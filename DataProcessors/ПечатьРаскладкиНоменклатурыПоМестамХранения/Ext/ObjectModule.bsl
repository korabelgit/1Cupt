	...
	Если мСпециальныеПоля[ДопПоле.Имя] = 0 Тогда // "МестоХранения"
		...
		Пока Выборка.Следующий() Цикл
			...
			Если СтрокаПредставленияПриоритета = Неопределено Тогда
				НазваниеКолонки = ЛокализацияПовтИсп.ПолучитьПереводТекста(ДопПоле.Псевдоним) + ?(Выборка.Количество() = 1, "", " " + (Выборка.Приоритет));
			Иначе
	...
	ИначеЕсли мСпециальныеПоля[ДопПоле.Имя] = 1 Тогда // "Остаток"
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Не ПустаяСтрока(ДопПоле.Псевдоним) Тогда
				НазваниеКолонки = НазваниеКолонки + Символы.ПС + ЛокализацияПовтИсп.ПолучитьПереводТекста(ДопПоле.Псевдоним);
			КонецЕсли;
		...
		КонецЦикла;
	Иначе
		Секция.Параметры.ДопПоле = ЛокализацияПовтИсп.ПолучитьПереводТекста(ДопПоле.Псевдоним);
		мТабДок.Присоединить(Секция);
...
Процедура Печать(НаПринтер = Ложь) Экспорт
	...
	Если ВыводитьЗаголовок Тогда // выведем заголовок
		...
		Секция.Параметры.ТекстЗаголовка = ОбщегоНазначения.СформироватьЗаголовокДокумента(Документ,,КодЯзыкаПечать);
		Секция.Параметры.Склад = СокрЛП(мСклад);
		мТабДок.Вывести(Секция);
	КонецЕсли;

	Секция = мМакет.ПолучитьОбласть("ВремяПечати");
	Секция.Параметры.ВремяПечати =  НСтр("ru='Дата и время печати: ';uk='Дата й час друку: '", КодЯзыкаПечать)
									+ ТекущаяДата() 
									+ НСтр("ru='. Пользователь: ';uk='. Користувач: '", КодЯзыкаПечать)
									+ СокрЛП(глЗначениеПеременной("глТекущийПользователь"));
	мТабДок.Вывести(Секция);
	...
	Секция.Параметры.ИтогВес =  НСтр("ru='Загальна вага товарів: ';uk='Загальна вага товарів: '", КодЯзыкаПечать)
								+ мИтогВес 
								+ " " 
								+ мЕдиницаИзмеренияВеса 
								+ "."
								+ ?(мВесВведенНеДляВсехТоваров, 
									НСтр("ru=' Вес введен не для всех товаров.';uk=' Вага введена не для всіх товарів.'", КодЯзыкаПечать),
									""
								   );
	мТабДок.Вывести(Секция);
...
Процедура ЗаполнитьПостроительОтчета(НеЗаполнятьНастройки = Ложь) Экспорт
	...
	"Номенклатура", "Ед. изм.", "Свободный остаток", "Место хранения", "Родитель мест хранения", "Склад отгрузки (оприходования)");
