...
Процедура ОбновитьДоговораНаДату(ПараметрОтборДоговоровКонтрагентов)
   ...
   ТекстЗапросаВозврат=
					...
					Пока ВыборкаДоговора.Следующий() Цикл  //ДоговорКонтрагента
						...
						Иначе
							...
							Если ВыборкаДоговора.ЗаТару Тогда
								ТекстСообщения = 
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Контрагент: %1, Договор: %2"
	...
	ДоговораКопия.Свернуть("ДоговорКонтрагента, Сделка, РасчетыВозврат","");
	...
	Для каждого СтрокаДоговоров Из ДоговораКопия Цикл
		...
		Если СтрокаДоговоров.РасчетыВозврат 	= Перечисления.РасчетыВозврат.Возврат Тогда
			Продолжить;
		КонецЕсли;
...
"Не указана схема налогового учета, либо в схеме не указан Момент определения базы НДС по продажам!", ВыборкаДоговора.Контрагент, ВыборкаДоговора.ДоговорКонтрагента); 
		...
		Если ВыдатьПредупраждающееСообщение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Контрагент: %1, Договор: %2, Дата:  %3"
"Нет возможности определить параметры (ставка, налоговое назначение) налоговых обязательств. Общая сумма обязательств указана в колонке ""Аванс""!", ВыборкаДоговора.Контрагент, ВыборкаДоговора.ДоговорКонтрагента, Формат(ВыборкаДоговора.Дата,"ДФ=dd.MM.yyyy")); 
	...
	// добавим строки-ссылки на номенклатуру, добавляемую пользователем.
	ДоговораКопия = Договора.Выгрузить();
	ДоговораКопия.Свернуть("Дата, ДоговорКонтрагента, Сделка, РасчетыВозврат","");
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетКомитентуОПродажах") Тогда 
		...
		Для каждого ДопПараметр Из ДопПараметры Цикл
			Если ДопПараметр.Ключ = "НалоговоеНазначение" Тогда
				СтрокаУслуг[ДопПараметр.Ключ] = ДокументОснование[ДопПараметр.Ключ+"ДоходовИЗатратПоВознаграждению"];
			Иначе
...
Процедура ЗаполнитьТабличнуюЧасть(ИмяТабличнойЧастиДокумента, СтруктураОтбора)
	...
	Для каждого СтрокаТЧДокумента Из Документ[ИмяТабличнойЧастиДокумента] Цикл 
		...
		Для каждого Колонка Из КолокниТЧОбработки Цикл
			...
			ИначеЕсли ИмяКолонки = "КодУКТВЭД" Тогда
				...
				ИначеЕсли НЕ КолокниТЧДокумента.Найти("Номенклатура") = Неопределено Тогда
					...
					ЗначениеНоменклатураГТД = СтрокаТЧДокумента.Номенклатура.НоменклатураГТД;	
					...
					Если ТипЗнч(ЗначениеНоменклатураГТД) = Тип("СправочникСсылка.НоменклатураГТД") Тогда
						СтрокаТЧОбработки.КодУКТВЭД = ЗначениеНоменклатураГТД.КодУКТВЭД;
					ИначеЕсли ТипЗнч(ЗначениеНоменклатураГТД) = Тип("СправочникСсылка.КлассификаторУКТВЭД") Тогда
						СтрокаТЧОбработки.КодУКТВЭД = ЗначениеНоменклатураГТД;
					КонецЕсли; 
		...
		Если    ВРЕГ(ИмяТабличнойЧастиДокумента) = "ОС"
			ИЛИ ВРЕГ(ИмяТабличнойЧастиДокумента) = "НематериальныеАктивы" Тогда
	...
	Для каждого СтрокаТЧОбработки Из СтрокиТабличнойЧастиОбработки Цикл
		...
		Для Каждого КолонкаТЧДокумента Из КолонкиТЧДокумента Цикл
			...
			Если  ИмяДокумента = "Документ.НалоговаяНакладная"
				И (Врег(ИмяТабличнойЧасти) = "ОС" ИЛИ Врег(ИмяТабличнойЧасти) = "НематериальныеАктивы") Тогда
...
Процедура ЗаписатьПровестиНапечататьНалоговыеДокументы(НалоговыеДокументы, УИД = Неопределено)
	...
	Для каждого ЭлементСоответствия Из НалоговыеДокументы Цикл
		...
		Если ПечататьДокументы Тогда
			...
			Если НЕ НалоговыйДокумент7 = Неопределено Тогда
				НалоговыйДокумент7.Печать("НалоговаяНакладная", , ПечататьДокументыНаПринтер);
			КонецЕсли;
		КонецЕсли;
		...
		Если ОткрыватьДокументы Тогда
			...
			КонецЕсли;	
			Если НЕ НалоговыйДокумент7 = Неопределено Тогда
				Если  ТипЗнч(НалоговыйДокумент7) = Тип("ДокументОбъект.НалоговаяНакладная") Тогда
					НалоговыйДокумент7.ОткрытиеИзОбработкиФормирования = Истина;
				КонецЕсли;	
			КонецЕсли;	
		...
		Если ОткрытиеИзУФ Тогда
			...
			Если НЕ НалоговыйДокумент7 = Неопределено Тогда
				АдресВременногоХранилищаДокумента = ПоместитьВоВременноеХранилище(НалоговыйДокумент7, УИД);
				МассивАдресов.Добавить(АдресВременногоХранилищаДокумента);
			КонецЕсли;
		КонецЕсли; 
