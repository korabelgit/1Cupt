	...
	Пока ОбходПоНоменклатуре.Следующий() Цикл
		...
		Пока ОбходПоХарактеристикам.Следующий() Цикл
			...
			Пока ОбходПоСериям.Следующий() Цикл
				ОбходПоДокументу= ОбходПоСериям.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
				Пока ОбходПоДокументу.Следующий() Цикл
					...
					Пока ОбходПоФизЛицам.Следующий() Цикл
						ОбходПоСхемам= ОбходПоФизЛицам.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
						Пока ОбходПоСхемам.Следующий() Цикл
							ОбходПоСпособам= ОбходПоСхемам.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
							Пока ОбходПоСпособам.Следующий() Цикл
								ОбходПоСрокам= ОбходПоСпособам.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
								Пока ОбходПоСрокам.Следующий() Цикл
									ОбходПоКачеству= ОбходПоСрокам.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
									Пока ОбходПоКачеству.Следующий() Цикл
										
										ВсегоКоличество = Макс(ОбходПоКачеству.КоличествоУУ, 0);
										
										ОбходПоНалогововуНазначению= ОбходПоКачеству.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
										Пока ОбходПоНалогововуНазначению.Следующий() Цикл
											
											ВсегоКоличествоБух = Макс(ОбходПоНалогововуНазначению.КоличествоБУ, 0);
											
											Если ДокОбъект.ОтражатьВБухгалтерскомУчете И ВсегоКоличествоБух = 0 Тогда
												Продолжить;
											КонецЕсли;
											
											Если ДокОбъект.ОтражатьВБухгалтерскомУчете Тогда
												ТекКоличество = Макс(ОбходПоНалогововуНазначению.КоличествоБУ, 0);
											Иначе
												ТекКоличество = Макс(ОбходПоНалогововуНазначению.КоличествоУУ, 0);
											КонецЕсли;
											...
											ЗаполнитьЗначенияСвойств(НоваяСтрока, ОбходПоНалогововуНазначению);
											НоваяСтрока.ЕдиницаИзмеренияМест 	= ОбходПоНалогововуНазначению.ЕдиницаИзмерения;
											
											НоваяСтрока.Количество = ТекКоличество;
											НоваяСтрока.КоличествоМест = НоваяСтрока.Количество;
											
											Если мУказДокПередачи Тогда
												
												НоваяСтрока.ДокументПередачи = ОбходПоНалогововуНазначению.ДокументПередачи;
												
											КонецЕсли;
											
											Если ЭтоДокПеремещение Тогда
												
												НоваяСтрока.СхемаНазначенияИспользованияНовое 	= ОбходПоНалогововуНазначению.СхемаНазначенияИспользования;
												НоваяСтрока.СпособОтраженияРасходовНовое 		= ОбходПоНалогововуНазначению.СпособОтраженияРасходов;
												НоваяСтрока.СрокПолезногоИспользованияНовое 	= ОбходПоНалогововуНазначению.СрокПолезногоИспользования;
												НоваяСтрока.КачествоНовое                		= ОбходПоНалогововуНазначению.Качество;
												
											КонецЕсли;
											
										КонецЦикла;
										
										Если ДокОбъект.ОтражатьВУправленческомУчете И ВсегоКоличество > 0 Тогда
											
											НоваяСтрока = ТаблицаМатериалов.Добавить();
											ЗаполнитьЗначенияСвойств(НоваяСтрока, ОбходПоКачеству);
											НоваяСтрока.ЕдиницаИзмеренияМест 	= ОбходПоКачеству.ЕдиницаИзмерения;
											...
											Если мУказДокПередачи Тогда
												...
												НоваяСтрока.ДокументПередачи = ОбходПоКачеству.ДокументПередачи;
												
											КонецЕсли;
											
											Если ЭтоДокПеремещение Тогда
												
												НоваяСтрока.СхемаНазначенияИспользованияНовое 	= ОбходПоКачеству.СхемаНазначенияИспользования;
												НоваяСтрока.СпособОтраженияРасходовНовое 		= ОбходПоКачеству.СпособОтраженияРасходов;
												НоваяСтрока.СрокПолезногоИспользованияНовое 	= ОбходПоКачеству.СрокПолезногоИспользования;
												НоваяСтрока.КачествоНовое                		= ОбходПоКачеству.Качество;
...
Процедура ЗаполнитьНачальныеНастройки(Отказ) Экспорт
	...
	|		0                   				КАК КоличествоУУ,
	|		СУММА(ПартииБУ.КоличествоОстаток)   КАК КоличествоБУ
	|	{ВЫБРАТЬ
	|		Номенклатура,
	|		ХарактеристикаНоменклатуры,
	|		СерияНоменклатуры,
...
Процедура ЗаполнитьНачальныеНастройки_РАУЗ(Отказ) Экспорт
	...
	|		СУММА(УчетЗатрат.КоличествоОстаток)   						КАК КоличествоУУ,
	|		0   														КАК КоличествоБУ
	|	{ВЫБРАТЬ
	|		Номенклатура,
	|		ХарактеристикаНоменклатуры,
	...
	|		РегистрАналитикаУчетаЗатрат.Качество                   		КАК Качество,
	|		0   														КАК КоличествоУУ,
	|		СУММА(УчетЗатрат.КоличествоОстаток)   						КАК КоличествоБУ
	|	{ВЫБРАТЬ
	|		Номенклатура,
	|		ХарактеристикаНоменклатуры,
	|		СерияНоменклатуры,
