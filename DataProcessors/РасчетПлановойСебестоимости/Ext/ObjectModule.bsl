...
//
Процедура ВывестиСтрокуРасчетаПлановойСебестоимости(ТабДокумент, Макет, СтрокаДерева, Уровень = 1)
	
	ОбластьМакета = Макет.ПолучитьОбласть(?(Уровень = 1, "Номенклатура", "Строка"));
	ОбластьМакета.Параметры.Заполнить(СтрокаДерева);
	
	Если Уровень = 1 Тогда
		ОбластьМакета.Параметры.НоменклатураСтатьяЗатрат =
			СокрЛП(СтрокаДерева.НоменклатураСтатьяЗатрат)
			+ ?(ЗначениеЗаполнено(СтрокаДерева.ХарактеристикаНоменклатуры), " (" + СтрокаДерева.ХарактеристикаНоменклатуры + ")", "") 
	...
	Для Каждого Строка Из СтрокаДерева.Строки Цикл
		ВывестиСтрокуРасчетаПлановойСебестоимости(ТабДокумент, Макет, Строка, Уровень + 1);
	КонецЦикла;
...
Процедура ПечатьРасчетаПлановойСебестоимости(ДеревоРезультат, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Для Каждого Строка Из ДеревоРезультат.Строки Цикл
		ВывестиСтрокуРасчетаПлановойСебестоимости(ТабДокумент, Макет, Строка);
	КонецЦикла;
