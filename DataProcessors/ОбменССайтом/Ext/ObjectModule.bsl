...
Перем мТаблицаСвойствНоменклатуры;
...
Процедура ЗаполнитьСписокЗначенийРеквизитовТовара(СтруктураИзменений, СписокЗначенийCML, НоменклатураСсылка, ВыгруженоКартинок, ВыгруженоФайлов)
	...
	Если НЕ НоменклатураСсылка.ДополнительноеОписаниеНоменклатурыВФорматеHTML
		И НЕ ПустаяСтрока(НоменклатураСсылка.ДополнительноеОписаниеНоменклатуры) Тогда
	...
	ВыгрузитьСвойстваНоменклатуры(СписокЗначенийCML, НоменклатураСсылка);
	
	Если ЗначениеЗаполнено(НоменклатураСсылка.СтавкаНДС)
		И НоменклатураСсылка.СтавкаНДС <> Перечисления.СтавкиНДС.БезНДС Тогда
	...
	Если НоменклатураСсылка.ДополнительноеОписаниеНоменклатурыВФорматеHTML
		И НЕ ПустаяСтрока(НоменклатураСсылка.ДополнительноеОписаниеНоменклатуры) Тогда
		
		ДобавитьЗначениеРеквизитаВСписок(СписокЗначенийCML, "ОписаниеВФорматеHTML", НоменклатураСсылка.ДополнительноеОписаниеНоменклатуры);
...
Процедура ВыгрузитьСвойстваНоменклатуры(СписокЗначенийCML, НоменклатураСсылка)

	Если мТаблицаСвойствНоменклатуры.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
		
	ДобавитьНачалоЭлементаCML(СписокЗначенийCML, "ЗначенияСвойств");
	
	Для Каждого ДанныеСвойства Из мТаблицаСвойствНоменклатуры Цикл
		
		ДобавитьНачалоЭлементаCML(СписокЗначенийCML, "ЗначенияСвойства");
		
		Ид = Строка(ДанныеСвойства.Свойство.УникальныйИдентификатор());
		ДобавитьУзелCML(СписокЗначенийCML, "Ид", Ид);
		
		мЗапросПоСвойствамНоменклатуры.УстановитьПараметр("НоменклатураСсылка", НоменклатураСсылка);
		мЗапросПоСвойствамНоменклатуры.УстановитьПараметр("СвойствоНоменклатурыСсылка", ДанныеСвойства.Свойство);
	
		Выборка = мЗапросПоСвойствамНоменклатуры.Выполнить().Выбрать();
		...
		Если ТипЗначенияСправочник(ТипЗнч(Значение)) Тогда
			ИдЗначения = Строка(Значение.УникальныйИдентификатор());
...
Процедура ВыгрузитьОсновныеРеквизитыСвойстваДляКлассификатора(ОбъектCML, ДанныеСвойства)
	
	Ид = ДанныеСвойства.Свойство.УникальныйИдентификатор();
	ЗаписатьТекстовойУзел(ОбъектCML, "Ид", Ид);
	
	Наименование = ДанныеСвойства.СвойствоНаименование;
	ЗаписатьТекстовойУзел(ОбъектCML, "Наименование", ФорматНаименованияДляCML(Наименование));
	
	Типы = ДанныеСвойства.Свойство.ТипЗначения.Типы();
	...
	Иначе
		...
		Если ТипЗначенияСправочник(Тип) Тогда
			...
			ВариантыЗначений = ПолучитьВариантыЗначенийСвойстваПоТипу(ДанныеСвойства.Свойство, Тип);
...
Процедура ВыгрузитьСвойства(ОбъектCML)
	...
	МассивГруппНоменклатуры = ПолучитьМассивГруппНоменклатуры();
	
	мТаблицаСвойствНоменклатуры = ПолучитьСвойстваНоменклатуры(МассивГруппНоменклатуры);
	...
	Для Каждого ДанныеСвойства Из мТаблицаСвойствНоменклатуры Цикл
		...
		ВыгрузитьОсновныеРеквизитыСвойстваДляКлассификатора(ОбъектCML, ДанныеСвойства);
...
Процедура ВыгрузитьКонтрагентаДок(ОбъектXML, Контрагент, Роль = Неопределено, СтруктурнаяЕдиница = Неопределено, ИмяГруппыВыгрузки = "Контрагент")
	...
	Если ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
		ЗаписатьТекстовойУзел(ОбъектXML, "ИНН", Контрагент.КодПоЕДРПОУ);
	Иначе
		СведенияОбОрганизации = УправлениеКонтактнойИнформацией.СведенияОЮрФизЛице(Контрагент, ТекущаяДата());
		ЗаписатьТекстовойУзел(ОбъектXML, "ИНН", СведенияОбОрганизации.КодПоЕДРПОУ);
	КонецЕсли;	
...
Процедура ВыгрузитьТоварДляЗаказаВCML(ОбъектCML, Док, СтрокаТЧ, ТЧУслуги = Ложь)
	...
	Если ЗначениеЗаполнено(СтрокаТЧ.Номенклатура.СтавкаНДС)
		И СтрокаТЧ.Номенклатура.СтавкаНДС <> Перечисления.СтавкиНДС.БезНДС Тогда
...
Функция ПолучитьМассивГруппНоменклатуры()
	...
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	ВремНоменклатураРазличные.НоменклатураСсылка КАК Группа
		|ИЗ
	...
	ТабГрупп = Запрос.Выполнить().Выгрузить();
	
	ТабГрупп.Свернуть("Группа");
	
	МассивГрупп = Новый Массив;
	
	Для Каждого СтрокаГруппы Из ТабГрупп Цикл
		
		Если ЗначениеЗаполнено(СтрокаГруппы.Группа)
			И СтрокаГруппы.Группа.ЭтоГруппа Тогда
			
			МассивГрупп.Добавить(СтрокаГруппы.Группа);
...
Функция ПолучитьСвойстваНоменклатуры(МассивГруппНоменклатуры)
	
	Запрос = Новый Запрос(
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ЗначенияСвойствНоменклатуры.СвойствоНоменклатурыСсылка КАК Свойство,
		|	ЗначенияСвойствНоменклатуры.СвойствоНаименование
		|ИЗ
		|	ВремЗначенияСвойствНоменклатуры КАК ЗначенияСвойствНоменклатуры
		|ГДЕ
		|	(ЗначенияСвойствНоменклатуры.Назначение ЕСТЬ NULL 
		|			ИЛИ НЕ ЗначенияСвойствНоменклатуры.Назначение.ЭтоГруппа
		|			ИЛИ ЗначенияСвойствНоменклатуры.Назначение.ЭтоГруппа
		|				И ЗначенияСвойствНоменклатуры.Назначение В (&МассивГруппНоменклатуры))");
...
Процедура ПодготовитьЗапросы();
	...
	мЗапросПоСвойствамНоменклатуры = Новый Запрос(
		...
		|	И ЗначенияСвойствНоменклатуры.СвойствоНоменклатурыСсылка = &СвойствоНоменклатурыСсылка");
...
Функция СоздатьДоговорПоПараметрам(Контрагент, Организация, ВалютаВзаиморасчетов)
	...
	НайденныйДоговорОбъект.ВедениеВзаиморасчетовРегл = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоЗаказам;
...
Функция Версия() Экспорт
	...
	Если Метаданные.Имя = "КомплекснаяАвтоматизация" Тогда
		Версия = "КА 1.1, типовая конфигурация";
	Иначе
		Версия = "УПП 1.3, типовая конфигурация";
	КонецЕсли;
