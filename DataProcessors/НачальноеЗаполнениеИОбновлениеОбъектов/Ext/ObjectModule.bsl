...
Функция СформированиеЗначениеСсылочногоТипа(ТипРеквизита, ЗначениеРеквизита, СоздаватьЭлементыСправочников = Ложь)
	...
	Если Значение = Неопределено Тогда
		Если НЕ ПустаяСтрока(ЗначениеРеквизита) Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Не удалось получить значение %1 по типу %2!", ЗначениеРеквизита, ТипРеквизита));
		КонецЕсли;
		Возврат ЗначениеРеквизита;
	Иначе
		Возврат Значение;
...
Функция ПолучитьДатуПоСтроке(Знач ДатаСтрокой, ИмяОбъекта, ИмяИзмерения)
	...
	ИначеЕсли Найти(ДатаСтрокой,":") <> 0 Тогда
		...
		Если СтрДлина(ДатаСтрокой)<>19 Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Ошибка при заполнении объекта %1! При заполнении измерения %2 не удалось преобразовать ""%3"" к дате!", ИмяОбъекта, ИмяИзмерения, ДатаСтрокой));
			Возврат '00010101';
		Иначе
	...
	//Формируем таблицу типов реквизитов
 	Для Сч = 4 по ЧислоРеквизитов Цикл
		...
		Если МетаданныеРеквизита = Неопределено Тогда
			...
			Если МетаданныеРеквизита = Неопределено Тогда
				Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Ошибка при заполнении объекта %1! Не найден реквизит %2."
"Объект не заполнен!", ИмяОбъекта, ИмяРеквизита));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Если СсылкаНаЭлемент = Неопределено Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Не найден предопределённый элемент %1 для справочника %2", Имя, ИмяОбъекта));
			Возврат;
		КонецЕсли;
		...
		Для СчК = 4 по ЧислоРеквизитов Цикл
			...
			Если МетаданныеРеквизита = Неопределено Тогда
				...
				Если МетаданныеРеквизита = Неопределено Тогда
					...
					Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Не найден реквизит %1 справочника %2!", ИмяРеквизита, ИмяОбъекта));
					Продолжить;
		...
		Если ФлагИзменений Тогда
			...
			Если МетаданныеСправочника.ДлинаКода > 0 И МетаданныеСправочника.КонтрольУникальности = Истина Тогда
				...
				Если НЕ КодЭлементаУникален("Справочник."+ИмяОбъекта,ТекущаяЗапись.Код,СсылкаНаЭлемент) Тогда
					...
					Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Не удалось записать объект справочника %1: %2, по причине того, что сгенерированный код был не уникален!", ИмяОбъекта, ТекущаяЗапись));
					ФлагИзменений = Ложь;
				КонецЕсли;
			Конецесли;
			Если ФлагИзменений Тогда
				...
				Исключение
					Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Не удалось записать объект справочника %1: %2!", ИмяОбъекта, ТекущаяЗапись));
				КонецПопытки;	
			КонецЕсли;	
		КонецЕсли;	
...
Процедура ЗаполнитьРегистрСведений(ИмяОбъекта)
	...
	Для Сч = 1 по ЧислоИзмерений Цикл
		...
		Если МетаданныеИзмерения = Неопределено Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Ошибка при заполнении объекта %1! Не найдено измерение %2."
"Объект не заполнен!", ИмяОбъекта, ИмяИзмерения));
	...
	Для Сч = 1 по ЧислоРесурсов Цикл
		...
		Если МетаданныеРесурса = Неопределено Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Ошибка при заполнении объекта %1! Не найден ресурс %2."
"Объект не заполнен!", ИмяОбъекта, МетаданныеРесурса));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Исключение
			...
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("В регистре сведений %1 не удалось создать запись с параметрами %2", ИмяОбъекта, ПараметрыЗаписи));
			Продолжить;
		КонецПопытки;	
...
Процедура ЗаполнитьТабличнуюЧасть(ТекущаяЗапись, Имя, ИмяТЧ, МакетТЧ, ТаблицаРеквизитовТЧ, МетаданныеТЧ, ФлагИзменений, ЭтоПВР = Ложь)
	...
	Для НомСтроки = 1 по ЧислоСтрок  Цикл
		...
		Для Сч = 1 по ЧислоРеквизитов Цикл
			...
			Если МетаданныеРеквизита = Неопределено Тогда
				Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Не найден реквизит %1 табличной части %2!", ИмяРеквизита, ИмяТЧ));
				Продолжить;
			КонецЕсли;
    ...
    Если ЕстьТЧ Тогда
		...
		Для Каждого Область из КоллекцияОбластей Цикл
			...
			Если ЕстьНужнаяТЧ = Неопределено Тогда
				Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Ошибка при заполнении объекта %1! Не найдена ТЧ %2!", ИмяОбъекта, ИмяТч));
				Возврат;
			КонецЕсли;
...
"Объект не заполнен!", ИмяОбъекта, ИмяРеквизита));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Если СсылкаНаЭлемент = Неопределено Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Не найден предопределённый элемент %1 для ПВХ %2", Имя, ИмяОбъекта));
			Возврат;
		КонецЕсли;
		ТекущаяЗапись = СсылкаНаЭлемент.ПолучитьОбъект();
		...
		Если ФлагИзменений Тогда
			...
			Попытка
				ТекущаяЗапись.Записать();
			Исключение
				Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Не удалось записать объект ПВХ %1: %2!", ИмяОбъекта, ТекущаяЗапись));
			КонецПопытки;	
		КонецЕсли;	
	КонецЦикла;	
...
"Объект не заполнен!", ИмяОбъекта, ИмяРеквизита));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Если СсылкаНаЭлемент = Неопределено Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Не найден предопределённый элемент %1 для плана счетов %2", Имя, ИмяОбъекта));
			Возврат;
		КонецЕсли;
		ТекущаяЗапись = СсылкаНаЭлемент.ПолучитьОбъект();
		...
		Для СчК = 4 по ЧислоРеквизитов Цикл
			...
			МетаданныеРеквизита = МетаданныеПС.Реквизиты.Найти(ИмяРеквизита);
			Если МетаданныеРеквизита = Неопределено Тогда
				Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Не найден реквизит %1 плана счетов %2!", ИмяРеквизита, ИмяОбъекта));
				Продолжить;
			КонецЕсли;
		...
		Если ФлагИзменений Тогда
			...
			Если МетаданныеПС.ДлинаКода > 0 И МетаданныеПС.КонтрольУникальности = Истина Тогда
				Если НЕ КодЭлементаУникален("ПланСчетов."+ИмяОбъекта,ТекущаяЗапись.Код,СсылкаНаЭлемент) Тогда
					// все равно есть лишние проблемы - не будем записывать
					Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Не удалось записать объект плана счетов %1: %2, по причине того, что сгенерированный код был не уникален!", ИмяОбъекта, ТекущаяЗапись));
					ФлагИзменений = Ложь;
				КонецЕсли;
			Конецесли;
			Если ФлагИзменений Тогда
				Попытка
					ТекущаяЗапись.Записать();
				Исключение
					Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Не удалось записать объект плана счетов %1: %2!", ИмяОбъекта, ТекущаяЗапись));
				КонецПопытки;	
			КонецЕсли;	
		КонецЕсли;	
...
"Объект не заполнен!", ИмяОбъекта, ИмяРеквизита));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Если СсылкаНаЭлемент = Неопределено Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Не найден предопределённый элемент %1 для плана видов расчета %2", Имя, ИмяОбъекта));
			Возврат;
		КонецЕсли;
		ТекущаяЗапись = СсылкаНаЭлемент.ПолучитьОбъект();
		...
		Для СчК = 4 по ЧислоРеквизитов Цикл
			...
			МетаданныеРеквизита = МетаданныеПВР.Реквизиты.Найти(ИмяРеквизита);
			Если МетаданныеРеквизита = Неопределено Тогда
				Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Не найден реквизит %1 плана видов расчета %2!", ИмяРеквизита, ИмяОбъекта));
				Продолжить;
			КонецЕсли;
			НовоеЗначение = СформироватьЗначение2(ТаблицаРеквизитов, ИмяРеквизита, ЗначениеРеквизита, Параметр);
			Если НовоеЗначение <> ТекущаяЗапись[ИмяРеквизита] Тогда
		...
		Если ФлагИзменений Тогда
			Попытка
				...
				ТекущаяЗапись.Записать();
			Исключение
				Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Не удалось записать объект план видов расчета %1: %2!", ИмяОбъекта, ТекущаяЗапись));
			КонецПопытки;	
		КонецЕсли;	
	КонецЦикла;	
