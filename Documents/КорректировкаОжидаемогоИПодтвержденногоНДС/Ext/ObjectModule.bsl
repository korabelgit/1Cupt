...
Процедура ДвиженияПоДоходамИЗатратам(СтруктураШапкиДокумента, СтрокаТаблицы, СуммаНДС)
	...
	Если СуммаНДС< 0 Тогда
		...
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 3 , СтрокаТаблицы.Сделка);
	...
	Иначе
		...
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 3 , СтрокаТаблицы.Сделка);
		...
		НоваяСтрока.СуммаНал = НалоговыйУчет.ОпределитьСтоимостьНУ(НоваяСтрока.НалоговоеНазначениеДоходовИЗатрат, СуммаНДС, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
