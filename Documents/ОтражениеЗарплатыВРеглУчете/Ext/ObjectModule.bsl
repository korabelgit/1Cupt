...
Процедура РассчитатьНачисленияПоКатегории( Сотрудники, Категория, НаборНачисления )
	...
	Если Истина Тогда
		...
		|   Основной.НалоговоеНазначениеДоходовИЗатрат		КАК НалоговоеНазначение,
		...
		|   Основной.ВзносыОсновногоНачисления	КАК СпособОтраженияЕСВФОТ,
	...
	Если Истина Тогда
		...
		|   БазаИзНачислений.НалоговоеНазначениеДоходовИЗатрат	КАК НалоговоеНазначение,
		...
		|   БазаИзНачислений.ВзносыОсновногоНачисления	КАК СпособОтраженияЕСВФОТ,
		...
		Пока ВыборкаНомерСтроки.Следующий() Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				ГруппаНалоговогоНазначенияЗатрат = НалоговыйУчетПовтИсп.ОпределитьГруппуНалоговогоНазначенияЗатрат(
					Движение.СчетДт, 
					Движение.СубконтоДт1, 
					Движение.СубконтоДт2, 
					Движение.СубконтоДт3,
				...
				Если ГруппаНалоговогоНазначенияЗатрат = Перечисления.ГруппыНалоговыхНазначений.НалоговыеНазначенияАктивовИВзаиморасчетовПоНДС Тогда
					// в НалоговоеНазначениеДоходовИЗатрат необходимо поместить НалоговоеНазначение 
					Движение.НалоговоеНазначениеДоходовИЗатрат     = Выборка.НалоговоеНазначение;
					СтрокаДанных.НалоговоеНазначениеДоходовИЗатрат = Выборка.НалоговоеНазначение;
				Иначе
					// ничего не делаем, в НалоговоеНазначениеДоходовИЗатрат уже и так НалоговоеНазначениеДоходовИЗатрат из способа
				КонецЕсли;	
...
Процедура РассчитатьВзносы( Сотрудники ) Экспорт
	...
	Если НЕ ПроведениеРасчетов.ИспользуетсяЕСВ(ПериодРегистрации) Тогда
		...
		Если Истина Тогда
			...
			|	
			|		ГДЕ		НАЧАЛОПЕРИОДА(Взаиморасчеты.Период, МЕСЯЦ) = &парамПериодРегистрации И Взаиморасчеты.Активность 
...
Процедура РассчитатьНДФЛ( Сотрудники ) Экспорт
	...
	Если Истина Тогда
		...
		|
		|		ГДЕ		НАЧАЛОПЕРИОДА(Основной.Период, МЕСЯЦ) = &парамПериодРегистрации  И Основной.Активность 
		...
		Пока ВыборкаЗнакСуммы.Следующий() Цикл
			...
			Пока ВыборкаСотрудники.Следующий() Цикл
				...
				Пока Выборка.Следующий() Цикл
					...
					ГруппаНалоговогоНазначенияЗатрат = НалоговыйУчетПовтИсп.ОпределитьГруппуНалоговогоНазначенияЗатрат(
						СтрокаДанных.СчетДт, 
						СтрокаДанных.СубконтоДт1, 
						СтрокаДанных.СубконтоДт2, 
						СтрокаДанных.СубконтоДт3,
						СтрокаДанных.БазовыйПериодНачало
					);
...
Процедура ДобавитьСтрокуОтражений(ВыборкаПоСтрокамДокумента, НаборЗаписей)
	...
	Если Дата >= глЗначениеПеременной("ДатаНКУ2015") 
		 И ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НалоговоеНазначениеДоходовИЗатрат)
		 И НалоговыйУчетПовтИсп.ПолучитьГруппуНалоговогоНазначения(ВыборкаПоСтрокамДокумента.НалоговоеНазначениеДоходовИЗатрат) <> Перечисления.ГруппыНалоговыхНазначений.НалоговыеНазначенияАктивовИВзаиморасчетовПоНДС
		 
		Тогда
		
		Движение.НалоговоеНазначениеДоходовИЗатрат = Справочники.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка();
...
Процедура ПроверитьЗаполнениеСтрокиОтраженияВУчете(ВыборкаПоДоходам, Отказ, Заголовок = "")
	...
	Если БухгалтерскийУчетПовтИсп.СчетЗатратБудущихПериодов(ВыборкаПоДоходам.СчетДт) Тогда
		СчетБУ = БухгалтерскийУчет.СчетСписанияРБП(ВыборкаПоДоходам.СубконтоДт1, ВыборкаПоДоходам.СубконтоДт2, ВыборкаПоДоходам.СубконтоДт3);
	Иначе
		СчетБУ = ВыборкаПоДоходам.СчетДт;
	КонецЕсли; 	
	
	ХарактерЗатрат = УправлениеЗатратами.ПолучитьХарактерЗатратПоСчетуЗатрат(СчетБУ, ВыборкаПоДоходам.СтатьяЗатрат);
	Если Дата < глЗначениеПеременной("ДатаНКУ2015") Тогда
		...
		КонецЕсли; 	
	Иначе
		Если БухгалтерскийУчетПовтИсп.УчетПоНалоговымНазначениямНДССчет(СчетБУ) И Не ЗначениеЗаполнено(ВыборкаПоДоходам.НалоговоеНазначениеДоходовИЗатрат) Тогда	
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не указано налоговое назначение!", Отказ);
		КонецЕсли; 
	КонецЕсли; 		
...
Процедура ДобавитьСтрокуВДвиженияПоОтражениюВУчете(СтруктураШапкиДокумента, ВыборкаПоДоходам, ВалютаРеглУчета, ТаблицаЗатрат)
	...
	НалоговоеНазначениеДоходовИЗатрат = ВыборкаПоДоходам.НалоговоеНазначениеДоходовИЗатрат;
	Если Дата >= глЗначениеПеременной("ДатаНКУ2015") 
		 И ЗначениеЗаполнено(ВыборкаПоДоходам.НалоговоеНазначениеДоходовИЗатрат)
		 И НалоговыйУчетПовтИсп.ПолучитьГруппуНалоговогоНазначения(ВыборкаПоДоходам.НалоговоеНазначениеДоходовИЗатрат) <> Перечисления.ГруппыНалоговыхНазначений.НалоговыеНазначенияАктивовИВзаиморасчетовПоНДС
	...
	Проводка.СуммаНУДт = НалоговыйУчет.ОпределитьСтоимостьНУ(Проводка.НалоговоеНазначениеДт, ВыборкаПоДоходам.Сумма, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015); 
	...
	НоваяСтрока.НалоговоеНазначение = НалоговоеНазначениеДоходовИЗатрат;
