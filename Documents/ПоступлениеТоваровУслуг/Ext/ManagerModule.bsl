...
Функция ПечатьПоступлениеТоваров(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого Ссылка Из МассивОбъектов Цикл
		...
		ОбластьМакета.Параметры.РеквизитыПоставщика 	= ФормированиеПечатныхФормСервер.ОписаниеОрганизации(СведенияОПоставщике, "НомерСчета,Банк,МФО,/,ЮридическийАдрес,Телефоны,/,КодПоЕДРПОУ,КодПоДРФО,ИНН,НомерСвидетельства,",,КодЯзыкаПечать);
		ТабДокумент.Вывести(ОбластьМакета);
		...
		ВыводитьМеста = Истина;
			Если ВыборкаСтрок.Итог("КоличествоМест") = 0 Тогда
				ВыводитьМеста = Ложь;
			КонецЕсли;
