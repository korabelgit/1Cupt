...
Функция КорректноеСочетаниеСчетов(Выборка,Сообщение)
	...
	Если (СчетДт = ПланыСчетов.Хозрасчетный.РасчетыПоОплатеТруда 
		...
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Проводки в дебет %1 не должны отражаться в налоговом учете!", Выборка.НаименованиеСчетДт);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиОтраженияВУчете(СтруктураШапкиДокумента, ВыборкаПоДоходам, Отказ, Заголовок = "")
	...
	СтрокаНачалаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("В строке номер ""%1"" табл. части ""Отражение в учете"": ", СокрЛП(ВыборкаПоДоходам.НомерСтроки));
