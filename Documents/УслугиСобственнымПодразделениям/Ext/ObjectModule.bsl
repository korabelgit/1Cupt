...
Процедура ПроверкаРеквизитов(Отказ, Заголовок, СтруктураШапкиДокумента, ТаблицаЗатрат)
	...
	Для Каждого СтрокаТЧ Из ТаблицаЗатрат Цикл
		...
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете И (СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 ИЛИ СтруктураШапкиДокумента.ЕстьНДС) Тогда 
...
//Процедура заполняет в структуре шапки документа поля НоменклатурнаяГруппа, СчетЗатрат, НалоговоеНазначение
Процедура ДополнитьСтруктуруШапкиДаннымиНоменклатуры(СтруктураШапкиДокумента)
	
	СтруктураЗаполняемыхПолей = Новый Структура("НоменклатурнаяГруппа,СчетЗатрат,НалоговоеНазначение");
	
	НеобходимоЗаполнениеПолей = Ложь;
	
	Для Каждого ПолеДляЗаполнения Из СтруктураЗаполняемыхПолей Цикл
		
		Если НЕ ЗначениеЗаполнено(СтруктураШапкиДокумента[ПолеДляЗаполнения.Ключ]) Тогда
			НеобходимоЗаполнениеПолей = Истина;
			Прервать;
		КонецЕсли;
		
	КонецЦикла;
	
	Если НеобходимоЗаполнениеПолей Тогда
		
		ЗначенияДляЗаполнения = Документы.УслугиСобственнымПодразделениям.ПараметрыУчетаНоменклатуры(СтруктураШапкиДокумента);
		ЗначениеПоля = Неопределено;
		
		Для Каждого ПолеДляЗаполнения Из СтруктураЗаполняемыхПолей Цикл
			
			Если НЕ ЗначениеЗаполнено(СтруктураШапкиДокумента[ПолеДляЗаполнения.Ключ])
			 И ЗначенияДляЗаполнения.Свойство(ПолеДляЗаполнения.Ключ, ЗначениеПоля) Тогда
			 	
			 	СтруктураШапкиДокумента[ПолеДляЗаполнения.Ключ] = ЗначениеПоля;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ДополнитьСтруктуруШапкиДаннымиНоменклатуры(СтруктураШапкиДокумента);
