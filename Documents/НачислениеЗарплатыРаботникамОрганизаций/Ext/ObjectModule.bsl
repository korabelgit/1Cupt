...
Функция ЗаполнитьТаблицуРасчетногоЛистка(ТекущийСотрудник) Экспорт
	...
	|			ИНАЧЕ """ + НСтр("ru='Способ выплаты не указан ';uk='Спосіб виплати не зазначений '",КодЯзыкаПечать) + """
	|		КОНЕЦ + 
	...
	Если ДобавитьДанныеИзДокумента Тогда
		...
		ОбластьШапка.Параметры.КодПоДРФО = НСтр("ru='Код по ДРФО: ';uk='Код за ДРФО: '",КодЯзыкаПечать) + ТекущийСотрудник.ФизЛицо.КодПоДРФО; 
		...
		Для Каждого СтрокаНачислений Из Строки Цикл
			...
			Если СтрокаНачислений.Назначение.ОсновноеНазначение = СтрокаНачислений.Сотрудник Тогда
				мВидЗанятости = НСтр("ru=' (по внутр. совмест-ву)';uk=' (за внутр. сумісн-ву)'",КодЯзыкаПечать)
			КонецЕсли;
		...
		Для Каждого СтрокаВзносы Из СтрокиВзносы Цикл
			...
			Если ИспользуетсяЕСВ Тогда
				СтрокаТаблицы.ВидРасчета			= НСтр("ru='ЕСВ';uk='ЕСВ'",КодЯзыкаПечать);
...
Процедура ПроверитьЗаполнениеСтрокиВзносы(ВыборкаПоСтрокамДокумента, Отказ, Заголовок = "")
	...
	ИмяТЧ = ?(ВыборкаПоСтрокамДокумента.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.Взносы,"Взносы","Взносы ФОТ");
	СтрокаНачалаСообщенияОбОшибке = "В строке номер """+ СокрЛП(ВыборкаПоСтрокамДокумента.НомерСтроки) +
									""" табл. части """+ИмяТЧ+""": ";
