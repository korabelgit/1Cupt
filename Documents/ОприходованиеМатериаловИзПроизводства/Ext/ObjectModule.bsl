...
Процедура ПроверкаРеквизитовТЧ(ТаблицаМатериалов, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	ПроверятьЗаполнениеНУ = СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете
							И (СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 ИЛИ СтруктураШапкиДокумента.ЕстьНДС);
...
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоМатериалам) Экспорт
	...
	СтруктураСложныхПолей.Вставить("ПринадлежностьНоменклатуры",  "ВЫБОР КОГДА СтатьяЗатрат.СтатусМатериальныхЗатрат = Значение(Перечисление.СтатусыМатериальныхЗатратНаПроизводство.ПринятыеВПереработку) ТОГДА Значение(Перечисление.ПринадлежностьНоменклатуры.Принятый) ИНАЧЕ Значение(Перечисление.ПринадлежностьНоменклатуры.ПустаяСсылка) КОНЕЦ");
	...
	ОбщегоНазначенияКлиентСервер.ЗаполнитьНоменклатурнуюГруппуИСтатьюЗатратВТаблицеДокумента(ТаблицаПоМатериалам, СтруктураОбрабатываемыхКолонок, СтруктураЗависимыхКолонок);
...
КонецПроцедуры // ДополнитьСтруктуруПолейДаннымиНоменклатуры
