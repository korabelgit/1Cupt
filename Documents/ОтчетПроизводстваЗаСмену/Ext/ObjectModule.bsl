...
Процедура ЗаполнитьТехОперации() Экспорт
	...
	Для каждого СтрокаПродукция из ВыпускПоДокументу Цикл
		...
		// Количество выпуска по всей спецификации, если это полная спецификация - учтем пропорционально
		ОбщееКоличествоВыпускаПоСпецификации = 0;
		...
		Для каждого СтрокаСпецификации Из СтрокаПродукция.Спецификация.ВыходныеИзделия Цикл
			...
			ОбщееКоличествоВыпускаПоСпецификации = ОбщееКоличествоВыпускаПоСпецификации + СтрокаСпецификации.Количество * ?(ЗначениеЗаполнено(СтрокаСпецификации.ЕдиницаИзмерения), СтрокаСпецификации.ЕдиницаИзмерения.Коэффициент, 1);
			
		КонецЦикла;
		
		Если ОбщееКоличествоВыпускаПоСпецификации > 0 Тогда	
			
			НоваяСтрокаВыпускПоСпецификациям.КоличествоВыпусков = СтрокаПродукция.Количество / ОбщееКоличествоВыпускаПоСпецификации;
...
Процедура ДобавитьСтрокуВТехОперации(ТаблицаТехнологическиеОперации, СтрокаВыпуска, ТехОпер)
	...
	СтруктПоиска.Вставить( "НоменклатурнаяГруппа", 	СтрокаВыпуска.НоменклатурнаяГруппа);
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаданиеНаПроизводство") Тогда
		...
		// Установим признак отражения документа в бух. учете для того,
		...
		#Если Клиент Тогда
		СписокПодразделений = РаботаСДиалогами.ПолучитьСписокПодразделенийОрганизаций(Основание.Подразделение, Организация);
