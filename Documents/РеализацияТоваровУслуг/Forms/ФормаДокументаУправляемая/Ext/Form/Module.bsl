...
Процедура УстановитьВидимостьДоверенности()
	...
	Элементы.ДоверенностьДатаНомер.Видимость = НЕ ПолучилПоДругомуДокументу;
...
КонецПроцедуры // УстановитьВидимостьДоверенности
...
Процедура ПолучилПоДругомуДокументуПриИзменении(Элемент)
	УстановитьВидимостьДоверенности();
КонецПроцедуры // ПолучилПоДругомуДокументуПриИзменении

&НаКлиенте
Процедура ДоверенностьПолучилНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	СписокПредставителей = ПолучитьСписокПредставителейКонтрагента(Объект.Контрагент);
	ДанныеВыбора = СписокПредставителей;
КонецПроцедуры
...
&НаСервереБезКонтекста
Функция ПолучитьСписокПредставителейКонтрагента(Контрагент) 
	...
	Пока Выборка.Следующий() Цикл
		СписокПредставителей.Добавить(?(ПустаяСтрока(Выборка.Должность),"",СокрЛП(Выборка.Должность) + " ") + 
						?(ПустаяСтрока(Выборка.Фамилия),"",СокрЛП(Выборка.Фамилия) + " ") + 
						?(ПустаяСтрока(Выборка.Имя),"",СокрЛП(Выборка.Имя) + " ") + 
						СокрЛП(Выборка.Отчество));
	КонецЦикла;
					
	Возврат СписокПредставителей;

КонецФункции // ПолучитьСписокПредставителейКонтрагента
