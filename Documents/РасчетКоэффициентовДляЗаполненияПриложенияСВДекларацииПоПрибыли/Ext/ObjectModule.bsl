...
Процедура ПроверитьЗаполнениеТабличнойЧастиСтатьи(ТаблицаПоСтатьям, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если  СуммаКоэффициентов <> 1 Тогда
		...
		ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сумма коэффициентов на конец периода в строках не единице!'; uk = 'Сума коефіцієнтів на кінець періода у рядках не дорівнюе одиниці!'"), Уровень1, Уровень2), Отказ);		
