		...
		Если ОтражатьВБухгалтерскомУчете Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТЧ.СпособНачисленияАмортизацииНУ) Тогда
				Если НЕ ЗначениеЗаполнено(СтрокаТЧ.СрокПолезногоИспользованияНУ) Тогда
					ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Бух. учет: Для основного средства ""%1"" в строке №%2 не заполнен реквизит ""Срок использ. (НУ)""!'"), СтрокаТЧ.ОсновноеСредство, СтрокаТЧ.НомерСтроки), Отказ, Заголовок);
				КонецЕсли;	
				Если НЕ ЗначениеЗаполнено(СтрокаТЧ.СрокИспользованияДляВычисленияАмортизацииНУ) Тогда
					ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Бух. учет: Для основного средства ""%1"" в строке №%2 не заполнен реквизит ""Срок для аморт. (НУ)""!'"), СтрокаТЧ.ОсновноеСредство, СтрокаТЧ.НомерСтроки), Отказ, Заголовок);
				КонецЕсли;	
				Если НЕ ЗначениеЗаполнено(СтрокаТЧ.СтоимостьДляВычисленияАмортизацииНУ) Тогда
					ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Бух. учет: Для основного средства ""%1"" в строке №%2 не заполнен реквизит ""Ст-ть для выч. аморт. (НУ)""!'"), СтрокаТЧ.ОсновноеСредство, СтрокаТЧ.НомерСтроки), Отказ, Заголовок);
				КонецЕсли;	
			КонецЕсли;	
		КонецЕсли;
	КонецЦикла;
