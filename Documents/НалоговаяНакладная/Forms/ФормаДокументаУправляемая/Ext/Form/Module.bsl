...
Процедура УстановитьВидимостьНаСервере()
	Элементы.ТребуетВключенияВЕдиныйРеестрНалоговыхНакладных.Доступность = НЕ (Объект.ТипПричиныНевыдачиПокупателю = 0) И (Объект.Дата < глЗначениеПеременной("ДатаВступленияВСилуПриказа10"));
	Элементы.ТоварыНомерГТД.Видимость               = (Объект.Дата < глЗначениеПеременной("ДатаВступленияВСилуПриказа10"));
...
Процедура ПриОткрытии(Отказ)
    ...
    Элементы.УточненияДатаНомер.Видимость = Объект.ВключаетсяВУточняющийРасчет;
КонецПроцедуры
...
Функция ИзменениеНоменклатурыСервер(ДанныеДляЗаполнения)
	...
	КодУКТВЭДНомерГТД = ПолучитьКодУКТВЭДНомерГТД(ДанныеДляЗаполнения);
	ЗначенияДляЗаполнения.Вставить("КодУКТВЭД",  КодУКТВЭДНомерГТД.КодУКТВЭД);
	ЗначенияДляЗаполнения.Вставить("НомерГТД",   КодУКТВЭДНомерГТД.НомерГТД);
...
КонецФункции // ИзменениеНоменклатурыСервер

&НаКлиенте
Процедура ИзменениеНоменклатурыКлиент(СтрокаТабличнойЧасти)
	ДанныеОбменаССервером = Новый Структура("Номенклатура, ХарактеристикаНоменклатуры, УчитыватьНДС, СуммаВключаетНДС");
...
КонецПроцедуры // ИзменениеНоменклатурыКлиент
...
&НаСервереБезКонтекста
Функция ПолучитьКодУКТВЭДНомерГТД(ДанныеДляЗаполнения)
	
	Результат = Новый Структура("КодУКТВЭД, НомерГТД");
	
	НоменклатураГТДХарактеристики = ДанныеДляЗаполнения.ХарактеристикаНоменклатуры.НоменклатураГТД;
	
	Если ДанныеДляЗаполнения.ХарактеристикаНоменклатуры.ОтечественныйНеподакцизныйТовар = Истина Тогда
		Результат.Вставить("КодУКТВЭД", Неопределено);
		Результат.Вставить("НомерГТД",  Неопределено);
	ИначеЕсли ЗначениеЗаполнено(НоменклатураГТДХарактеристики) Тогда
		Результат.Вставить("КодУКТВЭД", НоменклатураГТДХарактеристики.КодУКТВЭД);
		Результат.Вставить("НомерГТД",  НоменклатураГТДХарактеристики.НомерГТД);
	Иначе
		Результат.Вставить("КодУКТВЭД", ДанныеДляЗаполнения.Номенклатура.НоменклатураГТД.КодУКТВЭД);
		Результат.Вставить("НомерГТД",  ДанныеДляЗаполнения.Номенклатура.НоменклатураГТД.НомерГТД);
	КонецЕсли;
	
	Возврат Результат;
	
КонецФункции // ПолучитьКодУКТВЭДНомерГТД
...
Процедура ТоварыХарактеристикаНоменклатурыПриИзменении(Элемент)
	...
	ДанныеДляЗаполнения = Новый Структура("Номенклатура, ХарактеристикаНоменклатуры");
	ЗаполнитьЗначенияСвойств(ДанныеДляЗаполнения, СтрокаТабличнойЧасти);
	
	КодУКТВЭДНомерГТД = ПолучитьКодУКТВЭДНомерГТД(ДанныеДляЗаполнения);
	
	СтрокаТабличнойЧасти.КодУКТВЭД = КодУКТВЭДНомерГТД.КодУКТВЭД;
	СтрокаТабличнойЧасти.НомерГТД  = КодУКТВЭДНомерГТД.НомерГТД;
...
&НаСервереБезКонтекста
Функция ПолучитьНоменклатуруГТД(Номенклатура, КодУКТВЭД, НомерГТД)
...
КонецФункции // ПолучитьНоменклатуруГТД
...
Процедура НомерГТДНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Отбор = Новый Структура("Владелец", ТекущаяСтрокаТоваров.Номенклатура);
	
	ПараметрыФормы = Новый Структура("Отбор", Отбор);
	ПараметрыФормы.Вставить("ТекущаяСтрока", ПолучитьНоменклатуруГТД(ТекущаяСтрокаТоваров.Номенклатура, ТекущаяСтрокаТоваров.КодУКТВЭД, ТекущаяСтрокаТоваров.НомерГТД));
	
	ОткрытьФорму("Справочник.НоменклатураГТД.Форма.ФормаВыбораУправляемая", ПараметрыФормы, Элемент);
		
КонецПроцедуры // НомерГТДНачалоВыбора

&НаКлиенте
Процедура ТоварыНомерГТДНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	НомерГТДНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка);
КонецПроцедуры // ТоварыНомерГТДНачалоВыбора

&НаКлиенте
Процедура ТоварыКодУКТВЭДНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	НомерГТДНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка);
КонецПроцедуры // ТоварыКодУКТВЭДНачалоВыбора

&НаСервереБезКонтекста
Функция ПолучитьКодУКТВЭД(НоменклатураГТД)
	Возврат Новый Структура("КодУКТВЭД, НомерГТД", НоменклатураГТД.КодУКТВЭД, НоменклатураГТД.НомерГТД);
КонецФункции
...
Процедура ТоварыНомерГТДОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.НоменклатураГТД") Тогда
		ТекущаяСтрокаДанных = Элементы.Товары.ТекущиеДанные;
		Коды = ПолучитьКодУКТВЭД(ВыбранноеЗначение);
		Если НЕ ТекущаяСтрокаДанных = Неопределено Тогда
			ТекущаяСтрокаДанных.КодУКТВЭД = Коды.КодУКТВЭД;
		КонецЕсли;
		ВыбранноеЗначение = Коды.НомерГТД;	
	КонецЕсли;
...
Процедура ТоварыКодУКТВЭДОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.НоменклатураГТД") Тогда
		...
		Коды = ПолучитьКодУКТВЭД(ВыбранноеЗначение);
		Если НЕ ТекущаяСтрокаДанных = Неопределено Тогда
			ТекущаяСтрокаДанных.НомерГТД = Коды.НомерГТД;
		КонецЕсли;
		ВыбранноеЗначение = Коды.КодУКТВЭД;	
	КонецЕсли;
КонецПроцедуры // ТоварыКодУКТВЭДОбработкаВыбора
