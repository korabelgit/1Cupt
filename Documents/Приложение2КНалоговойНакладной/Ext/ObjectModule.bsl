...
Функция ПечатьДокумента(ТолькоФормироватьСтруктуруПоказателейПечатнойФормы = Ложь
	...
	Если РабочаяДата >= глЗначениеПеременной("ДатаВступленияВСилуПриказа1307") Тогда
		...
		Если ВыгрузкаПечатьНН_ПоФормеНаДатуДокумента = Ложь Тогда
			...
			Если РабочаяДата >= глЗначениеПеременной("ДатаВступленияВСилуПриказа276") Тогда
				Возврат ПечатьДокументаНалоговыйКодекс2017(ТолькоФормироватьСтруктуруПоказателейПечатнойФормы, ИмяМакета);
			ИначеЕсли РабочаяДата >= глЗначениеПеременной("ДатаВступленияВСилуПриказа1307") Тогда
	...
	Если Дата >= глЗначениеПеременной("ДатаВступленияВСилуПриказа276") Тогда
		Возврат ПечатьДокументаНалоговыйКодекс2017(ТолькоФормироватьСтруктуруПоказателейПечатнойФормы, ИмяМакета);
	ИначеЕсли Дата >= глЗначениеПеременной("ДатаВступленияВСилуПриказа1307") Тогда
...
Функция ПечатьДокументаНалоговыйКодекс2017(ТолькоФормироватьСтруктуруПоказателейПечатнойФормы, ИмяМакета = "Приложение2КНалоговойНакладной") Экспорт	
	...
	ТаблицаНоменклатуры.Свернуть(
		"КодВид, КодСХ, Причина, Номенклатура, Характеристика, КодУКТВЭД, ЕдиницаИзмерения, ЕдиницаИзмеренияКод, ЦенаМестаБезНДСБезСкидкиРегл,  Количество, СтавкаНДС, КодЛьготы, НомерСтрокиНН", 
	...
	Макет = ПолучитьОбщийМакет("Приложение2КНалоговойНакладной2017Альбом");
	...
	Для Страница = 1 По 1 Цикл
		...
		Для каждого СтрокаТаблицы Из ТаблицаНоменклатуры Цикл
			...
			ОбластьМакета.Параметры.Номенклатура = ОбластьМакета.Параметры.Номенклатура + ДопТекст;
...
КонецФункции // ПечатьДокументаНалоговыйКодекс2017()
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, 
	...
	Если//СтруктураШапкиДокумента.РегистрируетсяВЕРННПокупателем
		...
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru = 'Не указан период включения в Декларацию по НДС (реквизит ""Включить в Декларацию за:"")! Проводки сформированы частично.'; uk = 'Не зазначений період включення до Декларації з ПДВ (реквізит ""Включити до Декларації за:"")! Проводки сформовані частково.'"), , Заголовок);
		Возврат; 
