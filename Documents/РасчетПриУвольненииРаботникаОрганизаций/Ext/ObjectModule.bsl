...
Функция ПечатьСредняяОтпуск() Экспорт
	...
	Если ВидУчетаВремениДляСредней = Перечисления.ВидыУчетаВремениДляСредней.ПоРабочимДням Тогда
		ОбластьШапка2.Параметры.СредняяТекст = НСтр("ru='Среднечасовая (по рабочим дням):';uk='Середньогодинна (по робочим дням):'",КодЯзыкаПечать);
	ИначеЕсли ВидУчетаВремениДляСредней = Перечисления.ВидыУчетаВремениДляСредней.ПоРабочимЧасам Тогда
		ОбластьШапка2.Параметры.СредняяТекст = НСтр("ru='Среднечасовая (по рабочим часам):';uk='Середньогодинна (по робочим годинам):'",КодЯзыкаПечать);
	Иначе
		ОбластьШапка2.Параметры.СредняяТекст = НСтр("ru='Среднечасовая (по календарным):';uk='Середньогодинна (по календарним):'",КодЯзыкаПечать);
	КонецЕсли;
...
Функция ПечатьСредняяПособие() Экспорт
	...
	Если ВидУчетаВремениДляСредней = Перечисления.ВидыУчетаВремениДляСредней.ПоКалендарнымДням Тогда
		ОбластьШапка2.Параметры.СредняяТекст =  НСтр("ru='Среднедневная (по первичному отпуску):';uk='Середньоденна (по первинній відпустці):'",КодЯзыкаПечать);
...
Функция РассчитатьРасчетСреднего(КомментироватьРасчет = Ложь) Экспорт
	...
	Если КомментироватьРасчет Тогда
		...
		КомментарийРасчетаСредней = ОбщегоНазначения.КомментарийРасчета("Расчет средней с " + Формат(ПериодРасчетаСреднегоНачало,"ДФ='МММ гггг'")+ " по " + Формат(ПериодРасчетаСреднегоКонец,"ДФ='МММ гггг'"));
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ВидРасчета = ОсновнойВидРасчета Тогда
			...
			Если КомментироватьРасчет Тогда
				...
				ТекстКомментарияКоэффициента = "Коэффициент повышения оклада = " + нКоэффициентПовышенияОкладов;
			КонецЕсли;
		КонецЕсли;
		Если КомментироватьРасчет Тогда
	...
	Если КомментироватьРасчет Тогда
		...
		Для Каждого СтрокаКомментария из ТаблицаКомментариев Цикл
			КомментарийНомерСтроки = ОбщегоНазначения.КомментарийРасчета("Номер строки: " + СтрокаКомментария.НомерСтроки, КомментарийРасчетаСредней);
			КомментарийВидаРасчета = ОбщегоНазначения.КомментарийРасчета("	Вид расчета: " +  СтрокаКомментария.ВидРасчета, КомментарийНомерСтроки);
			ОбщегоНазначения.КомментарийРасчета("	Базовый период: " + Формат(СтрокаКомментария.БазовыйПериод,"ДФ='МММ гггг'"), КомментарийВидаРасчета);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если КомментироватьРасчет Тогда
			...
			Если мКоэффициентПовышенияОкладов <> 1 И Выборка.БазовыйПериодНачало < ПериодИзмененияКоэффициента Тогда
				ТекстКомментарияФормулаСКоэффициентом = "Результат: " + Выборка.РезультатПолный + "*" + мКоэффициентПовышенияОкладов + " = " + ОКР(мКоэффициентПовышенияОкладов * Выборка.Результат,2,0);
			КонецЕсли;
