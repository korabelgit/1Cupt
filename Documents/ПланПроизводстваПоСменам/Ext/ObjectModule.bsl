...
Функция ПечатьПланаПроизводстваПоСменам()
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = ОбщегоНазначения.СформироватьЗаголовокДокумента(Ссылка, НСтр("ru='План производства по сменам';uk='План виробництва по змінах';", КодЯзыкаПечать), КодЯзыкаПечать);
...
Функция ПечатьСоставаДокумента()
	...
	Пока ВыборкаПланов.Следующий() Цикл
		...
		ОбластьСтроки.Параметры.ПериодВыборки = НСтр("ru='С ';uk='З '", КодЯзыкаПечать)
		                                        + Формат(ВыборкаПланов.ДатаНачВыборки, "ДФ=dd.MM.yyyy")
												+ НСтр("ru=' по ';uk=' по '", КодЯзыкаПечать)
												+ Формат(ВыборкаПланов.ДатаКонВыборки, "ДФ=dd.MM.yyyy");
		Если ВыборкаПланов.ВариантРаспределенияВыпусковПланов = Перечисления.ВариантыРаспределенияПоПериоду.ТочноКСроку Тогда
			ОбластьСтроки.Параметры.РаспределениеПлана = НСтр("ru='Все к ';uk='Всі до '", КодЯзыкаПечать) + Формат(ВыборкаПланов.ДатаВыпускаПланов, "ДФ=dd.MM.yyyy");
		Иначе
			ОбластьСтроки.Параметры.РаспределениеПлана = НСтр("ru='С ';uk='З '", КодЯзыкаПечать)
														 + Формат(ВыборкаПланов.ДатаЗапускаПланов, "ДФ=dd.MM.yyyy")
														 + НСтр("ru=' по ';uk=' по '", КодЯзыкаПечать)
														 + Формат(ВыборкаПланов.ДатаВыпускаПланов, "ДФ=dd.MM.yyyy");
		КонецЕсли; 
	...
	Пока ВыборкаЗаказов.Следующий() Цикл
		...
		Если ВыборкаЗаказов.ВариантРаспределенияВыпусковЗаказа = Перечисления.ВариантыРаспределенияПоПериоду.ТочноКСроку Тогда
			ОбластьСтроки.Параметры.РаспределениеЗаказа = НСтр("ru='Все к ';uk='Всі до '", КодЯзыкаПечать) + Формат(ВыборкаЗаказов.ДатаВыпускаЗаказа, "ДФ=dd.MM.yyyy");
		Иначе
			ОбластьСтроки.Параметры.РаспределениеЗаказа = НСтр("ru='С ';uk='З '", КодЯзыкаПечать)
														  + Формат(ВыборкаЗаказов.ДатаЗапускаЗаказа, "ДФ=dd.MM.yyyy")
														  + НСтр("ru=' по ';uk=' по '", КодЯзыкаПечать)
														  + Формат(ВыборкаЗаказов.ДатаВыпускаЗаказа, "ДФ=dd.MM.yyyy");
		КонецЕсли; 
		ТабДокумент.Вывести(ОбластьСтроки);
	КонецЦикла;
	...
	Пока ВыборкаТоваров.Следующий() Цикл
		...
		Если ВыборкаТоваров.ВариантРаспределенияВыпусковТовара = Перечисления.ВариантыРаспределенияПоПериоду.ТочноКСроку Тогда
			ОбластьСтроки.Параметры.РаспределениеТовара = НСтр("ru='Все к ';uk='Всі до '", КодЯзыкаПечать) + Формат(ВыборкаТоваров.ДатаВыпускаТовара, "ДФ=dd.MM.yyyy");
		Иначе
			ОбластьСтроки.Параметры.РаспределениеТовара = НСтр("ru='С ';uk='З '", КодЯзыкаПечать)
			                                              + Формат(ВыборкаТоваров.ДатаЗапускаТовара, "ДФ=dd.MM.yyyy")
														  + НСтр("ru=' по ';uk=' по '", КодЯзыкаПечать)
														  + Формат(ВыборкаТоваров.ДатаВыпускаТовара, "ДФ=dd.MM.yyyy");
		КонецЕсли; 
		ТабДокумент.Вывести(ОбластьСтроки);
	КонецЦикла;
		...
		Пока ТекущаяДата >= ПериодДатаНач Цикл
			...
			Если РезультатРаспределения[ТекущаяДата] <> Неопределено Тогда
				...
				Иначе
					...
					Если Значение > _РаспределяемоеЗначение Тогда
						Значение = _РаспределяемоеЗначение;
					КонецЕсли; 
					РезультатРаспределения.Вставить(ТекущаяДата, Значение);
