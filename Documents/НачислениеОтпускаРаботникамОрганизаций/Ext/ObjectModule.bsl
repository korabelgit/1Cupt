...
Функция ПечатьСредняя() Экспорт
	...
	Иначе
		ОбластьШапка2.Параметры.СредняяТекст = НСтр("ru='Среднедневная (по календарным):';uk='Середньоденна (по календарним):'",КодЯзыкаПечать);
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если КомментироватьРасчет Тогда
			...
			Если мКоэффициентПовышенияОкладов <> 1 И (Выборка.БазовыйПериодНачало < ПериодИзмененияКоэффициента ИЛИ КоэффициентПовышенияОкладов <> 1) Тогда 
				ТекстКомментарияФормулаСКоэффициентом = "Результат: " + Выборка.Результат + "*" + нКоэффициентПовышенияОкладов + " = " + ОКР(нКоэффициентПовышенияОкладов * Выборка.Результат,2,0);
			КонецЕсли;
