...
Функция СуммаНУВсегдаНулевая(ТекущиеДанные) Экспорт
	...
	Если Не мУчетнаяПолитикаРегл.ЕстьНалогНаПрибыльДо2015 Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура ОбработкаПроведения_ЗатратыЗапасы(Отказ)
	...
	НалУчет    =  СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015;
	НалУчетОбщ = (СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 ИЛИ СтруктураШапкиДокумента.ЕстьНДС);	
