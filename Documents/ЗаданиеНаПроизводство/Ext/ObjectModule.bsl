	...
	Если ВидЗадания = Перечисления.ВидыЗаданийНаПроизводство.НаТехПроцесс Тогда
		...
		Для каждого Строка из ВыпускТехПроцесс Цикл
			...
			Если Строка.ТехОперация.НормаВремени > 0 Тогда
				...
				НоваяСтрокаЗаданияНаТехОперации.Количество = (Строка.ДатаОкончания - Строка.Период + 1) / (Строка.ТехОперация.НормаВремени * ?(Строка.ТехОперация.Коэффициент = 0, 1, Строка.ТехОперация.Коэффициент));
...
Функция ПечатьЗаданияНаПроизводство()
	...
	|	МИНИМУМ(ЗаданиеНаПроизводствоВыпускТехПроцесс.НомерСтроки) КАК НомерСтроки
	|ИЗ
