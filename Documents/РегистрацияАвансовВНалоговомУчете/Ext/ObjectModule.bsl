	...
	Для каждого Строка Из ТаблицаПоПриобретениям Цикл
		...
		Если Строка.РегистрацияПоНДС Тогда
				Если Строка.НалоговоеНазначение = Справочники.НалоговыеНазначенияАктивовИЗатрат.НДС_Пропорционально               
					И  НЕ (    Строка.СтавкаНДС = Перечисления.СтавкиНДС.НДС20 ИЛИ Строка.СтавкаНДС = Перечисления.СтавкиНДС.НДС0 ИЛИ Строка.СтавкаНДС = Перечисления.СтавкиНДС.НДС7) Тогда

				
					СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Для договора ""%1"" в табличной части ""Приобретение"" указан признак пропорционального включения НДС в кредит."
"При этом ставка НДС может быть только 20%/7%/0%! Исправьте указанные реквизиты!';uk='Для договору ""%1"" у табличній частині ""Придбання"" зазначена ознака пропорційного включення ПДВ у кредит."
"При цьому ставка ПДВ може бути тільки 20%/7%/0%, Виправте зазначені реквізити!'"), Строка(Строка.ДоговорКонтрагента));
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если НЕ СтруктураШапкиДокумента.ЕстьНДС И НЕ СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 Тогда
...
Функция НалоговыеРегистрыЗакрывалисьВТекущемМесяце(СтруктураШапкиДокумента)
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
				   ...
				   |	И ПродажиНалоговыйУчет.Активность 
				   ...
				   |	И ПриобретенияНалоговыйУчет.Активность 
