...
Процедура ДвиженияПоРегистрамРегл(СтруктураШапкиДокумента, ТаблицаПоОС, Отказ, Заголовок)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |				И Организация = &ТекОрганизация) КАК МестонахождениеОСБУ";
	...
	Для Каждого СтрокаТЧ Из ТаблицаПоОС Цикл
		...
		Если ТаблицаМестонахождений <> Неопределено Тогда
			...
			ТекМестонахождение = ТаблицаМестонахождений.Найти(ТекОС, "ОС_БУ");
		...
		Если    СтрокаТЧ.АмортизацияЗаМесяцБУ > 0 
			...
			НоваяСтрока.АмортизацияНУ           = СтрокаТЧ.АмортизацияЗаМесяцБУ;
			...
			Движение.АмортизацияНУ 		= СтрокаТЧ.АмортизацияЗаМесяцБУ;
		...
		Проводка.СуммаНУКт = НалоговыйУчет.ОпределитьСтоимостьНУ(Проводка.НалоговоеНазначениеКт, СтрокаТЧ.СтоимостьНУ);
		
		СтоимостьНУУвеличение = Проводка.СуммаНУДт - Проводка.СуммаНУКт;
		
		Если СтоимостьНУУвеличение <> 0 Тогда
			// Движения по регистру СтоимостьОС
			Движение = ТаблицаДвиженийСтоимость.Добавить();

			Движение.ВидДвижения		= ?(СтоимостьНУУвеличение > 0, ВидДвиженияНакопления.Приход, ВидДвиженияНакопления.Расход);
			Движение.Организация 		= ТекОрганизация;
			Движение.ОсновноеСредство 	= ТекОС;
			Движение.Стоимость			= 0;
			Движение.СтоимостьНУ		= Макс(СтоимостьНУУвеличение, - СтоимостьНУУвеличение);
		...
		ИначеЕсли СтрокаТЧ.МетодКорректировкиНалоговогоКредита = Перечисления.МетодыКорректировкиНалоговогоКредита.НаНалоговыйКредит Тогда
			...
			Если ПересчитыватьНакопленнуюАмортизациюВСвязиСКорректировкойНК Тогда
				...
				НоваяСтрока.АмортизацияНУ 			= НалоговыйУчет.ОпределитьСтоимостьНУ(СтрокаТЧ.НалоговоеНазначение, КоэффПересчетаАмортизации * (СтрокаТЧ.АмортизацияНУ + СтрокаТЧ.АмортизацияЗаМесяцНУ), СтруктураШапкиДокумента.ЕстьНалогНаПрибыль);
			Иначе				
				НоваяСтрока.Амортизация 			= КоэффПересчетаАмортизации * (СтрокаТЧ.АмортизацияЗаМесяцБУ);
				НоваяСтрока.АмортизацияНУ 			= НалоговыйУчет.ОпределитьСтоимостьНУ(СтрокаТЧ.НалоговоеНазначение, КоэффПересчетаАмортизации * (СтрокаТЧ.АмортизацияЗаМесяцНУ), СтруктураШапкиДокумента.ЕстьНалогНаПрибыль);
			КонецЕсли;
			...
			Если СтрокаТЧ.НДС <> 0 Тогда
				...
				Если ПересчитыватьНакопленнуюАмортизациюВСвязиСКорректировкойНК Тогда
					...
					Движение.АмортизацияНУ 		= НалоговыйУчет.ОпределитьСтоимостьНУ(СтрокаТЧ.НалоговоеНазначение, -КоэффПересчетаАмортизации * (СтрокаТЧ.АмортизацияНУ + СтрокаТЧ.АмортизацияЗаМесяцНУ), СтруктураШапкиДокумента.ЕстьНалогНаПрибыль);
				Иначе					
					Движение.Амортизация 		= -КоэффПересчетаАмортизации * (СтрокаТЧ.АмортизацияЗаМесяцБУ);
					Движение.АмортизацияНУ 		= НалоговыйУчет.ОпределитьСтоимостьНУ(СтрокаТЧ.НалоговоеНазначение, -КоэффПересчетаАмортизации * (СтрокаТЧ.АмортизацияЗаМесяцНУ), СтруктураШапкиДокумента.ЕстьНалогНаПрибыль);
				КонецЕсли;
			КонецЕсли;
	...
	ТаблицаДвиженийСтоимость.ЗаполнитьЗначения(ДатаДока,	"Период");
	ТаблицаДвиженийСтоимость.ЗаполнитьЗначения(Ссылка,		"Регистратор");
	ТаблицаДвиженийСтоимость.ЗаполнитьЗначения(Истина,		"Активность");
	
	СтоимостьОС.ВыполнитьДвижения();
...
Процедура ДвиженияПоРегиструОжидаемыйИПодтвержденныйНДСПродаж(СтруктураШапкиДокумента, 
	...
	ТаблицаКопия.Свернуть("СтавкаНДС","СуммаНДС, БазаНДС");
...
Функция ПодготовитьТаблицуОС(РезультатЗапросаПоОС, СтруктураШапкиДокумента)
	...
	Для каждого СтрокаТаблицы Из ТаблицаОС Цикл
		...
		СтрокаТаблицы.БазаНДС  = СтрокаТаблицы.НДС*100/Ценообразование.ПолучитьСтавкуНДС(СтрокаТаблицы.СтавкаНДС);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если НЕ (СтруктураШапкиДокумента.ЕстьНДС) Тогда
		...
		ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("На дату документа: %1 организация ""%2""%3не является плательщиком налога на прибыль или НДС.%4В этом случае документ не используется.", Формат(Дата, "ДЛФ=D"+";Л="+Локализация.ОпределитьКодЯзыкаДляФормат(Локализация.КодЯзыкаИнтерфейса())), Организация, Символы.ПС, Символы.ПС),
						Отказ, Заголовок);
		Возврат;				
