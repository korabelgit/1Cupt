...
Функция ПечатьАналитическогоПаспорта() Экспорт
	...
	АттестатКонтрагента                = УправлениеСертификациейНоменклатуры.ПолучитьАттестатАккредитации(Организация, Дата, КодЯзыкаПечать);
	Секция.Параметры.Аттестат          = АттестатКонтрагента;
	Секция.Параметры.НомерДок          = Строка(Номер) + НСтр("ru=' от ';uk=' від '", КодЯзыкаПечать) + Строка(Формат(Дата,"ДЛФ=DD" + ЛокализацияПовтИсп.ПолучитьФорматЯзыкаДляПредставленияВСхемахСКД()));
	...
	Секция.Параметры.СрокГодности      = Формат(СерияНоменклатуры.СрокГодности, "ДЛФ=DD"+ ЛокализацияПовтИсп.ПолучитьФорматЯзыкаДляПредставленияВСхемахСКД());
	Секция.Параметры.ДатаИзготовления  = Формат(УправлениеСертификациейНоменклатуры.ПолучитьДатуЗавершенияПроизводстваСерии(СерияНоменклатуры), "ДЛФ=DD"+ ЛокализацияПовтИсп.ПолучитьФорматЯзыкаДляПредставленияВСхемахСКД());
