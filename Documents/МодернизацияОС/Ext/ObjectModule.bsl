...
Функция ПечатьОЗ2(ПечатьПоДаннымУпрУчета = Истина)
	...
	Если Не ПечатьПоДаннымУпрУчета Тогда
		...
		СдалДолжность	= СокрЛП(СдалДанные.Должность);
		СдалПринялДолжности.Вставить("СдалДолжность",СдалДолжность);
		
		ПринялДанные	= ОбщегоНазначения.ДанныеФизЛица(Организация,Принял, Дата);
		ПринялДолжность = СокрЛП(ПринялДанные.Должность);
		СдалПринялДолжности.Вставить("ПринялДолжность",ПринялДолжность);
	Иначе
		...
		Для Каждого СтрокаТЧ Из ТаблицаПоОС Цикл
			...
			Если СуммаВСтоимостьБУ > 0 ИЛИ СуммаВСтоимостьНУ > 0 Тогда
				...
				Если СуммаПроводки <> 0 ИЛИ СуммаПроводкиНУ <> 0 Тогда	
					...
					Проводка.СуммаНУДт 				= НалоговыйУчет.ОпределитьСтоимостьНУ(Проводка.НалоговоеНазначениеДт, СуммаПроводкиНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыль);
					...
					Проводка.СуммаНУКт 				= НалоговыйУчет.ОпределитьСтоимостьНУ(Проводка.НалоговоеНазначениеКт, СуммаПроводкиНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
...
Процедура ФормированиеДвиженийПоПодсистемеНДС(СтруктураШапкиДокумента, ТЗНалоговыеНазначения, Отказ, Заголовок)
	...
	Если ОстатокСтоимостиНУ = 0 И ОстатокСтоимостиБУ = 0 Тогда	
		...
		Возврат
    ИначеЕсли СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 И ОстатокСтоимостиНУ < ТекСтоимостьНУ Тогда
