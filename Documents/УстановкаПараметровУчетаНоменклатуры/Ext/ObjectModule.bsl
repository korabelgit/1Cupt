...
Функция ПечатьУстановленныхСчетовУчета()
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, НСтр("ru='Установка параметров учета номенклатуры';uk='Установка параметрів обліку номенклатури'", КодЯзыкаПечать), КодЯзыкаПечать);
	...
	|	УстановкаПараметровУчетаНоменклатурыПараметрыУчетаУслуг.ВестиУчетЗатратПоЗаказам
	|ИЗ
...
КонецФункции // ПечатьУстановленныхСчетовУчета
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если НЕ Запрос.Выполнить().Пустой() Тогда
			ОбщегоНазначения.СообщитьОбОшибке("Проведенные документы установки параметров учета номенклатуры не должны иметь одинаковые даты", Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;
