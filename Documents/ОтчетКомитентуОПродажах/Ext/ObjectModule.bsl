...
Функция ПечатьОтчетаКомитенту()
	...
	ОбластьМакета.Параметры.РеквизитыПолучателя = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации, "ЮридическийАдрес,Телефоны,/,КодПоЕДРПОУ,КодПоДРФО,ИНН,НомерСвидетельства,/,ИнформацияОСтатусеПлательщикаНалогов,",,КодЯзыкаПечать);
...
Процедура ЗаполнитьТовары(ДокументПоставки = Неопределено, Реализованными = Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СуммаПоступленияВал = Выборка.СуммаВзаиморасчетовОстаток - СуммаЗаказанного;
		СуммаПоступленияВал = Ценообразование.ПересчитатьЦенуПриИзмененииФлаговНалогов(
								СуммаПоступленияВал, 
								Неопределено,     // СпособЗаполненияЦен
								Истина,           // ЦенаВключаетНДС СуммаВзаиморасчетовОстаток - всегда с НДС 
								УчитыватьНДС, 
								СуммаВключаетНДС, // СуммаВключаетНДС
								Ценообразование.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.СтавкаНДС)
							  );
							  
		СтрокаТабличнойЧасти.ЦенаПоступления            = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СуммаПоступленияВал, 
...
Процедура ЗаполнитьРеализованнымиУпр(ДокументПоставки = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если УправлениеЗапасами.ИспользуетсяРасширеннаяАналитикаУчета(?(ДокументПоставки = Неопределено, Дата, ДокументПоставки.Дата)) Тогда							
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугамВознаграждения, 
	...
	Если УправлениеЗапасами.ИспользуетсяРасширеннаяАналитикаУчета(Дата) Тогда
		...
		ТаблицаПродажи.Колонки.СчетВычетовИзДоходов.Имя      = "СчетДоходовБУ";
		ТаблицаПродажи.Колонки.СубконтоВычетовИзДоходов1.Имя = "СубконтоДоходовБУ1";
		ТаблицаПродажи.Колонки.СубконтоВычетовИзДоходов2.Имя = "СубконтоДоходовБУ2";
		ТаблицаПродажи.Колонки.СубконтоВычетовИзДоходов3.Имя = "СубконтоДоходовБУ3";
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам,
	...
	Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
		...
		СуммаВознагражденияСНДС = СуммаВознаграждения + 
		...
		СуммаДолга = ?(СтруктураШапкиДокумента.УдержатьКомиссионноеВознаграждение, СуммаДокумента - СуммаВознагражденияСНДС, СуммаДокумента);
		...
		Если Не СтруктураШапкиДокумента.УдержатьКомиссионноеВознаграждение 
		   И СуммаВознагражденияСНДС <> 0 Тогда
			...
			СтрокаДвижений.Сделка              = УправлениеВзаиморасчетами.ОпределитьСделку(ЭтотОбъект, СтруктураШапкиДокумента);
			СтрокаДвижений.СуммаВзаиморасчетов = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СуммаВознагражденияСНДС, СтруктураШапкиДокумента.ВалютаДокумента,
												 СтруктураШапкиДокумента.ВалютаВзаиморасчетов,
												 СтруктураШапкиДокумента.КурсДокумента, КурсВзаиморасчетов,
												 СтруктураШапкиДокумента.КратностьДокумента, КратностьВзаиморасчетов);
			
												 
			СтрокаДвижений.СуммаУпр            = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СуммаВознагражденияСНДС, СтруктураШапкиДокумента.ВалютаДокумента,
												 СтруктураШапкиДокумента.ВалютаУправленческогоУчета,
												 СтруктураШапкиДокумента.КурсДокумента, СтруктураШапкиДокумента.КурсВалютыУправленческогоУчета,
												 СтруктураШапкиДокумента.КратностьДокумента, СтруктураШапкиДокумента.КратностьВалютыУправленческогоУчета);
			НаборДвижений.мПериод              = Дата;
		...
		Если НЕ Отказ Тогда
		    ...
		    Если УчитыватьНДС И (Не СуммаВключаетНДС) Тогда 
				Для Каждого ТекущаяСтрока Из СтруктТаблицДокумента.ТаблицаПоТоварам Цикл
					ТекущаяСтрока.СуммаВзаиморасчетов = ТекущаяСтрока.СуммаВзаиморасчетов + ТекущаяСтрока.СуммаНДСПоступления;
				КонецЦикла;	
			КонецЕсли;									
		    ...
		    Если УчитыватьНДС И (Не СуммаВключаетНДС) Тогда 
				Для Каждого ТекущаяСтрока Из СтруктТаблицДокумента.ТаблицаПоТоварам Цикл
					ТекущаяСтрока.Выручка = ТекущаяСтрока.Выручка + ТекущаяСтрока.НДС;
				КонецЦикла;	
			КонецЕсли;									
...
Процедура ДвиженияПоРегистрамОперативныхВзаиморасчетов(РежимПроведения, ТаблицаПоВзаиморасчетам, Отказ, Заголовок, СтруктураШапкиДокумента)
	...
	Если Не СтруктураШапкиДокумента.УдержатьКомиссионноеВознаграждение 
		...
		НоваяСтрока.СуммаВзаиморасчетов = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СуммаВознагражденияСНДС, 
													СтруктураШапкиДокумента.ВалютаДокумента, СтруктураШапкиДокумента.ВалютаВзаиморасчетов,
													СтруктураШапкиДокумента.КурсДокумента, КурсВзаиморасчетов, 
													СтруктураШапкиДокумента.КратностьДокумента, КратностьВзаиморасчетов);
		НоваяСтрока.СуммаРегл   		= МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СуммаВознагражденияСНДС, 
...
КонецПроцедуры // ДвиженияПоРегистрамОперативныхВзаиморасчетов
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, 
	...
	БухгалтерскийУчетРасчетовСКонтрагентами.ЗачетАванса(СтруктураПараметровЗачетАванса, СтруктураДвижений, мВалютаРегламентированногоУчета,РежимПроведения, ЭтотОбъект, ДвиженияДляРегистраРасчетовПоПриобретению, Отказ);
		...
		КонецЕсли;
	ТаблицаКопия = ТаблицаПоТоварам.Скопировать();
	ТаблицаКопия.Свернуть("СделкаРегл, СчетВычетовИзДоходов, СубконтоВычетовИзДоходов1, СубконтоВычетовИзДоходов2, СубконтоВычетовИзДоходов3, НоменклатурнаяГруппа"+?(СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015,", НалоговоеНазначениеДоходовИЗатрат",""),"ПроводкиСуммаСНДСРегл,ПроводкиСуммаСНДСВал");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ОтражатьВБухгалтерскомУчете Тогда
		...
		Если УчетнаяПолитика <> Неопределено И НЕ УчетнаяПолитика.ЕстьНалогНаПрибыльДо2015 Тогда
