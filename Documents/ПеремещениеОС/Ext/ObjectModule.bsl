...
Функция ПолучитьДанныеФизЛицаУпр(ФизЛицо,ДатаСреза)
	
	ДанныеФизЛицаУпр = Новый Структура;
	...
	Запрос.Текст = 
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц.СрезПоследних(&ДатаСреза, ФизЛицо = &ФизЛицо) КАК ФИОФизЛицСрезПоследних
	...
	Если НЕ Запрос.Выполнить().Пустой() Тогда
		...
		ДанныеФизЛицаУпр.Вставить("Должность",ВыборкаДетальныеЗаписи.Должность);
		ДанныеФизЛицаУпр.Вставить("Представление", ОбщегоНазначения.ПолучитьФамилиюИмяОтчество(ВыборкаДетальныеЗаписи.Фамилия, 
																								ВыборкаДетальныеЗаписи.Имя, 
																									ВыборкаДетальныеЗаписи.Отчество, 
																										Истина));
		
		Возврат ДанныеФизЛицаУпр;
...
Функция ПечатьОЗ1(ПечатьПоДаннымУпрУчета = Истина)
	...
	Пока ВыборкаПоОС.Следующий() Цикл
		...
		Если ПечатьПоДаннымУпрУчета Тогда
			 ...
			 СдалДанные 	= ПолучитьДанныеФизЛицаУпр(Сдал,Дата);
			 ПолучилДанные  = ПолучитьДанныеФизЛицаУпр(Принял,Дата);
			 
			 Если НЕ СдалДанные = Неопределено Тогда 
				СдалПринял.Вставить("СдалДолжность",СокрЛП(СдалДанные.Должность));
				СдалПринял.Вставить("СдалФИО",СокрЛП(СдалДанные.Представление));
			КонецЕсли;
			
			Если НЕ ПолучилДанные = Неопределено Тогда 
				СдалПринял.Вставить("ПолучилДолжность",СокрЛП(ПолучилДанные.Должность));
				СдалПринял.Вставить("ПолучилФИО",СокрЛП(ПолучилДанные.Представление));
			КонецЕсли;
		 ...
		 Иначе
			...
			СдалДанные		= ОбщегоНазначения.ДанныеФизЛица(Организация,Сдал, Дата);
			СдалПринял.Вставить("СдалДолжность",СокрЛП(СдалДанные.Должность));
			СдалПринял.Вставить("СдалФИО",СокрЛП(СдалДанные.Представление));
		
			ПолучилДанные	= ОбщегоНазначения.ДанныеФизЛица(Организация,Принял, Дата);
			СдалПринял.Вставить("ПолучилДолжность",СокрЛП(ПолучилДанные.Должность));
			СдалПринял.Вставить("ПолучилФИО",СокрЛП(ПолучилДанные.Представление));
		...
		ОбластьМакета.Параметры.Заполнить(СдалПринял);
...
Функция ПолучитьСведенияОСдалПринял(ДокументОбъект) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	ФИОСдал.Фамилия + "" "" + ПОДСТРОКА(ФИОСдал.Имя, 1, 1) + "". "" + ПОДСТРОКА(ФИОСдал.Отчество, 1, 1) + ""."" КАК СдалФИО,
	               |	ФИОПринял.Фамилия + "" "" + ПОДСТРОКА(ФИОПринял.Имя, 1, 1) + "". "" + ПОДСТРОКА(ФИОПринял.Отчество, 1, 1) + ""."" КАК ПолучилФИО,
	               |	ДолжностьСдал.Должность КАК СдалДолжность,
	               |	ДолжностьПринял.Должность КАК ПолучилДолжность
	               |ИЗ
	               |	РегистрСведений.ФИОФизЛиц.СрезПоследних(&НаДату, ФизЛицо = &Сдал) КАК ФИОСдал
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц.СрезПоследних(&НаДату, ФизЛицо = &Принял) КАК ФИОПринял
	               |		ПО (ИСТИНА)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций.СрезПоследних(
	               |				&НаДату,
	               |				Сотрудник.ФизЛицо = &Сдал
	               |					И Организация = &Организация) КАК ДолжностьСдал
	               |		ПО (ИСТИНА)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций.СрезПоследних(
	               |				&НаДату,
	               |				Сотрудник.ФизЛицо = &Принял
	               |					И Организация = &Организация) КАК ДолжностьПринял
	               |		ПО (ИСТИНА)";
