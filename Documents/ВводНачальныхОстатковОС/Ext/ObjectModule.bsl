	...
	Если БухУчет или НалУчет или НалУчетОбщ тогда
		...
		Если НалУчет Тогда
			...
			Если Дата >= ДатаНКУ2015_ДляВводаНачальныхОстатков Тогда 
				ОбязательныеРеквизиты = ОбязательныеРеквизиты + ", СпособНачисленияАмортизацииНУ";
				Если  (СтрокаОС.СпособНачисленияАмортизацииНУ = Перечисления.СпособыНачисленияАмортизацииОС.Прямолинейный)
					...
					ОбязательныеРеквизиты = ОбязательныеРеквизиты + ", СрокИспользованияДляВычисленияАмортизацииНУ";
				КонецЕсли;	
			КонецЕсли;
	...
	Для каждого КлючЗначение Из РеквизитыДляПроверки Цикл
		...
		Если НЕ ЗначениеЗаполнено(Значение) Тогда // надо ругаться
			...
			Если Прав( КлючЗначение.Ключ, 2) = "БУ" Тогда
				ВидУчета = "(БУ) ";
			ИначеЕсли Прав( КлючЗначение.Ключ, 2) = "НУ" ИЛИ КлючЗначение.Ключ = "НалоговаяГруппаОС" ИЛИ КлючЗначение.Ключ = "НалоговоеНазначение" Тогда
				ВидУчета = "(НУ) ";
...
Процедура ДвижениеПоРегиструНачислениеАмортизацииНал( ТабОС, СтруктураШапкиДокумента)
	...
	Если НЕ СтруктураШапкиДокумента.ЕстьНалогНаПрибыль Тогда //ЕстьНДС
		Возврат;
	КонецЕсли;	
...
Процедура ДвижениеПоРегиструПараметрыАмортизацииНал( ТабОС, СтруктураШапкиДокумента)
	...
	Для Каждого СтрокаОС Из ТабОС Цикл
		...
		Если СтруктураШапкиДокумента.ЕстьНалогНаПрибыль Тогда
			Если НЕ ЗначениеЗаполнено(СтрокаОС.СтоимостьДляВычисленияАмортизацииНУ) Тогда
...
// Процедура формирования движений по регистру ПереоценкаОСБухгалтерскийУчет
//
...
Процедура ФормированиеПроводокБух( ТабОС, СтруктураШапкиДокумента)
	...
	Для Каждого СтрокаОС Из ТабОС Цикл
		...
		Если ЗначениеЗаполнено( СтрокаОС.СчетУчетаБУ) Тогда
				...
				Проводка.СуммаНУДт   = НалоговыйУчет.ОпределитьСтоимостьНУ(СтрокаОС.НалоговоеНазначение, СтрокаОС.БалансоваяСтоимостьНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыль);
		...
		Если ЗначениеЗаполнено(СтрокаОС.СчетАмортизацииБУ)
			   И (НЕ СтрокаОС.НакопленнаяАмортизацияБУ = 0 ИЛИ НЕ СтрокаОС.НакопленнаяАмортизацияНУ = 0) Тогда
				...
				Проводка.СуммаНУКт   = НалоговыйУчет.ОпределитьСтоимостьНУ(СтрокаОС.НалоговоеНазначение, СтрокаОС.НакопленнаяАмортизацияНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыль);
...
Процедура ФормированиеДвиженияОСВременнаяНалоговаяРазница(ЭтотОбъект, СтруктураШапкиДокумента, Заголовок)
	...
	НоваяСтрока.АмортизацияНУ      	= АмортизацияНУВременнаяНалоговаяРазница;
...
Процедура ДвиженияПоРегистрам( ТабОС, СтруктураШапкиДокумента, Заголовок)
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		ФормированиеДвиженияОСВременнаяНалоговаяРазница(ЭтотОбъект, СтруктураШапкиДокумента, Заголовок);
