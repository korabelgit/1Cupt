...
Функция ПечатьСредняя() Экспорт
	...
	Если ВидУчетаВремениДляСредней = Перечисления.ВидыУчетаВремениДляСредней.ПоРабочимДням Тогда
		ОбластьШапка2.Параметры.СредняяТекст = НСтр("ru='Среднедневная (по рабочим дням):';uk='Середньоденна (по робочим дням):'",КодЯзыкаПечать);
	...
	ИначеЕсли ВидУчетаВремениДляСредней = Перечисления.ВидыУчетаВремениДляСредней.ПоКалендарнымДням Тогда
		ОбластьШапка2.Параметры.СредняяТекст = НСтр("ru='Среднедневная (по календарным дням с вычетом праздников):';uk='Середньоденна (по календарним дням з вирахуванням свят):'",КодЯзыкаПечать);
	ИначеЕсли ВидУчетаВремениДляСредней = Перечисления.ВидыУчетаВремениДляСредней.ПоКалендарнымДням Тогда
		ОбластьШапка2.Параметры.СредняяТекст = НСтр("ru='Среднедневная (по календарным дням без вычета праздников):';uk='Середньоденна (по календарним дням без вирахування свят):'",КодЯзыкаПечать);
	КонецЕсли;
