...
Функция ПолучитьСведенияОСдалПринял(ДокументОбъект) Экспорт
	...
	СдалДолжностьФИО 	= ОбщегоНазначения.ДанныеФизЛица(Организация,ДокументОбъект.Сдал,ДокументОбъект.Дата); 
	ПолучилДолжностьФИО = ОбщегоНазначения.ДанныеФизЛица(Организация,ДокументОбъект.Принял,ДокументОбъект.Дата);
	
	СтруктураСдалПринял = Новый Структура;
	СтруктураСдалПринял.Вставить("СдалФИО",СдалДолжностьФИО.Представление);
	СтруктураСдалПринял.Вставить("СдалДолжность",СдалДолжностьФИО.Должность);
	СтруктураСдалПринял.Вставить("ПолучилФИО",ПолучилДолжностьФИО.Представление);
	СтруктураСдалПринял.Вставить("ПолучилДолжность",ПолучилДолжностьФИО.Должность);
	
	Возврат СтруктураСдалПринял;
...
Процедура ПроверитьОстаток(СтруктураШапкиДокумента, СчетУчета, Стоимость, СтоимостьНУ, Отказ, Заголовок)
	...
	Если СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 И ОстатокНУ < СтоимостьНУ Тогда
...
Процедура ПроверитьЗаполнениеШапкиРегл(РежимПроведения,СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 И СтруктураШапкиДокумента.НачислятьАмортизациюБУ И СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.Оборудование Тогда
		УправлениеВнеоборотнымиАктивами.ПроверитьСоответствиеНалоговогоНазначенияАмортизацииОбъекта(Номенклатура, СпособыОтраженияРасходовПоАмортизации, НалоговоеНазначение, Отказ, Заголовок, СтруктураШапкиДокумента.Дата);
	КонецЕсли;
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда 
		...
		Иначе // не оборудование
			...
			Для Каждого Строка Из ТаблицаПоОС Цикл
				...
				Иначе	
					НоваяСтрока.СуммаНУ     = НалоговыйУчет.ОпределитьСтоимостьНУ(СтруктураШапкиДокумента.НалоговоеНазначение, ТекСтоимостьБУ);
				КонецЕсли;	
		...
		Для Каждого Строка Из ТЗДанныеОС Цикл
			...
			Если СтруктураШапкиДокумента.ЕстьНалогНаПрибыль Тогда 
				...
				Движение.СтоимостьДляВычисленияАмортизации           = НалоговыйУчет.ОпределитьСтоимостьНУ(СтруктураШапкиДокумента.НалоговоеНазначение, Строка.СуммаНУ - Строка.СуммаНалоговогоКредитаНУ);
	...
	Если ОтражатьВБухгалтерскомУчете Тогда 
		...
		Если Дата >= ДатаНКУ2015 Тогда
			Если ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.ОбъектыСтроительства Тогда
				...
				Если НЕ УчетнаяПолитикаРегл.ЕстьНалогНаПрибыль Тогда
					СтоимостьНУ = 0;
				КонецЕсли;	
			КонецЕсли;	
			Если НЕ ЗначениеЗаполнено(СпособНачисленияАмортизацииНУ) Тогда
