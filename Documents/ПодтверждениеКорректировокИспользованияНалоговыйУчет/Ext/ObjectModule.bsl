...
Процедура ЗаполнитьПартииТоваровПоОстаткам() Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |	КорректировкиНалоговыйУчетОстатки.Номенклатура.СтавкаНДС КАК СтавкаНДС,
...
Функция ПодготовитьТаблицуПартииТоваров(РезультатЗапросаПоПартиямТоваров, СтруктураШапкиДокумента)
	...
	ТаблицаПартииТоваров.Колонки.Добавить("ВидКорректировкиНалоговогоКредита");
	...
	Для каждого СтрокаТаблицы Из ТаблицаПартииТоваров Цикл
		...
		СтрокаТаблицы.БазаНДС  = СтрокаТаблицы.СуммаОбязательствНДС * 100 / Ценообразование.ПолучитьСтавкуНДС(СтрокаТаблицы.СтавкаНДС);
		...
		СтрокаТаблицы.ВидКорректировкиНалоговогоКредита = НалоговыйУчетПовтИсп.ВидКорректировкиНалоговогоКредита(СтрокаТаблицы.НалоговоеНазначение, СтрокаТаблицы.НалоговоеНазначениеПоФакту);
...
Функция ПодготовитьТаблицуСтроительствоОбъектовОсновныхСредств(РезультатЗапросаПоСтроительствуОбъектовОсновныхСредств, СтруктураШапкиДокумента)
	...
	ТаблицаСтроительствоОбъектовОсновныхСредств.Колонки.Добавить("ВидКорректировкиНалоговогоКредита");
...
Процедура ДвиженияПоРегистрамНалоговогоУчета(РежимПроведения, СтруктураШапкиДокумента, 
	...
	// или документом Приложение2КНалоговойНакладной (Вид операции - Условная продажа (возврат)
	...
	Пока Сч < ТаблицаКопия.Количество() Цикл
		...
		Иначе
			Если Строка.ВидКорректировкиНалоговогоКредита = Перечисления.ВидыКорректировокНалоговогоКредита.ВосстановлениеПраваНаНалоговыйКредит Тогда
				...
				Строка.СуммаОбязательствНДС = - Строка.СуммаОбязательствНДС;
			Иначе	
	...
	ТаблицаКопия.Свернуть("НалоговоеНазначениеПоФакту, СтавкаНДС, СобытиеНДС","СуммаНДС, БазаНДС");
...
Процедура ПроводкиПоНДС(СтруктураШапкиДокумента, ТаблицаПоОбъектам, Отказ, Заголовок)
	...
	ТаблицаКопия.Свернуть("ВидКорректировкиНалоговогоКредита, МетодКорректировкиНалоговогоКредита, ВидНалоговойДеятельностиПоФакту","НДСКредит, НДСКредитПоФактуРекомендуемый, СуммаКорректировкиНДС, СуммаОбязательствНДС");
		
	Для Каждого СтрокаТаблицы Из ТаблицаКопия Цикл
		
		СуммаУсловнойПродажи       = 0;
		СуммаСторноУсловнойПродажи = 0;
		СуммаНаЗатраты             = 0;
		...
		Если СтрокаТаблицы.МетодКорректировкиНалоговогоКредита = Перечисления.МетодыКорректировкиНалоговогоКредита.НаНалоговыеОбязательства Тогда
			Если СтрокаТаблицы.ВидКорректировкиНалоговогоКредита = Перечисления.ВидыКорректировокНалоговогоКредита.ПотеряПраваНаНалоговыйКредит Тогда
				// условная продажа
				СуммаУсловнойПродажи = СтрокаТаблицы.СуммаОбязательствНДС;
			ИначеЕсли СтрокаТаблицы.ВидКорректировкиНалоговогоКредита = Перечисления.ВидыКорректировокНалоговогоКредита.ВосстановлениеПраваНаНалоговыйКредит Тогда 
				// сторно ("Возврат") условной продажи
		        СуммаСторноУсловнойПродажи = СтрокаТаблицы.СуммаКорректировкиНДС;
				СуммаНаЗатраты = СтрокаТаблицы.НДСКредитПоФактуРекомендуемый - СуммаСторноУсловнойПродажи;
			КонецЕсли;	
		ИначеЕсли СтрокаТаблицы.МетодКорректировкиНалоговогоКредита = Перечисления.МетодыКорректировкиНалоговогоКредита.НаНалоговыйКредит Тогда
			// исправление ошибок
			// проводок по НДС здесь не делаем, 6443 закроет ЗаписьКнигиПриобретения с видом операции "Исправление ошибок"
			// но разницу между скорректированой стоимостью запаса и фактической суммой корректировки НДС относим на затраты
			СуммаНаЗатраты = СтрокаТаблицы.НДСКредитПоФактуРекомендуемый - СтрокаТаблицы.СуммаКорректировкиНДС;
		ИначеЕсли СтрокаТаблицы.МетодКорректировкиНалоговогоКредита = Перечисления.МетодыКорректировкиНалоговогоКредита.НеКорректировать Тогда
			...
			СуммаНаЗатраты = СтрокаТаблицы.СуммаКорректировкиНДС;
		КонецЕсли;	
				
		Если СуммаУсловнойПродажи <> 0 Тогда
			// проводка по условной продаже 
			...
			Проводка.Сумма                      = СуммаУсловнойПродажи;
			...
			Если СтрокаТаблицы.ВидНалоговойДеятельностиПоФакту = Справочники.ВидыНалоговойДеятельности.НеОблагаемая Тогда
				// При корректировке налогового назначения на нехозяйственное проводка Дт Затраты Кт 6435 в любом случае должна формироваться по нехоз. назначению. 
			    // Нет права признать такие расходы в НУ. 
				// Налоговое назначение, которое указано на закладке Счета учета, 
				// действует только при изменении налогового назначения на хозяйственное – в этом случае право на признание расходов в НУ спорное.
			...
			КонецЕсли; 
			Проводка.СуммаНУДт = НалоговыйУчет.ОпределитьСтоимостьНУ(Проводка.НалоговоеНазначениеДт, СуммаУсловнойПродажи, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
			
			Проводка.Содержание					= "Корректировка налогового кредита по НДС (условная продажа)";
			
		КонецЕсли;
		
		Если СуммаСторноУсловнойПродажи <> 0 Тогда
			// проводка по сторно ("Возврату") условной продажи типа 6435 6443 +СуммаНДС
			...
			Проводка.Сумма                      = СуммаСторноУсловнойПродажи;
			...
			Проводка.Содержание					= "Корректировка налогового кредита по НДС (условная продажа сторно)";
			
		КонецЕсли;	
		
		Если СуммаНаЗатраты <> 0 Тогда
			...
			Проводка.НалоговоеНазначениеДт = СтруктураШапкиДокумента.НалоговоеНазначениеДоходовИЗатрат;
			Проводка.СуммаНУДт = НалоговыйУчет.ОпределитьСтоимостьНУ(Проводка.НалоговоеНазначениеДт, СуммаНаЗатраты, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
