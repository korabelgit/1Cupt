...
Процедура ПроверитьНалоговыеНазначения(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если СтруктураШапкиДокумента.Дата >= глЗначениеПеременной("ДатаНКУ2015") Тогда
		...
		Для Каждого СтрокаТЧ Из Затраты Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТЧ.СчетЗатрат) 
				 И БухгалтерскийУчетПовтИсп.УчетПоНалоговымНазначениямНДССчет(СтрокаТЧ.СчетЗатрат) 
				 И НЕ ЗначениеЗаполнено(СтрокаТЧ.НалоговоеНазначениеДоходовИЗатрат) 
				 Тогда
				 
				СтрокаСообщения = "В табличной части """ + "Затраты" + """ в строке " + СтрокаТЧ.НомерСтроки + 
				" не указано налоговое назначение!";
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщения, Отказ, Заголовок);
				
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СтрокаТЧ.СчетЗатратПолучатель) 
				 И БухгалтерскийУчетПовтИсп.УчетПоНалоговымНазначениямНДССчет(СтрокаТЧ.СчетЗатратПолучатель) 
				 И НЕ ЗначениеЗаполнено(СтрокаТЧ.НалоговоеНазначениеДоходовИЗатратПолучатель) 
				 Тогда
				 
				СтрокаСообщения = "В табличной части """ + "Получатели" + """ в строке " + СтрокаТЧ.НомерСтроки + 
				" не указано налоговое назначение - получатель!";
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщения, Отказ, Заголовок);
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоЗатратам, Отказ, Заголовок)
	...
	Для Каждого СтрокаТабличнойЧасти Из ТаблицаПоЗатратам Цикл
		...
		НоваяПроводка.СуммаНУДт = НалоговыйУчет.ОпределитьСтоимостьНУ(НоваяПроводка.НалоговоеНазначениеДт, СтрокаТабличнойЧасти.СуммаНал, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
		...
		НоваяПроводка.СуммаНУКт = НалоговыйУчет.ОпределитьСтоимостьНУ(НоваяПроводка.НалоговоеНазначениеКт, СтрокаТабличнойЧасти.СуммаНал, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
