...
Процедура ЗаполнитьПоКомиссионнойТорговлеНалоговыйКодекс() Экспорт
	...
	Если    ДоговорКонтрагента.ВедениеВзаиморасчетовРегл = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоЗаказам
		ИЛИ ДоговорКонтрагента.ВедениеВзаиморасчетовРегл = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоСчетам Тогда
		Запрос.УстановитьПараметр("Сделка", Сделка);
		УстановитьФильтрПоСделке= Истина;
	Иначе
		УстановитьФильтрПоСделке = Ложь
	КонецЕсли;
	Запрос.УстановитьПараметр("Курс", 				Данные.Курс/Данные.Кратность);
	...
	|ГДЕ
	|	  ОжидаемыйИПодтвержденныйНДСПриобретений.Регистратор 		 = &ДокументОснование
	|" + ?(УстановитьФильтрПоСделке, " И Сделка = &Сделка", "");
	...
	Иначе
		...
		Если НалоговыйУчет.ЕстьНалоговыйКредит(СтрокаТЧ) Тогда
			...
			Если    СтрокаТЧ.СтавкаНДС = Перечисления.СтавкиНДС.НДС0 
				...
				ИначеЕсли ВидОперации = Перечисления.ВидыОперацийЗаписьКнигиПриобретения.ВосстановлениеНалоговогоКредита Тогда
			...
			Иначе
				...
				Если    ВидОперации = Перечисления.ВидыОперацийЗаписьКнигиПриобретения.НалоговаяНакладная
					ИЛИ ВидОперации = Перечисления.ВидыОперацийЗаписьКнигиПриобретения.ИсправлениеОшибки
					ИЛИ ВидОперации = Перечисления.ВидыОперацийЗаписьКнигиПриобретения.ВосстановлениеНалоговогоКредита
					ИЛИ ВидОперации = Перечисления.ВидыОперацийЗаписьКнигиПриобретения.ТоварныйЧек Тогда
				...
				Если    СтрокаТЧ.СтавкаНДС = Перечисления.СтавкиНДС.НДС0 
					...
					Если    ВидОперации = Перечисления.ВидыОперацийЗаписьКнигиПриобретения.НалоговаяНакладная
						...
						ИначеЕсли ВидОперации = Перечисления.ВидыОперацийЗаписьКнигиПриобретения.РасчетКорректировкиВозврат
							...
							Если Дата < глЗначениеПеременной("ДатаВступленияВСилуПриказа957") Тогда
						...
						Иначе
							Если СтрокаТЧ.Амортизируется И Дата > '20160401'Тогда
								Возврат  СтатьиДеклараций.НДС_НКПокупкаСНДСвВРОблагПриобрОФ;
							Иначе
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ВидОперации = Перечисления.ВидыОперацийЗаписьКнигиПриобретения.ИсправлениеОшибки
 	 ИЛИ ВидОперации = Перечисления.ВидыОперацийЗаписьКнигиПриобретения.ВосстановлениеНалоговогоКредита Тогда
	...
	Если Не Отказ И СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийЗаписьКнигиПриобретения.РаботыОтНерезидента Тогда
	...
	Если ВидОперации = Перечисления.ВидыОперацийЗаписьКнигиПриобретения.ВосстановлениеНалоговогоКредита 
		И НЕ (СуммаДокумента = 0 И СуммаНДСДокумента = 0) Тогда
	
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru = 'Для данного вида операции сумма документа и сумма НДС должна быть нулевой!'; uk = 'Для даного виду операції сума документа та ПДВ повинна дорівнювати нулю!'"), Отказ, Заголовок);
			...
			Если  Строка.ПропорциональныйНДС Тогда
				...
				Если НЕ Строка.СтавкаНДС = Перечисления.СтавкиНДС.НДС20
				   ...
				   Тогда
					...
					СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("В строке ""%1"" указан признак пропорционального НДС! Ставка НДС в этом случае должна быть 20%(0%)!.", Строка.НомерСтроки);
				...
				ИначеЕсли Строка.ВидНалоговойДеятельности = Справочники.ВидыНалоговойДеятельности.НеОблагаемая Тогда
					...
					СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("В строке ""%1"" указан признак пропорционального НДС! В этом случае нельзя выбирать налоговое назначение с нехозяйственным видом деятельности!.", Строка.НомерСтроки);
					ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщения, Отказ, Заголовок);
...
Процедура ДвиженияПоРегистрам(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоТаре, Отказ, Заголовок);
	...
	ТаблицаДвижений.Свернуть("СделкаРегл, СчетУчетаНДС", "ПроводкиСуммаНДС, СуммаНДСУсловнойПродажи");

	Корректировка = ((ВидОперации = Перечисления.ВидыОперацийЗаписьКнигиПриобретения.РасчетКорректировкиВозврат) 
			     ИЛИ (ВидОперации = Перечисления.ВидыОперацийЗаписьКнигиПриобретения.РасчетКорректировкиКорректировка));
	...
	// с 2015 года, если дата документ и дата вх. нал. документа после 01.07.2015 - на налоговый кредит относятся все суммы вх. НДС, однако
	// если приобретение осуществляется для не облагаемых/не хоз. операций, необходимо оформить условную продажу
	Если    (НЕ Корректировка И ДатаВходящегоДокумента 				  >= глЗначениеПеременной("ДатаВступленияВСилуЗУ643") И Дата >= глЗначениеПеременной("ДатаВступленияВСилуЗУ643"))
		ИЛИ (   Корректировка И ДатаКорректируемогоВходящегоДокумента >= глЗначениеПеременной("ДатаВступленияВСилуЗУ643") И Дата >= глЗначениеПеременной("ДатаВступленияВСилуЗУ643")) Тогда
		...
		Для Каждого СтрокаТаблицы Из ТаблицаДвижений Цикл
			...
			Если    СтрокаТаблицы.СуммаНДСУсловнойПродажи <> 0 Тогда
				...
				Проводка.Содержание                 = "Налоговый кредит по операциям не обл. НДС после 07.2015";
				Проводка.НомерЖурнала               = "";
...
Процедура ДвиженияПоРегиструОжидаемыйИПодтвержденныйНДСПродаж(СтруктураШапкиДокумента, ТаблицаПоТоварам,  Отказ, Заголовок)
	...
	Если  ВидОперации <> Перечисления.ВидыОперацийЗаписьКнигиПриобретения.НалоговаяНакладная 
		И ВидОперации <> Перечисления.ВидыОперацийЗаписьКнигиПриобретения.ТоварныйЧек 
		И ВидОперации <> Перечисления.ВидыОперацийЗаписьКнигиПриобретения.РасчетКорректировкиВозврат
		И ВидОперации <> Перечисления.ВидыОперацийЗаписьКнигиПриобретения.РаботыОтНерезидентаПрошлогоПериода
		И ВидОперации <> Перечисления.ВидыОперацийЗаписьКнигиПриобретения.РасчетКорректировкиКорректировка
		И ВидОперации <> Перечисления.ВидыОперацийЗаписьКнигиПриобретения.ИсправлениеОшибки		Тогда
	...
	ТаблицаКопия.Свернуть("СделкаРегл, СтавкаНДС, НалоговоеНазначение", 
				  "СуммаВал, СуммаНДСУсловнойПродажи");
	ТаблицаКопия.Колонки.СделкаРегл.Имя = "Сделка";
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПодтверждениеКорректировокИспользованияНалоговыйУчет") Тогда 
	  ...
	  ВидОперации = Перечисления.ВидыОперацийЗаписьКнигиПриобретения.ВосстановлениеНалоговогоКредита;	
	  ВключаетсяВУточняющийРасчет = истина;
	  Возврат;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// Получим необходимые данные для проведения и проверки заполнения по табличной части "Тара".
	СтруктураПолей = Новый Структура();
	СтруктураПростыхПолей = Новый Структура();
