...
Процедура УстановитьВидимость()
	...
	ТекВключаетсяВУточняющийРасчет = Объект.ВключаетсяВУточняющийРасчет;
	Элементы.УточняемыйПериод.Видимость = ТекВключаетсяВУточняющийРасчет ;

	ТекДатаВключенияВЕдиныйРеестрНалоговыхНакладных = Объект.ДатаВключенияВЕдиныйРеестрНалоговыхНакладных;
	ТекДатаПолучения = ?(ЗначениеЗаполнено(ТекДатаВключенияВЕдиныйРеестрНалоговыхНакладных) И ТекДатаВключенияВЕдиныйРеестрНалоговыхНакладных > Объект.Дата, ТекДатаВключенияВЕдиныйРеестрНалоговыхНакладных, Объект.Дата);
	
	ТекДатаПолучения = Формат(ТекДатаПолучения, "ДФ=dd.MM.yyyy");
	Элементы.НадписьДатаПолучения.Заголовок = "Дата получения, отображаемая в реестре: " + ТекДатаПолучения;
	
	ТекВключатьВПриложение8Декларации =  Объект.ВключатьВПриложение8Декларации;
	Элементы.Жалоба.Видимость											= НЕ ТекВключаетсяВУточняющийРасчет;
	Элементы.СуммаПриложение8Декларации.Видимость						= ТекВключатьВПриложение8Декларации > 0;
	Элементы.СуммаНДСПриложение8Декларации.Видимость					= ТекВключатьВПриложение8Декларации > 0;
	Элементы.КонпкаРасчитатьСуммыПриложение8.Видимость 					= ТекВключатьВПриложение8Декларации > 0;
	Элементы.ОписаниеНарушенияПриложение8Декларации.Видимость			= ТекВключатьВПриложение8Декларации > 0;
	Элементы.ПараметрыДокОтказаПриложение8Декларации.Видимость 			= ТекВключатьВПриложение8Декларации = 1;
	
	ЗаголовокНадписиОписанияНарушения = "";
	Если ТекВключатьВПриложение8Декларации = 1 Тогда
	...
	ИначеЕсли ТекВключатьВПриложение8Декларации = 2 Тогда
	...
	Элементы.ОписаниеНарушенияПриложение8Декларации.Заголовок = ЗаголовокНадписиОписанияНарушения;
...
Процедура УстановитьВидимостьНаСервере()
	Элементы.НадписьДатаПолучения.Видимость 	 = Объект.Дата <  глЗначениеПеременной("ДатаВступленияВСилуПриказа10");
	Элементы.ДатаПолучения.Видимость 			 = Объект.Дата >= глЗначениеПеременной("ДатаВступленияВСилуПриказа10");
...
&НаКлиенте
Процедура РасчитатьСуммыПриложение8(Команда)
	РасчитатьСуммыДляПриложения8();
КонецПроцедуры
...
Процедура РасчитатьСуммыДляПриложения8()
	...
	// в тексте таблиц приложения 8 требуется указать сумму входящей налоговой накладной (поставки)
	// которая не просто предназначена для облагаемой деятельности (это учитывается в конфигурации)
	// но только ее часть, которая предназначена для продажи по ставке 20%, 0%, и отдельно по ставке 7%
	// Поэтому из налогового назначения в строке документа
	// определить эту сумму невозможно. Кроме того, не понятно что делать с товарами, которые 
	// пропорционально облагаются НДС - включать ли эти суммы пропорционально или в полном объеме?...
	// Таким образом, суммы которые будут показаны в таблицах Приложения 8 
	// пользователь корректирует вручную по своему разумению.
	
	СуммаПриложение8Декларации = 0;
	СуммаНДСПриложение8Декларации = 0;
	
	Сумма7Приложение8Декларации = 0;
	СуммаНДС7Приложение8Декларации = 0;
	
	Для каждого СтрокаТЧ Из Объект.Товары Цикл
	
		Если  СтрокаТЧ.НалоговоеНазначение.ВидДеятельностиНДС = Перечисления.ВидыДеятельностиНДС.Облагаемая 
			ИЛИ 
			  СтрокаТЧ.НалоговоеНазначение.ВидДеятельностиНДС = Перечисления.ВидыДеятельностиНДС.ПропорциональноОблагаемая Тогда
		
			Если СтрокаТЧ.СтавкаНДС = Перечисления.СтавкиНДС.НДС7 И Объект.Дата >= глЗначениеПеременной("ДатаВступленияВСилуПриказа957") Тогда
				Сумма7Приложение8Декларации 	= Сумма7Приложение8Декларации 	 + СтрокаТЧ.Сумма; 
				СуммаНДС7Приложение8Декларации 	= СуммаНДС7Приложение8Декларации + СтрокаТЧ.СуммаНДС;
			Иначе	
	...
	Объект.Сумма7Приложение8Декларации    = Сумма7Приложение8Декларации;
	Объект.СуммаНДС7Приложение8Декларации = СуммаНДС7Приложение8Декларации;
	
КонецПроцедуры // РасчитатьСуммыДляПриложения8
