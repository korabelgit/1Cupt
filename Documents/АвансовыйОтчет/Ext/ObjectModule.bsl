...
Функция ПечатьАвансовогоОтчета()
	...
	ПараметрыНеОбязательные = Новый Структура("СуммаДокументаКраткоРасписка",СтрЗаменить(ОбщегоНазначения.ФорматСумм(Шапка.СуммаДокумента, ВалютаДокумента),"грн","грн,коп"));
...
Процедура ЗаполнитьТоварыПоОснованию(ДокументОснование) Экспорт
	...
	ДоговорКонтрагента = ДокументОснование.ЗаказПоставщику.ДоговорКонтрагента;
...
Процедура ПодготовитьТаблицуПрочиеРегл(ТаблицаПрочие, СтруктураШапкиДокумента, ПогрешностиОкругления)
	...
	Если СтруктураШапкиДокумента.Дата >= глЗначениеПеременной("ДатаВступленияВСилуЗакона4014") Тогда
	...
	Для Каждого СтрокаТаблицы Из ТаблицаПрочие Цикл
		...
		Если СтрокаТаблицы.ЭтоСуточные Тогда
			СтрокаТаблицы.СтоимостьНУ = НалоговыйУчет.ОпределитьСтоимостьНУ(СтрокаТаблицы.НалоговоеНазначениеДоходовИЗатрат, СтрокаТаблицы.СуммаНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
		КонецЕсли; 
...
Процедура ПроверитьЗаполнениеТабличнойЧастиПрочее(ТаблицаПоПрочее, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Для Каждого СтрокаАвансовПрочее Из Прочее Цикл
	    ...
	    СтрокаНачалаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строке номер ""%1"" табличной части ""%2"": ';uk='У рядку номер ""%1"" табличної частини ""%2"": '"), СокрЛП(СтрокаАвансовПрочее.НомерСтроки), ПредставлениеПрочее);
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента,
	...
	Если СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015
		...
		ЗапросПоЕдинщикам.Текст = "ВЫБРАТЬ
		                          ...
		                          |			(ВЫБРАТЬ
		                          |				Документ.АвансовыйОтчет.Товары.Поставщик
		                          |			ИЗ
		                          ...
		                          |		
		                          |			ВЫБРАТЬ
		                          |				Документ.АвансовыйОтчет.Прочее.Поставщик
		                          |			ИЗ
		...
		Если КонтрагентыЕдинщики.Количество() > 0 Тогда
			
			Для каждого СтрокаТаблицы Из ТаблицаПоТоварам Цикл
			
				// проводки будем формировать только для контрагентов-единщиков
				Если НЕ КонтрагентыЕдинщики.Найти(СтрокаТаблицы.Контрагент) = Неопределено Тогда
					...
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Контрагенты",        СтрокаТаблицы.Контрагент  , Истина, Заголовок);
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Договоры",           СтрокаТаблицы.ДоговорКонтрагента  , Истина, Заголовок);
					...
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Договоры",           СтрокаТаблицы.ДоговорКонтрагента  , Истина, Заголовок);
