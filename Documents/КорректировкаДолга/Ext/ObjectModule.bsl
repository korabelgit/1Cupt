...
Процедура ПроверитьЗаполнениеТабличнойЧастиСуммыДолга(ТаблицаПоСуммамДолга, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Для каждого СтрокаТаблицы Из ТаблицаПоСуммамДолга Цикл
		...
		СтрокаНачалаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("В строке номер ""%1"" табличной части ""Суммы долга"": ", СокрЛП(СтрокаТаблицы.НомерСтроки));
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоСуммамДолга,
	...
	Если ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.СписаниеЗадолженности Тогда
		...
		Если СтруктураШапкиДокумента.ЕстьНалогНаПрибыль Тогда	
			...
			// суммы по списанию дебиторской задолженности в НУ не должны фигурировать
		...
		Если СтруктураШапкиДокумента.ЕстьНалогНаПрибыль Тогда	
			...
			// суммы по списанию кредиторской задолженности в НУ не должны фигурировать
		КонецЕсли;
	...
	Иначе
		...
		КонецЕсли;
		Если НЕ ТаблицаРасчетыСПоставщикамиДебиторскаяЗадолженность.Колонки.Найти("НалоговоеНазначение") = Неопределено Тогда
			ТаблицаРасчетыСПоставщикамиДебиторскаяЗадолженность.Колонки.Удалить("НалоговоеНазначение")
		КонецЕсли;
		Если НЕ ТаблицаРасчетыСПокупателямиКредиторскаяЗадолженность.Колонки.Найти("НалоговоеНазначение") = Неопределено Тогда
			ТаблицаРасчетыСПокупателямиКредиторскаяЗадолженность.Колонки.Удалить("НалоговоеНазначение")
		КонецЕсли;
		Если НЕ ТаблицаРасчетыСПоставщикамиКредиторскаяЗадолженность.Колонки.Найти("НалоговоеНазначение") = Неопределено Тогда
	...
	Если ТаблицаРасчетыСПокупателямиДебиторскаяЗадолженность.Количество() > 0 Тогда
		...
		Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.ПроведениеВзаимозачета 
			...
			КоррСчет = ВспомогательныйСчет;
		ИначеЕсли СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 
				И СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.СписаниеЗадолженности
			    И СтруктураШапкиДокумента.СписыватьИзРезерваСомнительныхДолгов Тогда
			КоррСчет = СтруктураШапкиДокумента.СчетРезерваСомнительныхДолгов;
		КонецЕсли;
		СтруктураШапкиДокумента.Вставить("Контрагент", КонтрагентДебитор);
	...
	Если СтруктураШапкиДокумента.ЕстьЕдиныйНалог И СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.СписаниеЗадолженности Тогда
		...
		Если (СтруктураШапкиДокумента.Дата < ДатаНачалаДействияЗУ71) ИЛИ (СтруктураШапкиДокумента.ЕстьНДС) Тогда
			ДвиженияПоЕдиномуНалогу(СтруктураШапкиДокумента, ТаблицаПоСуммамДолга);
		КонецЕсли; 
	КонецЕсли;
	
КонецПроцедуры

Процедура ДвиженияПоЕдиномуНалогу(СтруктураШапкиДокумента, ТаблицаПоСуммамДолга)
	
	СуммаЕН = 0;
	Для каждого СтрокаТЗ из ТаблицаПоСуммамДолга Цикл
		Если СтрокаТЗ.ВидЗадолженности = Перечисления.ВидыЗадолженности.Кредиторская Тогда
			СуммаЕН = СуммаЕН + СтрокаТЗ.СуммаСНДСРегл;
			Если СтруктураШапкиДокумента.ЕстьНДС Тогда
				СуммаЕН = СуммаЕН - СтрокаТЗ.СуммаНДСРегл;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПодготовитьПараметрыУчетнойПолитикиРегл(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ОтражатьВБухгалтерскомУчете Тогда
		...
		Иначе
			СтруктураШапкиДокумента.Вставить("ОпределятьАвансыПриПроведенииДокумента", УчетнаяПолитикаРегл.ОпределятьАвансыПриПроведенииДокумента);
		КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Дата >= глЗначениеПеременной("ДатаНКУ2015") 
		 И НЕ НалоговоеНазначениеДоходовИЗатрат = Справочники.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка()
		 И НЕ БухгалтерскийУчетПовтИсп.УчетПоНалоговымНазначениямНДССчет(СчетДт) 
		 И НЕ НалоговыйУчетПовтИсп.ПолучитьГруппуНалоговогоНазначения(НалоговоеНазначениеДоходовИЗатрат) = Перечисления.ГруппыНалоговыхНазначений.НалоговыеНазначенияАктивовИВзаиморасчетовПоНДС
