...
Функция ПечатьОтчетаПоЗарплате() Экспорт
	...
	индекс = 1;
	Для Каждого СтрокаВедомость ИЗ ПлатежныеВедомости Цикл
		
		ТекстПлатежнаяВедомость = ТекстПлатежнаяВедомость + " № "+ ОбщегоНазначения.ПолучитьНомерНаПечать(СтрокаВедомость.Ведомость) + НСтр("ru=' от ';uk=' від '", КодЯзыкаПечать) + Формат(СтрокаВедомость.Ведомость.Дата, "ДФ=dd.MM.yyyy");
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Если ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеЗП Тогда
		Возврат Новый Структура("ПлатежноеПоручение, ПлатежноеПоручениеДо2009, ПлатежноеПоручениеВВалюте, ПеречислениеЗП", "Платежное поручение", "Платежное поручение, старая версия ", "Платежное поручение в валюте", "Ведомость перечисления зарплаты");
    Иначе
