...
Функция ПечатьПКО (Вариант2009 = Истина)
	...
	Если ВидОперации = Перечисления.ВидыОперацийПКО.ПриходДенежныхСредствРозничнаяВыручка Тогда
		...
		СуммаНДС = ?(ТаблицаВыручки.Итог("Сумма") = 0, 0, ТаблицаВыручки.Итог("СуммаНДС") / ТаблицаВыручки.Итог("Сумма") * (ТаблицаВыручки.Итог("Сумма") - СуммаОплатыПлатежнымиКартами - СуммаОплатыБанковскимиКредитами));
...
Функция ПечатьПКОвБланк()
	...
	СуммаПрописью = СтрЗаменить( СокрЛП(ЧислоПрописью(Шапка.Сумма, "Л = "+КодЯзыкаПечать+"; НП = Ложь; НД = Ложь", НСтр("ru='гривна, гривны, гривен, ж, копейка, копейки, копеек, ж, 2';uk='гривня, гривні, гривень, ж, копійка, копійки, копійок, ж, 2'"))),
	                             Символы.НПП, " ");
...
Процедура ЗаполнитьПоРозничнойВыручкеУпр(СтрокаПлатеж)
	...
	|	ЕСТЬNULL(ОборотыРегл.СуммаАкцизногоНалогаОборот, 0)	КАК СуммаАкцизногоНалогаОборот,
	|	ЕСТЬNULL(ОстаткиРегл.СуммаОстаток, 0)		КАК СуммаОстаток,
	|	ЕСТЬNULL(ОстаткиРегл.СуммаНДСОстаток, 0)	КАК СуммаНДСОстаток,
	|	ЕСТЬNULL(ОстаткиРегл.СуммаАкцизногоНалогаОстаток, 0)	КАК СуммаАкцизногоНалогаОстаток,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Сумма > 0 Тогда
			...
			СтрокаВыручки.СуммаАкцизногоНалога = Мин(Макс(Выборка.СуммаАкцизногоНалогаОборот, - Выборка.СуммаАкцизногоНалогаОборот), Макс(Выборка.СуммаАкцизногоНалогаОстаток, -Выборка.СуммаАкцизногоНалогаОстаток));
...
Процедура ДвиженияПоРегистрамНалоговогоУчетаРозничныеПродажи(Режим, Отказ, Заголовок,СтруктураШапкиДокумента);
	...
	Для каждого СтрокаВыручки Из ТаблицаВыручки Цикл
		...
		СтрокаВыручки.БазаНДС = СтрокаВыручки.Сумма - СтрокаВыручки.СуммаНДС - СтрокаВыручки.СуммаАкцизногоНалога;
...
Процедура ПроверитьЗаполнениеДокументаРегл(Отказ, Заголовок, СтруктураШапкиДокумента)
	...
	Если ОтражатьВБухгалтерскомУчете Тогда
		...
		Если ВидОперации = Перечисления.ВидыОперацийПКО.ПолучениеНаличныхДенежныхСредствВБанке И СчетУчетаРасчетовСКонтрагентом.Пустая() Тогда
			СтрокаСообщения = "Не указан счет учета безналичных денежных средств!";
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщения, Отказ, Заголовок);
		КонецЕсли;
