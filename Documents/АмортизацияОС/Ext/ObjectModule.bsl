...
Процедура ПодготовитьПараметрыУчетнойПолитикиРегл(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ОтражатьВБухгалтерскомУчете Тогда
		...
		Если НЕ Отказ Тогда
			...
			СтруктураШапкиДокумента.Вставить("ВключатьСуммуДооценокОСВСоставНП", мУчетнаяПолитикаРегл.ВключатьСуммуДооценокОСВСоставНП);
...
Процедура ДвиженияПоРегистрамРегл(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если Не СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете И НЕ ОтражатьВНалоговомУчете Тогда
		Возврат;
	КонецЕслИ;
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете ИЛИ ОтражатьВНалоговомУчете Тогда

		ОтражатьВНУ = Истина;
		Если ДатаДок >= ДатаНКУ2015 Тогда 
			ОтражатьВНУ = ОтражатьВНалоговомУчете;
		КонецЕсли;	
		ТабАмортизации  = УправлениеВнеоборотнымиАктивами.РасчетАмортизацииРегл(ДатаДок,СтруктураШапкиДокумента.Организация, , СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете, ОтражатьВНУ, , Отказ);
