...
Функция ПечатьЗаказа()
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, НСтр("ru='Заказ на производство';uk='Замовлення на виробництво'", КодЯзыкаПечать), КодЯзыкаПечать);
	...
	Если НЕ РезультатЗапросаПродукция.Пустой() Тогда
		...
		Область.Параметры.ТекстРаздела = НСтр("ru='Продукция (услуги)';uk='Продукція (послуги)'", КодЯзыкаПечать);
	...
	Если НЕ РезультатЗапросаМат.Пустой() Тогда
		...
		Область = Макет.ПолучитьОбласть("ЗаголовокРаздела");
		Область.Параметры.ТекстРаздела = НСтр("ru='Материалы';uk='Матеріали'", КодЯзыкаПечать);
		ТабДокумент.Вывести(Область);
	...
	ОбластьМакета.Параметры.ИтоговаяСтрока = НСтр("ru='Всего наименований продукции: ';uk='Всього найменувань продукції: '", КодЯзыкаПечать) + ВсегоНаименований;
	ТабДокумент.Вывести(ОбластьМакета);
...
Процедура ЗаполнитьПродукциюПоЗаказуНаПроизводство(ПроизвЗаказ) Экспорт
	...
	ТаблицаПродукция.Свернуть(
		"Номенклатура, ХарактеристикаНоменклатуры, ЕдиницаИзмерения, ЕдиницаИзмеренияМест, Коэффициент, Спецификация, ВидВоспроизводства, Заказ, КлючСвязи",
