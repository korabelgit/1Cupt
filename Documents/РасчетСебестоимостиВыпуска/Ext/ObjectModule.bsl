...
Процедура ПодготовитьСтруктуруШапкиДокумента(
	...
	Иначе
		Если ОтражатьВБухгалтерскомУчете Тогда
			Если мНачДата < глЗначениеПеременной("ДатаНачалаРаспределенияОПЗвНУ") Тогда
				ДиректКостингОбщепроизводственныеЗатраты = мУчетнаяПолитика["НеРаспределятьОПЗнаСебестоимостьПродукции"];
				ДиректКостингОбщепроизводственныеЗатратыНУ = Истина;
			Иначе
				ДиректКостингОбщепроизводственныеЗатраты = Ложь;
				Если мУчетнаяПолитика["НеРаспределятьОПЗнаСебестоимостьПродукции"] Тогда
					ОбщегоНазначения.Сообщение("В учетной политике установлен флаг ""Не распределять в БУ общепроизводственные затраты на себестоимость продукции"". Значение флага с " + глЗначениеПеременной("ДатаНачалаРаспределенияОПЗвНУ") + " не учитывается, ОПЗ в БУ распределяются всегда!");
				КонецЕсли; 
				ДиректКостингОбщепроизводственныеЗатратыНУ = Ложь;
			КонецЕсли;	
			СтруктураШапкиДокумента.Вставить("ДиректКостингОбщепроизводственныеЗатратыНУ", ДиректКостингОбщепроизводственныеЗатратыНУ);
		Иначе
	...
	Если ОтражатьВБухгалтерскомУчете Тогда
		Если мНачДата >= глЗначениеПеременной("ДатаНачалаРаспределенияОПЗвНУ") Тогда
			Если НЕ мУчетнаяПолитика["ЗаполнятьСтатьиИПодразделенияВПроводкахПриРаспределенииЗатрат"] Тогда
				ОбщегоНазначения.Сообщение("В учетной политике значение параметра ""Проводки бухгалтерского учета по распределению затрат формируются"" установлено в ""Сводно"". При таком значении параметра с " + глЗначениеПеременной("ДатаНачалаРаспределенияОПЗвНУ") + " скорей всего будет неправильно заполнена Декларация по прибыли, крайне рекомендуется установить значение параметра в ""Детально""!");
			КонецЕсли; 
			СтруктураШапкиДокумента.Вставить("НалоговоеНазначениеНераспределенныхПостоянныхОПЗ", мУчетнаяПолитика.НалоговоеНазначениеНераспределенныхПостоянныхОПЗ);
			Если НЕ ЗначениеЗаполнено(СтруктураШапкиДокумента.НалоговоеНазначениеНераспределенныхПостоянныхОПЗ) Тогда
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		мЕстьНалогНаПрибыльДо2015 = (мУчетнаяПолитика.ЕстьНалогНаПрибыльДо2015 = Истина);
		мЕстьНДС            = (мУчетнаяПолитика.ЕстьНДС            = Истина);
		СтруктураШапкиДокумента.Вставить("ЕстьНалогНаПрибыль", мЕстьНалогНаПрибыль);
		СтруктураШапкиДокумента.Вставить("ЕстьНалогНаПрибыльДо2015", мЕстьНалогНаПрибыльДо2015);
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		ЗаполнятьСтатьиИПодразделенияВПроводкахПриРаспределенииЗатрат = мУчетнаяПолитика.ЗаполнятьСтатьиИПодразделенияВПроводкахПриРаспределенииЗатрат;
		СтруктураШапкиДокумента.Вставить("ЗаполнятьСтатьиИПодразделенияВПроводкахПриРаспределенииЗатрат", ЗаполнятьСтатьиИПодразделенияВПроводкахПриРаспределенииЗатрат);
	Иначе
		СтруктураШапкиДокумента.Вставить("ЗаполнятьСтатьиИПодразделенияВПроводкахПриРаспределенииЗатрат", Ложь);
	КонецЕсли;
...
// Процедура выполнения действия "Распределение производственных расходов".
//
// Параметры
//  СтруктураШапкиДокумента – Структура - Реквизиты документа "Расчет себестоимости"
//
