...
Процедура ЗаполнитьТоварыПоОснованиюУпр(ДокументОснование, ТабличнаяЧасть) Экспорт
	...
	?(ТабличнаяЧасть = Продукция, " * ДокТовары.Коэффициент 
	...
	|           Остатки.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент
	|       ИНАЧЕ
	|           ЕСТЬNULL(ЦеныНоменклатурыКонтрагентовСрезПоследних.ЕдиницаИзмерения.Коэффициент, 1) 
	|       КОНЕЦ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТабличнаяЧасть = Продукция Тогда
			...
			Количество = Мин(Выборка.КоличествоОстатокКомпании, Выборка.КоличествоПоСерии);
			
			СтрокаТабличнойЧасти.Номенклатура     = Выборка.Номенклатура;
			СтрокаТабличнойЧасти.Количество       = Количество * Выборка.КоэффициентЕдиницыХранения / Выборка.Коэффициент;
...
Процедура ЗаполнитьМатериалыПоЗаказу() Экспорт
	...
	|	
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ЗаказыПокупателей.ЕдиницаИзмерения.Коэффициент,0) = 0 
	|		ТОГДА 0
	|		ИНАЧЕ СУММА(ЗаказыПокупателей.КоличествоПриход - ЗаказыПокупателей.КоличествоРасход)
	|				* ЗаказыПокупателей.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент
	|				/ ЗаказыПокупателей.ЕдиницаИзмерения.Коэффициент
...
Процедура ПроверкаРеквизитов(СтруктураШапкиДокумента, Отказ, Заголовок) Экспорт
	...
	Если ИспользоватьПрочиеЗатраты И Не АвтораспределениеПрочихЗатрат Тогда
		...
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете И (СтруктураШапкиДокумента.ЕстьНДС ИЛИ СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015) И ИспользоватьРасширеннуюАналитику Тогда
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТаре, Отказ, Заголовок);
	...
	Для Каждого СтрокаТаблицы Из ТаблицаПоТаре Цикл
		...
		Проводка.СуммаНУДт               = НалоговыйУчет.ОпределитьСтоимостьНУ(Проводка.НалоговоеНазначениеДт, СтрокаТаблицы.СтоимостьНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
