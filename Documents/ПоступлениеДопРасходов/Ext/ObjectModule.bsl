...
Процедура РассчитатьПропорциональныйНДС() Экспорт
	...
	Для каждого СтрокаПропорциональногоНДС Из СтрокиПропорциональногоНДС Цикл
		...
		Если СпособРаспределения = Перечисления.СпособыРаспределенияДопРасходов.ПоВесу Тогда
			ДоляПропорциональных = ДоляПропорциональных + СтрокаПропорциональногоНДС.ЕдиницаИзмерения.Вес * СтрокаПропорциональногоНДС.Количество;
		ИначеЕсли СпособРаспределения = Перечисления.СпособыРаспределенияДопРасходов.ПоКоличеству Тогда	
		    ДоляПропорциональных = ДоляПропорциональных + СтрокаПропорциональногоНДС.Количество;
		Иначе
		    ДоляПропорциональных = ДоляПропорциональных + СтрокаПропорциональногоНДС.СуммаТовара;
		КонецЕсли;
	КонецЦикла;
	...
	СуммаНДСПропорциональноВсего = СуммаНДСПропорциональноВсего + Окр(СуммаНДС * ?(ОбщаяДоля = 0, 0, ДоляПропорциональных/ОбщаяДоля),2);
