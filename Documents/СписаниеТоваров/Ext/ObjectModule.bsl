...
Функция ПечатьСписаниеТоваров(ВидПечати = "Регл")
	...
	|	ПодразделениеЗатраты КАК Подразделение,
	|	ПодразделениеОрганизацииЗатраты КАК ПодразделениеОрганизации,
	...
	Если ОтражатьПоЗатратам Тогда
		...
		Если ВидПечати = "Регл"	Тогда
			Если ЗначениеЗаполнено(Шапка.НалоговоеНазначениеДоходовИЗатрат) Тогда
				ОбластьМакета.Параметры.ВидСубконто	= НСтр("ru='Налоговое назначение:';uk='Податкове призначення:'", КодЯзыкаПечать);
				ОбластьМакета.Параметры.Субконто	= Шапка.НалоговоеНазначениеДоходовИЗатрат;
				ТабДокумент.Вывести(ОбластьМакета);
			КонецЕсли;
		КонецЕсли;	
		
		Если ЗначениеЗаполнено(Шапка.Подразделение) Тогда
			ОбластьМакета.Параметры.ВидСубконто	= НСтр("ru='Подразделение:';uk='Підрозділ:'", КодЯзыкаПечать);
			ОбластьМакета.Параметры.Субконто	= Шапка.Подразделение;
			ТабДокумент.Вывести(ОбластьМакета);
		КонецЕсли;
		
		Если ВидПечати = "Регл"	Тогда
			Если ЗначениеЗаполнено(Шапка.ПодразделениеОрганизации) Тогда
				ОбластьМакета.Параметры.ВидСубконто	= НСтр("ru='Подразделение орган.:';uk='Підрозділ  орган.:'", КодЯзыкаПечать);
				ОбластьМакета.Параметры.Субконто	= Шапка.ПодразделениеОрганизации;
				ТабДокумент.Вывести(ОбластьМакета);
			КонецЕсли;
		КонецЕсли;	
		
		Если ЗначениеЗаполнено(Шапка.СтатьяЗатрат) Тогда
			ОбластьМакета.Параметры.ВидСубконто	= НСтр("ru='Статья затрат:';uk='Стаття витрат:'", КодЯзыкаПечать);
			ОбластьМакета.Параметры.Субконто	= Шапка.СтатьяЗатрат;
			ТабДокумент.Вывести(ОбластьМакета);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Шапка.НоменклатурнаяГруппа) Тогда
			ОбластьМакета.Параметры.ВидСубконто	= НСтр("ru='Номенклатурная группа:';uk='Номенклатурна група:'", КодЯзыкаПечать);
			ОбластьМакета.Параметры.Субконто	= Шапка.НоменклатурнаяГруппа;
			ТабДокумент.Вывести(ОбластьМакета);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Шапка.Заказ) Тогда
			ОбластьМакета.Параметры.ВидСубконто	= НСтр("ru='Заказ:';uk='Замовлення:'", КодЯзыкаПечать);
			ОбластьМакета.Параметры.Субконто	= Шапка.Заказ;
			ТабДокумент.Вывести(ОбластьМакета);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Шапка.Проект) Тогда
			ОбластьМакета.Параметры.ВидСубконто	= НСтр("ru='Проект:';uk='Проект:'", КодЯзыкаПечать);
			ОбластьМакета.Параметры.Субконто	= Шапка.Проект;
			ТабДокумент.Вывести(ОбластьМакета);
		КонецЕсли;
	...
	ОбластьДанных = Макет.ПолучитьОбласть("ШапкаТаблицы|КоличествоЦенаСумма");

	
	ОбластьТовар.Параметры.Товар = ?(Шапка.ВидОперации = Перечисления.ВидыОперацийСписаниеТоваров.ТоварыПродукция, НСтр("ru='Товар';uk='Товар'",КодЯзыкаПечать), НСтр("ru='Оборудование';uk='Устаткування'",КодЯзыкаПечать));
	...
	ОбластьДанных = Макет.ПолучитьОбласть("Строка|КоличествоЦенаСумма");
	...
	ОбластьДанных = Макет.ПолучитьОбласть("Итого|КоличествоЦенаСумма");
...
Процедура ПодготовитьТаблицуБланковРегл(ТаблицаБланков, СтруктураШапкиДокумента)
	...
	ТаблицаБланков.Колонки.Добавить("СчетЗатрат");
	ТаблицаБланков.Колонки.Добавить("НалоговоеНазначениеДоходовИЗатрат");
	ТаблицаБланков.Колонки.Добавить("ВидНалоговойДеятельностиДоходовИЗатрат");
	ТаблицаБланков.Колонки.Добавить("ВидДеятельностиНДСДоходовИЗатрат");
	
	ТаблицаБланков.ЗаполнитьЗначения(СтруктураШапкиДокумента.СчетДт, "СчетЗатрат");	
	ТаблицаБланков.ЗаполнитьЗначения(СтруктураШапкиДокумента.НалоговоеНазначениеДоходовИЗатрат, "НалоговоеНазначениеДоходовИЗатрат");
	ТаблицаБланков.ЗаполнитьЗначения(СтруктураШапкиДокумента.НалоговоеНазначениеДоходовИЗатрат.ВидНалоговойДеятельности, "ВидНалоговойДеятельностиДоходовИЗатрат");	
	ТаблицаБланков.ЗаполнитьЗначения(СтруктураШапкиДокумента.НалоговоеНазначениеДоходовИЗатрат.ВидДеятельностиНДС      , "ВидДеятельностиНДСДоходовИЗатрат");	
...
Процедура ДвиженияПоТоварамОрганизацийРегл(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоТаре, Отказ, Заголовок)
	...
	Если НЕ Отказ Тогда
		...
		ТаблицаПоТоварамКопия.Колонки.Добавить("Склад");
		
		Для Каждого СтрокаТоваров Из ТаблицаПоТоварамКопия Цикл
			
			Если ЗначениеЗаполнено(СтрокаТоваров.ДокументРезерва) 
				 И ТипЗнч(СтрокаТоваров.ДокументРезерва) = Тип("ДокументСсылка.ЗаказПокупателя") 
				 И СтрокаТоваров.ДокументРезерва.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.Переработка
		...
		ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "Организация", Организация);
		ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "Комиссионер", Неопределено);
		ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "Склад",       ЗначСклад, "ТаблицаПоТаре");
...
Процедура ЗаполнитьКолонкиРегистраСписанныеТоварыПоТоварамРегл(ТаблицаДвижений, СтруктураШапкиДокумента, ТаблицаПоТоварам)
	...
	Для каждого Строка Из ТаблицаДвижений Цикл
		...
		Строка.НалоговоеНазначениеНовое   = СтрокаТЧ.НалоговоеНазначениеДоходовИЗатрат;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	НалоговыйУчет.ОчиститьНалоговыеНазначенияЗатратВДокументеСписания(
		СтруктураШапкиДокумента, 
		ТаблицаПоТоварам
	);	
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		Если СтруктураШапкиДокумента.ИспользоватьРАУЗ И СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийСписаниеТоваров.БланкиСтрогогоУчета И Движения.Хозрасчетный.Модифицированность() Тогда
