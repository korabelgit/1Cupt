	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПокупкаПродажаВалюты.ПродажаВалюты Тогда
		...
		Если Дата >= '20120101' Тогда
			...
			ИначеЕсли СуммаРазницы < 0 Тогда
				...
				Проводка.Содержание  = "Расходы на продажу валюты";
		...
		Иначе
			//Алгоритм до Приказа Минфина 1591
			...
			Если СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 Тогда 
				Проводка.СуммаНУКт   = НалоговыйУчет.ОпределитьСтоимостьНУ(НалоговоеНазначениеДоходовИЗатрат, ?(СуммаРазницы > 0, СуммаРазницы, 0));
				Проводка.НалоговоеНазначениеКт = НалоговоеНазначениеДоходовИЗатрат;
			КонецЕсли;
			...
			Если СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 Тогда 
				Проводка.СуммаНУДт   = НалоговыйУчет.ОпределитьСтоимостьНУ(НалоговоеНазначениеДоходовИЗатрат, ?(СуммаРазницы < 0, -СуммаРазницы, 0));
				Проводка.НалоговоеНазначениеДт = НалоговоеНазначениеДоходовИЗатрат;
			КонецЕсли;
