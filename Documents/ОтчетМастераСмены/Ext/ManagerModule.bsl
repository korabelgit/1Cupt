...
Функция ПечатьОтчетаМастераСмены(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого Ссылка Из МассивОбъектов Цикл
		...
		ОбластьМакета.Параметры.ТекстЗаголовка 		= ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, НСтр("ru='Отчет мастера смены';uk='Звіт майстра зміни'", КодЯзыкаПечать), КодЯзыкаПечать);
		...
		Если Выборка.Количество() > 0 Тогда
			...
			Иначе
				Область 	  = Макет.ПолучитьОбласть("СтрокаНазначениеПараметров");
				Область.Параметры.НазначениеПараметров = НСтр("ru='Общие технологические параметры:';uk='Загальні технологічні параметри:'", КодЯзыкаПечать);
				ТабДокумент.Вывести(Область);
			КонецЕсли;
