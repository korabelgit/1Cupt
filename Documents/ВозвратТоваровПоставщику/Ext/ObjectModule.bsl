...
Процедура ПодготовитьПараметрыУчетнойПолитики(Отказ, Заголовок, СтруктураШапкиДокумента) Экспорт
	...
	Если (НЕ СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете) И (НЕ СтруктураШапкиДокумента.ОтражатьВРегламентированномУчете) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.СчетНаОплатуПоставщика") Тогда
		...
		мРеквизитыЗаполненныеПоОснованию.Вставить("УчитыватьНДС");
		мРеквизитыЗаполненныеПоОснованию.Вставить("СуммаВключаетНДС");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ОтражатьВБухгалтерскомУчете Тогда
		НалоговыйУчет.ЗаполнитьНалоговыеНазначенияВТабличныхЧастяхПередЗаписьюДокумента(
		    ?(ВидОперации = Перечисления.ВидыОперацийВозвратТоваровПоставщику.ИзПереработки, "Переработка", "Поступление"),
			Дата,
			Организация,
