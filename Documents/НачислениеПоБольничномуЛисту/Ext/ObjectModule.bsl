...
Процедура ПолучитьПериодИПрочиеПараметрыСреднейПоРаботнику() 
	...
	Если ДатаНачала < ПроведениеРасчетов.ДатаИзмененияПорядка1266() Тогда
		...
		Если ВыборкаПараметры.Следующий() Тогда
			...
			ПроведениеРасчетов.РазобратьРазностьДат(ВыборкаПараметры.ДатаСтажаЗа12Месяцев,ВыборкаПараметры.ДатаСтажа, тДень12, тМесяц12, тГод12);  
			Стаж12 = ""; 
			Если ВыборкаПараметры.СтажЗа12МесВКаленДнях < 182 Тогда

				Если Стаж = "" И ДатаНачала >= '20150101000000' Тогда 
					ОплачивалВзносыМеньше6Месяцев = Истина;
				КонецЕсли; 
				Стаж12 = Символы.ПС + "(стаж за последние 12 мес. меньше 182 дн.)";
			КонецЕсли;
			Стаж = "Стаж для больничного: "+тГод+"г."+тМесяц+"мес. "+тДень+"д." + Стаж12;
...
Функция		ПолучитьГрафикВыслугуПредел()
	...
	Если Истина Тогда
		...
		|ГДЕ
		|	СотрудникиОрганизаций.Ссылка = &парамСотрудник";
		Запрос.Текст = ТекстЗапросаВТ;
		Запрос.Выполнить();
		Запрос.УстановитьПараметр("ДатаВводаЕСВ", ПроведениеРасчетов.ДатаВводаЕСВ());
		Если ДатаНачала < ПроведениеРасчетов.ДатаВключенияВСтажОтпускаПоУходуЗаребенком() Тогда
			ТекстЗапросаВТ = "
			|ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	СУММА(РАЗНОСТЬДАТ(ОсновныеНачисленияРаботниковОрганизацийФактическийПериодДействия.ПериодДействияНачало, ОсновныеНачисленияРаботниковОрганизацийФактическийПериодДействия.ПериодДействияКонец, ДЕНЬ)+1) КАК ДнейОтпускаПоУходуЗаРебенком
			|ПОМЕСТИТЬ ВТОтпускПоУходуЗаРебенком	
			|ИЗ
			|	РегистрРасчета.ОсновныеНачисленияРаботниковОрганизаций.ФактическийПериодДействия(
			|			ВидРасчета = &ОтпускПоУходуЗаРебенком
			|				И Сотрудник = &парамСотрудник) КАК ОсновныеНачисленияРаботниковОрганизацийФактическийПериодДействия
			|ГДЕ
			|	ОсновныеНачисленияРаботниковОрганизацийФактическийПериодДействия.ПериодДействияКонец < &ДатаНачалаСобытия
			| И ОсновныеНачисленияРаботниковОрганизацийФактическийПериодДействия.ПериодДействияКонец >= &ДатаВводаЕСВ";
	
			Запрос.Текст = ТекстЗапросаВТ;
			Запрос.Выполнить();
		Иначе
			ТекстЗапросаВТ = "ВЫБРАТЬ
			|	0 КАК ДнейОтпускаПоУходуЗаРебенком
			|ПОМЕСТИТЬ ВТОтпускПоУходуЗаРебенком";
		...
		ТекстЗапросаВТ = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ВЫБОР КОГДА ЕСТЬNULL(ВТДатаПриема.ДатаПриема, ДАТАВРЕМЯ(1,1,1)) >= ЕСТЬNULL(ОбщийСтаж.Период, ДАТАВРЕМЯ(1,1,1))
		|	ТОГДА ВТДатаПриема.ДатаПриема 
		|	ИНАЧЕ ОбщийСтаж.Период
		|	КОНЕЦ КАК ДатаПриема,
		|	ЕСТЬNULL(ОбщийСтаж.ДнейСтажа, 0) - ЕСТЬNULL(ОтпускПоУходуЗаРебенком.ДнейОтпускаПоУходуЗаРебенком, 0) КАК ДнейСтажа
		|ПОМЕСТИТЬ ВТСведенияОСтаже
		|ИЗ
		|	ВТДатаПриема КАК ВТДатаПриема
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НачальныеСведенияОСтажеРаботниковОрганизации.СрезПоследних(
		|				&ДатаНачалаСобытия,
		|				Организация = &парамОрганизация
		|					И ФизЛицо = &парамФизЛицо
		|					И ВидСтажа = ЗНАЧЕНИЕ(Справочник.ВидыСтажа.СтраховойСтажДляБольничного)) КАК ОбщийСтаж
		|		ПО (ИСТИНА)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОтпускПоУходуЗаРебенком КАК ОтпускПоУходуЗаРебенком
		|		ПО (ИСТИНА)";
		Запрос.Текст = ТекстЗапросаВТ;
		Запрос.Выполнить();
	КонецЕсли;
	...
	Если Сотрудник.ДатаПриемаНаРаботу > ДатаСтажа12МесяцевДоСобытия Тогда
		
		СтруктураПоиска = Новый Структура("ВидСтажа", Справочники.ВидыСтажа.СтраховойСтажДляБольничного); 
		МассивНайденныхСтрок = Сотрудник.Физлицо.Стажи.НайтиСтроки(СтруктураПоиска);
		ТрудоваяДеятельность = Сотрудник.Физлицо.Стажи.Выгрузить(МассивНайденныхСтрок,"ДатаНачала, ДатаОкончания");
		
		Запрос.УстановитьПараметр("ДатаСтажа12МесяцевДоСобытия", ДатаСтажа12МесяцевДоСобытия);
		Запрос.УстановитьПараметр("ТрудоваяДеятельность", ТрудоваяДеятельность);
		ТекстЗапросаВТ = "ВЫБРАТЬ
		|	ТрудоваяДеятельность.ДатаНачала КАК ДатаНачала,
		|	ВЫБОР
		|		КОГДА ТрудоваяДеятельность.ДатаОкончания > &ДатаСтажа12МесяцевДоСобытия
		|			ТОГДА &ДатаСтажа12МесяцевДоСобытия
		|		ИНАЧЕ ТрудоваяДеятельность.ДатаОкончания
		|	КОНЕЦ КАК ДатаОкончания
		|ПОМЕСТИТЬ ВТСведенияОСтаже12МесяцевДоСобытия
		|ИЗ
		|	&ТрудоваяДеятельность КАК ТрудоваяДеятельность
		|ГДЕ
		|	ТрудоваяДеятельность.ДатаНачала < &ДатаСтажа12МесяцевДоСобытия
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СУММА(РАЗНОСТЬДАТ(СведенияОСтаже12МесяцевДоСобытия.ДатаНачала, СведенияОСтаже12МесяцевДоСобытия.ДатаОкончания, ДЕНЬ)) КАК ДнейСтажа
		|ПОМЕСТИТЬ ВТСведенияОСтаже12
		|ИЗ
		|	ВТСведенияОСтаже12МесяцевДоСобытия КАК СведенияОСтаже12МесяцевДоСобытия";
	...
	Запрос.Текст = "ВЫБРАТЬ * ИЗ ВТСведенияОСтаже12";

	Выборка = Запрос.Выполнить().Выбрать();
	
	Если Выборка.Следующий() Тогда
		ДнейСтажаЗа12Месяцев = Выборка.ДнейСтажа;	
	КонецЕсли;	
	Запрос.УстановитьПараметр("ДнейСтажаЗа12Месяцев",ДнейСтажаЗа12Месяцев);
	...
	| 	ДОБАВИТЬКДАТЕ(&ДатаНачалаСобытия, ДЕНЬ, ВЫБОР	КОГДА ЕСТЬNULL(&ДнейСтажаЗа12Месяцев,0)=0 ТОГДА 0 ИНАЧЕ - &ДнейСтажаЗа12Месяцев КОНЕЦ
	|  )	КАК ДатаСтажаЗа12Месяцев,
	|РАЗНОСТЬДАТ(ДОБАВИТЬКДАТЕ( Стаж.ДатаПриема, ДЕНЬ, "+СтрЗаменить(МинусДней,"Дней","Стаж.ДнейСтажа")+"),
	|ДОБАВИТЬКДАТЕ(&ДатаНачалаСобытия, ДЕНЬ, ВЫБОР	КОГДА ЕСТЬNULL(&ДнейСтажаЗа12Месяцев,0)=0 ТОГДА 0 ИНАЧЕ - &ДнейСтажаЗа12Месяцев КОНЕЦ),ДЕНЬ) КАК СтажЗа12МесВКаленДнях
...
Функция РассчитатьСреднююПорядок1266( КомментироватьРасчет = Ложь ) Экспорт
	...
	Если ДатаНачала < ПроведениеРасчетов.ДатаИзмененияПорядка1266() Тогда
		...
		Если КомментироватьРасчет Тогда
			...
			КомментарийРасчетаСредней = ОбщегоНазначения.КомментарийРасчета("Расчет средней с " + Формат(ПериодРасчетаСреднегоНачало,"ДФ='МММ гггг'")+ " по "+
		...
		Пока Выборка.Следующий() Цикл
			...
			Если КомментироватьРасчет Тогда
				...
				КомментарийНомерСтроки = ОбщегоНазначения.КомментарийРасчета("Номер строки: " + Выборка.НомерСтроки, КомментарийРасчетаСредней,,,,Выборка.Результат <> 0);
				КомментарийВидаРасчета = ОбщегоНазначения.КомментарийРасчета("	Вид расчета: " +  СтрокаТабличнойЧасти.ВидРасчета, КомментарийНомерСтроки);
				ОбщегоНазначения.КомментарийРасчета("	Базовый период: " +Формат(Выборка.БазовыйПериодНачало,"ДФ='МММ гггг'"), КомментарийВидаРасчета);
				КомментарийБазовыйПериод = ОбщегоНазначения.КомментарийРасчета("	Базовые начисления составили: " + ОКР(Выборка.РезультатПолный,2,0), КомментарийВидаРасчета,,,,Ложь);
				...
				Пока ВыборкаКомментарии.НайтиСледующий(СтруктураПоиска) Цикл
					...
					Если ВыборкаКомментарии.ВидРасчетаРазрез = NULL Тогда
						ТекстКомментарияБазаСОграничением = "База с учетом ограничений: " + ОКР(Выборка.Результат,2,0);
					...
					Иначе   
						Если Выборка.Результат <> 0 и ВыборкаКомментарии.Результат <> 0 Тогда
							...
							Если Выборка.Результат <> Выборка.РезультатПолный Тогда 
								Если ВыборкаКомментарии.ВидРасчета = ПланыВидовРасчета.СреднийЗаработок.ПоЗаработкуБЛПропорционально Тогда
									Если Выборка.ОтработаноЧасовДляПропорции <> Выборка.НормаЧасовДляПропорции Тогда
										ТекстКомментарияПропорцияВремя = "Отработано часов = " + Выборка.ОтработаноЧасовДляПропорции + ", норма часов = " + Выборка.НормаЧасовДляПропорции;
							...
							Если ВыборкаКомментарии.ОтработаноДней <> 0 Тогда
								ТекстКомментария = ?(ВыборкаКомментарии.ОтработаноДней = 0,"",ТекстКомментария + " (отработано дней: " +  ВыборкаКомментарии.ОтработаноДней+")");
							КонецЕсли;
