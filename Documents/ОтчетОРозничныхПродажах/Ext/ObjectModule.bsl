...
Функция ПечатьОтчетаОПродажахККМ()
	...
	Если Шапка.УчитыватьНДС Тогда
		...
		ОбластьДанных.Параметры.НДС = ?(Шапка.СуммаВключаетНДС, НСтр("ru='В т.ч. НДС:';uk='У т.ч. ПДВ:'",КодЯзыкаПечать), НСтр("ru='Сумма НДС:';uk='Сума ПДВ:'",КодЯзыкаПечать));
		...
		Если СуммаАкцизногоНалога <> 0 Тогда
			...
			ОбластьДанных.Параметры.НДС      = ?(Шапка.СуммаВключаетНДС, НСтр("ru='В т.ч. акциз:';uk='У т.ч. акциз:'",КодЯзыкаПечать), НСтр("ru='Сумма акциза:';uk='Сума акцизу:'",КодЯзыкаПечать));
			ТабДокумент.Присоединить(ОбластьДанных);
			...
			ОбластьСуммы.Параметры.ВсегоНДС = ОбщегоНазначения.ФорматСумм(СуммаАкцизногоНалога);
			ТабДокумент.Присоединить(ОбластьСуммы);
		...
		Если НЕ Шапка.СуммаВключаетНДС Тогда
			...
			ОбластьСклада = Макет.ПолучитьОбласть("ИтогоДополнительно|Склад");		
			...
			КонецЕсли;
			Если СуммаАкцизногоНалога <> 0  Тогда
				ОбластьДанных.Параметры.Подпись = НСтр("ru='Всего с налогами:';uk='Всього з податками:'", КодЯзыкаПечать);
			Иначе
			...
			Если ЕстьСкидки Тогда
				ТабДокумент.Присоединить(ОбластьСкидок);
			КонецЕсли;
			ОбластьСуммы.Параметры.Сумма = ОбщегоНазначения.ФорматСумм(Сумма + СуммаНДС + СуммаАкцизногоНалога);
			ТабДокумент.Присоединить(ОбластьСуммы);
		КонецЕсли;		
	...
	Иначе
		...
		Если СуммаАкцизногоНалога <> 0  Тогда
			...
			Если ВыводитьКоды Тогда
				ТабДокумент.Присоединить(ОбластьКодов);
			КонецЕсли;
			ОбластьДанных.Параметры.НДС = НСтр("ru='В т.ч. акциз:';uk='У т.ч. акциз:'", КодЯзыкаПечать);
			ТабДокумент.Присоединить(ОбластьДанных);
	...
	СуммаКПрописи = Сумма + ?(Шапка.СуммаВключаетНДС, 0, СуммаНДС) + ?(Шапка.УчитыватьНДС И Шапка.СуммаВключаетНДС, 0, СуммаАкцизногоНалога);
	...
	КонецЕсли;
  	ОбластьМакета.Параметры.СуммаПрописью  = ОбщегоНазначения.СформироватьСуммуПрописью(СуммаКПрописи, мВалютаРегламентированногоУчета, КодЯзыкаПечать)
	 										 + ?(НЕ Шапка.УчитыватьНДС И СуммаНалоговКПрописи = 0, 
											 		"", 
													Символы.ПС + ЗаголовокНалоговКПрописи + ОбщегоНазначения.СформироватьСуммуПрописью(СуммаНалоговКПрописи, мВалютаРегламентированногоУчета, КодЯзыкаПечать)
												);
...
Процедура ПодготовитьТаблицуТоваровРегл(ТаблицаТоваров, СтруктураШапкиДокумента)
	...
	Для каждого СтрокаТаблицы Из ТаблицаТоваров Цикл
		...
		Если СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 = Ложь Тогда
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете И Товары.Итог("СуммаАкцизногоНалога") <> 0 Тогда
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, 
	...
	ТаблицаРозничнойВыручки.Свернуть("РасчетыВозврат, СтавкаНДС", "СуммаНДСРегл, СуммаБезНДСРегл, ПроводкиСуммаАкцизногоНалогаРегл");
	...
	Для каждого СтрокаТЧ из ТаблицаРозничнойВыручки Цикл
		...
		СтрокаДвижений.Сумма           		= СтрокаТЧ.СуммаБезНДСРегл + СтрокаТЧ.СуммаНДСРегл + СтрокаТЧ.ПроводкиСуммаАкцизногоНалогаРегл;
		СтрокаДвижений.СуммаНДС        		= СтрокаТЧ.СуммаНДСРегл;
		СтрокаДвижений.СуммаАкцизногоНалога = СтрокаТЧ.ПроводкиСуммаАкцизногоНалогаРегл;
	...
	ТаблицаСуммРеализации.Свернуть("РасчетыВозврат,
								   ...
								   ,"СуммаСНДСРегл, СуммаНДСРегл, ПроводкиСуммаАкцизногоНалогаРегл");
	...
	Для каждого СтрокаТЧ из ТаблицаСуммРеализации Цикл		
		...
		Если СтрокаТЧ.РасчетыВозврат = Перечисления.РасчетыВозврат.Расчеты Тогда
			...
			Проводка.Сумма        = СтрокаТЧ.СуммаСНДСРегл + СтрокаТЧ.ПроводкиСуммаАкцизногоНалогаРегл;
		...
		Если НЕ СтрокаТЧ.ПроводкиСуммаАкцизногоНалогаРегл = 0 Тогда
			...
			Если СтрокаТЧ.РасчетыВозврат = Перечисления.РасчетыВозврат.Расчеты Тогда
				...
				Проводка.Содержание   = "Акцизный налог розничной выручки";
				Проводка.Сумма        = СтрокаТЧ.ПроводкиСуммаАкцизногоНалогаРегл; 
			...
			Иначе
				...
				Проводка.Содержание   = "Акцизный налог розничной выручки (возврат)";
				Проводка.Сумма        = СтрокаТЧ.ПроводкиСуммаАкцизногоНалогаРегл; //сумма отрицательная
...
Процедура ДвиженияРозничныеПродажиПодакцизныхТоваров(СтруктураШапкиДокумента, ТаблицаПоТоварам, Отказ)
	...
	ТаблицаАкцизныйНалог.Свернуть("Склад, СтатьяДекларацииПоАкцизномуНалогу, ПодакцизныеТоварыДляКоммерческогоИспользования",
								  "ПроводкиСуммаАкцизногоНалогаРегл, ПроводкиСуммаСНДСРегл");
...
КонецПроцедуры // ДвиженияРозничныеПродажиПодакцизныхТоваров
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	СуммаДокумента = Ценообразование.ПолучитьСуммуДокументаСНДС(ЭтотОбъект, "Товары") + ?(НЕ УчитыватьНДС ИЛИ (УчитыватьНДС И СуммаВключаетНДС), 0, Товары.Итог("СуммаАкцизногоНалога"));
...
Процедура ЗаполнитьСчетаВШапкеРегл(ЗаполнятьБУ) Экспорт;
	...
	ИначеЕсли ЗаполнятьБУ = Ложь Тогда
		...
		СчетУчетаАкцизногоНалога = ПланыСчетов.Хозрасчетный.ПустаяСсылка();
