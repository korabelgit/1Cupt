...
Процедура ОтключитьНедоступныеРегламентныеОперации(МассивНедоступныхРегламентныхОпераций = Неопределено) Экспорт
	...
	Если МассивНедоступныхРегламентныхОпераций = Неопределено Тогда
		МассивНедоступныхРегламентныхОпераций = БизнесПроцессЗакрытиеМесяца.ЗаполнитьМассивНедоступныхРегламентныхОпераций(
												 ?(ЗначениеЗаполнено(ДатаНачалаДействияНастройки),ДатаНачалаДействияНастройки,ТекущаяДата())
											     , // Организация
											     ,"ИзСправочникаНастроек"
											    );
	КонецЕсли;
