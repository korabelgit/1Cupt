...
Функция ПолучитьКодОперацииПоДокументу(Документ, СтруктураШапкиДокумента, ИмяТабличнойЧасти, ИмяДокумента = "")
	...
	ИначеЕсли (ТипЗНЧ(Документ) = Тип("ДокументСсылка.ВозвратТоваровПоставщику")) Тогда
		...
		Если Документ.ДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПоставщиком Тогда
			...
			КонецЕсли;
		Иначе
			Если ИмяТабличнойЧасти = "ТаблицаПоТаре" И Документ.ДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомитентом Тогда
				// возврат тары комитенту обрабатываем так же, как и возврат поставщику
				СтруктураКодовОпераций.Вставить("ВозвратПоставщику");
			Иначе
	...
	ИначеЕсли (ТипЗНЧ(Документ) = Тип("ДокументСсылка.РегламентнаяПереоценкаЗапасов")) Тогда
		ИмяДокумента = "РегламентнаяПереоценкаЗапасов";
		СтруктураКодовОпераций.Вставить("ПереоценкаКупленных");
	...
	Если ТипЗНЧ(Документ) = Тип("ДокументСсылка.ОприходованиеМатериаловИзПроизводства") И Документ.ВидОперации = Перечисления.ВидыОперацийОприходованиеМатериаловИзПроизводства.Материалы Тогда
	    СтруктураКодовОпераций.Вставить("СписаниеНаСтроительствоОбъектовОС", Перечисления.КодыОперацийПартииТоваров.СписаниеНаСтроительствоОбъектовОС);
	КонецЕсли;	
...
Процедура СформироватьДвиженияПоПереоценкеТоваров(ДокументПоступления, Коэфф, ВидУчета) Экспорт
	...
	ПогрешностьСтоимость = 0; ПогрешностьСтоимостьНУ = 0; ПогрешностьНДСВходящий = 0; ПогрешностьНДСКредит = 0;
	Для ТекСтрока = 0 по КоличествоДвижений - 1 Цикл
		...
		НоваяЗапись.Стоимость     = ОбщегоНазначения.ОкруглитьСУчетомПогрешности(НоваяЗапись.Стоимость   * Коэфф, 2, ПогрешностьСтоимость);
		НоваяЗапись.СтоимостьНУ   = ОбщегоНазначения.ОкруглитьСУчетомПогрешности(НоваяЗапись.СтоимостьНУ * Коэфф, 2, ПогрешностьСтоимостьНУ);
		НоваяЗапись.НДСВходящий   = ОбщегоНазначения.ОкруглитьСУчетомПогрешности(НоваяЗапись.НДСВходящий * Коэфф, 2, ПогрешностьНДСВходящий);
		НоваяЗапись.НДСКредит     = ОбщегоНазначения.ОкруглитьСУчетомПогрешности(НоваяЗапись.НДСКредит   * Коэфф, 2, ПогрешностьНДСКредит);
...
//
Функция ПолучитьАналитикуПоКлючуАналитики(ИмяКлючаАналитики, КлючАналитики, СтруктураКлючиАналитики) Экспорт	
...
Функция ПолучитьТаблицуДвиженийПоРегиструПараметрыАмортизацииОС(ТаблицаЗатрат,СтруктураКлючиАналитики,ДатаДокумента, ВидОтраженияВУчете)
	...
	Если ТаблицаСписания.Количество() > 0 Тогда
		...
		Если ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВНалоговомУчете Тогда
			...
			Если ДатаДокумента >= глЗначениеПеременной("ДатаНКУ2015") Тогда
				Для Каждого СтрокаТаблицаСписания Из ТаблицаСписания Цикл
					СтрокаТаблицаСписания.СтоимостьНУ = СтрокаТаблицаСписания.Стоимость;
				КонецЦикла; 							 
			КонецЕсли; 						 
		Иначе
			СтрокаСтоимостьНУ = "";
...
Функция ПолучитьТаблицуДвиженийПоРегиструПервоначальныеСведенияОС(ТаблицаЗатрат,СтруктураКлючиАналитики,ДатаДокумента, ВидОтраженияВУчете)
	...
	Если ТаблицаСписания.Количество() > 0 Тогда
		...
		|	//ДляНалУчета	ПервоначальныеСведенияОС.СпособНачисленияАмортизации КАК СпособНачисленияАмортизации,
		|
...
Функция ПолучитьТаблицуДвиженийПоРегиструЗатратыНаСтроительствоОбъектовОсновныхСредствБУ(ТаблицаЗатрат,СтруктураКлючиАналитики)
	...
	Если ТаблицаСписания.Количество() > 0 Тогда
		...
		ЗаполнитьТаблицуДвиженийПоЗатратамАналитикой(ТаблицаСписания, Новый Структура("Организация"), Отказ, СтруктураКлючиАналитики);
		ЗаполнитьТаблицуДвиженийПоЗатратамАналитикой(ТаблицаСписания, Новый Структура("ОбъектСтроительства, СчетУчета, НалоговоеНазначение, Субконто2","ОбъектСтроительства","СчетУчета","НалоговоеНазначение","СтатьяЗатрат"), Отказ, СтруктураКлючиАналитики,Истина);
...
//  Таблица значений.
//
Функция ПолучитьТаблицуДвиженийПоРегиструСтоимостьОС(ТаблицаЗатрат, СтруктураКлючиАналитики, ВидОтраженияВУчете, Период)	
	...
	Если ТаблицаСписания.Количество() > 0 Тогда
		...
		Если ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВБухгалтерскомУчете Тогда	
			...
			Если Период >= глЗначениеПеременной("ДатаНКУ2015") Тогда
				Для Каждого СтрокаТаблицаСписания Из ТаблицаСписания Цикл
					СтрокаТаблицаСписания.СтоимостьНУ = СтрокаТаблицаСписания.Стоимость;
				КонецЦикла; 							 
			КонецЕсли; 						 
...
Процедура СформироватьДвиженияПоАналитическимРегистрам(
	...
	ТаблицаДвиженийПоРегиструСтоимостьОС = ПолучитьТаблицуДвиженийПоРегиструСтоимостьОС(ТаблицаЗатрат,СтруктураКлючиАналитики, ВидОтраженияВУчете, Период);
...
Процедура ДобавитьПроводку(СтруктураПараметров, ИмяРегистра, Движение,КэшПоИерархииСчетов, Сумма, СуммаНУ, СуммаНУПолучатель, СуммаВидУчета = Неопределено) Экспорт
	...
	Проводка.СуммаНУКт = НалоговыйУчет.ОпределитьСтоимостьНУВПроводке(Проводка.Период, Сумма, СуммаНУ, Проводка.СчетКт, Проводка.СчетДт, Проводка.НалоговоеНазначениеКт, СтруктураПараметров.ЕстьНалогНаПрибыльДо2015, СтруктураПараметров.ЕстьНалогНаПрибыльС2015);
	Проводка.СуммаНУДт = НалоговыйУчет.ОпределитьСтоимостьНУВПроводке(Проводка.Период, Сумма, СуммаНУПолучатель, Проводка.СчетДт, Проводка.СчетКт, Проводка.НалоговоеНазначениеДт, СтруктураПараметров.ЕстьНалогНаПрибыльДо2015, СтруктураПараметров.ЕстьНалогНаПрибыльС2015);
...
Процедура СформироватьДополнительныеДвиженияПоРегистрамУчетаЗатрат(Источник,СтруктураНаборовЗаписейПоВидамУчета,ТаблицаДляФормированияДвиженийПоРегистру)  
	...
	Если ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.ПередачаМатериаловВЭксплуатацию") Тогда
		...
		УчетнаяПолитика = ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(Источник.Дата, Источник.мСтруктураШапкиДокумента.Организация, Ложь);
	  	Если ЗначениеЗаполнено(УчетнаяПолитика) Тогда
	  		СтруктураПараметров.Вставить("ЕстьНалогНаПрибыль", УчетнаяПолитика.ЕстьНалогНаПрибыль);
			СтруктураПараметров.Вставить("ЕстьНалогНаПрибыльДо2015", УчетнаяПолитика.ЕстьНалогНаПрибыльДо2015);
			СтруктураПараметров.Вставить("ЕстьНалогНаПрибыльС2015", УчетнаяПолитика.ЕстьНалогНаПрибыльС2015);
	  	Иначе
...
Функция ПолучитьСтруктуруДвиженийПоБухгалтерскимРегистрам(Регистратор)
	...
	Возврат Движения;
КонецФункции // ПолучитьСтруктуруДвиженийПоБухгалтерскимРегистрам
...
Процедура СформироватьДвиженияПоОтчетуКомитенту(СтруктураШапкиДокумента, ТаблицаПоТоварам) Экспорт
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		Для Каждого ТекущаяСтрока Из ТаблицаПоТоварам Цикл
			...
			// Учет товаров комиссионера ведется без НДС
			СебестоимостьБУ = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(ТекущаяСтрока.СуммаСНДС - ТекущаяСтрока.СуммаНДСПоступления
			...
			Если ТекущаяСтрока.ПринадлежностьНоменклатуры = Перечисления.ПринадлежностьНоменклатуры.Принятый Тогда
				...
				Иначе	
					...
					Иначе	
						...
						СтруктураПериод = Новый Структура("ДатаОкончания", СтруктураШапкиДокумента.Дата);
						НеВключатьНДСВСтоимостьПартий = ОбщегоНазначения.ПолучитьНеВключатьНДСВСтоимостьПартий(0, СтруктураПериод);
						Если НеВключатьНДСВСтоимостьПартий Тогда
							ВыручкаСебестоимость = ТекущаяСтрока.СуммаБезНДС;
						Иначе
							ВыручкаСебестоимость = ТекущаяСтрока.Стоимость;	
						КонецЕсли; 
					КонецЕсли;						
				...
				СтрокаДвижений.Количество = Количество;
				СтрокаДвижений.Стоимость = ВыручкаСебестоимость;	
...
КонецПроцедуры // СформироватьДвиженияПоРеализацииПринятогоНаКомиссиюТовара
...
КонецПроцедуры // СформироватьДвиженияПоПередачеДавальческихМатериаловВПроизводство
...
Процедура СформироватьДвиженияВозвратаПринятыхМатериалов(СтруктураШапкиДокумента, СтруктураТабличныхЧастей, Ссылка, Движения, ВидОперации)
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		Для Каждого ТекущаяСтрока Из СтруктураТабличныхЧастей.ТаблицаПоТоварам Цикл
			...
			// Учет товаров комиссионера и принятых материалов ведется без НДС
			СебестоимостьБУ = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(ТекущаяСтрока.СуммаБезНДСВал
										,СтруктураШапкиДокумента.ВалютаВзаиморасчетов,СтруктураШапкиДокумента.ВалютаРегламентированногоУчета
										,СтруктураШапкиДокумента.КурсВзаиморасчетов,1
										,СтруктураШапкиДокумента.КратностьВзаиморасчетов,1);
			
			Проводка.Сумма = СебестоимостьБУ;
			Проводка.КоличествоКт = ТекущаяСтрока.КоличествоДок;
...
КонецПроцедуры // СформироватьДвиженияПоУчетуПринятыхТМЦ
...
КонецПроцедуры	// РасчитатьЦенуНоменклатуры
...
КонецПроцедуры // РасчитатьСебестоимость
...
Функция ПодготовитьТаблицуДляФормированияДвижений(СтруктураНаборовЗаписейПоВидамУчета, Источник)
	...
	ИначеЕсли ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.РегламентнаяПереоценкаЗапасов")Тогда
		ТаблицаДляФормированияДвижений.Колонки.Добавить("НомерСтроки_РегламентнаяПереоценкаЗапасов", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 0)));
	КонецЕсли;
...
КонецФункции // ПодготовитьТаблицуДляФормированияДвижений
...
Процедура ЗаполнитьСтрокуВТаблицеДляФормированияДвижений(
	...
	ИначеЕсли КодОперацииПартийТоваров = Перечисления.КодыОперацийПартииТоваров.ПереоценкаКупленных Тогда	
		НоваяСтрока.ФормироватьПроводки = Ложь; // проводки формируются независимо от движений по регистру учета затрат
	Иначе
	...
	Если НЕ ТекущаяСтрокаОприходования = Неопределено Тогда
		...
		Если ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.ПоступлениеДопРасходов") 
		   И ЗначениеЗаполнено(ТекущаяСтрокаОприходования.СтрокаИсходнойТаблицы.ДокументОприходования) Тогда
			НоваяСтрока.ДатаДокументаПартии = ТекущаяСтрокаОприходования.СтрокаИсходнойТаблицы.ДокументОприходования.Дата;
		КонецЕсли;	
	КонецЕсли;	
	
	Если ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.РегламентнаяПереоценкаЗапасов") Тогда
        // Запоминаем номер строки при приходе раздеру МПЗ и оборота по разделу Затраты
		Если НЕ ТекущаяСтрокаСписания = Неопределено Тогда
			НоваяСтрока.НомерСтроки_РегламентнаяПереоценкаЗапасов = ТекущаяСтрокаСписания.СтрокаИсходнойТаблицы.НомерСтроки;
		ИначеЕсли НЕ ТекущаяСтрокаОприходования = Неопределено Тогда 
			НоваяСтрока.НомерСтроки_РегламентнаяПереоценкаЗапасов = ТекущаяСтрокаОприходования.СтрокаИсходнойТаблицы.НомерСтроки;
		КонецЕсли;
	КонецЕсли;
...
Процедура СформироватьДвиженияПоСтрокеДляСписания(СтруктураШапкиДокумента,ТекущаяСтрока,ТекущийНабор)
	...
	Если ТекущаяСтрока.ВыполнятьСписание И ТекущаяСтрока.ВыполнятьДвижениеСписание Тогда					
		  ...
		  РасширеннаяАналитикаУчета.СформироватьДвиженияПоРегиструУчетЗатрат(
			...
			ТекущийНабор,
			Неопределено, // ДатаДокументаПартии
	...
	Для Каждого ТекущийНаборЗаписей Из СтруктураНаборовЗаписейПоВидамУчета Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ИмяРегистраУчетаЗатрат = "УчетЗатратРегл" Тогда
				...
				КлючАналитикиПолучатель = ПолучитьАналитикуПоКлючуАналитики("АналитикаВидаУчета", СтрокаТаблицы.КорАналитикаВидаУчета, СтруктураКлючиАналитики);
				Если КлючАналитикиПолучатель <> Неопределено Тогда
					НалоговоеНазначениеПолучатель = КлючАналитикиПолучатель.НалоговоеНазначение;
					СтрокаТаблицы.СтоимостьНУПолучатель = НалоговыйУчет.ОпределитьСтоимостьНУ(НалоговоеНазначениеПолучатель, СтрокаТаблицы.СтоимостьНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
				Иначе
				...
				Если СпособРасчетаСебестоимостиСписанияМПЗ = Перечисления.СпособыРасчетаСебестоимостиВыпускаПоОперациям.ПоПлановойСебестоимости Тогда	
					...
					НалоговоеНазначение           = ПолучитьАналитикуПоКлючуАналитики("АналитикаВидаУчета", СтрокаТаблицы.АналитикаВидаУчета, СтруктураКлючиАналитики).НалоговоеНазначение;
					...
					Если НалоговыйУчетПовтИсп.ПолучитьВидНалоговойДеятельностиНалоговогоНазначения(НалоговоеНазначение) <> Справочники.ВидыНалоговойДеятельности.НеОблагаемая Тогда
						СтрокаТаблицы.НДСВходящий = СтрокаТаблицы.Стоимость * 0.2;
					Иначе
						СтрокаТаблицы.НДСВходящий = 0;
					КонецЕсли;	
					...
					Если (НалоговыйУчетПовтИсп.ПолучитьВидДеятельностиНДСНалоговогоНазначения(НалоговоеНазначение) <> Перечисления.ВидыДеятельностиНДС.Необлагаемая 
						...
						СтрокаТаблицы.НДСКредит = СтрокаТаблицы.Стоимость * 0.2;
					Иначе
				...
				Иначе
					...
					СтрокаТаблицы.НДСВходящий = СтрокаТаблицы.КоличествоНУ * Выборка.ЦенаНДСВходящий;
					СтрокаТаблицы.НДСКредит   = СтрокаТаблицы.КоличествоНУ * Выборка.ЦенаНДСКредит;
					
				КонецЕсли;	
				
				СтрокаТаблицы.НДСВходящийПолучатель = СтрокаТаблицы.НДСВходящий;
				СтрокаТаблицы.НДСКредитПолучатель   = СтрокаТаблицы.НДСКредит;
			КонецЕсли;
...
КонецПроцедуры // ЗаполнитьСуммыВДвижениях

// Процедура расчитывает стоимость списания/приходовани по документу "Регламентная переоценка запасов"
// Параметры:
//  СтруктураШапкиДокумента - структура, содержащая реквизиты шапки документа,
//  ТаблицаДляФормированияДвижений - таблица значений, содержащая перечень списываемых затрат,
//  Отказ - флаг отказа от проведения документа.
//
Процедура ЗаполнитьСуммыВДвиженияхДокументаРегламентнаяПереоценкаЗапасов(Источник, ТаблицаДляФормированияДвижений, Отказ)
				
	СтруктураШапкиДокумента = Источник.мСтруктураШапкиДокумента;
	
	// Подготовим таблицу переоцененных товаров и оборотов по затратам
	Запрос = Новый Запрос();
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц();
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ТаблицаДляФормированияДвижений.НомерСтроки_РегламентнаяПереоценкаЗапасов КАК НомерСтроки,
	|	ТаблицаДляФормированияДвижений.КорАналитикаВидаУчета КАК АналитикаВидаУчета,
	|	ТаблицаДляФормированияДвижений.КорАналитикаУчетаЗатрат КАК АналитикаУчетаЗатрат,
	|	ТаблицаДляФормированияДвижений.КорАналитикаУчетаПартий КАК АналитикаУчетаПартий,
	|	ТаблицаДляФормированияДвижений.КорАналитикаРаспределенияЗатрат КАК АналитикаРаспределенияЗатрат,
	|	ТаблицаДляФормированияДвижений.СтоимостьПолучатель КАК НоваяЦена
	|ПОМЕСТИТЬ ТаблицаДляФормированияДвижений
	|ИЗ
	|	&ТаблицаДляФормированияДвижений КАК ТаблицаДляФормированияДвижений
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаДляФормированияДвижений.НомерСтроки КАК НомерСтроки,
	|	ТаблицаДляФормированияДвижений.АналитикаВидаУчета КАК АналитикаВидаУчета,
	|	ТаблицаДляФормированияДвижений.АналитикаУчетаЗатрат КАК АналитикаУчетаЗатрат,
	|	ТаблицаДляФормированияДвижений.АналитикаУчетаПартий КАК АналитикаУчетаПартий,
	|	ТаблицаДляФормированияДвижений.АналитикаРаспределенияЗатрат КАК АналитикаРаспределенияЗатрат,
	|	ТаблицаДляФормированияДвижений.НоваяЦена КАК НоваяЦена
	|ПОМЕСТИТЬ ТаблицаСписания
	|ИЗ
	|	ТаблицаДляФормированияДвижений КАК ТаблицаДляФормированияДвижений
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаВидаУчета КАК РегистрСведенийАналитикаВидаУчета
	|		ПО ТаблицаДляФормированияДвижений.АналитикаВидаУчета = РегистрСведенийАналитикаВидаУчета.Ссылка
	|ГДЕ
	|	РегистрСведенийАналитикаВидаУчета.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчета.МПЗ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ТаблицаДляФормированияДвижений
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ТаблицаСписания.АналитикаВидаУчета,
	|	ТаблицаСписания.АналитикаУчетаЗатрат,
	|	ЕСТЬNULL(УчетЗатратОстатки.КоличествоОстаток, 0) КАК Количество,
	|	ЕСТЬNULL(УчетЗатратОстатки.КоличествоОстаток, 0) * ТаблицаСписания.НоваяЦена КАК СтоимостьНовая,
	|	ЕСТЬNULL(УчетЗатратОстатки.СтоимостьОстаток, 0) - ЕСТЬNULL(УчетЗатратОстатки.КоличествоОстаток, 0) * ТаблицаСписания.НоваяЦена КАК СуммаУценкиИзменение
	|ИЗ
	|	ТаблицаСписания КАК ТаблицаСписания
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.УчетЗатратРегл.Остатки(
	|				&ДатаДокумента,
	|				(АналитикаВидаУчета, АналитикаРаспределенияЗатрат, АналитикаУчетаЗатрат, АналитикаУчетаПартий) В
	|					(ВЫБРАТЬ
	|						ТаблицаСписания.АналитикаВидаУчета,
	|						ТаблицаСписания.АналитикаРаспределенияЗатрат,
	|						ТаблицаСписания.АналитикаУчетаЗатрат,
	|						ТаблицаСписания.АналитикаУчетаПартий
	|					ИЗ
	|						ТаблицаСписания КАК ТаблицаСписания)) КАК УчетЗатратОстатки
	|		ПО (УчетЗатратОстатки.АналитикаВидаУчета = ТаблицаСписания.АналитикаВидаУчета)
	|			И (УчетЗатратОстатки.АналитикаРаспределенияЗатрат = ТаблицаСписания.АналитикаРаспределенияЗатрат)
	|			И (УчетЗатратОстатки.АналитикаУчетаЗатрат = ТаблицаСписания.АналитикаУчетаЗатрат)
	|			И (УчетЗатратОстатки.АналитикаУчетаПартий = ТаблицаСписания.АналитикаУчетаПартий)";
	
	Запрос.УстановитьПараметр("ТаблицаДляФормированияДвижений", ТаблицаДляФормированияДвижений);
	
	// Расчет себестоимости выпуска делает движения на конец месяца, не зависимо от даты указанной в документе
	// Регламентная переоценка выполняется после расчета себестоимости, соответственно необходимо получать остатки учитывая
	// особеность проведения документа "Расчет себестоимости выпуска"
	Запрос.УстановитьПараметр("ДатаДокумента", Новый Граница(СтруктураШапкиДокумента.Дата, ВидГраницы.Включая));					  			
	
	Заголовок = ОбщегоНазначения.ПредставлениеДокументаПриПроведении(СтруктураШапкиДокумента.Ссылка);
	
	// Паралельно с заполнением сумм по регистру УчетЗатратРегл сформируем проводки
	НаборЗаписей = РегистрыБухгалтерии.Хозрасчетный.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Регистратор.Установить(СтруктураШапкиДокумента.Ссылка);
	
	ТаблицаПоТоварам = Источник.мСтруктураТабличныхЧастей.ТаблицаПоТоварам;
	...
	Если НЕ Результат.Пустой() Тогда
		Выборка = Результат.Выбрать();	
		ОтборСтрок = Новый Структура("НомерСтроки_РегламентнаяПереоценкаЗапасов");
		Пока Выборка.Следующий() Цикл
			// Проверки аналогичны проведению в традиционном режиме
			// однако в РАУЗ не реализован контроль суммы при сторнировании в случае, когда
			// уцененные товары обратно дооцениваются до первоначальной себестоимости.
			Если (Не Выборка.Количество > 0) ИЛИ (Выборка.СуммаУценкиИзменение = 0) Тогда
				// нужно сообщить об ошибке  
				ОбщегоНазначения.СообщитьОбОшибке("Бухгалтерский учет. Не выполнена переоценка товара """ + 
				                                  Выборка.АналитикаУчетаЗатрат+""" по виду учета """ +
												  Выборка.АналитикаВидаУчета+"""", Отказ, Заголовок);
				Продолжить; 
			КонецЕсли;
			
			// Зафиксируем сумму уценки по разделу МПЗ и оборотам по разделу Затраты
			ОтборСтрок.НомерСтроки_РегламентнаяПереоценкаЗапасов = Выборка.НомерСтроки;		
			СтрокиДляФормированияДвижений = ТаблицаДляФормированияДвижений.НайтиСтроки(ОтборСтрок);
			Для каждого СтрокаДляФормированияДвижений Из СтрокиДляФормированияДвижений Цикл
				
				Если Выборка.АналитикаВидаУчета = СтрокаДляФормированияДвижений.КорАналитикаВидаУчета Тогда
					// Приход по МПЗ
					// Колонка СтоимостьПолучатель использовалась для транспорта новой цены - больше нам не нужна
					СтрокаДляФормированияДвижений.СтоимостьПолучатель = -Выборка.СуммаУценкиИзменение;
				Иначе
					
					Если Выборка.СуммаУценкиИзменение > 0 Тогда
						// Для оборота по Затратам
						СтрокаДляФормированияДвижений.Стоимость           = Выборка.СуммаУценкиИзменение;
						СтрокаДляФормированияДвижений.СтоимостьПолучатель = Выборка.СуммаУценкиИзменение;		
					Иначе
						// Если это дооценивание до превоначальной стоимости, то движений по 
						// разделу учета "Затраты" быть не должно, это доход согласно 
						// п. 28 положення (стандарта) бухгалтерського обліку 9 "Запаси"
						СтрокаДляФормированияДвижений.ВыполнятьДвижениеОприходование = Ложь;
						СтрокаДляФормированияДвижений.ВыполнятьОприходование = Ложь;
						СтрокаДляФормированияДвижений.ВыполнятьДвижениеСписание = Ложь;
						СтрокаДляФормированияДвижений.ВыполнятьСписание = Ложь;
			...
			// Получаем исходную строку документа из таблицы по товарам
			СтрокаДокумента = ТаблицаПоТоварам[Выборка.НомерСтроки-1];
			
			// Добавим проводку по корректировке стоимости номенклатуры
			...
			Если Выборка.СуммаУценкиИзменение > 0 Тогда
				...
				Проводка.Сумма = Выборка.СуммаУценкиИзменение;
			Иначе			
				// Увеличение стоимости
				Проводка.СчетДт		= СтрокаДокумента.СчетУчетаБУ;
			...
			СтруктураПараметровДляСодержания = Новый Структура("КодыОпераций, ТипЗначенияРегистратора",Перечисления.КодыОперацийПартииТоваров, ТипЗнч(СтруктураШапкиДокумента.Ссылка));
			СтрокаДвиженияДляСодержания = Новый Структура("КодОперацииПартииТоваров", Перечисления.КодыОперацийПартииТоваров.ПереоценкаКупленных);
			Проводка.Содержание = УправлениеЗапасамиПартионныйУчетФормированиеПроводокРегл.ПолучитьСодержаниеПроводки(СтрокаДвиженияДляСодержания, СтруктураПараметровДляСодержания);
			
			// И проводку списания на результаты деятельности
			ДобавленнаяПроводка = Проводка;
			...
			Если Выборка.СуммаУценкиИзменение > 0 Тогда
				...
				КонецЦикла; 				
				Проводка.НалоговоеНазначениеДт = ДобавленнаяПроводка.НалоговоеНазначениеДт;
				Проводка.НалоговоеНазначениеКт = ДобавленнаяПроводка.НалоговоеНазначениеДт;
				
				Проводка.Содержание    = "Списание прочих операционных расходов";
			Иначе
				Проводка.СчетДт    	   = ДобавленнаяПроводка.СчетКт;
				Для каждого ВидСубконто Из ДобавленнаяПроводка.СчетКт.ВидыСубконто Цикл				
					Проводка.СубконтоДт[ВидСубконто.ВидСубконто] = ДобавленнаяПроводка.СубконтоКт[ВидСубконто.ВидСубконто];				
				КонецЦикла; 				

				//счет доходов 
				Проводка.СчетКт        = ПланыСчетов.Хозрасчетный.РезультатОперационнойДеятельности;
				
				Проводка.НалоговоеНазначениеДт = ДобавленнаяПроводка.НалоговоеНазначениеКт;
				Проводка.НалоговоеНазначениеКт = ДобавленнаяПроводка.НалоговоеНазначениеКт;
				
				Проводка.Содержание    = "Оприходование прочих операционных доходов";
	...
	Если НаборЗаписей.Модифицированность() И НаборЗаписей.Количество() > 0 Тогда
...
Процедура РасширеннаяАналитикаЗапасовИзменениеСостояния(Источник, Отказ, РежимПроведения) Экспорт
	...
	Если ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.РегламентнаяПереоценкаЗапасов") Тогда
		ЗаполнитьСуммыВДвиженияхДокументаРегламентнаяПереоценкаЗапасов(Источник, ТаблицаДляФормированияДвижений, Отказ);
		Если Отказ Тогда
			ПодготовитьИсточникДляВозвратаКлиенту(Источник);
			Возврат;
		КонецЕсли;
	...
	Если НЕ СтруктураНаборовЗаписейПоВидамУчета.Свойство("УчетЗатратРегл") = Неопределено 
	  И ТаблицаДляФормированияПроводок.Количество() > 0	Тогда
		...
		Если Не Источник.мСтруктураШапкиДокумента.Свойство("ЕстьНалогНаПрибыль") Тогда
		  	...
		  	КонецЕсли; 
		 Иначе 
		  	СтруктураПараметров.Вставить("ЕстьНалогНаПрибыль", Источник.мСтруктураШапкиДокумента.ЕстьНалогНаПрибыль);
			СтруктураПараметров.Вставить("ЕстьНалогНаПрибыльДо2015", Источник.мСтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
			СтруктураПараметров.Вставить("ЕстьНалогНаПрибыльС2015", Источник.мСтруктураШапкиДокумента.ЕстьНалогНаПрибыльС2015);
		 КонецЕсли;			
...
КонецПроцедуры // РасширеннаяАналитикаЗапасовИзменениеСостояния
...
Функция СформироватьДвиженияПоКорректировкамИспользованияНУ(
	...
	Если ТипЗнч(КорАналитикаВидаУчета) = Тип("СправочникСсылка.КлючиАналитикиУчетаПрочихЗатрат") Тогда
		...
		|	ЕСТЬNULL(РегистрКорАналитикаУчетаПрочихЗатрат.Субконто1, Неопределено) КАК Субконто1,
		|	ЕСТЬNULL(РегистрКорАналитикаУчетаПрочихЗатрат.Субконто2, Неопределено) КАК Субконто2,
		|	ЕСТЬNULL(РегистрКорАналитикаУчетаПрочихЗатрат.Субконто3, Неопределено) КАК Субконто3,
	...
	Если ИспользуетсяАналитикаПрочихЗатрат Тогда
		...
		Иначе
			...
			КорСубконто1                    = Выборка.Субконто1;
			КорСубконто2                    = Выборка.Субконто2;
			КорСубконто3                    = Выборка.Субконто3;
			КорИмяРегистраКорректировки     = "КорректировкиРеализацияНалоговыйУчет";
		КонецЕсли; 
	Иначе	
	...
	Если Количество = 0 И КорректировкаНДСКредит = 0 И НДСКредитПоФакту = 0 И СуммаКорректировкиСтоимость = 0 И СуммаКорректировкиСтоимостьНУ = 0 Тогда
		Возврат Ложь;
	КонецЕсли; 
	...
	Если СтруктураКорректировкаНДСКредит.ВосстановлениеНДС Тогда
		...
		ИначеЕсли КорИмяРегистраКорректировки = "КорректировкиСтроительствоОбъектовОсновныхСредствНалоговыйУчет" Тогда
			...
			БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетДт, НоваяПроводка.СубконтоДт, "СтатьиЗатрат",         КорСтатьяЗатрат);
		ИначеЕсли КорИмяРегистраКорректировки = "КорректировкиРеализацияНалоговыйУчет" Тогда
			БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетДт, НоваяПроводка.СубконтоДт, 1, КорСубконто1);
			БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетДт, НоваяПроводка.СубконтоДт, 2, КорСубконто2);
			БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетДт, НоваяПроводка.СубконтоДт, 3, КорСубконто3);
		КонецЕсли;	
	...
	Если КорректировкаНДСКредит <> 0 ИЛИ СуммаКорректировкиСтоимость <> 0 ИЛИ СуммаКорректировкиСтоимостьНУ <> 0 Тогда
		...
		Если КорИмяРегистраКорректировки <> "КорректировкиСтроительствоОбъектовОсновныхСредствНалоговыйУчет" 
			И КорИмяРегистраКорректировки <> "КорректировкиРеализацияНалоговыйУчет" 
...
Функция СформироватьДвиженияПоПередачеМатериаловВЭксплуатацию(
	...
	Если Сумма = 0 И СуммаНУ = 0 И НДСВходящий = 0 И НДСКредит = 0 И Количество = 0 И КоличествоНУ = 0 Тогда
		Возврат Ложь;			                   
	КонецЕсли; 
	...
	Если ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВРегламентированномУчете Тогда
		...
		Если БухгалтерскийУчетПовтИсп.УчетСуммНУСчет(СтрокаКорАналитика.СчетУчета) Тогда
			НоваяСтрока.СтоимостьНУ = НалоговыйУчет.ОпределитьСтоимостьНУОС(СтрокаКорАналитика.НалоговоеНазначение, НоваяСтрока.Стоимость, СуммаНУ, СтруктураПараметров.ЕстьНалогНаПрибыльДо2015, СтруктураПараметров.ЕстьНалогНаПрибыльС2015);
		Иначе
			НоваяСтрока.СтоимостьНУ = НалоговыйУчет.ОпределитьСтоимостьНУ(СтрокаКорАналитика.НалоговоеНазначение, СуммаНУ, СтруктураПараметров.ЕстьНалогНаПрибыльДо2015);
		КонецЕсли; 
