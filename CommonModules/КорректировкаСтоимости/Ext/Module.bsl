		...
		Для Каждого ЭлементПриемник Из СоотвПриемники Цикл
			...
			Если СтруктураДопПараметров.Учет = "Бух" Тогда
				...
				Если НалоговоеНазначениеИсточник <> НалоговоеНазначениеПриемник Тогда
					...
					Состояние.СтруктураСумм.СтоимостьНУ	= НалоговыйУчет.ОпределитьСтоимостьНУ(НалоговоеНазначениеПриемник, Состояние.СтруктураСумм.СтоимостьНУ, СтруктураДопПараметров.ЕстьНалогНаПрибыльДо2015);
...
КонецФункции // ПолучитьМассивИсключаемыхСчетов()
...
КонецФункции // ПолучитьМассивИсключаемыхСтатусовПартий()
...
Процедура ЗаполнитьЗапросПоСписаниюБух(Запрос, МассивНоменклатуры, ДопПараметры)
	...
	|	Источник.Качество,
	|	БракВПроизводстве.Продукция,
	|	ЕСТЬNULL(СтроительствоОС.ОбъектСтроительства, Источник.КорСубконто1) КАК ОбъектСтроительства,
	...
	|	Источник.Качество,
	|	БракВПроизводстве.Продукция,
	|	ЕСТЬNULL(СтроительствоОС.ОбъектСтроительства, Источник.КорСубконто1),
|	Источник.Организация,
	|	Источник.СчетУчета,
	|	Источник.КорСчет,
...
Процедура ОцифроватьСостояния(Таб, СоотвПараметровСостояний, ПрефиксПараметровНовогоСостояния, Учет)  
	...
	| 		ТОГДА ИсхТаб1.Стоимость - ЕСТЬNULL(ИсхТаб2.Стоимость, 0) ИНАЧЕ 0 КОНЕЦ КАК Стоимость,
	|";
	...
	Если Учет = "Бух" Тогда
		...
		|	ВЫБОР 
		|		КОГДА (ИсхТаб1.Количество > ЕСТЬNULL(ИсхТаб2.Количество, 0)) ИЛИ (ИсхТаб1.Количество = ЕСТЬNULL(ИсхТаб2.Количество, 0) И (ИсхТаб1.Источник < ИсхТаб1.Приемник))  
		|		ТОГДА ИсхТаб1.СтоимостьНУ - ЕСТЬNULL(ИсхТаб2.СтоимостьНУ, 0) ИНАЧЕ 0 КОНЕЦ КАК СтоимостьНУ,
		|	ВЫБОР 
		|		КОГДА (ИсхТаб1.Количество > ЕСТЬNULL(ИсхТаб2.Количество, 0)) ИЛИ (ИсхТаб1.Количество = ЕСТЬNULL(ИсхТаб2.Количество, 0) И (ИсхТаб1.Источник < ИсхТаб1.Приемник))  
	    |       ТОГДА ИсхТаб1.НДСВходящий - ЕСТЬNULL(ИсхТаб2.НДСВходящий, 0) ИНАЧЕ 0 КОНЕЦ КАК НДСВходящий,
		|	ВЫБОР 
		|		КОГДА (ИсхТаб1.Количество > ЕСТЬNULL(ИсхТаб2.Количество, 0)) ИЛИ (ИсхТаб1.Количество = ЕСТЬNULL(ИсхТаб2.Количество, 0) И (ИсхТаб1.Источник < ИсхТаб1.Приемник))  
	    |       ТОГДА ИсхТаб1.НДСКредит - ЕСТЬNULL(ИсхТаб2.НДСКредит, 0) ИНАЧЕ 0 КОНЕЦ КАК НДСКредит,
		|";
	КонецЕсли;	
	...
	Если Учет = "Бух" Тогда
		...
		|	ИЛИ (ВЫБОР 
		|		КОГДА (ИсхТаб1.Количество > ЕСТЬNULL(ИсхТаб2.Количество, 0)) ИЛИ (ИсхТаб1.Количество = ЕСТЬNULL(ИсхТаб2.Количество, 0) И (ИсхТаб1.Источник < ИсхТаб1.Приемник))  
	    |       ТОГДА ИсхТаб1.НДСВходящий - ЕСТЬNULL(ИсхТаб2.НДСВходящий, 0) ИНАЧЕ 0 КОНЕЦ) <> 0 
		|	ИЛИ (ВЫБОР 
		|		КОГДА (ИсхТаб1.Количество > ЕСТЬNULL(ИсхТаб2.Количество, 0)) ИЛИ (ИсхТаб1.Количество = ЕСТЬNULL(ИсхТаб2.Количество, 0) И (ИсхТаб1.Источник < ИсхТаб1.Приемник))  
	    |       ТОГДА ИсхТаб1.НДСКредит - ЕСТЬNULL(ИсхТаб2.НДСКредит, 0) ИНАЧЕ 0 КОНЕЦ) <> 0 
		|";
	КонецЕсли;
...
КонецФункции // ПолучитьСписокВершин
