...
Процедура ВыполнитьОбновлениеИнформационнойБазы() Экспорт
	...
	// При переходе на новый механизм обновлений БСП
	// регистр сведений ВерсииПодсистем может оказаться неинициализированным.
	// Инициализируем его значениями из константы НомерВерсииКонфигурации.
	ОбновлениеИнформационнойБазы.ОтказатьсяОтКонстантыНомерВерсииКонфигурации();
