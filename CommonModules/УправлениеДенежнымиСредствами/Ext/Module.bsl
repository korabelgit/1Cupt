...
Процедура ЗаполнитьПриходПоОснованию(ДокументОбъект, Основание, ПользовательДокумента) Экспорт
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаказПокупателя")
	 ИЛИ ТипЗнч(Основание) = Тип("ДокументСсылка.СчетНаОплатуПокупателю") Тогда
		...
		|ВЫБРАТЬ
		|
		|	ДокументСчет.СтавкаНДС КАК СтавкаНДС,
		|   СУММА(ДокументСчет.Сумма) КАК СУММА
		|
		|ИЗ 
		|
		|(
		...
		|	ВЫБОР
		|		КОГДА ДокументУслуги.Ссылка.УчитыватьНДС
		|				И (НЕ ДокументУслуги.Ссылка.СуммаВключаетНДС)
		|			ТОГДА ДокументУслуги.Сумма + ДокументУслуги.СуммаНДС
		|		ИНАЧЕ ДокументУслуги.Сумма
		|	КОНЕЦ КАК Сумма
		|ИЗ
		|	Документ."+ТипДокДляЗапроса+".Услуги КАК ДокументУслуги
		|ГДЕ
		|	ДокументУслуги.Ссылка = &СчетНаОплату
		|
		|ОБЪЕДИНИТЬ ВСЕ 
		|
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НеНДС),
		|   ДокументТара.Сумма КАК Сумма
		|ИЗ
		|	Документ."+ТипДокДляЗапроса+".ВозвратнаяТара КАК ДокументТара
		|ГДЕ
		|	ДокументТара.Ссылка = &СчетНаОплату
		|
		|) КАК ДокументСчет
		|
		|СГРУППИРОВАТЬ ПО
		|	ДокументСчет.СтавкаНДС
		|";
...
Процедура ОпределитьСчетаУчетаПлатежаПоУмолчанию(ДокументОбъект,СтрокаПлатеж,ПерезаписыватьЗначения)
	...
	Если НЕ ЕстьРасчетыПоКредитам(ВидОперации) Тогда
		...
		ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.АккредитивПолученный") 
			...
			ИЛИ ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПлатежноеТребованиеПоручениеВыставленное")
		...
		ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.АккредитивПереданный") 
			...
			ИЛИ ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПлатежноеТребованиеПоручениеПолученное")
...
Процедура ЗаполнитьРеквизитыРасчетногоДокумента(ДокументОбъект, ТекПользователь, ВалютаРегламентированногоУчета = Неопределено, 
	...
	Если ЕстьРасчетыСКонтрагентами ИЛИ ЕстьРасчетыПоКредитам Тогда
		...
		Если флВыполнитьЗаполнениеСтруктуры Тогда
			...
			ОсновнойДоговорКонтрагента                     = ОбщегоНазначения.ПолучитьЗначениеРеквизита(текКонтрагент, "ОсновнойДоговорКонтрагента",  Истина);
			Если ЗначениеЗаполнено(ОсновнойДоговорКонтрагента) Тогда
				ОсновнойДоговорКонтрагентаОрганизация          = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ОсновнойДоговорКонтрагента, "Организация", Истина);
				ОсновнойДоговорКонтрагентаВидДоговора          = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ОсновнойДоговорКонтрагента, "ВидДоговора", Истина);
				ОсновнойДоговорКонтрагентаВалютаВзаиморасчетов = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ОсновнойДоговорКонтрагента, "ВалютаВзаиморасчетов", Истина);
				ОсновнойДоговорКонтрагентаТипЦен               = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ОсновнойДоговорКонтрагента, "ТипЦен", Истина);
			Иначе
				ОсновнойДоговорКонтрагентаОрганизация          = Неопределено;
				ОсновнойДоговорКонтрагентаВидДоговора          = Неопределено;
				ОсновнойДоговорКонтрагентаВалютаВзаиморасчетов = Неопределено;
				ОсновнойДоговорКонтрагентаТипЦен               = Неопределено;
			КонецЕсли;	
			
			ДанныеКонтрагент.Вставить("ОсновнойДоговорКонтрагента", ОсновнойДоговорКонтрагента);
			ДанныеКонтрагент.Вставить("ОсновнойДоговорКонтрагентаОрганизация", ОсновнойДоговорКонтрагентаОрганизация);
			ДанныеКонтрагент.Вставить("ОсновнойДоговорКонтрагентаВидДоговора", ОсновнойДоговорКонтрагентаВидДоговора);
			ДанныеКонтрагент.Вставить("ОсновнойДоговорКонтрагентаВалютаВзаиморасчетов", ОсновнойДоговорКонтрагентаВалютаВзаиморасчетов);
			ДанныеКонтрагент.Вставить("ОсновнойДоговорКонтрагентаТипЦен", ОсновнойДоговорКонтрагентаТипЦен); 
...
Процедура ПровестиСписаниеДенежныхСредствРегл(
	...
	Если Ссылка.ОтражатьВБухгалтерскомУчете И Оплачено Тогда
		...
		ИначеЕсли ВидОперацииИмя = "ПеречислениеДенежныхСредствПодотчетнику" 
			...
			Если ВидОперацииИмя = "ПрочееСписаниеБезналичныхДенежныхСредств" Или ВидОперацииИмя = "РасходДенежныхСредствПрочее" Тогда
				...
				Проводка.СуммаНУДт = НалоговыйУчет.ОпределитьСтоимостьНУ(Проводка.НалоговоеНазначениеДт, Проводка.Сумма, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
		...
		ИначеЕсли ВидОперацииИмя = "ПеречислениеНалога" Тогда
			...
			Для каждого ТекущаяСтрока Из Ссылка.ПеречислениеНалогов Цикл
				...
				Если ТекущаяСтрока.СубконтоДт1 = Справочники.Налоги.ВоенныйСбор
					...
					Тогда
					ДвиженияПоВзаиморасчетыПоНДФЛ(СтруктураШапкиДокумента, мСтруктураПараметровДенежныхСредств, СтруктураДвижений, Истина);
					Прервать;
				КонецЕсли;
		...
		Если ВидОперацииИмя = "ПрочееСписаниеБезналичныхДенежныхСредств" Или ВидОперацииИмя = "РасходДенежныхСредствПрочее" Тогда
			...
			Если СтруктураШапкиДокумента.ОтражатьПоЗатратам Тогда
				...
				СтрокаЗатрат.СтоимостьНУ						= НалоговыйУчет.ОпределитьСтоимостьНУ(СтруктураШапкиДокумента.НалоговоеНазначениеДоходовИЗатрат, ТаблицаПлатежейУпр[0].СуммаРегл, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015); 
		...
		Если  ВидОперацииИмя = "ВозвратДенежныхСредствПокупателю" 
			 И СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 Тогда

			 Для Каждого СтрокаРасшифровки Из РасшифровкаПлатежа Цикл
				 
				Попытка
					ВозвратАвансаДо01042011НУ = СтрокаРасшифровки.ВозвратАвансаДо01042011НУ;
				Исключение
				    Продолжить;
				КонецПопытки; 
				...
				ЭтоПриобретение = (Найти(ВРЕГ(СтруктураШапкиДокумента.ВидДокумента), "ВХОДЯЩ") > 0) ИЛИ (Найти(ВРЕГ(СтруктураШапкиДокумента.ВидДокумента), "ПРИХОД") > 0) ИЛИ (Найти(ВРЕГ(СтруктураШапкиДокумента.ВидДокумента), "ПОСТУПЛЕНИЕ") > 0);
	...
	Если Ссылка.ОтражатьВБухгалтерскомУчете И (Оплачено ИЛИ НЕ Ссылка.ОтражатьВУправленческомУчете) Тогда
		...
		Если    СтруктураШапкиДокумента.ЕстьНДС 
			ИЛИ СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 Тогда
		...
		Если НЕ СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 Тогда
			Для каждого КлючСтруктуры из СтруктураДвижений Цикл
				...
				Если ДвижениеКЗаписи.Модифицированность() Тогда
...
Процедура ДвиженияПоВзаиморасчетыПоНДФЛ(СтруктураШапкиДокумента, мСтруктураПараметровДенежныхСредств, СтруктураДвижений, ВоенныйСбор = Ложь)
...
Процедура ДвиженияПоЕдиномуНалогуРегл(СтруктураШапкиДокумента, мСтруктураПараметровДенежныхСредств, СтруктураДвижений = Неопределено)
	...
	Если МетаданныеДокумента.Имя = "АвансовыйОтчет" Тогда
						  		 ...
						  		 ТаблицаПлатежей = Ссылка.Товары.			Выгрузить().Свернуть("СтатьяДекларацииПоЕдиномуНалогу","Сумма,СуммаНДС");
		ОбщегоНазначения.ЗагрузитьВТаблицуЗначений(Ссылка.ОплатаПоставщикам.Выгрузить().Свернуть("СтатьяДекларацииПоЕдиномуНалогу","Сумма,СуммаНДС"), ТаблицаПлатежей);
		ОбщегоНазначения.ЗагрузитьВТаблицуЗначений(Ссылка.Прочее.			Выгрузить().Свернуть("СтатьяДекларацииПоЕдиномуНалогу","Сумма,СуммаНДС"), ТаблицаПлатежей);
		ТаблицаПлатежей.Свернуть("СтатьяДекларацииПоЕдиномуНалогу","Сумма,СуммаНДС");
	...
	ИначеЕсли ЕстьРасчетыСКонтрагентами Или ВидОперацииИмя = "ПрочееСписаниеБезналичныхДенежныхСредств" Тогда
		...
		Если ТаблицаПлатежей.Колонки.Найти("ОстаточнаяСтоимостьОС") = Неопределено Тогда
			...
			ТаблицаПлатежей.Колонки.Добавить("ОстаточнаяСтоимостьОС", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
		Иначе
			ТаблицаПлатежей.Свернуть("СтатьяДекларацииПоЕдиномуНалогу","СуммаПлатежа,СуммаНДС,ОстаточнаяСтоимостьОС");
		КонецЕсли;
	Иначе
		Если НЕ Ссылка.Оплачено Тогда
			Возврат;
		...
		ТаблицаПлатежей.Колонки.Добавить("СуммаНДС", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
		...
		СтрокаТаблицыПлатежей.СуммаНДС  						= Ссылка.РасшифровкаПлатежа.Итог("СуммаНДС");
		
	КонецЕсли;
	
	РозничнаяВыручка = (ТипЗнч(Ссылка) = Тип("ДокументСсылка.ПриходныйКассовыйОрдер"))
		И (СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПКО.ПриходДенежныхСредствРозничнаяВыручка)
		И (СтруктураШапкиДокумента.Дата >= глЗначениеПеременной("ДатаВступленияВСилуЗакона4014"));
	Если РозничнаяВыручка И СтруктураШапкиДокумента.ЕстьНДС Тогда
		СуммаНДСПродажа = 0;
		СуммаНДСВозврат = 0;
		Для каждого СтрокаТЧ из Ссылка.ПриемРозничнойВыручки Цикл
	...
	Для Каждого СтрокаПлатеж Из ТаблицаПлатежей Цикл
		...
		Если СтруктураШапкиДокумента.ЕстьНДС И СтруктураШапкиДокумента.Дата >= глЗначениеПеременной("ДатаВступленияВСилуЗакона4014") Тогда
			СуммаЕН = СтрокаПлатеж.СуммаПлатежа - СтрокаПлатеж.СуммаНДС;
		Иначе
			СуммаЕН = СтрокаПлатеж.СуммаПлатежа;
		КонецЕсли;
		Если РозничнаяВыручка Тогда
			СуммаЕН = СтруктураШапкиДокумента.СуммаПродаж;
			Если СтруктураШапкиДокумента.ЕстьНДС Тогда
				СуммаЕН = СуммаЕН - СуммаНДСПродажа;	
			КонецЕсли;
			СуммаЕН = СуммаЕН - СуммаАкцизаПродажа;
		КонецЕсли;
		
		Если СчетОрганизации.ВалютаДенежныхСредств <> ВалютаРег Тогда
			Строка.Сумма	= МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СуммаЕН, СтруктураШапкиДокумента.ВалютаДокумента, ВалютаРег,
	...
	Если Ссылка.ОтражатьВБухгалтерскомУчете И (Оплачено ИЛИ НЕ Ссылка.ОтражатьВУправленческомУчете) Тогда
		...
		Если ВидОперацииИмя = "ПрочееПоступлениеБезналичныхДенежныхСредств" Или
			...
			ИначеЕсли ВидОперацииИмя = "ПолучениеНаличныхДенежныхСредствВБанке" Тогда
				...
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "БанковскиеСчета", Ссылка.СчетОрганизации);
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "СтатьиДвиженияДенежныхСредств", РасшифровкаПлатежа[0].СтатьяДвиженияДенежныхСредств);
			ИначеЕсли ВидОперацииИмя = "ПокупкаПродажаВалюты" Тогда
			...
			Если ВидОперацииИмя = "ПрочееПоступлениеБезналичныхДенежныхСредств" Или ВидОперацииИмя = "ПриходДенежныхСредствПрочее" Тогда
				...
				Проводка.СуммаНУКт = НалоговыйУчет.ОпределитьСтоимостьНУ(Проводка.НалоговоеНазначениеКт, Проводка.Сумма, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
			КонецЕсли;	
		...
		ИначеЕсли ВидОперацииИмя = "ПриходДенежныхСредствРозничнаяВыручка" Тогда
			...
			Если (СуммаПродаж > 0) И (СуммаПроводки <> 0) Тогда
			...
			Если (СуммаВозврата > 0) И (СуммаПроводкаи <> 0) Тогда
			...
			// Движения по регистру РозничнаяВыручкаОрганизаций
			...
			ТаблицаДвижений = НаборДвижений.Выгрузить();
			ТаблицаВыручки = ПриемРозничнойВыручки.Выгрузить();
			ТаблицаВыручки.Свернуть("Возврат, СтавкаНДС", "Сумма, СуммаНДС, СуммаАкцизногоНалога");
            Для Каждого СтрокаНДС из ТаблицаВыручки Цикл
				
				Если СтрокаНДС.Сумма = 0 И СтрокаНДС.СуммаНДС = 0 И СтрокаНДС.СуммаАкцизногоНалога = 0 Тогда
					Продолжить;
				КонецЕсли;
				...
				Если СтрокаНДС.Возврат = Истина Тогда
					...
					СтрокаДвижений.СуммаАкцизногоНалога = -СтрокаНДС.СуммаАкцизногоНалога;
		...
		Если ВидОперацииИмя = "ПоступлениеОплатыПоПлатежнымКартам" Тогда
			Если СтруктураШапкиДокумента.СуммаУслуг <> 0 Тогда
				...
				Иначе  
					...
					НоваяСтрока.СтоимостьНУ				 = НалоговыйУчет.ОпределитьСтоимостьНУ(СтруктураШапкиДокумента.НалоговоеНазначениеДоходовИЗатрат, СуммаУслугРегл, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015); 
		...
		Если  ВидОперацииИмя = "ВозвратДенежныхСредствПоставщиком" 
			 И СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 Тогда
