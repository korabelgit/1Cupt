	...
	Если СтрокаДвижений.Авторасчет Тогда
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ИндексацияАлиментов Тогда
			...
			Если ИсходныеДанные.БазаИндексации = NULL Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + ": Не определен размер алиментов для индексации  ");
			...
			Иначе
				...
				Если КомментироватьРасчет Тогда
					ОбщегоНазначения.КомментарийРасчета("Базовый месяц: " + БазовыйМесяц, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета("Коэффициент индексации: " + КоэффициентИндексации, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета("База: " + ИсходныеДанные.БазаИндексации, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета("Прожиточный минимум: " + ИсходныеДанные.АлиментыТрудоспособные, КомментарийВидаРасчета);
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ИндексацияЗарплаты Тогда
			...
			Иначе	
				...
				Если КомментироватьРасчет Тогда
					...
					ОбщегоНазначения.КомментарийРасчета("Коэффициент индексации: " + КоэффициентИндексации, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета("Размер фиксированной индексации: " + ИсходныеДанные.РезультатФиксированнаяИндексация, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета("Коэффициент индексации пропорционально: " + Окр(КоэффициентИндексацииПропорционально, 4), КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета("Фиксированная индексация: " + РезультатФиксированнаяИндексация, КомментарийВидаРасчета);
				КонецЕсли;
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ДоначислениеОтОбратного Тогда
			...
			Если ИсходныеДанные.РезультатБаза = NULL Тогда
				...
				Если КомментироватьРасчет Тогда
					ОбщегоНазначения.КомментарийРасчета("Сумма ""чистая"": " + СтрокаДвижений[ПоказательРазмер], КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета("Сумма ""от обратного"": " + СуммаОтОбратного, КомментарийВидаРасчета);
				КонецЕсли;
			Иначе
				Если КомментироватьРасчет Тогда
					КомментарийБазы = ОбщегоНазначения.КомментарийРасчета("Базовые начисления составили всего: " + ИсходныеДанные.РезультатБаза, КомментарийВидаРасчета,,,, Ложь);
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ДоплатаДоМЗП Тогда
			...
			Если РезультатБаза = 0 Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + ": Нет данных о базе для доначисления до минимальной зарплаты ");
				
			ИначеЕсли МинимальнаяЗарплата = 0 Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + ": Нет данных о размере минимальной зарплаты ");
			...
			Иначе	
				...
				Если КомментироватьРасчет Тогда
					...
					ОбщегоНазначения.КомментарийРасчета("Минимальная зарплата: " + МинимальнаяЗарплата, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета("Минимальная зарплата пропорционально: " + МинимальнаяЗарплатаПропорционально, КомментарийВидаРасчета);
				...
				Если НЕ СтрокаДвижений.Сторно Тогда
					...
					Иначе
						...
						Если КомментироватьРасчет Тогда
							...
							КомментарийБазы = ОбщегоНазначения.КомментарийРасчета("Базовые начисления составили всего: " + БазаВсего, КомментарийВидаРасчета,,,, Ложь);
							РасшифровкаБазы = ИсходныеДанные.Выбрать();
							Пока РасшифровкаБазы.Следующий() Цикл
							...
							Если БазаДоначисление <> 0 Тогда
								ОбщегоНазначения.КомментарийРасчета("	в т.ч. доначисление до минимальной базы : " + БазаДоначисление, КомментарийВидаРасчета);
							КонецЕсли;
							Если	СтрокаДвижений.ВидРасчета.СпособРасчетаПоШкале = Перечисления.СпособыРасчетаПоШкале.Прогрессивный Тогда 
							...
							Если МинимальнаяБаза <> 0 Тогда
								ОбщегоНазначения.КомментарийРасчета("Минимальная база : " + МинимальнаяБаза, КомментарийВидаРасчета);
							КонецЕсли;
							ОбщегоНазначения.КомментарийРасчета("Ставка : " + Ставка, КомментарийВидаРасчета);
							Если КоэффициентСтавки <> 0 И КоэффициентСтавки <> 1 Тогда
								 ОбщегоНазначения.КомментарийРасчета("Коэффициент ставки : " + КоэффициентСтавки, КомментарийВидаРасчета);
								 ОбщегоНазначения.КомментарийРасчета("Ставка с учетом коэф. : " + Окр(Ставка * КоэффициентСтавки,5), КомментарийВидаРасчета);
							КонецЕсли;	
		...
		Если КомментироватьРасчет Тогда
			...
			ИЛИ	СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ВзносыФОТ Тогда
				Если СтрокаДвижений.РезультатДоначисление <> 0 Тогда
					ОбщегоНазначения.КомментарийРасчета("	в т.ч. на сумму доначисления до минимальной базы: " + СтрокаДвижений.РезультатДоначисление, КомментарийВидаРасчета);
				КонецЕсли;
			КонецЕсли;	
		КонецЕсли;
	...
	Если НеобходимыеДанные.ПоказателиСхемМотивации Тогда
		...
		Если НеобходимыеДанные.ИндексацияЗарплаты Тогда
			...
			Если Истина Тогда
				...
				|      ИЛИ ПараметрыРасчетаИндексации.Сотрудник = ЗНАЧЕНИЕ(Справочник.СотрудникиОрганизаций.ПустаяСсылка))
				|    И КОНЕЦПЕРИОДА(Основной.ДатаНачала, МЕСЯЦ) >= ПараметрыРасчетаИндексации.Период
		...
		Если КомментироватьРасчет Тогда
			...
			ИначеЕсли ИсходныеДанные.ВариантНДФЛ = Перечисления.ВариантыАлгоритмовРасчетНДФЛ17.ДоначислениеНаСуммуПревышения Тогда 	
				...
				Если Превышение <> 0 Тогда
					ОбщегоНазначения.КомментарийРасчета("	по обычной ставке: " + (Налог-НалогПревышение), КомментарийВидаРасчета);
...
Процедура ПроверкаБазыЕСВ(ВидРасчета) Экспорт
	...
	Если табПересечений.Количество() > 0 Тогда 
		...
		Сообщить("Проверьте состав базы для взноса " + ВидРасчета + "!", СтатусСообщения.Важное);
		Сообщить("Виды расчета");
		...
		КонецЦикла;
		Сообщить("входят также в базу взносов "+СтрокаСообщенияВзносы);
	КонецЕсли;
...
Процедура ВыполнитьДвиженияПоЗарплате( ДокументВыплаты, Движения, ПроводкиБУ="", СчетКт, СпособОплаты, ВыплаченностьЗарплаты, 
	...
	|
	|ИТОГИ СУММА(СуммаВзаиморасчетов), СУММА(СуммаОкругления), МАКСИМУМ(Период), МАКСИМУМ(ПериодВзаиморасчетов), МАКСИМУМ(СтатьяДвиженияДенежныхСредств), МАКСИМУМ(РегистраторРанееВыплаченные)
	...
	Пока ВыборкаОрганизация.Следующий() Цикл
		...
		Пока ВыборкаСотрудник.Следующий() Цикл
			...
			Пока ВыборкаВедомость.Следующий() Цикл
				...
				Если НЕ (ВыборкаВедомость.РегистраторРанееВыплаченные <> NULL И ВыборкаВедомость.РегистраторРанееВыплаченные <> ДокументВыплаты) Тогда
