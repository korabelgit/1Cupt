...
Функция РегламентФормированияДокументовПоОрдерам_СформироватьДокументы(Настройка, ДатаНачала, ДатаОкончания, 
	...
	Иначе
		...
		ИмяРегистраЦен		= "ЗаказыПокупателей";
		ИмяИзмеренияОтбора 	= "ЗаказПокупателя";
	КонецЕсли;
...
//  Ложь   - при формировании документов произошли ошибки
//
Функция РегламентФормированияДокументовОтработанногоВремени_СформироватьДокументы(Настройка, ДатаНачала, ДатаОкончания,
	    Ошибка = "", СформированныеДокументы = Неопределено, УчтенныеДокументы = Неопределено)
	...
	|ПОМЕСТИТЬ ВТВсеФизЛицаЗаПериод
	|ИЗ
	...
	|			Сотрудник.Физлицо В
	|					(ВЫБРАТЬ
	|						ВсеФизЛицаЗаПериод.Сотрудник
	|					ИЗ
	|						ВТВсеФизЛицаЗаПериод КАК ВсеФизЛицаЗаПериод)
	...
	Если (ОписаниеПериодаДетализации = "Месяц") и (Настройка.СпособВводаДанных = Перечисления.СпособыВводаДанныхОВремени.ВЦеломЗаПериод) Тогда
	...
	Иначе
		...
		|	СУММА(ВложенныйЗапрос.ЧасовПоГрафику) 		КАК ЧасовПоГрафику,
		|	ВЫБОР
		|		КОГДА СУММА(ВложенныйЗапрос.КоличествоДнейПоГрафику - ВложенныйЗапрос.КоличествоОтработанныхДней) > 0
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ 										КАК Неявка
		|ИЗ 
		...
		|		1								КАК КоличествоДнейПоГрафику,
		|		0								КАК КоличествоОтработанныхДней
		|	ИЗ ВТДниПлановые 					КАК ДниПлановые	
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|		0,
		|		1
		|	ИЗ ВТДниФактические КАК ДниФактические) КАК ВложенныйЗапрос
		|	
		|СГРУППИРОВАТЬ ПО
		|	ВложенныйЗапрос.Сотрудник,
		|	ВложенныйЗапрос.ЗанимаемыхСтавок,
	...
	Пока ВыборкаКлючейДокументов.Следующий() Цикл
		...
		Если ОписаниеПериодаДетализации = "Месяц" Тогда
		...
		Иначе
			...
			КонстантыДляЗаполнения.Вставить("СпособУказанияПериода",	Перечисления.ПериодНачисленияЗарплаты.УказанныйПериод);
			КонстантыДляЗаполнения.Вставить("ДатаНачалаПериода",		НачалоДня(ВыборкаКлючейДокументов.Период));
			КонстантыДляЗаполнения.Вставить("ДатаОкончанияПериода",		КонецДня(ВыборкаКлючейДокументов.Период));
			КонстантыДляЗаполнения.Вставить("СпособВводаДанных",		Перечисления.СпособыВводаДанныхОВремени.ПоДням);
		...
		Если (ОписаниеПериодаДетализации = "Месяц") и (Настройка.СпособВводаДанных = Перечисления.СпособыВводаДанныхОВремени.ВЦеломЗаПериод) Тогда
			...
			Для каждого СтрокаИзТаблицы Из ТабличнаяЧастьТабеля Цикл
				
				СтокаТабличнойЧасти = НовыйДокумент.ОтработанноеВремяВЦеломЗаПериод.Добавить();
				СтокаТабличнойЧасти.Сотрудник	= СтрокаИзТаблицы.Сотрудник;
				СтокаТабличнойЧасти.Назначение	= СтрокаИзТаблицы.Сотрудник;
				СтокаТабличнойЧасти.ВидВремени1 = Явка;
				СтокаТабличнойЧасти.Часов1		= СтрокаИзТаблицы.ОтработаноЧасов;
				СтокаТабличнойЧасти.Дней1		= СтрокаИзТаблицы.КоличествоОтработанныхДней;
				
				РазницаЧасов = СтрокаИзТаблицы.ЧасовПоГрафику - СтрокаИзТаблицы.ОтработаноЧасов;
				Если РазницаЧасов > 0 Тогда
				
					СтокаТабличнойЧасти.ВидВремени2 = Неявка;
					СтокаТабличнойЧасти.Часов2		= РазницаЧасов;
					СтокаТабличнойЧасти.Дней2		= СтрокаИзТаблицы.КоличествоДнейНеявки;
		...
		Иначе
			...
			Для каждого СтрокаИзТаблицы Из ТабличнаяЧастьТабеля Цикл
				...
				НайденнаяСтрока = НовыйДокумент.ОтработанноеВремя.Найти(СтрокаИзТаблицы.Сотрудник, "Сотрудник");
				Если НайденнаяСтрока = Неопределено Тогда
					СтокаТабличнойЧасти =  НовыйДокумент.ОтработанноеВремя.Добавить();
					ПредыдущийНомерДня = 0;
					
					// Определение номера максимального отработанного дня.
					СтруктураПоиска = Новый Структура("Сотрудник", СтрокаИзТаблицы.Сотрудник); 
					МассивОтобранныхСтрокПоСотруднику = ТабличнаяЧастьТабеля.НайтиСтроки(СтруктураПоиска);
					КоличествоЗаписейВМассиве = МассивОтобранныхСтрокПоСотруднику.Количество();
					Если КоличествоЗаписейВМассиве > 0 Тогда
						ПоследнийОтработанныйДень = МассивОтобранныхСтрокПоСотруднику[КоличествоЗаписейВМассиве - 1].НомерДня
					Иначе
						ПоследнийОтработанныйДень = 31
					КонецЕсли; 
				Иначе
					СтокаТабличнойЧасти = НайденнаяСтрока;  
				КонецЕсли; 
				СтокаТабличнойЧасти.Сотрудник = СтрокаИзТаблицы.Сотрудник;
				СтокаТабличнойЧасти.Назначение	= СтрокаИзТаблицы.Сотрудник;
				
				РеквизитТЧВидВремени = "ПервыйВидВремени" + Строка(СтрокаИзТаблицы.НомерДня);
				РеквизитТЧПервыйЧасов = "ПервыйЧасов" + (СтрокаИзТаблицы.НомерДня);
				СтокаТабличнойЧасти[РеквизитТЧВидВремени]	= ?(СтрокаИзТаблицы.Неявка, Неявка, Явка);
				СтокаТабличнойЧасти[РеквизитТЧПервыйЧасов]	= ?(СтрокаИзТаблицы.Неявка, СтрокаИзТаблицы.ЧасовПоГрафику,СтрокаИзТаблицы.ОтработаноЧасов);
				
				// Если существует пропуск в отработаных днях, нужно проставить выходной.
				Если СтрокаИзТаблицы.НомерДня - ПредыдущийНомерДня > 1 Тогда
					
					Для сч = 2 По СтрокаИзТаблицы.НомерДня - ПредыдущийНомерДня Цикл
						СтокаТабличнойЧасти["ПервыйВидВремени" + Строка(ПредыдущийНомерДня + Сч -1)] = Выходной;
					КонецЦикла
					
				КонецЕсли;	
				
				// Если это последний отработаный день до окончания месяца, заполним дни до конца месяца
				Если СтрокаИзТаблицы.НомерДня = ПоследнийОтработанныйДень и ПоследнийОтработанныйДень < ПоследнийДеньМесяца Тогда
					Для Сч = СтрокаИзТаблицы.НомерДня + 1 По ПоследнийДеньМесяца Цикл
						
						СтокаТабличнойЧасти["ПервыйВидВремени" + Строка(Сч)] = Выходной;
						
					КонецЦикла;	
				КонецЕсли;
				
				ПредыдущийНомерДня = СтрокаИзТаблицы.НомерДня;
...
Функция ПолучитьДатуОкончанияПериодаФормированияДокументовПоРасписанию(Настройка)
...
КонецФункции // ПолучитьДатуОкончанияПериодаФормированияДокументов()
...
Функция УстановитьПериодФормированияДокументов(ОбъектНастройки, ГраницаОбработки, Ошибка = "") Экспорт
...
Функция ПолучитьДатуОкончанияПериодаФормированияДокументов(Регламент, ДатаНачалаПериода)
	...
	Если ТипЗнч(Регламент) = Тип("СправочникСсылка.НастройкиФормированияДокументовВыпускаПродукции")
		ИЛИ ТипЗнч(Регламент) = Тип("СправочникСсылка.НастройкиФормированияДокументовПоОрдерам")
		ИЛИ ТипЗнч(Регламент) = Тип("СправочникСсылка.НастройкиФормированияДокументовОтработанногоВремени") Тогда
	
		Возврат ПолучитьДатуОкончанияПериодаФормированияДокументовПоРасписанию(Регламент)
...
Функция СформироватьДокументы(Регламент, ДатаНачала, ДатаОкончания, Ошибка = "", СформированныеДокументы=Неопределено, УчтенныеДокументы=Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Регламент) = Тип("СправочникСсылка.НастройкиФормированияДокументовОтработанногоВремени") Тогда
		Возврат РегламентФормированияДокументовОтработанногоВремени_СформироватьДокументы(Регламент, ДатаНачала, ДатаОкончания, Ошибка, СформированныеДокументы, УчтенныеДокументы);
	Иначе
...
Функция ПолучитьМассивДокументовДляПроведения(Регламент, ПроверяемыеДокументы) Экспорт
	...
	ИначеЕсли ТипЗнч(Регламент) = Тип("СправочникСсылка.НастройкиФормированияДокументовПоОрдерам")
		ИЛИ ТипЗнч(Регламент) = Тип("СправочникСсылка.НастройкиФормированияДокументовОтработанногоВремени") Тогда
		Возврат ПроверяемыеДокументы;
	Иначе
...
Функция УдалятьИПроводитьДокументыВТранзакции(Регламент) 
	...
	ИначеЕсли ТипЗнч(Регламент) = Тип("СправочникСсылка.НастройкиФормированияДокументовВыпускаПродукции") 
		ИЛИ ТипЗнч(Регламент) = Тип("СправочникСсылка.НастройкиФормированияДокументовОтработанногоВремени") Тогда
