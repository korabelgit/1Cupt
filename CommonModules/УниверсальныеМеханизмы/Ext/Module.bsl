		...
		Пока ВыборкаСсылок.Следующий() Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Не ЭтоЗаменяемаяФорма Тогда
					НаименованиеПечатнойФормы = Выборка.Наименование;
				КонецЕсли;
...
Функция НапечататьДокументПоУмолчанию(Объект, КоличествоЭкземпляров = 0, НаПринтер = Ложь, Форма = Неопределено) Экспорт	
