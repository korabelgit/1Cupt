...
Процедура ЗаполнитьОбъектРедактированияАдресаПоСтруктуре(Объект, СтруктураАдреса) Экспорт
	...
	Иначе
		...
		Если ТипЗнч(ТипДома) = Тип("Строка") И ВРег(ТипДома) = "ВЛАДЕНИЕ" ИЛИ ВРег(ТипДома) = "ВОЛОДІННЯ" ИЛИ ТипДома = Перечисления.ТипыДомов.Владение Тогда
		...
		Если ТипЗнч(ТипКорпуса) = Тип("Строка") И ВРег(ТипКорпуса) = "СТРОЕНИЕ" ИЛИ ВРег(ТипКорпуса) = "CПОРУДА" ИЛИ ТипКорпуса = Перечисления.ТипыКорпусов.Строение Тогда
...
Функция ПолучитьПредставлениеАдресаПоСтруктуре(СтруктураАдреса, КодЯзыкаИБ=Неопределено) Экспорт
	...
	Если Не АдресЗаПределамиУкраины Тогда
		...
		Если ВПочтовомФормате Тогда
			...
			ДополнитьПредставлениеАдреса(ТекПредставление, СокрЛП(СтруктураАдреса.Дом), ", " +      ?(СтруктураАдреса.Свойство("ТипДома")     И ЗначениеЗаполнено(СтруктураАдреса.ТипДома),     Локализация.ПолучитьЛокализованныйСинонимОбъекта(СтруктураАдреса.ТипДома,     КодЯзыкаИБ), НСтр("ru = 'дом'; uk = 'будинок'",   КодЯзыкаИБ)) + " № ");
			ДополнитьПредставлениеАдреса(ТекПредставление, СокрЛП(СтруктураАдреса.Корпус), ", " +   ?(СтруктураАдреса.Свойство("ТипКорпуса")  И ЗначениеЗаполнено(СтруктураАдреса.ТипКорпуса),  Локализация.ПолучитьЛокализованныйСинонимОбъекта(СтруктураАдреса.ТипКорпуса,  КодЯзыкаИБ), НСтр("ru = 'корпус'; uk = 'корпус'", КодЯзыкаИБ)) + " ");
			ДополнитьПредставлениеАдреса(ТекПредставление, СокрЛП(СтруктураАдреса.Квартира), ", " + ?(СтруктураАдреса.Свойство("ТипКвартиры") И ЗначениеЗаполнено(СтруктураАдреса.ТипКвартиры), Локализация.ПолучитьЛокализованныйСинонимОбъекта(СтруктураАдреса.ТипКвартиры, КодЯзыкаИБ), НСтр("ru = 'кв.'; uk = 'кв.'",       КодЯзыкаИБ)) + "");
...
Функция ПолучитьПредставлениеАдреса(Запись, КодЯзыкаИБ=Неопределено) Экспорт
	...
	ДополнитьПредставлениеАдреса(ТекПредставление, СокрЛП(СтруктураЗаписи.Поле7), ", " + ?(ЗначениеЗаполнено(СтруктураЗаписи.ТипДома),     Локализация.ПолучитьЛокализованныйСинонимОбъекта(СтруктураЗаписи.ТипДома,     КодЯзыкаИБ), НСтр("ru='дом';uk='будинок'",   КодЯзыкаИБ)) + " № ");
	ДополнитьПредставлениеАдреса(ТекПредставление, СокрЛП(СтруктураЗаписи.Поле8), ", " + ?(ЗначениеЗаполнено(СтруктураЗаписи.ТипКорпуса),  Локализация.ПолучитьЛокализованныйСинонимОбъекта(СтруктураЗаписи.ТипКорпуса,  КодЯзыкаИБ), НСтр("ru='корпус';uk='корпус'", КодЯзыкаИБ)) + " ");
	ДополнитьПредставлениеАдреса(ТекПредставление, СокрЛП(СтруктураЗаписи.Поле9), ", " + ?(ЗначениеЗаполнено(СтруктураЗаписи.ТипКвартиры), Локализация.ПолучитьЛокализованныйСинонимОбъекта(СтруктураЗаписи.ТипКвартиры, КодЯзыкаИБ), НСтр("ru='кв.';uk='кв.'",         КодЯзыкаИБ)) + "");
...
Функция СведенияОЮрФизЛице(ЮрФизЛицо, ДатаПериода, ДляФизЛицаТолькоИнициалы = Истина, БанковскийСчет = Неопределено, КодЯзыка = Неопределено) Экспорт
	...
	Сведения = Новый Структура("Представление, ПолноеНаименование, КодПоЕДРПОУ, КодПоДРФО, ИНН, НомерСвидетельства, Телефоны, ЮридическийАдрес, ФактическийАдрес, Банк, МФО, КоррСчет, НомерСчета, АдресБанка, ПлательщикНалогаНаПрибыль, ИнформацияОСтатусеПлательщикаНалогов");
