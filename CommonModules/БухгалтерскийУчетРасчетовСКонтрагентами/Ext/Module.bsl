	...
	Для каждого СтрокаСчетаКредита Из ДвиженияКредит Цикл
		...
		Для каждого СтрокаСчетаДебета Из ДвиженияДебет Цикл
			...
			Если СтрокаСчетаКредита.ВалютнаяСуммаКт < СтрокаСчетаДебета.ВалютнаяСуммаДт Тогда
				...
				//КурсСоответствия	 = СтрокаСчетаКредита.Сумма / СуммаСоответствияВал;
				КурсСоответствия	 = ?(ЗначениеЗаполнено(СуммаСоответствияВал), СтрокаСчетаКредита.Сумма / СуммаСоответствияВал, 0);
			Иначе
				СуммаСоответствияВал = СтрокаСчетаДебета.ВалютнаяСуммаДт;
				//КурсСоответствия	 = СтрокаСчетаДебета.Сумма / СуммаСоответствияВал;
				КурсСоответствия	 = ?(ЗначениеЗаполнено(СуммаСоответствияВал), СтрокаСчетаДебета.Сумма / СуммаСоответствияВал, 0);				
			КонецЕсли;
...
Функция ОпределениеНаправленияДвиженияДляДокументаПриобретенияРеализации(ВидДокумента,ВидОперации = Неопределено) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ВидОперации) тогда
		...
		РасчетВозвратПоВидуОпераций.Вставить(Перечисления.ВидыОперацийПередачаТоваров.ВПереработкуБрак          		, ВидОперацииРасчет);
...
Функция ЗафиксироватьДвижениеДенегВРегистреРасчетов(Объект, СтруктураПараметров, ЗаписыватьДвижения, СтруктураДвижений) экспорт
	...
	Для каждого Реквизит из СтруктураПараметров Цикл
		...
		Если РеестрПлатежейКопия.Колонки.Найти(Реквизит.Ключ) = Неопределено Тогда
			РеестрПлатежейКопия.Колонки.Добавить(Реквизит.Ключ);
			РеестрПлатежейКопия.ЗаполнитьЗначения(Реквизит.Значение, Реквизит.Ключ);
		КонецЕсли;
		
	КонецЦикла;
	
	Если РеестрПлатежейКопия.Колонки.Найти("Документ") = Неопределено Тогда
    	РеестрПлатежейКопия.Колонки.Добавить("Документ");
	КонецЕсли;
	Если РеестрПлатежейКопия.Колонки.Найти("ВидОперации") = Неопределено Тогда
    	РеестрПлатежейКопия.Колонки.Добавить("ВидОперации");
	КонецЕсли;
	
	РеестрПлатежейКопия.ЗаполнитьЗначения(СтруктураПараметров.Ссылка,      "Документ");
	РеестрПлатежейКопия.ЗаполнитьЗначения(СтруктураПараметров.ВидОперации, "ВидОперации");
...
Процедура ДвижениеДенегПрочийРасход(Объект, РасчетыВВалюте, СчетКт, КурсДокумента=1, КратностьДокумента=1, СтруктураДвижений=Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ПлатежноеПоручениеИсходящее") Тогда
		...
		Иначе
			...
			ВидОперации = "Оплата";
			Если Объект.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПрочиеРасчетыСКонтрагентами Тогда
				ПрочиеРасчеты = Истина;
			КонецЕсли;	
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.АккредитивПереданный")
		...
		Иначе
			...
			ВидОперации = "Оплата";
			Если Объект.ВидОперации = Перечисления.ВидыОперацийСписаниеБезналичныхДенежныхСредств.ПрочиеРасчетыСКонтрагентами Тогда
				ПрочиеРасчеты = Истина;
			КонецЕсли;	
...
Процедура ДвижениеДенегПрочийПриход(Объект, РасчетыВВалюте, СчетДт, КурсДокумента=1, КратностьДокумента=1, СтруктураДвижений) Экспорт
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ПлатежноеПоручениеВходящее")
		...
		Контрагент      = Ссылка.Контрагент;
		СчетОрганизации = Ссылка.СчетОрганизации;
		Если Ссылка.ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ВозвратДенежныхСредствПоставщиком ИЛИ Ссылка.ВидОперации = Перечисления.ВидыОперацийППВходящее.ВозвратДенежныхСредствПоставщиком Тогда
			ВидОперации = "Возврат";
		Иначе
		//Если Ссылка.ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ОплатаПокупателя Тогда
			ВидОперации = "Оплата";
			Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ПрочиеРасчетыСКонтрагентами ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийППВходящее.ПрочиеРасчетыСКонтрагентами Тогда
				ПрочиеРасчеты = Истина;
			КонецЕсли;	
		КонецЕсли;
...
Функция ДвижениеДенег(СтруктураПараметров, СтруктураДвижений, ВалютаРегламентированногоУчета, РежимПроведения=Неопределено, Объект=Null, Отказ=Ложь, ЗаписыватьДвижения=Истина) Экспорт
	...
	Если ЭтоВозврат И (НЕ СтруктураПараметров.Свойство("ВидДокумента") ИЛИ (СтруктураПараметров.Свойство("ВидДокумента") И СтруктураПараметров.ВидДокумента <> "КорректировкаДолга")) Тогда	
	...
	Для каждого ТекущийПлатеж из РеестрПлатежей Цикл
		...
		Если (СтруктураПараметров.РасчетыВозврат = Перечисления.РасчетыВозврат.Возврат) И (СтруктураПараметров.ВалютаДокумента <> ВалютаРегламентированногоУчета) Тогда
			...
			//Получение основных данных документа
			ПараметрыОтбора = Новый Структура("ДоговорКонтрагента, СделкаРегл", ТекущийПлатеж.ДоговорКонтрагента, ТекущийПлатеж.СделкаРегл);
			ТаблицаПоВзаиморасчетам = СтруктураПараметров.Таблица.Скопировать(ПараметрыОтбора);
				...
				Если НайденныеСтроки.Количество() > 0 Тогда
					...
					Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
						...
						Если СтруктураПараметров.РасчетыВозврат = Перечисления.РасчетыВозврат.Расчеты Тогда
							...
							Возмещаем		= Макс(ОсталосьВозместить, НайденнаяСтрока.СуммаСНДСВал);//ведь здесь суммы отрицательные
...
Функция ВзаиморасчетыВВалютеОплата(СтруктураПараметров,ВалютаРегламентированногоУчета,РежимПроведения=Неопределено,ПроводкиБУ,Объект = Null,Отказ=Ложь,УдаляемыеВидыРегОпераций = Неопределено) Экспорт
	...
	Если Не (УчитыватьАванс или ВызванРегламентнойПроцедурой) Тогда
		...
		Для Каждого ТекущийПлатеж Из ТаблицаПлатежей Цикл
			...
			Движение.Контрагент			 = ТекущийПлатеж.ДоговорКонтрагента.Владелец;
			Движение.Организация 		 = Организация;
	...
	Иначе
		...
		Для Каждого Платеж Из ТаблицаПлатежей Цикл
			...
			Для Каждого ОстатокЗадолженности Из СостояниеРасчетов.РеестрЗадолженности Цикл
				...
				Движение.Активность			 = Истина;
				Движение.Период 			 = ДатаДока;
				Движение.Контрагент			 = Платеж.ДоговорКонтрагента.Владелец;
				Движение.Организация 		 = Организация;
				Движение.СчетОплаты			 = Платеж.СчетОплаты;
				Движение.СуммаВзаиморасчетов = ТекущееПокрытиеВал;
				Движение.СуммаРегл			 = ТекущееПокрытиеРегл;		
...
КонецФункции // ВзаиморасчетыВВалютеОплата
...
Функция ВзаиморасчетыВВалютеПриобретение(СтруктураПараметров,ВалютаРегламентированногоУчета,РежимПроведения=Неопределено,ПроводкиБУ,Объект = Null,Отказ=Ложь,УдаляемыеВидыРегОпераций=Неопределено, СтруктураДвижений=Неопределено) Экспорт
	...
	Для Каждого Долг Из ТаблицаЗадолженности Цикл
		...
		// Накапливаем погрешность пропорциональном изменении
		// стоимостей в разрезе Контрагент-Договор-Сделка 
        Погрешность = 0; ПогрешностьНУДт = 0; ПогрешностьНУКт = 0;
		...
		Для Каждого СтрокаТаблицы из ТаблицаДвижений Цикл
			...
			Если    СтрокаТаблицы.СчетКт             = Долг.СчетОплаты 
				...
				Если НЕ (   СтрокаТаблицы.СубконтоДт3 = Долг.СделкаРегл
					...
					Если СтрокаТаблицы.СчетДополнительный <> Долг.СчетОплаты Тогда
						Продолжить;
					КонецЕсли;
				...
				Проводка.Сумма                   = ОбщегоНазначения.ОкруглитьСУчетомПогрешности(СтрокаТаблицы.Сумма * Коэфф, 2, Погрешность);
				...
				Проводка.СуммаНУКт             = НалоговыйУчет.ОпределитьСтоимостьНУ(Проводка.НалоговоеНазначениеКт, ОбщегоНазначения.ОкруглитьСУчетомПогрешности(СтрокаТаблицы.СуммаНУКт * Коэфф, 2, ПогрешностьНУКт));
				...
				Проводка.СуммаНУДт             = НалоговыйУчет.ОпределитьСтоимостьНУ(Проводка.НалоговоеНазначениеДт, ОбщегоНазначения.ОкруглитьСУчетомПогрешности(СтрокаТаблицы.СуммаНУДт * Коэфф, 2, ПогрешностьНУДт));
		...
		Если Покупки Тогда
			...
			Иначе	
				...
				Если  НаборДвиженийПартииТоваров <> Неопределено
					...
					// Накапливаем погрешность пропорциональном изменении
					// стоимостей в разрезе Контрагент-Договор 
					// (ввести валютный документ поступления с разными сделками невозможно)
			        Погрешность = 0; ПогрешностьНУ = 0;
					Для ТекСтрока = 0 по КоличествоДвижений-1 Цикл
						...
						НоваяЗапись.Стоимость   = ОбщегоНазначения.ОкруглитьСУчетомПогрешности(НоваяЗапись.Стоимость   * Коэфф, 2, Погрешность);
						НоваяЗапись.СтоимостьНУ = ОбщегоНазначения.ОкруглитьСУчетомПогрешности(НоваяЗапись.СтоимостьНУ * Коэфф, 2, ПогрешностьНУ);
				...
				Если  НаборДвиженийЗатраты <> Неопределено
					...
					Для ТекСтрока = 0 по КоличествоДвижений-1 Цикл
						...
						НоваяЗапись.Сумма   = ОбщегоНазначения.ОкруглитьСУчетомПогрешности(НоваяЗапись.Сумма   * Коэфф, 2, Погрешность);
						НоваяЗапись.СуммаНУ = ОбщегоНазначения.ОкруглитьСУчетомПогрешности(НоваяЗапись.СуммаНУ * Коэфф, 2, ПогрешностьНУ);
						НоваяЗапись.ВидРегламентнойОперации = Перечисления.ВидыРегламентныхОпераций.КорректировкиПриВзаиморасчетахВВалюте;
					КонецЦикла;	
