...
Функция ПроверитьВозможностьОткрытияДочернейФормыРегламентиованногоОтчета(ЭтаФорма) Экспорт	
	...
	ТекстВопорса = НСтр("ru = 'Внимание! Начиная с "+ДатаСтрока+" для формирования регламентированных отчетов по организации """+ОрганизацияСтрока+""" используется ""1С:Звіт"". 
	                    |
...
Процедура ЗаписатьФайлДокументаНаДискету(ДокументОбъект, ТекущийПользователь, ИмяФайла) Экспорт
	...
	Длг.Заголовок = НСтр("ru='Укажите каталог';uk='Укажіть каталог'");
...
"Завантажити обраний звіт в інформаційну базу?'");
	...
	// Проверка на сущестрование отчета Объект.Метаданные().Имя произведена в начале процедуры
	ТекОтчет = РеглОтчеты(Объект.Метаданные().Имя); // Помещение отчета во внутренний кэш, без него ФормаРеглОтчета(...) не рабтает
	ТекФорма = ФормаРеглОтчета(Объект.Метаданные().Имя);
	ТекФорма.РежимВыбора = Ложь;
	ТекФорма.Открыть();
...
Функция РеглОтчеты(Наим, СоздаватьИНеЗапоминать = Ложь) Экспорт
	...
	Если МассивДублей.Количество() > 0 Тогда
		Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В справочнике ""Регламентированные отчеты"" обрнаружены дубли для объекта ""%1""'; uk = 'У довіднику ""Регламентовані звіти"" обрнаружени дублі для об''єкта ""%1""'"), СокрП(Наим)), СтатусСообщения.Информация);
		Для каждого СсылкаДубль Из МассивДублей Цикл
			Попытка
				...
				Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Элемент ""%1"" (%2) помечен на удаление'; uk = 'Елемент ""%1"" (%2) позначений на видалення'"), Строка(СсылкаДубль), Строка(СсылкаДубль.Код)), СтатусСообщения.БезСтатуса);
			Исключение
