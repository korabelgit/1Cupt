...
Функция ДанныеФизЛица(Организация, ФизЛицо, ДатаСреза, ФИОКратко = Истина) Экспорт
	...
	|		РаботникиОрганизацийСрезПоследних.ПодразделениеОрганизации КАК ПодразделениеОрганизации
	|	ИЗ
	|		РегистрСведений.РаботникиОрганизаций.СрезПоследних(&ДатаСреза,	
	|														   Сотрудник.ФизЛицо = &ФизЛицо 
	|														   И Организация = &Организация) КАК РаботникиОрганизацийСрезПоследних
	|	ГДЕ
	|		РаботникиОрганизацийСрезПоследних.ПричинаИзмененияСостояния <> ЗНАЧЕНИЕ(Перечисление.ПричиныИзмененияСостояния.Увольнение)
	// Выбор должности сначала по основному назначению. 
	|	УПОРЯДОЧИТЬ ПО
	|		ВЫБОР
...
КонецФункции // СформироватьМассивПрефиксовДляРИБИОрганизации
...
КонецФункции // СформироватьМассивПрефиксовПоОрганизации
...
Функция СформироватьЗаголовокДокумента(ДокументОбъект, НазваниеДокумента = "", КодЯзыка = "ru") Экспорт	
	...
	Если ДокументОбъект = Неопределено Тогда
		Возврат "";
	Иначе
			ЛокализованоеНазваниеДокумента = НазваниеДокумента;
			Если НЕ ЗначениеЗаполнено(НазваниеДокумента) Тогда
				ЛокализованоеНазваниеДокумента = Локализация.ПолучитьЛокализованныйСинонимОбъекта(ДокументОбъект, КодЯзыка);
			КонецЕсли; 
			Возврат ЛокализованоеНазваниеДокумента
					+ " № " 
					+ ПолучитьНомерНаПечать(ДокументОбъект)
					+ НСтр("ru=' от ';uk=' від '", КодЯзыка) 
					+ Формат(ДатаДока, "ДФ='дд ММММ гггг';Л=" + ЛокализацияПовтИсп.ОпределитьКодЯзыкаДляФормат(КодЯзыка));
	КонецЕсли;
...
Процедура ПроверитьНомерНалоговогоДокумента(Документ) Экспорт
	...
	Если НЕ СтарыеДанные.Филиал = ТекФилиал
		И Документ.Дата < глЗначениеПеременной("ДатаВступленияВСилуПриказа1307")
...
Процедура ДобавитьПрефиксыНалоговогоДокумента(ДокументОбъект, Префикс) Экспорт
   ...
   КонецЕсли;		
	...
	Если УчетнаяПолитика.ВестиНумерациюНалоговыхДокументовБезУчетаОбособленныхПодразделений = Ложь И ДокументОбъект.Дата < глЗначениеПеременной("ДатаВступленияВСилуПриказа1307") Тогда
		Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.НалоговаяНакладная") Тогда
...
Функция ПредставлениеДокументаПриПроведении(Источник) Экспорт
	...
	Иначе
		...
		Если МетаданныеДокумента.Реквизиты.Найти("ВидОперации") <> Неопределено Тогда
			ВидОперацииСтр = " (" + ПолучитьЗначениеРеквизита(Источник, "ВидОперации") + ")";
		КонецЕсли;
...
Функция СформироватьСтруктуруШапкиДокумента(ДокументОбъект) Экспорт
	...
	СтруктураШапкиДокумента.Вставить("ЕстьНалогНаПрибыльС2015" , Ложь);
	...
	Если СтруктураШапкиДокумента.Свойство("Организация")
		...
		И ((СтруктураШапкиДокумента.Свойство("ОтражатьВБухгалтерскомУчете") 
			...
			ИЛИ 
			(СтруктураШапкиДокумента.Свойство("ОтражатьВНалоговомУчете") 
		...
		УчетнаяПолитика = ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(СтруктураШапкиДокумента.Дата, СтруктураШапкиДокумента.Организация, Ложь);
	  	...
	  	Если ЗначениеЗаполнено(УчетнаяПолитика) Тогда
			...
			СтруктураШапкиДокумента.Вставить("ЕстьНалогНаПрибыльДо2015", СтруктураШапкиДокумента.ЕстьНалогНаПрибыль И (СтруктураШапкиДокумента.Дата <  глЗначениеПеременной("ДатаНКУ2015")));
			СтруктураШапкиДокумента.Вставить("ЕстьНалогНаПрибыльС2015" , СтруктураШапкиДокумента.ЕстьНалогНаПрибыль И (СтруктураШапкиДокумента.Дата >= глЗначениеПеременной("ДатаНКУ2015")));
...
Функция ПолучитьРегистрыПоВидамУчета()
	...
	РегистрыПоВидамУчета.Вставить("ОтражатьВУправленческомУчете", 
				"ГрафикиАмортизацииОС,
				|МестонахождениеОС,
	...
	РегистрыПоВидамУчета.Вставить("ОтражатьВБухгалтерскомУчете", 
				"ГрафикиАмортизацииОСБухгалтерскийУчет,
				...
				|КорректировкиПартииТоваровНалоговыйУчет,
				|КорректировкиСтроительствоОбъектовОсновныхСредствНалоговыйУчет,
				|НезавершенноеПроизводствоБухгалтерскийУчет,
				|ПартииТоваровНаСкладахБухгалтерскийУчет,
				|СтоимостьОСБухгалтерскийУчет,
				|СтроительствоОбъектовОсновныхСредствБухгалтерскийУчет,
				|ТоварыОрганизаций,
	...
	РегистрыПоВидамУчета.Вставить("ОтражатьВМеждународномУчете", 
...
Функция ПолучитьПараметрыУчетнойПолитикиРегл(Период, Организация, СообщатьОбОшибке = Истина, Заголовок = "") Экспорт	
	...
	Для каждого СтрокаУчетнойПолитики Из УчетнаяПолитика Цикл
		...
		Если СтрокаУчетнойПолитики.Организация = Организация
			...
			Результат.Вставить("ЕстьНалогНаПрибыльДо2015", Результат.ЕстьНалогНаПрибыль И (Период <  глЗначениеПеременной("ДатаНКУ2015")));
			Результат.Вставить("ЕстьНалогНаПрибыльС2015" , Результат.ЕстьНалогНаПрибыль И (Период >= глЗначениеПеременной("ДатаНКУ2015")));
...
// Возвращает НеВключатьНДСВСтоимостьПартий с упр. учетной политики, используется в схеме компоновки отчета ВаловаяПрибыльРасширеннаяАналитика
//
// Параметры:
//
// ВидУчета - Число - 0 - Упр. учет, 1 - Регл. учет 
// Период   - СтандартныйПериод - период, установленный в отчете
//
Функция ПолучитьНеВключатьНДСВСтоимостьПартий(ВидУчета, Период) Экспорт
	...
	Если Период = Неопределено ИЛИ НЕ ЗначениеЗаполнено(Период) ИЛИ НЕ ЗначениеЗаполнено(Период.ДатаОкончания) Тогда
		ДатаПолучения = ТекущаяДата();
	Иначе
		ДатаПолучения = Период.ДатаОкончания;
	КонецЕсли; 
	
	УчетнаяПолитикаУпр = ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиУпр(ДатаПолучения, Ложь); 
						
	Возврат ?(НЕ ЗначениеЗаполнено(УчетнаяПолитикаУпр), 
				Истина, 
				УчетнаяПолитикаУпр.НеВключатьНДСВСтоимостьПартий
			);
							
КонецФункции // ПолучитьНеВключатьНДСВСтоимостьПартий
...
Функция ОбщиеПараметрыБазовойФункциональности() Экспорт
	...
	ОбщиеПараметры.Вставить("МинимальноНеобходимаяВерсияПлатформы", "8.2.19.130");
