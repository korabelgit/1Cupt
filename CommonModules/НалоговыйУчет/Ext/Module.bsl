...
Функция ПолучитьСхемуНалогообложения(Дата, Организация) Экспорт
	...
	Если ЗначениеЗаполнено(УчетнаяПолитикаНУ.СхемаНалогообложения) Тогда
		СхемаНалогообложения = УчетнаяПолитикаНУ.СхемаНалогообложения;
	Иначе
		СхемаНалогообложения = Справочники.СхемыНалогообложения.НеПлательщик;
	КонецЕсли; 
	
	РеквизитыСхемаНалогообложения = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(СхемаНалогообложения, "НалогНаПрибыль, НДС, ЕдиныйНалог");
	СтруктураСхемаНалогообложения = Новый Структура;
	СтруктураСхемаНалогообложения.Вставить("НалогНаПрибыль",       РеквизитыСхемаНалогообложения.НалогНаПрибыль);
	СтруктураСхемаНалогообложения.Вставить("НДС",                  РеквизитыСхемаНалогообложения.НДС); 
	СтруктураСхемаНалогообложения.Вставить("ЕдиныйНалог",          РеквизитыСхемаНалогообложения.ЕдиныйНалог);
	СтруктураСхемаНалогообложения.Вставить("НалогНаПрибыльДо2015", СтруктураСхемаНалогообложения.НалогНаПрибыль И (Дата <  глЗначениеПеременной("ДатаНКУ2015")));
	СтруктураСхемаНалогообложения.Вставить("НалогНаПрибыльС2015" , СтруктураСхемаНалогообложения.НалогНаПрибыль И (Дата >= глЗначениеПеременной("ДатаНКУ2015")));
	
	Возврат СтруктураСхемаНалогообложения;
...
Процедура УстановитьВидимостьНалоговогоНазначенияПриВыводеСтроки(Знач Дата, Знач СчетЗатрат, Знач Субконто1, Знач Субконто2, Знач Субконто3, НалоговоеНазначение, ОформлениеСтроки, Знач ИмяНалоговогоНазначения = "НалоговоеНазначение") Экспорт
	
	Если Дата < глЗначениеПеременной("ДатаНКУ2015") Тогда
		Возврат;
	КонецЕсли;	
	
	Если БухгалтерскийУчетПовтИсп.СчетЗатратБудущихПериодов(СчетЗатрат) Тогда
		СчетБУ = БухгалтерскийУчет.СчетСписанияРБП(Субконто1, Субконто2, Субконто3);
	Иначе
		СчетБУ = СчетЗатрат;
	КонецЕсли; 	
		
	Если ЗначениеЗаполнено(СчетБУ) И НЕ БухгалтерскийУчетПовтИсп.УчетПоНалоговымНазначениямНДССчет(СчетБУ) Тогда
		...
		КонецЕсли; 
		ОформлениеСтроки.Ячейки[ИмяНалоговогоНазначения].ТолькоПросмотр        = Истина;		
		ОформлениеСтроки.Ячейки[ИмяНалоговогоНазначения].ОтметкаНезаполненного = Ложь;		
	КонецЕсли;		

КонецПроцедуры // УстановитьВидимостьНалоговогоНазначения
...
#КонецЕсли		
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		...
		Если ЕстьОблагаемыеОперации Тогда
			...
			Если ЗначениеЗаполнено(НалоговаяНакладнаяОбъект.ДоговорКонтрагента.ФормаРасчетов) Тогда
				НалоговаяНакладнаяОбъект.ФормаРасчетов = НалоговаяНакладнаяОбъект.ДоговорКонтрагента.ФормаРасчетов;
				Если НалоговаяНакладнаяОбъект.Дата >= '2014-03-01' И НалоговаяНакладнаяОбъект.Дата < '2015-01-01' Тогда
					Если НалоговаяНакладнаяОбъект.ФормаРасчетов = "Оплата з поточного рахунка" Тогда
						НалоговаяНакладнаяОбъект.ФормаРасчетов = "Оплата з поточного рахунку";
					КонецЕсли;
				ИначеЕсли НалоговаяНакладнаяОбъект.Дата >= '2015-01-01' Тогда
					Если НалоговаяНакладнаяОбъект.ФормаРасчетов = "Оплата з поточного рахунку" Тогда
						НалоговаяНакладнаяОбъект.ФормаРасчетов = "Оплата з поточного рахунка";
					КонецЕсли;
				КонецЕсли;
			Иначе	
				НалоговаяНакладнаяОбъект.ФормаРасчетов = "Оплата з поточного рахунку";
				Если глЗначениеПеременной("ДатаВступленияВСилуПриказа1379") <= НалоговаяНакладнаяОбъект.Дата И НалоговаяНакладнаяОбъект.Дата < '2014-03-01' ИЛИ НалоговаяНакладнаяОбъект.Дата >= '2015-01-01' Тогда 
					НалоговаяНакладнаяОбъект.ФормаРасчетов = "Оплата з поточного рахунка";
				КонецЕсли;
			КонецЕсли;
			НалоговаяНакладнаяОбъект.УстановитьТипПричиныНевыдачиПокупателюПоУмолчанию();
...
Функция ОпределитьСтоимостьНУ(Знач НалоговоеНазначение, Знач СтоимостьНУ, ЕстьНалогНаПрибыльДо2015 = Истина) Экспорт	

	Если (НЕ ЕстьНалогНаПрибыльДо2015)
		ИЛИ НалоговоеНазначение = Справочники.НалоговыеНазначенияАктивовИЗатрат.НКУ_НеХозДеятельность 
...
Процедура ПроверитьЗаполнениеНалоговыхНазначений(
	...
	Если Отказ 
		...
		ИЛИ НЕ (СтруктураШапкиДокумента.ЕстьНДС ИЛИ СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015) 
		Тогда
		Возврат;
	КонецЕсли;	
	...
	Если (ВидОперации = "Поступление" ИЛИ ВидОперации = "Списание" ИЛИ ВидОперации = "ОтражениеЗатрат") И ЭтоЗатраты Тогда
		...
		Для Каждого СтрокаТаблицы Из ТаблицаДокумента Цикл
			...
			// проверим на РБП
			Если БухгалтерскийУчетПовтИсп.СчетЗатратБудущихПериодов(СтрокаТаблицы[ИмяРеквизитаСчетЗатрат]) Тогда
				СчетБУ = БухгалтерскийУчет.СчетСписанияРБП(СтрокаТаблицы[ИмяРеквизитаСубконтоЗатрат+"1"], СтрокаТаблицы[ИмяРеквизитаСубконтоЗатрат+"2"], СтрокаТаблицы[ИмяРеквизитаСубконтоЗатрат+"3"]);
			Иначе
				СчетБУ = СтрокаТаблицы[ИмяРеквизитаСчетЗатрат];
			КонецЕсли; 	
			ХарактерЗатратБУ = УправлениеЗатратами.ПолучитьХарактерЗатратПоСчетуЗатрат(СчетБУ, Неопределено);
			
			Если СтруктураШапкиДокумента.Дата < глЗначениеПеременной("ДатаНКУ2015") Тогда
				Если ВидОперации = "Списание" Тогда
					Продолжить;
				КонецЕсли; 
				Если ХарактерЗатратБУ = Перечисления.ХарактерЗатрат.ПроизводственныеРасходы 
					...
					ИЛИ (ХарактерЗатратБУ = Перечисления.ХарактерЗатрат.ОбщепроизводственныеРасходы И СтруктураШапкиДокумента.Дата >= глЗначениеПеременной("ДатаНачалаРаспределенияОПЗвНУ"))
				...
				Иначе
					...
					Если СтруктураШапкиДокумента.ЕстьНДС И СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 И ЕстьКолонкаНалоговоеНазначение 
					...
					Если СтруктураШапкиДокумента.ЕстьНДС И СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 И ЕстьКолонкаНалоговоеНазначение	
						...
						СтрокаСообщения = ?(ЭтоПроверкаШапки, "В документе", "В табличной части " + ИмяТабличнойЧасти + " в строке " + СтрокаТаблицы.НомерСтроки)+  
										  ...
										  "при этом в налоговом назначении затрат не указан нехозяйственный вид налоговой деятельности!";
						ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщения, Отказ, Заголовок);
					
					КонецЕсли; 	
					
				КонецЕсли; 
			Иначе // СтруктураШапкиДокумента.Дата >= глЗначениеПеременной("ДатаНКУ2015")
				Если СтруктураШапкиДокумента.ЕстьНДС 
					 И ЕстьКолонкаНалоговоеНазначение 
					 И (БухгалтерскийУчетПовтИсп.УчетПоНалоговымНазначениямНДССчет(СчетБУ) ИЛИ ВидОперации = "Поступление")
					 И Не ЗначениеЗаполнено(СтрокаТаблицы.НалоговоеНазначение) 
				...
				Если СтруктураШапкиДокумента.ЕстьНДС 
					 И ЕстьКолонкаНалоговоеНазначениеДоходовИЗатрат 
					 И (БухгалтерскийУчетПовтИсп.УчетПоНалоговымНазначениямНДССчет(СчетБУ) 
					    ИЛИ (ВидОперации = "Списание" И ЭтоЗатраты )
					 )
					 И Не ЗначениеЗаполнено(СтрокаТаблицы.НалоговоеНазначениеДоходовИЗатрат) Тогда
	...
	Если ВидОперации = "Реализация" Тогда
		...
		Для Каждого СтрокаТаблицы Из ТаблицаДокумента Цикл
			...
			Если СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015
				...
				Если ГруппаНалоговогоНазначенияЗатрат <> Перечисления.ГруппыНалоговыхНазначений.НалоговыеНазначенияДоходовИЗатрат Тогда
					СтрокаСообщения = ?(ЭтоПроверкаШапки, "В документе", "В табличной части " + ИмяТабличнойЧасти + " в строке " + СтрокаТаблицы.НомерСтроки)+ 
					                  " в налоговом назначении доходов должно быть налоговое назначение из группы Доходов и затрат!"; 
					ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщения, Отказ, Заголовок);
				КонецЕсли; 
				...
				Иначе
					ВидНалоговойДеятельностиДоходовИЗатрат = НалоговыйУчетПовтИсп.ПолучитьВидНалоговойДеятельностиНалоговогоНазначения(СтрокаТаблицы.НалоговоеНазначениеДоходовИЗатрат);
				КонецЕсли;	
				// в налоговом назначении доходов не может быть указано налоговое назначение с видом налоговой деятельности - распределяется
				Если СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 	
					И ВидНалоговойДеятельностиДоходовИЗатрат = Справочники.ВидыНалоговойДеятельности.Распределяется Тогда
					
					СтрокаСообщения = ?(ЭтоПроверкаШапки, "В документе", "В табличной части " + ИмяТабличнойЧасти + " в строке " + СтрокаТаблицы.НомерСтроки)+  
					                  " в налоговом назначении доходов указан вид налоговой деятельности ""Распределяется""!"; 
					ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщения, Отказ, Заголовок);
...
Функция ПолучитьСуммуКорректировкиНалоговогоКредита(
	...
	Иначе	
		...
		Если ДатаДокумента <> Неопределено И ДатаДокумента >= глЗначениеПеременной("ДатаНКУ2015") Тогда
			СтруктураВозврата.Вставить("СуммаКорректировкиСтоимостьНУ", 0);	
		Иначе
...
Процедура ПроверитьНалоговыеНазначенияПриВыпускеПродукции(
	...
	Если НЕ ЗначениеЗаполнено(СчетЗатратПолучатель) Тогда
		Возврат;
	КонецЕсли; 
	
	// Контролируем соответствие группы НалоговоеНазначениеДоходовИЗатрат счету затрат-получателю
	ГруппаСчетаЗатрат = НалоговыйУчетПовтИсп.ОпределитьГруппуНалоговогоНазначенияПоСчету(СчетЗатратПолучатель, СтруктураШапкиДокумента.Дата);
	
	// Контроль заполнения налогового назначения-получателя в зависимости от группы и учетной политики
	Если (ГруппаСчетаЗатрат = Перечисления.ГруппыНалоговыхНазначений.НалоговыеНазначенияАктивовИВзаиморасчетовПоНДС 
		И СтруктураШапкиДокумента.ЕстьНДС 
		И Не ЗначениеЗаполнено(НалоговоеНазначениеДоходовИЗатрат)) 
		
		ИЛИ (ГруппаСчетаЗатрат = Перечисления.ГруппыНалоговыхНазначений.НалоговыеНазначенияДоходовИЗатрат 
		...
		Тогда
			...
			Если ЕстьНаправленияСписания Тогда
				СтрокаСообщения = "В табличной части """ + ИмяТабличнойЧасти + """ в строке " + НомерСтроки + 
				" не заполнено налоговое назначение затрат-получателя в направлениях списания для строки " + НомерСтрокиНаправления + " !";
			Иначе
				СтрокаСообщения = "В табличной части """ + ИмяТабличнойЧасти + """ в строке " + НомерСтроки + 
				" не заполнено налоговое назначение затрат-получателя!";
			КонецЕсли;	
...
Процедура ЗаполнитьНалоговыеНазначенияВТабличныхЧастяхПередЗаписьюДокумента(
	...
	УстанавливатьНалоговоеНазначениеДоходовИЗатратПоНалоговомуНазначениюНДС = Истина
	) Экспорт
	...
	ЕстьНалогНаПрибыльДо2015 = УчетнаяПолитика.ЕстьНалогНаПрибыльДо2015;
	
	ЭтоПереработка = (ВидОперации = "Переработка");
	
	Если ЕстьНДС И ЕстьНалогНаПрибыльДо2015 И (НЕ ЭтоПереработка) Тогда	
		Возврат;
	КонецЕсли; 
	
	ЗаполнятьНалоговоеНазначениеДоходовИЗатрат = (НЕ ЕстьНалогНаПрибыльДо2015) И (ВидОперации = "Реализация");
	
	УслугиПоступление = (НЕ ЕстьНалогНаПрибыльДо2015) И (ВидОперации = "Поступление");
	
	УслугиРеализация = (НЕ ЕстьНалогНаПрибыльДо2015) И (ВидОперации = "Реализация");
	...
	ПустоеНалоговоеНазначение = Справочники.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка();
	...
	Если ТабличнаяЧастьТовары <> Неопределено Тогда
		Если ЭтоПереработка Тогда
			Для Каждого СтрокаТабличнойЧасти Из ТабличнаяЧастьТовары Цикл
			    СтрокаТабличнойЧасти.НалоговоеНазначение = ПустоеНалоговоеНазначение;
			КонецЦикла; 
		ИначеЕсли Не ЕстьНДС Тогда
			Для Каждого СтрокаТабличнойЧасти Из ТабличнаяЧастьТовары Цикл
	...
	Если ЕстьНДС И ЕстьНалогНаПрибыльДо2015 Тогда
		Возврат;
	КонецЕсли; 
	...
	Если ТабличнаяЧастьУслуги <> Неопределено Тогда
		...
		ИначеЕсли УслугиПоступление Тогда
			...
			Для Каждого СтрокаТабличнойЧасти Из ТабличнаяЧастьУслуги Цикл
				...
				ГруппаНалоговогоНазначенияЗатратБезУчетаДаты = НалоговыйУчетПовтИсп.ОпределитьГруппуНалоговогоНазначенияЗатрат(
					СтрокаТабличнойЧасти[ИмяРеквизитаСчетЗатрат], 
					Субконто1, 
					Субконто2, 
					Субконто3,
				...
				Если ГруппаНалоговогоНазначенияЗатрат = Перечисления.ГруппыНалоговыхНазначений.НалоговыеНазначенияАктивовИВзаиморасчетовПоНДС Тогда
					Если Не ЕстьНДС Тогда
						Если ГруппаНалоговогоНазначенияЗатратБезУчетаДаты = Перечисления.ГруппыНалоговыхНазначений.НалоговыеНазначенияАктивовИВзаиморасчетовПоНДС Тогда
							СтрокаТабличнойЧасти.НалоговоеНазначениеДоходовИЗатрат = НалоговоеНазначениеНеПлательщикНДС;
						Иначе
							// налоговое назначение доходов и затрат устанавливаем пустым
							СтрокаТабличнойЧасти.НалоговоеНазначениеДоходовИЗатрат = Справочники.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка(); 
						КонецЕсли;	
					Иначе
						Если НЕ НеЗаполнятьНалоговоеНазначение И УстанавливатьНалоговоеНазначениеДоходовИЗатратПоНалоговомуНазначениюНДС И Дата >= глЗначениеПеременной("ДатаНКУ2015") Тогда
							Если ГруппаНалоговогоНазначенияЗатратБезУчетаДаты = Перечисления.ГруппыНалоговыхНазначений.НалоговыеНазначенияАктивовИВзаиморасчетовПоНДС Тогда
								// налоговое назначение доходов и затрат устанавливаем равным налоговому назначению НДС 
								СтрокаТабличнойЧасти.НалоговоеНазначениеДоходовИЗатрат = СтрокаТабличнойЧасти.НалоговоеНазначение;								
							Иначе
								// налоговое назначение доходов и затрат устанавливаем пустым
								СтрокаТабличнойЧасти.НалоговоеНазначениеДоходовИЗатрат = Справочники.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка(); 
							КонецЕсли;	
...
Процедура ЗаполнитьНалоговыеНазначенияВШапкеПередЗаписьюДокумента(
	...
	УчитыватьДатуДокументаПриОпределенииГруппыНалоговогоНазначенияЗатрат = Ложь
	) Экспорт
	...
	ГруппаНалоговогоНазначенияЗатрат = НалоговыйУчетПовтИсп.ОпределитьГруппуНалоговогоНазначенияЗатрат(
		...
		Субконто3,
		ДокументОбъект.Дата,
		УчитыватьДатуДокументаПриОпределенииГруппыНалоговогоНазначенияЗатрат
	);
...
Процедура ОчиститьНалоговыеНазначенияЗатратВДокументеСписания(
	СтруктураШапкиДокумента, 
	ТаблицаДокумента = Неопределено, // Неопределено - в случае очистки шапки документа
	ИмяРеквизитаСчетЗатрат = "СчетЗатрат",
	ИмяРеквизитаНалоговоеНазначение = "НалоговоеНазначение",
	ИмяРеквизитаНалоговоеНазначениеДоходовИЗатрат = "НалоговоеНазначениеДоходовИЗатрат"
	) Экспорт
	...
	ЭтоОчисткаШапки = (ТаблицаДокумента = Неопределено);
	
	Если ЭтоОчисткаШапки Тогда
		Если НЕ БухгалтерскийУчетПовтИсп.УчетПоНалоговымНазначениямНДССчет(СтруктураШапкиДокумента[ИмяРеквизитаСчетЗатрат]) 
			 И СтруктураШапкиДокумента[ИмяРеквизитаНалоговоеНазначение] = СтруктураШапкиДокумента[ИмяРеквизитаНалоговоеНазначениеДоходовИЗатрат] 
			 
			 Тогда
			 
			 СтруктураШапкиДокумента[ИмяРеквизитаНалоговоеНазначениеДоходовИЗатрат] = Справочники.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка(); 
			
		
		КонецЕсли; 
	Иначе
		Для Каждого СтрокаТаблицы Из ТаблицаДокумента Цикл
			Если НЕ БухгалтерскийУчетПовтИсп.УчетПоНалоговымНазначениямНДССчет(СтрокаТаблицы[ИмяРеквизитаСчетЗатрат]) 
				 И СтрокаТаблицы[ИмяРеквизитаНалоговоеНазначение] = СтрокаТаблицы[ИмяРеквизитаНалоговоеНазначениеДоходовИЗатрат] 
				 
				 Тогда
				 
				 СтрокаТаблицы[ИмяРеквизитаНалоговоеНазначениеДоходовИЗатрат] = Справочники.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка(); 
...
КонецПроцедуры // ОчиститьНалоговыеНазначенияЗатратВДокументеСписания()
...
Функция ПолучитьСуммыОблагаемыхПоставокПоНДС(Организация, НачПериода, КонПериода, СпецРежимНалогообложения = 0) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |				КОГДА НДСНалоговыеОбязательстваОбороты.СтатьяКнигиПродаж В (&МассивОблагаемыеОперации)
	               ...
	               |ИЗ
	               |	РегистрНакопления.КнигаПродаж.Обороты(
	               |			&НачПериода,
	               |			&КонПериода,
	               |			Регистратор,
	               |			Организация = &Организация
	               |				И СтатьяКнигиПродаж В (&МассивВсеОперации)) КАК НДСНалоговыеОбязательстваОбороты
				   | ГДЕ 
...
КонецФункции // ПолучитьСуммыОблагаемыхПоставокПоНДС
...
Функция ПолучитьСписокВыбораСпецРежимНалогообложения() Экспорт
	...
	мСписокВыбораСпецРежимНалогообложения.Добавить(3, "3 (до 01.01.2017)  сільськогосподарські підприємства усіх форм власності,
...
КонецФункции // ПолучитьСписокВыбораСпецРежимНалогообложения
...
Функция ОпределитьСтоимостьНУОС(Знач НалоговоеНазначение, Знач СтоимостьБУ, Знач СтоимостьНУ, Знач ЕстьНалогНаПрибыльДо2015, Знач ЕстьНалогНаПрибыльС2015) Экспорт
	...
	ИначеЕсли ЕстьНалогНаПрибыльС2015 Тогда
		Результат = СтоимостьБУ;
	ИначеЕсли ЕстьНалогНаПрибыльДо2015 Тогда 
		Результат = СтоимостьНУ;
	Иначе
		Результат = 0;		
	КонецЕсли; 
...
КонецФункции // ОпределитьСтоимостьНУОС()

Функция ОпределитьСтоимостьНУВПроводке(Знач Период, Знач СтоимостьБУ, Знач СтоимостьНУ, Знач СчетДт, Знач СчетКт, Знач НалоговоеНазначениеДт, Знач ЕстьНалогНаПрибыльДо2015 = Истина, Знач ЕстьНалогНаПрибыльС2015 = Истина) Экспорт	
	
	Если Период < глЗначениеПеременной("ДатаНКУ2015") Тогда
		
		Если (НЕ ЕстьНалогНаПрибыльДо2015)
			ИЛИ НалоговоеНазначениеДт = Справочники.НалоговыеНазначенияАктивовИЗатрат.НКУ_НеХозДеятельность 
			ИЛИ НалоговоеНазначениеДт = Справочники.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяНеХозДеятельность
			ИЛИ (НЕ ЗначениеЗаполнено(НалоговоеНазначениеДт)) Тогда
	...
	Иначе
		...
		Если (НЕ ЕстьНалогНаПрибыльС2015)
			ИЛИ НалоговоеНазначениеДт = Справочники.НалоговыеНазначенияАктивовИЗатрат.НКУ_НеХозДеятельность 
			ИЛИ НалоговоеНазначениеДт = Справочники.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяНеХозДеятельность
			ИЛИ (НЕ ЗначениеЗаполнено(НалоговоеНазначениеДт)) Тогда
		...
		Иначе
			...
			Если БухгалтерскийУчетПовтИсп.УчетСуммНУСчет(СчетДт) Тогда
				Если БухгалтерскийУчетПовтИсп.УчетСуммНУСчет(СчетКт) Тогда
					Результат = СтоимостьНУ;
				Иначе
	                Результат = СтоимостьБУ;
				КонецЕсли; 
...
КонецФункции // ОпределитьСтоимостьНУВПроводке
