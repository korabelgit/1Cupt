...
Функция ПолучитьПотребность(Источники, Результат, Параметры = Неопределено) Экспорт
	...
	Если Источники.Колонки.Найти("НомерСтрокиПродукция") <> Неопределено Тогда   
		...
		ПотребностиПоСпецификациям.Колонки.Добавить("НомерСтрокиПродукция", Новый ОписаниеТипов("Число"));
	КонецЕсли;  
    ...
    Для каждого Источник Из Источники Цикл
		...
		Если ТипЗнч(СтруктураИсточник.Номенклатура) = Тип("СправочникСсылка.Номенклатура") И НЕ СтруктураИсточник.Номенклатура.Пустая() Тогда
			...
			СтруктураДляПоиска = Новый Структура("Спецификация, Номенклатура, ХарактеристикаНоменклатуры, СерияНоменклатуры", ПараметрыТекущейСтроки.Спецификация, СтруктураИсточник.Номенклатура, СтруктураИсточник.ХарактеристикаНоменклатуры, СтруктураИсточник.СерияНоменклатуры);
			Если ЕстьВидВыпуска Тогда
				СтруктураДляПоиска.Вставить("ВидВыпуска", Источник.ВидВыпуска);
			КонецЕсли;
			Если ЕстьНомерСтрокиПродукция Тогда
				СтруктураДляПоиска.Вставить("НомерСтрокиПродукция", Источник.НомерСтрокиПродукция);
			КонецЕсли;
			НайденныеСтроки = ПотребностиПоСпецификациям.НайтиСтроки(СтруктураДляПоиска);
			...
			Если НайденныеСтроки.Количество() = 0 Тогда
				...
				КонецЕсли;	
				Если ЕстьНомерСтрокиПродукция Тогда
					СтрокаПотребностиПоСпецификации.НомерСтрокиПродукция = Источник.НомерСтрокиПродукция;
				КонецЕсли;
	...
	Для каждого СтрокаПотребностиПоНоменклатуре Из ПотребностиПоНоменклатуре Цикл
		...
		СтруктураДляПоиска = Новый Структура("Спецификация, Номенклатура, ХарактеристикаНоменклатуры, СерияНоменклатуры", СтрокаПотребностиПоНоменклатуре.Спецификация, СтрокаПотребностиПоНоменклатуре.Номенклатура, СтрокаПотребностиПоНоменклатуре.ХарактеристикаНоменклатуры, СтрокаПотребностиПоНоменклатуре.СерияНоменклатуры);
		Если ЕстьВидВыпуска Тогда
			СтруктураДляПоиска.Вставить("ВидВыпуска", СтрокаПотребностиПоНоменклатуре.ВидВыпуска);
		КонецЕсли;
		Если ЕстьНомерСтрокиПродукция Тогда
			СтруктураДляПоиска.Вставить("НомерСтрокиПродукция", СтрокаПотребностиПоНоменклатуре.НомерСтрокиПродукция);
		КонецЕсли;
		НайденныеСтроки = ПотребностиПоСпецификациям.НайтиСтроки(СтруктураДляПоиска);
