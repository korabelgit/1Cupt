...
Функция ОпределитьГруппуНалоговогоНазначенияЗатрат(СчетЗатрат, Субконто1 = Неопределено, Субконто2 = Неопределено, Субконто3 = Неопределено, ДатаДокумента = Неопределено, УчитыватьДатуДокумента = Истина) Экспорт	
	...
	Если УчитыватьДатуДокумента И ?(НЕ ЗначениеЗаполнено(ДатаДокумента), ТекущаяДата(), ДатаДокумента) >= глЗначениеПеременной("ДатаНКУ2015") Тогда
		Возврат Перечисления.ГруппыНалоговыхНазначений.НалоговыеНазначенияАктивовИВзаиморасчетовПоНДС;
	КонецЕсли; 
	...
	ИначеЕсли ХарактерЗатратБУ = Перечисления.ХарактерЗатрат.ОбщепроизводственныеРасходы Тогда
		...
		ДатаПроверки = ?(НЕ ЗначениеЗаполнено(ДатаДокумента), ТекущаяДата(), ДатаДокумента);
		
		Если ДатаПроверки < глЗначениеПеременной("ДатаНачалаРаспределенияОПЗвНУ") Тогда
	...
	ИначеЕсли СчетЗатрат = ПланыСчетов.Хозрасчетный.ЗатратыБудущихПериодов
		...
		Если ЗначениеЗаполнено(Субконто1) И ТипЗнч(Субконто1) = ТипРБП Тогда
			Возврат ОпределитьГруппуНалоговогоНазначенияЗатрат(Субконто1.СчетБУ,,,,ДатаДокумента, УчитыватьДатуДокумента);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Субконто2) И ТипЗнч(Субконто2) = ТипРБП Тогда
			Возврат ОпределитьГруппуНалоговогоНазначенияЗатрат(Субконто2.СчетБУ,,,,ДатаДокумента, УчитыватьДатуДокумента);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Субконто3) И ТипЗнч(Субконто3) = ТипРБП Тогда
			Возврат ОпределитьГруппуНалоговогоНазначенияЗатрат(Субконто3.СчетБУ,,,,ДатаДокумента, УчитыватьДатуДокумента);
		КонецЕсли;
...
Функция ОпределитьГруппуНалоговогоНазначенияПоСчету(СчетЗатрат, ДатаДокумента) Экспорт 
	...
	Если ХарактерЗатратБУ = Перечисления.ХарактерЗатрат.ПроизводственныеРасходы 
		...
		ИЛИ ХарактерЗатратБУ = Перечисления.ХарактерЗатрат.ВложенияВоВнеоборотныеАктивы 
	    ИЛИ ХарактерЗатратБУ = Перечисления.ХарактерЗатрат.ТранспортноЗаготовительныеРасходы 
		ИЛИ (ХарактерЗатратБУ = Перечисления.ХарактерЗатрат.ОбщепроизводственныеРасходы И ДатаДокумента >= глЗначениеПеременной("ДатаНачалаРаспределенияОПЗвНУ"))
...
Функция ПолучитьСчетУчетаПоКлючуАналитики(АналитикаВидаУчета) Экспорт
	
	Если АналитикаВидаУчета = Неопределено Тогда
	    Возврат Неопределено;
	КонецЕсли; 
	
	РезультатЗапроса = РасширеннаяАналитикаУчета.СформироватьЗапросПоЗначениямАналитики(АналитикаВидаУчета);
	
	Если РезультатЗапроса = Неопределено ИЛИ РезультатЗапроса.Пустой() ИЛИ РезультатЗапроса.Колонки.Найти("СчетУчета") = Неопределено Тогда
		Возврат Неопределено;
	Иначе
		Выборка = РезультатЗапроса.Выбрать();
		Выборка.Следующий();
...
КонецФункции // ПолучитьСчетУчетаПоКлючуАналитики()
...
Функция ВидКорректировкиНалоговогоКредита(НалоговоеНазначение, НалоговоеНазначениеПоФакту) Экспорт

	НалоговыйКредитВход  = НалоговоеНазначение = Справочники.НалоговыеНазначенияАктивовИЗатрат.НДС_Пропорционально ИЛИ НалоговыйУчетПовтИсп.ЕстьПравоНаНалоговыйКредит(НалоговоеНазначение);
	НалоговыйКредитВыход = НалоговоеНазначениеПоФакту = Справочники.НалоговыеНазначенияАктивовИЗатрат.НДС_Пропорционально ИЛИ НалоговыйУчетПовтИсп.ЕстьПравоНаНалоговыйКредит(НалоговоеНазначениеПоФакту);
	
	Если НалоговыйКредитВход = НалоговыйКредитВыход Тогда
		ВидКорректировкиНалоговогоКредита = Перечисления.ВидыКорректировокНалоговогоКредита.НетКорректировок;
	ИначеЕсли НалоговыйКредитВход И НЕ НалоговыйКредитВыход Тогда
		ВидКорректировкиНалоговогоКредита = Перечисления.ВидыКорректировокНалоговогоКредита.ПотеряПраваНаНалоговыйКредит;
	Иначе
		ВидКорректировкиНалоговогоКредита = Перечисления.ВидыКорректировокНалоговогоКредита.ВосстановлениеПраваНаНалоговыйКредит;
	КонецЕсли;
	
	Возврат ВидКорректировкиНалоговогоКредита;

КонецФункции // ВидКорректировкиНалоговогоКредита()
