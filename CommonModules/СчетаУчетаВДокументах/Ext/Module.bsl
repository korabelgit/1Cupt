...
Функция ПроверитьБалансовостьСчета(Значение, ДанныеРеквизита, ХозяйственнаяСитуация, ДолженБытьЗабалансовым = Неопределено)
	...
	Возврат БухгалтерскийУчетПовтИсп.ЗабалансовыйСчет(Значение) = ДолженБытьЗабалансовым;
...
Процедура УстановитьСчетаУчета(ЗаполняемыйОбъект, ТаблицаРеквизитов, СтруктураСчетовУчета, ХозяйственнаяСитуация, СтруктураШапкиДокумента = Неопределено, ДанныеСклада = Неопределено, РозничныйСклад=Неопределено, ЗаполнятьБУ=Истина, Перезаполнять=Ложь, ЗаполнятьКлоныКолонок=Истина, Объект = Неопределено)
	...
	Для Каждого ДанныеРеквизита Из СтрокиРеквизитов Цикл
		...
		Если Реквизит = "НалоговоеНазначениеДоходовИЗатрат" Тогда 
			...
			Если СтруктураШапкиДокумента.Дата >= глЗначениеПеременной("ДатаНКУ2015") И НалоговыйУчетПовтИсп.ПолучитьГруппуНалоговогоНазначения(НалоговоеНазначениеЗатрат) = Перечисления.ГруппыНалоговыхНазначений.НалоговыеНазначенияДоходовИЗатрат Тогда
				НалоговоеНазначениеЗатрат = Справочники.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка();
...
Процедура ПроверитьСчетаУчетаТабличнойЧасти(ИмяТабличнойЧасти, ДанныеТабличнойЧасти, СтруктураШапкиДокумента, Отказ, Заголовок) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ДанныеТабличнойЧасти Цикл
		...
		Для Каждого КлючЗначение Из СтруктураОбязательныхПолей Цикл
			...
			// заполнение НалоговоеНазначение для услуг после 01.01.2015 здесь не проверяем, проверка происходит в
			// НалоговыйУчет.ПроверитьЗаполнениеНалоговыхНазначений или в модулях документов
			Если СтруктураШапкиДокумента.Дата >= глЗначениеПеременной("ДатаНКУ2015") И Услуга И КлючЗначение.Ключ = "НалоговоеНазначение" Тогда
				Продолжить;
			КонецЕсли;
...
Функция ОпределитьХозяйственнуюСитуациюПоСтроке(ИмяТабличнойЧасти, ИмяДокумента, СтруктураШапкиДокумента, ДанныеСтроки, ХозяйственнаяСитуация)
	...
	ИначеЕсли ИмяДокумента = "ПеремещениеТоваров" Тогда
		...
		Если БухгалтерскийУчетПовтИсп.ЗабалансовыйСчет(ДанныеСтроки.СчетУчетаБУ) Тогда
...
//  ЕстьНДС							    - булево 
//  ЕстьНалогНаПрибыльДо2015			- булево 
Процедура УстановитьСвойстваПолейСчетов(Дата, ИмяДокумента, ЭлементыФормы, ТабличныеЧасти, ПоказыватьВДокументахСчетаУчета, ОтражатьВБухгалтерскомУчете, ЕстьНДС=Истина, ЕстьНалогНаПрибыльДо2015=Истина, ЭтоДоходы = Ложь) Экспорт
	...
	Иначе
		...
		СчетаНДСИлиПрибыль = Новый Структура;
		СчетаПрибыль       = Новый Структура;
		СчетаНДС           = Новый Структура("НалоговоеНазначение, СчетУчетаНДС"); 
		
		Если НЕ ЭтоДоходы Тогда
			СчетаНДС.Вставить("НалоговоеНазначениеДоходовИЗатрат");
		Иначе
			СчетаПрибыль.Вставить("НалоговоеНазначениеДоходовИЗатрат");
		КонецЕсли; 
		...
		Для Каждого ДанныеСчета Из ТаблицаРеквизитов Цикл
			...
			Если ПоказыватьВДокументахСчетаУчета <> Неопределено Тогда
				...
				Если СчетаНДСИлиПрибыль.Свойство(Счет) Тогда
					УсловиеВидимостиСчета = УсловиеВидимостиСчета И (ЕстьНДС ИЛИ ЕстьНалогНаПрибыльДо2015);
				...
				ИначеЕсли СчетаПрибыль.Свойство(Счет) Тогда
					УсловиеВидимостиСчета = УсловиеВидимостиСчета И ЕстьНалогНаПрибыльДо2015;
				КонецЕсли; 
