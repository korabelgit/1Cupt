...
Функция КомиссионныйТовар(СчетУчета) Экспорт
	Возврат УправлениеЗапасамиПартионныйУчетПовтИсп.КомиссионныйТовар(СчетУчета);
...
Функция ПолучитьУчетнуюПолитику(МоментКон, Упр = Истина, Бух = Истина, Меж = Истина, Организация = Неопределено) Экспорт	
	...
	Возврат УправлениеЗапасамиПартионныйУчетПовтИсп.ПолучитьУчетнуюПолитику(ДатаСреза, Упр, Бух, Меж, Организация);
...
Функция ПолучитьНаправлениеСписанияПоКодуОперации(КодОперации, СтатьяЗатрат = Неопределено, СчетЗатрат = Неопределено) Экспорт	
	Возврат УправлениеЗапасамиПартионныйУчетПовтИсп.ПолучитьНаправлениеСписанияПоКодуОперации(КодОперации, СтатьяЗатрат, СчетЗатрат);
...
Функция ПолучитьИсточникПоКодуОперации(КодОперации)
	Возврат УправлениеЗапасамиПартионныйУчетПовтИсп.ПолучитьИсточникПоКодуОперации(КодОперации);
...
Процедура ЗаполнитьПоляЗаписиСписанияУпр(Движение, СтрокаПартии, СтрокаДокумента, СтруктураПараметров, РегистрУчета, КоэффСписания, КоэффПоступления)
	...
	Если СтрокаДокумента.ОтражатьВУправленческомУчете Тогда
		...
		Движение.СуммаНДСВыручкиБУ  = Окр(СтрокаДокумента.ПроводкиСуммаНДСРегл *КоэффПоступления, 2);
...
Процедура СформироватьДвиженияСписанияНаСебестоимостьПродажУпр(СтрокаДокумента, СтруктураПараметров, Строка, ТипЗаписи = "Прямая")
	...
	Если ЗначениеЗаполнено(СтрокаДокумента.НоменклатураКомплекта) И НЕ ЗначениеЗаполнено(СтрокаДокумента.ХарактеристикаКомплекта) Тогда 
...
Процедура ПоступлениеНаСкладБух(СписаноИз, СтрокаДокумента, СтруктураПараметров, Строка, Корректировка = Ложь)
	...
	Если НЕ СтрокаДокумента.КодОперацииПартииТоваров = СтруктураПараметров.КодыОпераций.ВозвратОтКомиссионера И
		ЗначениеЗаполнено(СтрокаДокумента.НалоговоеНазначениеНовое) Тогда
	...
	Иначе
		Движение.СтоимостьНУ = НалоговыйУчет.ОпределитьСтоимостьНУ(Движение.НалоговоеНазначение, Строка.СтоимостьНУ, СтруктураПараметров.ЕстьНалогНаПрибыльДо2015);
	КонецЕсли;
...
Процедура СписаниеНаПостоянныеЗатратыБух(СтрокаДокумента, СтруктураПараметров, Строка)
	...
	Движение.СуммаНУ		= НалоговыйУчет.ОпределитьСтоимостьНУ(Движение.НалоговоеНазначение, Строка.СтоимостьНУ, СтруктураПараметров.ЕстьНалогНаПрибыльДо2015);
	Движение.НДСВходящий	= Строка.НДСВходящий;
	Движение.НДСКредит		= Строка.НДСКредит;
...
Процедура СписаниеНаАмортизациюБух(СтрокаДокумента, СтруктураПараметров, Строка)
	...
	Движение.Стоимость = Строка.Стоимость;
	Движение.СтоимостьНУ = НалоговыйУчет.ОпределитьСтоимостьНУОС(Строка.НалоговоеНазначениеПоФакту, Строка.Стоимость, Строка.СтоимостьНУ, СтруктураПараметров.ЕстьНалогНаПрибыльДо2015, СтруктураПараметров.ЕстьНалогНаПрибыльС2015);
...
Процедура ПередачаМатериаловВЭксплуатациюБух(СтрокаДокумента, СтруктураПараметров, Строка)
	...
	Движение.СтоимостьНУ			 = НалоговыйУчет.ОпределитьСтоимостьНУОС(Движение.НалоговоеНазначение, Строка.Стоимость, Строка.СтоимостьНУ, СтруктураПараметров.ЕстьНалогНаПрибыльДо2015, СтруктураПараметров.ЕстьНалогНаПрибыльС2015);
	Движение.НДСВходящий			 = Строка.НДСВходящий;
	Движение.НДСКредит				 = Строка.НДСКредит;
...
Функция ПодготовитьТаблицуЗаписейОснования(ТаблицаЗаписейОснования, СтруктураШапкиДокумента, ДокументПартии, СтрУчет)
	...
	|ПОМЕСТИТЬ 
	|	ДокументыВозвратТоваровОтПокупателя
	|ИЗ
	...
	|	
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВозвратТоваровОтПокупателя.Ссылка КАК ВозвратТоваровОтПокупателяСсылка
	|ИЗ
	|	Документ.ВозвратТоваровОтПокупателя КАК ВозвратТоваровОтПокупателя
	|
	|ГДЕ
	|	ВозвратТоваровОтПокупателя.ВозвратнаяТара.ДокументПартии = &ДокументПартии
	|	И ВозвратТоваровОтПокупателя.МоментВремени > &МоментНач
	|	И ВозвратТоваровОтПокупателя.МоментВремени < &МоментКон
	|
	|;
	...
	|	ВЫБОР
	|		КОГДА ПартииТоваровНаСкладах.КодОперации = &ВозвратТекущегоМесяца
	|			ТОГДА -ПартииТоваровНаСкладах.СтоимостьНУ
	|		ИНАЧЕ ПартииТоваровНаСкладах.СтоимостьНУ
	|	КОНЕЦ КАК СтоимостьНУ,
	|	ВЫБОР
	|		КОГДА ПартииТоваровНаСкладах.КодОперации = &ВозвратТекущегоМесяца
	|			ТОГДА -ПартииТоваровНаСкладах.НДСВходящий 
	|		ИНАЧЕ ПартииТоваровНаСкладах.НДСВходящий
	|	КОНЕЦ КАК НДСВходящий,
	|	ВЫБОР
	|		КОГДА ПартииТоваровНаСкладах.КодОперации = &ВозвратТекущегоМесяца
	|			ТОГДА -ПартииТоваровНаСкладах.НДСКредит 
	|		ИНАЧЕ ПартииТоваровНаСкладах.НДСКредит
	|	КОНЕЦ КАК НДСКредит,","") + " 
	|
	|	ПартииТоваровНаСкладах.НомерСтрокиСписанныхТоваров
	|
	|ИЗ
	|	ДокументыВозвратТоваровОтПокупателя КАК ВозвратТоваровОтПокупателя
	...
	Если Не РезультатЗапроса.Пустой() тогда
		...
		Для Каждого Строка Из ТаблицаЗаписейОснования Цикл
			...
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				...
				Если СтрУчет = "Бухгалтерский" Тогда
					
					Строка.СтоимостьНУ  = Строка.СтоимостьНУ - НайденнаяСтрока.СтоимостьНУ;
					Строка.НДСВходящий  = Строка.НДСВходящий - НайденнаяСтрока.НДСВходящий;
					Строка.НДСКредит    = Строка.НДСКредит   - НайденнаяСтрока.НДСКредит;
					
					НайденнаяСтрока.СтоимостьНУ = 0;
					НайденнаяСтрока.НДСВходящий = 0;
					НайденнаяСтрока.НДСКредит   = 0;
...
Процедура ВыполнитьДвиженияВозврата(СтруктураПараметров, СтрокаДокумента, Строка, ИмяРегистраПартии, 
	...
	Если ИмяРегистраПартии = "ПартииТоваровНаСкладахМеж" Тогда
		ВозвратТекущегоМесяца = КонецМесяца(ДокументПартии.ПериодКонец) = КонецМесяца(СтруктураПараметров.Период);	
	Иначе
	...
	Если ВозвратТекущегоМесяца тогда
		...
		//Если СтрокаДокумента.ОтражатьВУправленческомУчете Тогда
		//	Движение.Заказ = СтрокаДокумента.ЗаказПартии;
		//КонецЕсли;		
		// Движение.Заказ = Строка.Заказ;
	Иначе
		Движение.КодОперации = СтрокаДокумента.КодОперацииПартииТоваров;
...
Функция ПолучитьТаблицуСписанияРасходнымОрдером(СтрокаДокумента, СтруктураПараметров) Экспорт
	...
	Ресурсы = ОбщегоНазначенияПовтИсп.ПолучитьМассивИменРесурсовРегистраСведенийСписанныеТовары();
...
Процедура ПолучитьОстатки(СтруктураПараметров, ТаблицаСписания, МоментКон, Организация)
	...
	МенеджерВременныхТаблицСписанныеТовары = Неопределено;
	Если НЕ СтруктураПараметров.ЕстьСтрокиОтражатьВМеждународномУчете Тогда	
	
		МенеджерВременныхТаблицСписанныеТовары = Новый МенеджерВременныхТаблиц();
		...
		Если ОсновнойДокумент <> Неопределено Тогда
			УсловиеРегистратор = " СписанныеТовары.Регистратор В (&Ссылка, &ОсновнойДокумент) ";
		Иначе
			УсловиеРегистратор = " СписанныеТовары.Регистратор = &Ссылка ";
		КонецЕсли;
		...
		Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблицСписанныеТовары;
		Запрос.Выполнить();
	...
	ПолучитьОстаткиУпр(
		...
		ТаблицаСписания, 
		МоментКон, 
		МенеджерВременныхТаблицСписанныеТовары
	);
	...
	Если МенеджерВременныхТаблицСписанныеТовары <> Неопределено Тогда
...
Процедура ВыполнитьДопДвиженияПоРезультатуСписанияМеж(СтруктураПараметров, ТаблицаСписания)
	...
	Для Каждого СтрокаСписания Из СтруктураПараметров.ТаблицаСписанныхПартий Цикл
		...
		ИначеЕсли СтрокаСписания.ВидОперации = "Возврат" Тогда
			УправлениеЗапасамиПартионныйУчетФормированиеПроводокМежд.СписаниеНаСчетМеж("", СтрокаСписания, СтруктураПараметров, СтрокаСписания.Движение, СтрокаСписания.ТипЗаписи);
		КонецЕсли;	
...
Процедура ВыполнитьСписание(СтруктураПараметров, ТаблицаСписания, МоментКон, Останавливаться = Ложь, ПроведениеОстановлено = Ложь )
	...
	Если ТребуетсяПолучитьОстатки Тогда
		...
		Если ТипЗнч(СтруктураПараметров.Регистратор) = Тип("ДокументСсылка.РегламентнаяПереоценкаЗапасов") Тогда
	        // Документ РегламентнаяПереоценкаЗапасов вводится после документа РасчетСебестоимостиВыпуска. А документ
			// РасчетСебестоимостиВыпуска в свою очередь делает движения на конец месяца
			ПолучитьОстатки(СтруктураПараметров, ТаблицаСписания, Новый Граница(КонецМесяца(МоментКон.Дата), ВидГраницы.Включая), Организация);
		Иначе
...
Функция ОпределитьНеобходимостьСдвигаГраницы(ДокументМоментВремени, Учет, Отбор = Неопределено, СписыватьПартииПриПроведенииДокументов = Ложь)
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               ...
	               |	И (НЕ Последовательность.Регистратор ССЫЛКА Документ.РегламентнаяПереоценкаЗапасов)
...
Процедура ДвижениеПартийТоваров(ОбрабатываемыйДокумент, ТаблицаСписания = Неопределено,
								...
								ФормироватьПроводки = Неопределено, УчетнаяПолитика = Неопределено, УчитыватьСуществующиеСтрокиДвиженийПартийТоваров = Ложь, Отказ = Неопределено) Экспорт
	...
	Если НЕ СписаниеВыполняетсяОбработкой Тогда
		// Документ РегламентнаяПереоценкаЗапасов делает движения только при проведении 
		// вручную, т.к. результат его проведения основан на данных формируемых документом
		// РасчетСебестоимостиВыпуска который так же не перепроводится обработкой проведения
		// по партиям (только сдвигается граница последовательности).
		Если ТипЗнч(ОбрабатываемыйДокумент) <> Тип("ДокументСсылка.РегламентнаяПереоценкаЗапасов") Тогда
	...
	Если КоличествоСтрокВДокументе > 0 Тогда
		...
		Если СтруктураПараметров.ЕстьСтрокиОтражатьВБухгалтерскомУчете И Бух Тогда
			СтруктураПараметров.Вставить("ЕстьНалогНаПрибыль", СтруктураПараметров.УчетнаяПолитика.Бух.ЕстьНалогНаПрибыль);
			СтруктураПараметров.Вставить("ЕстьНалогНаПрибыльДо2015", СтруктураПараметров.УчетнаяПолитика.Бух.ЕстьНалогНаПрибыльДо2015);
			СтруктураПараметров.Вставить("ЕстьНалогНаПрибыльС2015", СтруктураПараметров.УчетнаяПолитика.Бух.ЕстьНалогНаПрибыльС2015);
		КонецЕсли; 
	...
	Если (НЕ СписаниеВыполняетсяОбработкой)
		// Документ не должен сдвигать границу. Граница сдвигается при восстановлении последовательности обработкой	"Проведени по партиям"
		// аналогично документу РасчетСебестоимостиВыпуска
		И (НЕ ТипЗнч(ОбрабатываемыйДокумент) = Тип("ДокументСсылка.РегламентнаяПереоценкаЗапасов")) Тогда  
...
Процедура ЗаполнитьЗапросПартийНаСкладахДляОтложеннойОтгрузкиУпр(Запрос, ВестиПартионныйУчетПоСкладам)
	...
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
	|			СписанныеТовары КАК СписанныеТоварыОрдера
	|		ПО 
...
Процедура ЗаполнитьЗапросПартийНаСкладахДляСписанияПоОрдернойСхемеУпр(Запрос,ВестиПартионныйУчетПоСкладам, СтратегияСтатусПартии, СпособОценкиМПЗ)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СписанныеТовары КАК СписанныеТоварыОрдера
...
Функция ПолучитьДеревоПартийНаСкладахУпр(МоментКон, СтруктураПараметров, МенеджерВременныхТаблицСписанныеТовары = Неопределено)	
...
Функция ПолучитьДеревоПартийПереданныеУпр(МоментКон, Регистратор, ДоговорКонтрагента, СпособОценкиМПЗ, СтратегияСтатусПартии, МенеджерВременныхТаблицСписанныеТовары = Неопределено)	
...
КонецФункции // ПолучитьДеревоПартийПереданныеУпр
...
Функция ПолучитьДеревоПартийНаСкладахБух(МоментКон, Организация, СтруктураПараметров, МенеджерВременныхТаблицСписанныеТовары = Неопределено)
...
Функция ПолучитьДеревоПартийПереданныеБух(МоментКон, Регистратор, Организация, ДоговорКонтрагента, СпособОценкиМПЗ, СтратегияСтатусПартии, МенеджерВременныхТаблицСписанныеТовары)	
	...
	|	ПартииТоваровПереданные.Количество,
	|	ПартииТоваровПереданные.Стоимость,
	|	ПартииТоваровПереданные.СтоимостьНУ, 
	|	ПартииТоваровПереданные.НДСВходящий,  
	|	ПартииТоваровПереданные.НДСКредит,
  	|	ПартииТоваровПереданные.СуммаУценки,
	|	ПартииТоваровПереданные.НалоговоеНазначение, 
	|	ПартииТоваровПереданные.СчетУчета,
	|	ВЫБОР
	...
	|						ИЛИ ПартииТоваровПереданные.НалоговоеНазначение = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка))
	|			И (ВЫБОР
	|				КОГДА СписанныеТовары.КодОперацииПартииТоваров = &СписаниеПартийПереданныхВПроизводство
	|						ИЛИ СписанныеТовары.КодОперацииПартииТоваров = &ВозвратОтПереработчика
	|					ТОГДА СписанныеТовары.ЗаказПартии = ПартииТоваровПереданные.ЗаказПередачи
...
//	МоментКон
//
Процедура ПолучитьОстаткиПартийНаСкладахУпр(СтруктураПараметров, МоментКон, МенеджерВременныхТаблицСписанныеТовары = Неопределено)
	...
	Если СтруктураПараметров.ЕстьСтрокиОтражатьВУправленческомУчете Тогда
		...
		Измерения = ОбщегоНазначенияПовтИсп.ПолучитьМассивИменИзмеренийРегистра("ПартииТоваровНаСкладах");
		Для Каждого Измерение ИЗ Измерения Цикл
			СтруктураИзмерений.Вставить(Измерение);
			ТаблицаОстатковПартий.Колонки.Добавить(Измерение);
		КонецЦикла;
...
Процедура ПолучитьОстаткиПартийПереданныхУпр(СтруктураПараметров, МоментКон, ДоговорКонтрагента, МенеджерВременныхТаблицСписанныеТовары	= Неопределено)	

	Если СтруктураПараметров.ЕстьСтрокиОтражатьВУправленческомУчете Тогда
		СтруктураПараметров.Вставить("ДеревоПартииТоваровПереданныеУпр", 
			ПолучитьДеревоПартийПереданныеУпр(
				МоментКон, 
				СтруктураПараметров.Регистратор, 
				ДоговорКонтрагента, 
				СтруктураПараметров.СпособОценкиМПЗУпр, 
				СтруктураПараметров.СтратегияСтатусПартииУпр,
				МенеджерВременныхТаблицСписанныеТовары
			)
		);
		...
		Измерения = ОбщегоНазначенияПовтИсп.ПолучитьМассивИменИзмеренийРегистра("ПартииТоваровПереданные");
		Для Каждого Измерение ИЗ Измерения Цикл
			СтруктураИзмерений.Вставить(Измерение);
			ТаблицаОстатковПартий.Колонки.Добавить(Измерение);
		КонецЦикла;
...
//	Нет.
//
Процедура ПолучитьОстаткиПартийНаСкладахРегл(СтруктураПараметров, МоментКон, Организация, МенеджерВременныхТаблицСписанныеТовары = Неопределено)	
	
	Если СтруктураПараметров.ЕстьСтрокиОтражатьВБухгалтерскомУчете Тогда
		
		СтруктураПараметров.Вставить("ДеревоПартииТоваровНаСкладахБух",
			ПолучитьДеревоПартийНаСкладахБух(
				МоментКон, 
				Организация, 
				СтруктураПараметров, 
				МенеджерВременныхТаблицСписанныеТовары
		...
		СтруктураИзмерений = Новый Структура;
		ТаблицаОстатковПартий = Новый ТаблицаЗначений;
		Измерения = ОбщегоНазначенияПовтИсп.ПолучитьМассивИменИзмеренийРегистра("ПартииТоваровНаСкладахБухгалтерскийУчет");
		Для Каждого Измерение ИЗ Измерения Цикл
			СтруктураИзмерений.Вставить(Измерение);
			ТаблицаОстатковПартий.Колонки.Добавить(Измерение);
		КонецЦикла;
...
//	Нет.
//
Процедура ПолучитьОстаткиПартийПереданныхРегл(СтруктураПараметров, МоментКон, Организация, ДоговорКонтрагента, МенеджерВременныхТаблицСписанныеТовары)
	
	Если СтруктураПараметров.ЕстьСтрокиОтражатьВБухгалтерскомУчете Тогда
		СтруктураПараметров.Вставить("ДеревоПартииТоваровПереданныеБух", 
			ПолучитьДеревоПартийПереданныеБух(
				МоментКон, 
				СтруктураПараметров.Регистратор, 
				Организация, 
				ДоговорКонтрагента, 
				СтруктураПараметров.СпособОценкиМПЗБух, 
				СтруктураПараметров.СтратегияСтатусПартииБух,
				МенеджерВременныхТаблицСписанныеТовары
			)
		);		
		...
		СтруктураИзмерений = Новый Структура;
		ТаблицаОстатковПартий = Новый ТаблицаЗначений;
		Измерения = ОбщегоНазначенияПовтИсп.ПолучитьМассивИменИзмеренийРегистра("ПартииТоваровПереданныеБухгалтерскийУчет");
		Для Каждого Измерение ИЗ Измерения Цикл
			СтруктураИзмерений.Вставить(Измерение);
			ТаблицаОстатковПартий.Колонки.Добавить(Измерение);
		КонецЦикла;
...
Процедура ПолучитьОстаткиПартийНаСкладахМеж(СтруктураПараметров, МоментКон,  Организация)
	...
	Если СтруктураПараметров.ЕстьСтрокиОтражатьВМеждународномУчете Тогда
		...
		СтруктураИзмерений = Новый Структура;
		ТаблицаОстатковПартий = Новый ТаблицаЗначений;
		Измерения = ОбщегоНазначенияПовтИсп.ПолучитьМассивИменИзмеренийРегистра("ПартииТоваровНаСкладахМеждународныйУчет");
		Для Каждого Измерение ИЗ Измерения Цикл
			СтруктураИзмерений.Вставить(Измерение);
			ТаблицаОстатковПартий.Колонки.Добавить(Измерение);
		КонецЦикла;
...
Процедура ПолучитьОстаткиПартийПереданныхМеж(СтруктураПараметров, МоментКон, Организация, ДоговорКонтрагента)
	...
	Если СтруктураПараметров.ЕстьСтрокиОтражатьВМеждународномУчете Тогда
		...
		СтруктураИзмерений = Новый Структура;
		ТаблицаОстатковПартий = Новый ТаблицаЗначений;
		Измерения = ОбщегоНазначенияПовтИсп.ПолучитьМассивИменИзмеренийРегистра("ПартииТоваровПереданныеМеждународныйУчет");
		Для Каждого Измерение ИЗ Измерения Цикл
			СтруктураИзмерений.Вставить(Измерение);
			ТаблицаОстатковПартий.Колонки.Добавить(Измерение);
		КонецЦикла;
...
//	Нет.
//
Процедура ПолучитьОстаткиУпр(СтруктураПараметров, ТаблицаСписания, МоментКон, МенеджерВременныхТаблицСписанныеТовары = Неопределено)	
	...
	Если СтруктураПараметров.ЕстьПереданные Тогда
		...
		ПолучитьОстаткиПартийПереданныхУпр(
			СтруктураПараметров, 
			МоментКон,
			ТаблицаСписания[0].ДоговорКонтрагента, 
			МенеджерВременныхТаблицСписанныеТовары
		);
...
//	Нет.
//
Процедура ПолучитьОстаткиРегл(СтруктураПараметров, ТаблицаСписания, МоментКон, Организация, МенеджерВременныхТаблицСписанныеТовары = Неопределено)
...
Процедура ВыполнитьПриходПоРегистрамПартий(ТаблицаСтаройРегистрацииВПоследовательности, СтруктураШапкиДокумента, Отказ=Ложь, ТаблицаПоТоварам=Неопределено, ТаблицаПоТаре=Неопределено, ТаблицаПоОборудованию_ТаблицаПоКомплектам=Неопределено, ЗНАЧ Упр=Ложь, ЗНАЧ Бух=Ложь) Экспорт
	...
	ИначеЕсли ТипРегистратора = Тип("ДокументСсылка.ОприходованиеТоваров") Тогда
		Если НЕ СтруктураШапкиДокумента.ПризнаватьЗатратыВНУ Тогда
			КопияТаблицаПоТоварам.ЗаполнитьЗначения(0, "СуммаНУ");
		КонецЕсли;
	...
	Для каждого СтрокаДвижений Из ТаблицаДвижений Цикл
		...
		Если ВестиПартионныйУчетПоСкладам Тогда
			Если ТипДокумента = Тип("ДокументСсылка.ОприходованиеМатериаловИзПроизводства") Тогда
				...
				КонецЕсли;
            ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ПоступлениеДопРасходов")
		...
		Если СтруктТаблицДокумента.Свойство("ТаблицаПоТоварам") Тогда
			Для Каждого СтрокаТаблицы Из СтруктТаблицДокумента.ТаблицаПоТоварам Цикл
				...
				Если ТипДокумента = Тип("ДокументСсылка.ОприходованиеТоваров") Тогда
					...
					СтрокаТД.СтоимостьНУ    = НалоговыйУчет.ОпределитьСтоимостьНУ(СтрокаТаблицы.НалоговоеНазначение, СтрокаТаблицы.СуммаНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
				...
				ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") Тогда
					...
					СтрокаТД.СтоимостьНУ    = НалоговыйУчет.ОпределитьСтоимостьНУ(СтрокаТаблицы.НалоговоеНазначение, СтрокаТаблицы.СебестоимостьНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015) * СтрокаТаблицы.Количество;
				...
				ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ОприходованиеМатериаловИзПроизводства") Тогда
					...
					СтрокаТД.СтоимостьНУ = НалоговыйУчет.ОпределитьСтоимостьНУ(СтрокаТаблицы.НалоговоеНазначение, СтрокаТаблицы.СуммаНал, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);;
				Иначе
					// общий случай
					СтрокаТД.Стоимость      = СтрокаТаблицы.ПроводкиСуммаБезНДСРегл;
					СтрокаТД.СтоимостьНУ	= СтрокаТаблицы.СтоимостьНУ;
...
Процедура ВыполнитьПриходПоРегистрамПартийПоВидуУчета(СтруктураШапкиДокумента, УчетнаяПолитика, КодОперацииПартииТоваров, Отказ, СтруктТаблицДокумента, Движения, ВидУчета)
	...
	Если ТипДокумента <> Тип("ДокументСсылка.ОприходованиеМатериаловИзПроизводства") Тогда	//Документ ОприходованиеМатериаловИзПроизводства содержит код операции в табличной части
		...
		Если ТипДокумента = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") И ТаблицыДанныхДокумента.Свойство("ТаблицаПоТоварам") Тогда 
			ТаблицыДанныхДокумента.ТаблицаПоТоварам.ЗаполнитьЗначения(Перечисления.КодыОперацийПартииТоваров.Поступление, "КодОперации");
		КонецЕсли; 		
	КонецЕсли;
	...
	Для каждого СтрокаТЧ из ТаблицаТЧ цикл
		...
		Если СтруктураПараметрыПроводок.Свойство("ПриходТМЦ") Тогда
			...
			Если СтруктураСуммыВПроводки.Свойство("СтоимостьНУ") Тогда
				Проводка.СуммаНУДт = НалоговыйУчет.ОпределитьСтоимостьНУ(Проводка.НалоговоеНазначениеДт, СтруктураСуммыВПроводки.СтоимостьНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
				Проводка.СуммаНУКт = НалоговыйУчет.ОпределитьСтоимостьНУ(Проводка.НалоговоеНазначениеКт, СтруктураСуммыВПроводки.СтоимостьНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
			Иначе
			...
			ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ОприходованиеТоваров") И НЕ СтруктураШапкиДокумента.ПризнаватьЗатратыВНУ Тогда
				Проводка.СуммаНУДт  = 0;
			КонецЕсли;
...
Процедура СформироватьСтрокуДвиженияПоРегиструПартииТоваровНаСкладах(
	...
	Если ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВУправленческомУчете Тогда
		...
		Если СтрокаВыборки.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.Переработка 
			И СтрокаВыборки.СтатусМатериальныхЗатрат = Перечисления.СтатусыМатериальныхЗатратНаПроизводство.ВозвратныеОтходы Тогда
	...
	Если ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВБухгалтерскомУчете 
		...
		НоваяСтрока.СтоимостьНУ = НалоговыйУчет.ОпределитьСтоимостьНУ(НоваяСтрока.НалоговоеНазначение, СтоимостьНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
...
Функция ПолучитьМассивКодыОперацийВыбытиеПартий()
	Возврат УправлениеЗапасамиПартионныйУчетПовтИсп.ПолучитьМассивКодыОперацийВыбытиеПартий();
...
Функция ПолучитьМассивКодыОперацийПеремещение()
	Возврат УправлениеЗапасамиПартионныйУчетПовтИсп.ПолучитьМассивКодыОперацийПеремещение();
КонецФункции // ПолучитьМассивКодыОперацийВыбытиеПартий()
	...
	Если СтрокаДереваПартий <> Неопределено Тогда
		...
		Для Каждого СтрокаПартииРаспределения ИЗ СтрокаДереваПартий.Строки Цикл
			...
			Если СуммаУценкиИзменение < 0 Тогда
				...
				СтрокаДокумента.КорСчетБУ      = СтрокаДокумента.СчетДоходовБУ;
				СтрокаДокумента.КорСубконтоБУ1 = СтрокаДокумента.СубконтоДоходовБУ1;
				СтрокаДокумента.КорСубконтоБУ2 = СтрокаДокумента.СубконтоДоходовБУ2;
				СтрокаДокумента.КорСубконтоБУ3 = СтрокаДокумента.СубконтоДоходовБУ3;
			Иначе
				СтрокаДокумента.КорСчетБУ      = СтрокаДокумента.СчетРасходовБУ;
				СтрокаДокумента.КорСубконтоБУ1 = СтрокаДокумента.СубконтоРасходовБУ1;
				СтрокаДокумента.КорСубконтоБУ2 = СтрокаДокумента.СубконтоРасходовБУ1;
				СтрокаДокумента.КорСубконтоБУ3 = СтрокаДокумента.СубконтоРасходовБУ1;
			КонецЕсли;
			...
			// 1. Корректировка стоимости партии
			Движение = ДобавитьДвижениеВСтруктуруПараметров(ИмяРегистра, СтруктураПараметров);
			...
			Движение.Регистратор 			= СтрокаДокумента.Регистратор;
			Движение.Активность 			= Истина;
			Движение.ВидДвижения 			= ?(СуммаУценкиИзменение > 0, ВидДвиженияНакопления.Расход, ВидДвиженияНакопления.Приход);
			...
			// Ресурсы 
			Движение.Стоимость	            = ?(СуммаУценкиИзменение > 0, 1, -1)*СуммаУценкиИзменение;			
			Движение.СуммаУценки	        = -Движение.Стоимость;			
			...
			// Сохраним ссылку на текущую строку партии 
			Строка = Движение;
			...
			// Движения по затратам делаем только в случае уменьшения стоимости
			Если СуммаУценкиИзменение > 0 Тогда
				// ВНИМАНИЕ! Данное условие неообходимо для нужд традиционного режима. Аналогичное условие находится в модуле документа "Регламентная переоценка запасов" для нужд РАУЗ
			...
			Иначе
				УправлениеЗапасамиПартионныйУчетФормированиеПроводокРегл.ОприходованиеНаСчетБух(СписыватьИз, СтрокаДокумента, СтруктураПараметров, Строка);
			КонецЕсли;
					
			// берем последнюю строку
			ДобавленнаяПроводка = СтруктураПараметров.ДвиженияХозрасчетный[СтруктураПараметров.ДвиженияХозрасчетный.Количество()-1];
			
			// Переоценка регламентная, поэтому затраты нужно сразу списать на результаты деятельности			
			Если НЕ ДобавленноеДвижениеЗатраты = Неопределено Тогда
				...
				Движение.Организация 			= ДобавленноеДвижениеЗатраты.Организация;
				Движение.НалоговоеНазначение	= ДобавленноеДвижениеЗатраты.НалоговоеНазначение;
				...
				Строка.НомерКорСтроки = ДобавленноеДвижениеЗатраты.НомерСтроки;
			КонецЕсли; 
