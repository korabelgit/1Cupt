...
КонецФункции // СформироватьТекстЗапросаДляДокументаКомплектацияНоменклатуры
...
Функция СформироватьТекстЗапросаДляДокументаСписаниеНЗП()
	...
	|	
	|   ВЫБОР 
	|		КОГДА Док.Ссылка.Дата < &ДатаНКУ2015 ТОГДА
	|   		Док.Ссылка.НалоговоеНазначениеДоходовИЗатрат
	|		КОГДА Док.Ссылка.СчетДт.УчетПоНалоговымНазначениямНДС ТОГДА
	|   		Док.Ссылка.НалоговоеНазначениеДоходовИЗатрат
	|		КОГДА Док.НалоговоеНазначение <> Док.Ссылка.НалоговоеНазначениеДоходовИЗатрат ТОГДА
	|   		Док.Ссылка.НалоговоеНазначениеДоходовИЗатрат
	|	ИНАЧЕ 
	|		ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
	|	КОНЕЦ КАК НалоговоеНазначениеПолучатель,
	|	
	|	Ложь КАК НеКорректироватьНУ,
	|
	|	Док.НоменклатурнаяГруппа КАК НоменклатурнаяГруппаНЗП,
...
Процедура СформироватьДвиженияПоРегистрамЗатратыНаВыпускПродукции(
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		Если (НЕ СтрокаВыборки.НеКорректироватьНУ) И (СтрокаВыборки.КодОперации <> Перечисления.КодыОперацийЗатратыНаВыпускПродукции.КорректировкаНЗПРасч) Тогда 
...
Процедура УстановитьПараметрыЗапроса(
	...
	Запрос.УстановитьПараметр("ДатаНКУ2015", глЗначениеПеременной("ДатаНКУ2015"));
...
Функция СформироватьТекстЗапросаПоВыпускуПродукцииКорректировкаНЗП()
	...
	|	ВЫБОР КОГДА ТабличнаяЧасть.Ссылка.ТипСтоимости = ЗНАЧЕНИЕ(Перечисление.ВидыНормативнойСтоимостиПроизводства.Рассчитывается)
	|		ТОГДА 
	|			Истина // Корректировки по НУ произойдут только при расчете себестоимости
	|		ИНАЧЕ
	|			Ложь
	|	КОНЕЦ КАК НеКорректироватьНУ,
	|
	|	ТабличнаяЧасть.Номенклатура КАК Затрата,
	|	ТабличнаяЧасть.ХарактеристикаНоменклатуры КАК ХарактеристикаЗатраты,
	|	ТабличнаяЧасть.СерияНоменклатуры КАК СерияЗатраты,
...
Функция СформироватьТекстЗапросаПоВыпускуПродукцииСписаниеНЗП()
	...
	|	ТабличнаяЧасть.НалоговоеНазначение КАК НалоговоеНазначение,
	|   ВЫБОР 
	|		КОГДА ТабличнаяЧасть.Ссылка.Дата < &ДатаНКУ2015 ТОГДА
	|   		ТабличнаяЧасть.Ссылка.НалоговоеНазначениеДоходовИЗатрат
	|		КОГДА ТабличнаяЧасть.Ссылка.СчетДт.УчетПоНалоговымНазначениямНДС ТОГДА
	|   		ТабличнаяЧасть.Ссылка.НалоговоеНазначениеДоходовИЗатрат
	|		КОГДА ТабличнаяЧасть.НалоговоеНазначение <> ТабличнаяЧасть.Ссылка.НалоговоеНазначениеДоходовИЗатрат ТОГДА
	|   		ТабличнаяЧасть.Ссылка.НалоговоеНазначениеДоходовИЗатрат
	|	ИНАЧЕ 
	|		ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
	|	КОНЕЦ КАК НалоговоеНазначениеПолучатель,
...
Процедура ПровестиДокументПоПроизводственнымРегистрам(ДокументСсылка, Упр, Бух, ОшибкаТекущегоДокумента = Ложь) Экспорт
	  ...
	  И (Бух ИЛИ Упр) Тогда
		...
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
			...
			Иначе
				...
				СтруктураШапкиДокумента.Вставить("ЕстьНалогНаПрибыльС2015" , УчетнаяПолитикаРегл.ЕстьНалогНаПрибыльС2015);
...
Процедура ЗаполнитьЗначенияРесурсовРегистра(
	...
	ИначеЕсли ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВБухгалтерскомУчете Тогда
		СтрокаДвижения[ИмяРесурсаСтоимость]   = Стоимость;
		СтрокаДвижения[ИмяРесурсаСтоимостьНУ] = НалоговыйУчет.ОпределитьСтоимостьНУ(СтрокаДвижения.НалоговоеНазначение, СтоимостьНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
...
Процедура СформироватьПроводкуПоРегиструБухгалтерии(
	...
	Если ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВБухгалтерскомУчете Тогда
		...
		Тогда
			...
			НоваяПроводка.СуммаНУДт = НалоговыйУчет.ОпределитьСтоимостьНУ(НалоговоеНазначениеДт, СуммаНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
			...
			НоваяПроводка.СуммаНУКт = НалоговыйУчет.ОпределитьСтоимостьНУ(НалоговоеНазначениеКт, СуммаНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
		...
		Тогда
			...
			ИначеЕсли СтрокаВыборки.СтатусМатериальныхЗатрат <> Перечисления.СтатусыМатериальныхЗатратНаПроизводство.ПринятыеВПереработку Тогда
				...
				БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетДт, НоваяПроводка.СубконтоДт, "СтатьиЗатрат", СтрокаВыборки.СтатьяЗатратПолучатель);
			КонецЕсли;
			Если СтрокаВыборки.ХарактерЗатрат = Перечисления.ХарактерЗатрат.ВложенияВоВнеоборотныеАктивы Тогда
				БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетДт, НоваяПроводка.СубконтоДт, "ОбъектыСтроительства", СтрокаВыборки.ОбъектСтроительства);
			КонецЕсли; 
...
Процедура ДополнитьСтруктуруШапкиДокументаНастройкиУчета(
	...
	Тогда
		Если ЗначениеЗаполнено(УчетнаяПолитикаБух) Тогда
			...
			СтруктураШапкиДокумента.Вставить("ЕстьНалогНаПрибыльДо2015", УчетнаяПолитикаБух.ЕстьНалогНаПрибыльДо2015);
			СтруктураШапкиДокумента.Вставить("ЕстьНалогНаПрибыльС2015", УчетнаяПолитикаБух.ЕстьНалогНаПрибыльС2015);
		Иначе
			...
			СтруктураШапкиДокумента.Вставить("ЕстьНалогНаПрибыльС2015", Истина);
		КонецЕсли;
	КонецЕсли;
