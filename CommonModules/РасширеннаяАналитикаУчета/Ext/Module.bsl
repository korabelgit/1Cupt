...
Процедура СкорректироватьЗначенияКлючаАналитикиВидаУчета(
	...
	// После 01.01.15 очищаем налоговое назначение для счетов, для которых не ведется учет по налоговым назначениям
	// кроме специальных случаев корректировки использования НДС при списании на затраты
	Если СтруктураШапкиДокумента.Дата >= глЗначениеПеременной("ДатаНКУ2015")
		 И СтруктураИзмерений.Свойство("СчетУчета") И ЗначениеЗаполнено(СтруктураИзмерений.СчетУчета) 
		 И СтруктураИзмерений.Свойство("НалоговоеНазначение") И ЗначениеЗаполнено(СтруктураИзмерений.НалоговоеНазначение) 
		 И НЕ БухгалтерскийУчетПовтИсп.УчетПоНалоговымНазначениямНДССчет(СтруктураИзмерений.СчетУчета)
		 И СтруктураИзмерений.Свойство("ВидДокумента") 
		 И НЕ (СтруктураШапкиДокумента.ВидДокумента = "ТребованиеНакладная"
		      ИЛИ СтруктураШапкиДокумента.ВидДокумента = "СписаниеТоваров"
			  ИЛИ СтруктураШапкиДокумента.ВидДокумента = "СписаниеНЗП"
			  ИЛИ СтруктураШапкиДокумента.ВидДокумента = "КорректировкаПрочихЗатрат"
		      )
	Тогда	 
		 СтруктураИзмерений.НалоговоеНазначение = Справочники.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка();
	КонецЕсли;	
...
Процедура СкорректироватьЗначенияКлючаАналитикиУчетаПрочихЗатрат(
	...
	Если ВидОтраженияВУчете <> Перечисления.ВидыОтраженияВУчете.ОтражатьВУправленческомУчете 
		И ТипЗнч(ПараметрыАналитики) = Тип("СтрокаТаблицыЗначений") 
		И ПараметрыАналитики.СтрокаИсходнойТаблицы.Владелец().Колонки.Найти("НалоговоеНазначение") <> Неопределено 
		И НЕ ЗначениеЗаполнено(СтруктураИзмерений["НалоговоеНазначение"])
		Тогда
		
		СтруктураИзмерений["НалоговоеНазначение"] = ПараметрыАналитики.СтрокаИсходнойТаблицы.НалоговоеНазначение;
...
//	НДСВходящий - Число - НДС входящий затрат
//	НДСКредит - Число - НДС кредит затрат
//	НаборЗаписейУчетЗатрат - РегистрНакопленияНаборЗаписей - Набор записей регистра накопления "Учет затрат"
//  ДатаДокументаПартии - дата - Если задана и в месяце даты документа, то движение будет сформировано на ДатаДокументаПартии
//	ЗаписыватьДвижения - Булево - записывать движения 
//
Процедура СформироватьДвиженияПоРегиструУчетЗатрат(
	...
	Если ТипЗнч(НаборЗаписейУчетЗатрат) = Тип("РегистрНакопленияНаборЗаписей.УчетЗатратРегл") Тогда
		...
		Если СтруктураШапкиДокумента.Дата >= глЗначениеПеременной("ДатаНКУ2015") Тогда
			СчетУчета = НалоговыйУчетПовтИсп.ПолучитьСчетУчетаПоКлючуАналитики(АналитикаВидаУчета);
			Если БухгалтерскийУчетПовтИсп.УчетСуммНУСчет(СчетУчета) Тогда
				КорСчетУчета = НалоговыйУчетПовтИсп.ПолучитьСчетУчетаПоКлючуАналитики(КорАналитикаВидаУчета);
				Если БухгалтерскийУчетПовтИсп.УчетСуммНУСчет(КорСчетУчета) Тогда
					НоваяСтрока.СтоимостьНУ = СтоимостьНУ;
				Иначе
					НоваяСтрока.СтоимостьНУ = Стоимость;
				КонецЕсли;	
			Иначе
		...
		Иначе
			...
			НоваяСтрока.СтоимостьНУ = НалоговыйУчет.ОпределитьСтоимостьНУ(НалоговоеНазначение, СтоимостьНУ, ?(СтруктураШапкиДокумента.Свойство("ЕстьНалогНаПрибыльДо2015"), СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015, Истина));	
		КонецЕсли; 
	...
	ИначеЕсли ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВРегламентированномУчете Тогда
		...
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
			...
			Тогда
				...
				СуммаНал      = НалоговыйУчет.ОпределитьСтоимостьНУ(СтрокаВыборки.НалоговоеНазначениеПолучатель, СтрокаВыборки.СуммаНал, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
...
Функция УчитыватьПродажиПоЗаказамПокупателей() Экспорт
	Возврат Истина;
КонецФункции // УчитыватьПродажиПоЗаказамПокупателей
...
Процедура СформироватьДвиженияПоРегиструУчетПродажИСебестоимости(
	...
	НоваяСтрока.СхемаРеализации     = СхемаРеализации;
...
Процедура СформироватьДвиженияПоПродажам(
	...
	Для Каждого СтрокаТабличнойЧасти Из ТаблицаПродаж Цикл
		...
		СформироватьДвиженияПоРегиструУчетПродажИСебестоимости(
			...
			0, // Себестоимость
			СтрокаТабличнойЧасти.СхемаРеализации,
