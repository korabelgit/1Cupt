...
//  СтруктураЗаполненныеРеквизиты - структура, в которой "возвращаются" реквизиты, заполненные при вводе на основании
//
Процедура ЗаполнитьШапкуДокументаПоОснованию(ДокументОбъект, ДокументОснование, СтруктураЗаполненныеРеквизиты = Неопределено) Экспорт
	...
	Если МетаданныеДокумента.Реквизиты.Найти("УчитыватьНДС") <> Неопределено
	   И МетаданныеДокументаОснования.Реквизиты.Найти("УчитыватьНДС") <> Неопределено Тогда
		...
		Если СтруктураЗаполненныеРеквизиты <> Неопределено Тогда
			СтруктураЗаполненныеРеквизиты.Вставить("УчитыватьНДС");
		КонецЕсли;	
	КонецЕсли;
	...
	Если МетаданныеДокумента.Реквизиты.Найти("СуммаВключаетНДС") <> Неопределено
	   И МетаданныеДокументаОснования.Реквизиты.Найти("СуммаВключаетНДС") <> Неопределено Тогда
		...
		Если СтруктураЗаполненныеРеквизиты <> Неопределено Тогда
			СтруктураЗаполненныеРеквизиты.Вставить("СуммаВключаетНДС");
		КонецЕсли;	
	КонецЕсли;
	...
	Если МетаданныеДокумента.Реквизиты.Найти("АвторасчетНДС") <> Неопределено
	   И МетаданныеДокументаОснования.Реквизиты.Найти("АвторасчетНДС") <> Неопределено Тогда
		...
		Если СтруктураЗаполненныеРеквизиты <> Неопределено Тогда
			СтруктураЗаполненныеРеквизиты.Вставить("АвторасчетНДС");
		КонецЕсли;	
	КонецЕсли;
	...
	Если МетаданныеДокумента.Реквизиты.Найти("ИспользоватьПлановуюСебестоимость") <> Неопределено Тогда
		Если МетаданныеДокументаОснования.Реквизиты.Найти("ИспользоватьПлановуюСебестоимость") <> Неопределено Тогда
			...
			Если СтруктураЗаполненныеРеквизиты <> Неопределено Тогда
				СтруктураЗаполненныеРеквизиты.Вставить("ИспользоватьПлановуюСебестоимость");
			КонецЕсли;	
		КонецЕсли;
	КонецЕсли;
...
//  СтруктураЗаполненныеРеквизиты   - структура, ключи - имена реквизитов, уже заполненных при вводе на основании документа
//
Процедура ЗаполнитьШапкуДокумента(ДокументОбъект, ТипОперации = "", ОбъектКопирования = Неопределено, СтруктураЗаполненныеРеквизиты = Неопределено) Экспорт	
	...
	СтруктураПараметров.Вставить("МетаданныеИмя",МетаданныеДокумента.Имя);
	
	// Не перезаполняем повторно реквизиты, уже заполненные при вводе на основании
	Если СтруктураЗаполненныеРеквизиты <> Неопределено Тогда
		Для Каждого ЗаполненныйРеквизит Из СтруктураЗаполненныеРеквизиты Цикл
			Если СтруктураПараметров.Свойство(ЗаполненныйРеквизит.Ключ) Тогда
				СтруктураПараметров.Удалить(ЗаполненныйРеквизит.Ключ);
			КонецЕсли; 
		КонецЦикла; 
	КонецЕсли; 
	...
	Пока ЕстьДокументы Цикл
		...
		Если ДокументСсылка.Проведен Тогда
			...
			Если (ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.КорректировкаСтоимостиСписанияТоваров"))
				 ИЛИ (ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РегламентнаяПереоценкаЗапасов"))
			...
			Если НЕ ИспользуетсяРасширеннаяАналитикаУчета Тогда
				УправлениеПроизводствомДвиженияПоРегистрам.ПровестиДокументПоПроизводственнымРегистрам(ДокументСсылка, Упр, Бух, ОшибкаТекущегоДокумента);
