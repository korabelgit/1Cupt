...
Процедура ЗаполнитьСубконтоПоСписаниюТоваровРегл(Проводка, СтрокаДокумента, Движение, СтруктураПараметров, ВидУчета) Экспорт
	...
	Если ЗначениеЗаполнено(Проводка.СчетДт) Тогда
		...
		Для каждого ВидСубконто Из Проводка.СчетДт.ВидыСубконто Цикл
			...
			Если Не (Движение.КодОперации = Перечисления.КодыОперацийПартииТоваров.ВозвратОтКомиссионера) И 
				 // В случае дооценки Дт 2* Кт 7*
				 Не (Движение.КодОперации = Перечисления.КодыОперацийПартииТоваров.ПереоценкаКупленных И Движение.ВидДвижения = ВидДвиженияНакопления.Приход)  Тогда
	...
	Если ЗначениеЗаполнено(Проводка.СчетКт) Тогда
		...
		Для каждого ВидСубконто Из Проводка.СчетКт.ВидыСубконто Цикл
			...
			Если Движение.КодОперации = Перечисления.КодыОперацийПартииТоваров.ПереоценкаКупленных 
			   И Движение.ВидДвижения = ВидДвиженияНакопления.Приход Тогда
			
				ЗаполнитьСубконтоПоРеквизитам(ВидСубконто, Проводка.СубконтоКт, КорСубконто1, КорСубконто2, КорСубконто3, , СтруктураПараметров);
				
				Если Проводка.СубконтоКт[ВидСубконто.ВидСубконто] <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
...
////////////////////////////////////////////////////////////////////////////////
// ОПРИХОДОВАНИЕ ПО СЧЕТАМ БУХ УЧЕТА

// Формирование проводок по оприходованию на счет по бухгалтерскому учету
Процедура ОприходованиеНаСчетБух(СписаноИз, СтрокаДокумента, СтруктураПараметров, Движение, ТипЗаписи = "Прямая") Экспорт
	...
	НалоговоеНазначениеДт        = Движение.НалоговоеНазначение;
	Если ЗначениеЗаполнено(СтрокаДокумента.НалоговоеНазначениеНовое) Тогда
		НалоговоеНазначениеКт    = СтрокаДокумента.НалоговоеНазначениеНовое;
	ИначеЕсли НЕ ЗначениеЗаполнено(СтрокаДокумента.НалоговоеНазначениеНовое)
		...
		НалоговоеНазначениеКт    = НалоговоеНазначениеДт;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаДокумента.КорСчетБУ)
		И Проводка.СчетДт.Забалансовый = СтрокаДокумента.КорСчетБУ.Забалансовый Тогда
		
		Проводка.СчетКт    = СтрокаДокумента.КорСчетБУ;
	...
	Если НЕ СтрокаДокумента.КодОперацииПартииТоваров = Перечисления.КодыОперацийПартииТоваров.ВозвратОтПокупателя тогда
		Если Проводка.СчетДт.Забалансовый
			И (ЗначениеЗаполнено(СтрокаДокумента.ПринятыеКорСчетБУ)) Тогда
			
			Проводка.СчетКт    = СтрокаДокумента.ПринятыеКорСчетБУ;
	...
	Если Проводка.СчетКт.Валютный Тогда
		...
		Проводка.ВалютнаяСуммаКт = Движение.ВалютнаяСуммаЗадолженностиБУ;
	...
	Проводка.СуммаНУДт = НалоговыйУчет.ОпределитьСтоимостьНУ(Проводка.НалоговоеНазначениеДт, СуммаНУ, СтруктураПараметров.ЕстьНалогНаПрибыльДо2015);
	Проводка.СуммаНУКт = НалоговыйУчет.ОпределитьСтоимостьНУ(Проводка.НалоговоеНазначениеКт, СуммаНУ, СтруктураПараметров.ЕстьНалогНаПрибыльДо2015);
	...
	Если ЗначениеЗаполнено(Проводка.СчетКт) Тогда
		...
		Если Проводка.СчетКт.Количественный  Тогда
			...
			Если Движение.КоличествоПоступление<>0 Тогда 
				...
				Проводка.КоличествоКт	= Движение.КоличествоПоступление;
			КонецЕсли;
		КонецЕсли;
		
	КонецЕсли;
	
	Проводка.Содержание = ПолучитьСодержаниеПроводки(СтрокаДокумента, СтруктураПараметров, Проводка.СчетДт);	
	...
	Движение.КорСчет      = Проводка.СчетКт;

	// В реквизиты движения регистра занесем налоговое назначение списания
	Движение.НалоговоеНазначениеПоФакту = Проводка.НалоговоеНазначениеКт;
	...
	Для Каждого ВидСубконто Из Проводка.СчетКт.ВидыСубконто Цикл
		...
		Если ЗначениеЗаполнено(Проводка.СубконтоКт[ВидСубконто.ВидСубконто]) тогда
			Движение["КорСубконто"+Сч] = Проводка.СубконтоКт[ВидСубконто.ВидСубконто];
		Иначе
...
Процедура СписаниеНаСчетБух(СписаноИз, СтрокаДокумента, СтруктураПараметров, Движение, ТипЗаписи = "Прямая") Экспорт
	...
	Если Движение.КодОперации = СтруктураПараметров.КодыОпераций.ВозвратОтКомиссионера Тогда 
		// В случае возврата товаров от комиссионера они оприходуются 
		// на склад. Счет затрат (отличие от "возврата от покупателя") 
		// не сторнируется
		НалоговоеНазначениеДт    = НалоговоеНазначениеКт;
	
	ИначеЕсли ЗначениеЗаполнено(СтрокаДокумента.НалоговоеНазначениеНовое) Тогда
	...
	Проводка.НалоговоеНазначениеДт    = ?(СтруктураПараметров.Период >= глЗначениеПеременной("ДатаНКУ2015"), 
											?(БухгалтерскийУчетПовтИсп.УчетПоНалоговымНазначениямНДССчет(Проводка.СчетДт), 
												НалоговоеНазначениеДт, 
											  	Справочники.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка()
											),
											НалоговоеНазначениеДт
										);	
	Проводка.НалоговоеНазначениеКт    = ?(СтруктураПараметров.Период >= глЗначениеПеременной("ДатаНКУ2015"), 
											?(БухгалтерскийУчетПовтИсп.УчетПоНалоговымНазначениямНДССчет(Проводка.СчетКт), 
	...
	Иначе
		...
		Проводка.Сумма = Сумма;
		Проводка.СуммаНУДт = НалоговыйУчет.ОпределитьСтоимостьНУВПроводке(Проводка.Период, Сумма, СуммаНУ, Проводка.СчетДт, Проводка.СчетКт, Проводка.НалоговоеНазначениеДт, СтруктураПараметров.ЕстьНалогНаПрибыльДо2015, СтруктураПараметров.ЕстьНалогНаПрибыльС2015);
		Проводка.СуммаНУКт = НалоговыйУчет.ОпределитьСтоимостьНУ(Проводка.НалоговоеНазначениеКт, СуммаНУ, СтруктураПараметров.ЕстьНалогНаПрибыльДо2015);
	...
	// В реквизиты движения регистра занесем налоговое назначение списания
	Движение.НалоговоеНазначениеПоФакту = НалоговоеНазначениеДт;
...
Процедура СформироватьПроводкиОтклоненийВСтоимостиПриВозвратеБух(СтруктураПараметров, СтоимостьСписания) 
	...
	Если (НЕ СуммаОтклонения = 0 ИЛИ НЕ СуммаОтклоненияВал = 0) И 
		...
		Если СуммаОтклонения > 0 Тогда
			...
			КонецЦикла;
			Для каждого ВидСубконто Из Проводка.СчетДт.ВидыСубконто Цикл
				ЗаполнитьСубконтоПоРеквизитам(ВидСубконто, Проводка.СубконтоДт, СтруктураРеквизитов.ДоговорКонтрагента, СтруктураРеквизитов.Контрагент, СтруктураРеквизитов.СделкаРегл, , СтруктураПараметров);
			КонецЦикла;
		...
		// Если стоимость списанных партий больше стоимости по документу
			...
			КонецЦикла;
			Для каждого ВидСубконто Из Проводка.СчетКт.ВидыСубконто Цикл
				ЗаполнитьСубконтоПоРеквизитам(ВидСубконто, Проводка.СубконтоКт, СтруктураРеквизитов.ДоговорКонтрагента, СтруктураРеквизитов.Контрагент, СтруктураРеквизитов.СделкаРегл, , СтруктураПараметров);
			КонецЦикла;
			// разницу по НУ не показываем
...
Функция ПолучитьСтруктуруШапкиДокументаПоСсылке(Знач Ссылка) Экспорт
	...
	Если СтруктураШапкиДокумента.Свойство("ДоговорКонтрагента") тогда
		...
		Если СтруктураШапкиДокумента.Свойство("Сделка") Тогда
			ВедениеВзаиморасчетовРегл = СтруктураДоговора.ВедениеВзаиморасчетовРегл;
			...
			КонецЕсли;
			Если ВедениеВзаиморасчетовРегл = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоДоговоруВЦелом Тогда
