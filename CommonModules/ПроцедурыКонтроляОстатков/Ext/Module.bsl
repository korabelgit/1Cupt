...
Процедура ТоварыПереданныеКонтрольОстатков_ЗаказВШапке(СтруктураШапкиДокумента, СтруктураПараметров, МетаданныеТабЧасти, Заголовок, Отказ)
	...
	Иначе	
		...
		Если СтруктураПараметров.ИмяДокумента = "ВозвратПереданныхТоваров" И СтруктураПараметров.СтатусПередачи = Перечисления.СтатусыПолученияПередачиТоваров.ВПереработку Тогда
			ЕстьСерия = Ложь;
		КонецЕсли;	
