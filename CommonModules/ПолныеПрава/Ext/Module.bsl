...
Процедура ПодготовитьСтруктуруДляЗаполненияШапкиДокумента(СтруктураПараметров, ТипОперации, ОбъектКопирования) Экспорт
	...
	Если  СтруктураПараметров.Свойство("МетаданныеИмя")
		  И (    СтруктураПараметров.МетаданныеИмя = "НалоговаяНакладная"
		     ИЛИ СтруктураПараметров.МетаданныеИмя = "ЗаписьКнигиПриобретения" 
		     ИЛИ СтруктураПараметров.МетаданныеИмя = "РегистрацияВходящегоНалоговогоДокумента") Тогда
	
		Если СвойствоЕстьИОноНЕЗаполнено("ОбособленноеПодразделение", СтруктураПараметров) Тогда
			СтруктураПараметров.ОбособленноеПодразделение = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ТекПользователь, "ОсновноеОбособленноеПодразделениеОрганизации");
			Если СтруктураПараметров.ОбособленноеПодразделение.Владелец <> СтруктураПараметров.Организация Тогда
				СтруктураПараметров.ОбособленноеПодразделение = Неопределено;
			КонецЕсли;
		КонецЕсли;
