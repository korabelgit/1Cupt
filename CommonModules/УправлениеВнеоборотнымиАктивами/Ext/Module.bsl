...
//
Функция РасчетАмортизацииРегл(ДатаРасчета, Организация, ОсновноеСредство = Неопределено, ОтражатьВБУ = Истина, ОтражатьВНУ = Истина, ВыдаватьСообщения = Истина, Отказ = Ложь) Экспорт       
	...
	|	ПервоначальныеСведенияНалоговыйУчетСрезПоследних.СпособНачисленияАмортизации 		КАК СпособНачисленияАмортизацииНУ
	|ПОМЕСТИТЬ
	...
	|	СписокАмортизируемыхОС.СпособНачисленияАмортизацииНУ                          КАК СпособНачисленияАмортизацииНУ,
	...
	|			ТОГДА ЕСТЬNULL(СтоимостьОС.СтоимостьОстаток, 0)
	|		ИНАЧЕ ЕСТЬNULL(СтоимостьОСНаНачало.СтоимостьОстаток, 0)
	...
	|	ВЫБОР
	|		КОГДА СписокАмортизируемыхОС.СпособНачисленияАмортизации В (&СписокУпрощенныхМетодов)
	|			ТОГДА ЕСТЬNULL(СтоимостьОС.СтоимостьНУОстаток, 0)
	|		ИНАЧЕ ЕСТЬNULL(СтоимостьОСНаНачало.СтоимостьНУОстаток, 0)
	|	КОНЕЦ                                                                         КАК СтоимостьКонечныйОстатокНУ,
	...
	|	ВЫБОР
	|		КОГДА СписокАмортизируемыхОС.СпособНачисленияАмортизации В (&СписокУпрощенныхМетодов)
	|			ТОГДА СпособыОтраженияРасходовПоАмортизацииБУДляМалоценных.СпособыОтраженияРасходовПоАмортизации
	|		ИНАЧЕ СпособыОтраженияРасходовПоАмортизацииБУ.СпособыОтраженияРасходовПоАмортизации
	...
	|	ЕСТЬNULL(ПереоценкаОСБУ.СуммаПереоценкиОстаток, 0)          				  КАК СуммаПереоценки,
	|	ЕСТЬNULL(ПереоценкаОСБУОборот.СуммаПереоценкиОборот, 0)                       КАК СуммаПереоценкиОборот,
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.СтоимостьОСБухгалтерскийУчет.Остатки(&ГраницаНачалоПериода,
	|		                  Организация = &Организация И ОсновноеСредство В (ВЫБРАТЬ ОсновноеСредство ИЗ СписокАмортизируемыхОС)) КАК СтоимостьОСНаНачало
	|	ПО СписокАмортизируемыхОС.ОсновноеСредство = СтоимостьОСНаНачало.ОсновноеСредство
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	...
	|		                &НачалоПериода, Организация = &Организация И ОсновноеСредство В (ВЫБРАТЬ ОсновноеСредство ИЗ СписокАмортизируемыхОС)
	...
	|		                ) КАК СпособыОтраженияРасходовПоАмортизацииБУДляМалоценных
	|	ПО СписокАмортизируемыхОС.ОсновноеСредство = СпособыОтраженияРасходовПоАмортизацииБУДляМалоценных.ОсновноеСредство
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.ПереоценкаОСБухгалтерскийУчет.Остатки(&ГраницаКонецПериода,
	|		                  Организация = &Организация И ОсновноеСредство В (ВЫБРАТЬ ОсновноеСредство ИЗ СписокАмортизируемыхОС)
	|		                  ) КАК ПереоценкаОСБУ
	|	ПО СписокАмортизируемыхОС.ОсновноеСредство = ПереоценкаОСБУ.ОсновноеСредство
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.ПереоценкаОСБухгалтерскийУчет.Обороты(&ГраницаНачалоПериода, &ГраницаКонецПериода, ,
	|		                  Организация = &Организация И ОсновноеСредство В (ВЫБРАТЬ ОсновноеСредство ИЗ СписокАмортизируемыхОС)) КАК ПереоценкаОСБУОборот
	|	ПО СписокАмортизируемыхОС.ОсновноеСредство = ПереоценкаОСБУОборот.ОсновноеСредство
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|			РегистрСведений.СостоянияОСОрганизаций КАК СостоянияОС
	...
	|	ЛОЖЬ	                           												КАК НачислятьАмортизациюНаНачало,
	...
	|		    	              Организация = &Организация И ОсновноеСредство В (ВЫБРАТЬ ОсновноеСредство ИЗ СписокОСПринятыхВТекущемМесяце)
	|		        	          ) КАК ПереоценкаОСБУ
	|		ПО СписокОСПринятыхВТекущемМесяце.ОсновноеСредство = ПереоценкаОСБУ.ОсновноеСредство
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.ПереоценкаОСБухгалтерскийУчет.Обороты(&ГраницаНачалоПериода, &ГраницаКонецПериода, ,
	|		                  Организация = &Организация И ОсновноеСредство В (ВЫБРАТЬ ОсновноеСредство ИЗ СписокОСПринятыхВТекущемМесяце)) КАК ПереоценкаОСБУОборот
	|	ПО СписокОСПринятыхВТекущемМесяце.ОсновноеСредство = ПереоценкаОСБУОборот.ОсновноеСредство
	|		ЛЕВОЕ СОЕДИНЕНИЕ 
	...
	Пока ВыборкаПоОС.Следующий() Цикл
		...
		ТекстСообщенияНУ = "Нал. расчет амортизации: для основного средства " + СокрЛП(ВыборкаПоОС.ОсновноеСредство)
		               + " (" + СокрЛП(ВыборкаПоОС.Код) + "), инв. № " + СокрЛП(ВыборкаПоОС.ИнвентарныйНомер);
		...
		Если ОтражатьВБУ И (ВыборкаПоОС.АмортизацияОборот <> 0) И (ВыборкаПоОС.АмортизацияОборот <> Null) И НЕ ЗначениеЗаполнено(ВыборкаПоОС.СуммаПереоценкиОборот) Тогда
		...
		Если ОтражатьВБУ И НЕ ЗначениеЗаполнено(ВыборкаПоОС.СпособНачисленияАмортизации) Тогда
		...
		Если НачислятьАмортизациюНУ И ЗначениеЗаполнено(ВыборкаПоОС.АмортизацияОборотНУ) Тогда
			...
			Если ВыдаватьСообщения Тогда
				...
				ОбщегоНазначения.СообщитьИнформацию(ТекстСообщенияНУ + " амортизация в этом месяце уже начислялась");
		...
		Если НачислятьАмортизациюНУ И НЕ ЗначениеЗаполнено(СпособНачисленияАмортизацииНУ) Тогда
			...
			Если ВыдаватьСообщения Тогда
				...
				ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщенияНУ + " не указан способ амортизации",
								 Отказ);
		...
		Если НачислятьАмортизациюНУ И СпособНачисленияАмортизацииНУ = Перечисления.СпособыНачисленияАмортизацииОС.Производственный И НачалоМесяца(ДатаРасчета) >= ДатаНКУ2015 Тогда
			...
			Если ВыдаватьСообщения Тогда
				...
				ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщенияНУ + " указан недопустимый производственный метод начисления амортизации. Измените метод документом ""Изменение параметров начисления амортизации ОС"". ",
							 Отказ);
		...
		Если НачислятьАмортизациюНУ И ВыборкаПоОС.НалоговоеНазначение = Справочники.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяНеХозДеятельность И НачалоМесяца(ДатаРасчета) >= ДатаНКУ2015 Тогда
			...
			Если ВыдаватьСообщения Тогда
				...
				ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщенияНУ + " указано нехоз. налоговое назначение. Амортизация (НУ) не начислена.",
								 );
		...
		Если (ЛиквидационнаяСтоимость <> 0) 
		   И (ЛиквидационнаяСтоимость >= ОстаточнаяСтоимость) И (ЛиквидационнаяСтоимость >= ОстаточнаяСтоимостьНУ) Тогда
			...
			СтоимостьИтоговая = ОбщегоНазначения.ПреобразоватьВЧисло(СтоимостьИтоговая);
 			...
 			Если ДатаВводаВЭксплуатацию = NULL Тогда
				...
				Если ВыдаватьСообщения Тогда
					...
					ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Бух. расчет амортизации: для основного средства %1 при расчете по способу %2 не определилась дата ввода в эксплуатацию!';uk='Бух. розрахунок амортизації: для основного засобу %1 при розрахунку способом %2 не визначилася дата уведення в експлуатацію!'"),
		...
		Если НачислятьАмортизациюНУ Тогда
			...
			ИначеЕсли СпособНачисленияАмортизацииНУ = Перечисления.СпособыНачисленияАмортизацииОС.УменьшенияОстатка Тогда
				...
				Если СрокИспользованияДляВычисленияАмортизацииНУ = 0 Тогда
					...
					Если ВыдаватьСообщения Тогда
						...
						ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщенияНУ + " при расчете по способу " 
										 + ВыборкаПоОС.СпособНачисленияАмортизации 
										 + " не указан срок полезного использования (НУ)!", 
										 Отказ);
				...
				СтоимостьИтоговая = ОбщегоНазначения.ПреобразоватьВЧисло(СтоимостьИтоговая);
	 			...
	 			Если ДатаВводаВЭксплуатациюНУ = NULL Тогда
					...
					Если ВыдаватьСообщения Тогда
						...
						ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Нал. расчет амортизации: для основного средства %1 при расчете по способу %2 не определилась дата ввода в эксплуатацию!';uk='Бух. розрахунок амортизації: для основного засобу %1 при розрахунку способом %2 не визначилася дата уведення в експлуатацію!'"),
						                           ВыборкаПоОС.ОсновноеСредство, ВыборкаПоОС.СпособНачисленияАмортизации), Отказ);
				...
				Иначе // Срок использования исчерпан
					...
					Если ВыдаватьСообщения Тогда
						...
						Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Нал. расчет амортизации: для основного средства %1 при расчете по способу %2 срок использования исчерпан!';uk='Бух. розрахунок амортизації: для основного засобу %1 при розрахунку способом %2 строк використання вичерпаний!'"), ВыборкаПоОС.ОсновноеСредство, ВыборкаПоОС.СпособНачисленияАмортизации));
			...
			ИначеЕсли СпособНачисленияАмортизацииНУ = Перечисления.СпособыНачисленияАмортизацииОС.Кумулятивный Тогда
				...
				Если ДатаВводаВЭксплуатациюНУ = NULL Тогда
					...
					Если ВыдаватьСообщения Тогда
						...
						ОбщегоНазначения.СообщитьИнформацию(ТекстСообщенияНУ + " при расчете по способу " 
										   + ВыборкаПоОС.СпособНачисленияАмортизации 
										   +  " не определилась дата ввода в эксплуатацию!");
			...
			Иначе 
				...
				Если ВыдаватьСообщения  Тогда
					...
					ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщенияНУ 
									 + " установлен способ расчета амортизации, не поддерживаемый в регламентированном учете (" 
		...
		Если ОстаточнаяСтоимость > 0 Тогда
    		СтрокаАмортизации.СуммаПереоценкиПропорциональнаяАмортизации = ВыборкаПоОС.СуммаПереоценки * СуммаАмортизации / ОстаточнаяСтоимость;
		КонецЕсли;	
		...
		Если НачислятьАмортизацию И СуммаАмортизации = 0 И ВыдаватьСообщения И Не Метод_50_50 Тогда
...
Функция ПолучитьРаспределениеАмортизацииПоНаправлениямРегл(ДокОбъект, 
	...
	Для Каждого СтрокаТЗ Из ТабАмортизации Цикл
		...
		МассивСумм = ОбщегоНазначения.РаспределитьПропорционально( СтрокаТЗ.Амортизация, МассивКоэф, 2, Ложь);
		...
		Для Каждого СтрокаНапр Из СтрокаТЗ.НаправлениеАмортизации.Способы Цикл
			...
			ГруппаНалоговогоНазначенияЗатрат = НалоговыйУчетПовтИсп.ОпределитьГруппуНалоговогоНазначенияЗатрат(
				СтрокаНапр.СчетЗатрат, 
				СтрокаНапр.Субконто1, 
				СтрокаНапр.Субконто2, 
				СтрокаНапр.Субконто3,
				ДатаРасчета
			);
			...
			Если ГруппаНалоговогоНазначенияЗатрат = Перечисления.ГруппыНалоговыхНазначений.НалоговыеНазначенияАктивовИВзаиморасчетовПоНДС Тогда
				...
				Если НЕ ЗначениеЗаполнено(НоваяСтрока.НалоговоеНазначениеДоходовИЗатрат) Тогда
					ХарактерЗатратБУ = УправлениеЗатратами.ПолучитьХарактерЗатратПоСчетуЗатрат(СтрокаНапр.СчетЗатрат, Неопределено);
					Если ХарактерЗатратБУ = Перечисления.ХарактерЗатрат.ОбщепроизводственныеРасходы Тогда
						// для 91 счета после 01.08.11, если НалоговоеНазначение в способе не заполнено - используем налоговое назначение объекта
						НоваяСтрока.НалоговоеНазначениеДоходовИЗатрат = СтрокаТЗ.НалоговоеНазначение;
					КонецЕсли;	
				КонецЕсли; 
			Иначе
				...
				Иначе	
					НоваяСтрока.НалоговоеНазначениеДоходовИЗатрат 	= Справочники.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка();	
				КонецЕсли;	
			КонецЕсли;	
			...
			Если СтруктураШапкиДокумента.ЕстьНДС И БухгалтерскийУчетПовтИсп.УчетПоНалоговымНазначениямНДССчет(СтрокаНапр.СчетЗатрат) Тогда
				Если Улучшение Тогда
...
Процедура ПроверитьСоответствиеНалоговогоНазначенияАмортизации(ОбъектУчета, НалоговоеНазначениеЗатрат, НалоговоеНазначениеОбъекта, Отказ = Ложь, СтрокаСообщенияДоп = "") Экспорт 
	...
	ИначеЕсли НЕ НалоговоеНазначениеЗатратНДСное Тогда 
		...
		Если НехозДеятельностьНКУ И НалоговоеНазначениеОбъекта = Справочники.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяХозДеятельность Тогда
			// разрешено
		ИначеЕсли НехозДеятельностьНДС <> НехозДеятельностьНКУ Тогда
			
			СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Налоговое назначение объекта %1 <%2> не соответствует налоговому назначению амортизации (<%3>)'; uk = 'Налоговое назначение объекта %1 <%2> не соответствует налоговому назначению амортизации (<%3>)'"), ОбъектУчета, НалоговоеНазначениеОбъекта, НалоговоеНазначениеЗатрат) + СтрокаСообщенияДоп;
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщения, Отказ);			
...
Процедура ПроверитьСоответствиеНалоговогоНазначенияОбъектов(ОбъектУчетаКт, ОбъектУчетаДт, НалоговоеНазначениеКт, НалоговоеНазначениеДт, Отказ = Ложь, Заголовок = "") Экспорт
	...
	ИначеЕсли НалоговоеНазначениеКт = Справочники.НалоговыеНазначенияАктивовИЗатрат.НДС_Пропорционально Тогда
		...
		Если  НалоговоеНазначениеДт <> Справочники.НалоговыеНазначенияАктивовИЗатрат.НДС_Облагаемая
			...
			СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Налоговое назначение %1 <%2> не соответствует налоговому назначению %3 <%4>';uk='Налоговое назначение %1 <%2> не соответствует налоговому назначению %3 <%4>'"), СтрокаОбъектУчетаКт, НалоговоеНазначениеКт, СтрокаОбъектУчетаДт, НалоговоеНазначениеДт);
...
Процедура ПроверитьСоответствиеНалоговогоНазначенияАмортизацииОбъекта(ОбъектУчета, СпособОтраженияРасходовПоАмортизации, НалоговоеНазначение, Отказ = Ложь, Заголовок = "", ДатаДокумента = Неопределено) Экспорт
		...
		Если (ЛиквидационнаяСтоимость <> 0) 
		   И (ЛиквидационнаяСтоимость >= СтоимостьКонечныйОстаток) Тогда
			...
			Если ВыдаватьСообщения Тогда
				...
				ОбщегоНазначения.СообщитьИнформацию(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Упр. расчет амортизации: остаточная стоимость нематериального актива %1 достигла ликвидационной <%2>, амортизация не начислена.",
		...
		ИначеЕсли ВыборкаПоНМА.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииНМА.УскоренногоУменьшенияОстатка Тогда
			...
			Если СрокИспользованияДляВычисленияАмортизации <= 24 Тогда
				...
				Если ВыдаватьСообщения Тогда
					...
					ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Упр. расчет амортизации: для нематериального актива %1 при расчете по способу %2 указан срок полезного использования <%3> мес.!"
"Для корректного начисления амортизации срок использования должен быть более 24 месяца.",
		...
		Иначе 
			...
			Если ВыдаватьСообщения  Тогда
				...
				ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Упр. расчет амортизации: для нематериального актива %1 установлен способ расчета амортизации, не поддерживаемый в бухгалтерском учете (%2)!", ВыборкаПоНМА.НематериальныйАктив, ВыборкаПоНМА.СпособНачисленияАмортизации), 
								 Отказ);
...
//   ТаблицаЗначений начисленной амортизации
//
Функция РасчетАмортизацииНМАРегл(ДатаРасчета, Организация, НематериальныйАктив = Неопределено, ОтражатьВБУ = Истина, ОтражатьВНУ = Истина, ВыдаватьСообщения = Истина, Отказ = Ложь) Экспорт
	...
	Пока ВыборкаПоНА.Следующий() Цикл
		...
		ТекстСообщенияНУ = "Нал. расчет амортизации: для нематериального актива " + ПредставлениеНМА;
		...
		Если ОтражатьВБУ И (ВыборкаПоНА.АмортизацияОборот <> 0) И (ВыборкаПоНА.АмортизацияОборот <> Null)  Тогда
		...
		Если ОтражатьВБУ И НЕ ЗначениеЗаполнено(ВыборкаПоНА.СпособНачисленияАмортизации) Тогда
		...
		Если ОтражатьВБУ И НЕ ЗначениеЗаполнено(ВыборкаПоНА.СтоимостьДляВычисленияАмортизации) Тогда
		...
		СпособНачисленияАмортизацииНУ = ВыборкаПоНА.СпособНачисленияАмортизацииНУ;
		Если НЕ ЗначениеЗаполнено(СпособНачисленияАмортизацииНУ) Тогда
			СпособНачисленияАмортизацииНУ  = ВыборкаПоНА.СпособНачисленияАмортизации;
		КонецЕсли;	
		...
		Если НачислятьАмортизациюНУ И СпособНачисленияАмортизацииНУ = Перечисления.СпособыНачисленияАмортизацииНМА.Производственный И НачалоМесяца(ДатаРасчета) >= ДатаНКУ2015 Тогда
			...
			Если ВыдаватьСообщения Тогда
				...
				ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщенияНУ + " указан недопустимый производственный метод начисления амортизации. Измените метод документом ""Изменение параметров начисления амортизации НМА"". ",
							 Отказ);
		...
		Если НачислятьАмортизациюНУ И ВыборкаПоНА.НалоговоеНазначение = Справочники.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяНеХозДеятельность И НачалоМесяца(ДатаРасчета) >= ДатаНКУ2015 Тогда
			...
			ИначеЕсли ВыборкаПоНА.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииНМА.УменьшенияОстатка Тогда
				...
				КонецЕсли;
	 			...
	 			Если ДатаПервойАмортизации = NULL Тогда
					...
					Если ВыдаватьСообщения Тогда
						...
						ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Бух. расчет амортизации: для нематериального актива %1 при расчете по способу %2 не определилась дата первой амортизации!';uk='Бух. розрахунок амортизації: для нематеріального активу %1 при розрахунку способом %2 не визначилася дата першої амортизації!'"),
				...
				Иначе // Срок использования исчерпан
					...
					Если ВыдаватьСообщения Тогда
						...
						Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Бух. расчет амортизации: для нематериального актива %1 при расчете по способу %2 срок использования исчерпан!';uk='Бух. розрахунок амортизації: для нематеріального активу %1 при розрахунку способом %2 строк використання вичерпаний!'"),
		...
		Если НачислятьАмортизациюНУ Тогда
			...
			Если СпособНачисленияАмортизацииНУ = Перечисления.СпособыНачисленияАмортизацииНМА.Прямолинейный Тогда
				...
				Если СрокИспользованияДляВычисленияАмортизацииНУ = 0 Тогда
					...
					Если ВыдаватьСообщения Тогда
						...
						ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщенияНУ + " при расчете по способу " + СпособНачисленияАмортизацииНУ + " не указан срок полезного использования!", Отказ);
			...
			ИначеЕсли СпособНачисленияАмортизацииНУ = Перечисления.СпособыНачисленияАмортизацииНМА.УменьшенияОстатка Тогда
				...
				КонецЕсли;
	 			...
	 			Если ДатаПервойАмортизацииНУ = NULL Тогда
					...
					Если ВыдаватьСообщения Тогда
						...
						ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Нал. расчет амортизации: для нематериального актива %1 при расчете по способу %2 не определилась дата первой амортизации!';uk='Под. розрахунок амортизації: для нематеріального активу %1 при розрахунку способом %2 не визначилася дата першої амортизації!'"),
						                           ПредставлениеНМА, СпособНачисленияАмортизацииНУ), Отказ);
				...
				Иначе // Срок использования исчерпан
					...
					Если ВыдаватьСообщения Тогда
						...
						Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Нал. расчет амортизации: для нематериального актива %1 при расчете по способу %2 срок использования исчерпан!';uk='Бух. розрахунок амортизації: для нематеріального активу %1 при розрахунку способом %2 строк використання вичерпаний!'"),
						                        ПредставлениеНМА, СпособНачисленияАмортизацииНУ));
			...
			ИначеЕсли СпособНачисленияАмортизацииНУ = Перечисления.СпособыНачисленияАмортизацииНМА.УскоренногоУменьшенияОстатка Тогда
				...
				Если СрокИспользованияДляВычисленияАмортизацииНУ <= 24 Тогда
					...
					Если ВыдаватьСообщения Тогда
						...
						"Для коректного нарахування амортизації строк використання повинен бути більше 24 місяців.'"),
						ПредставлениеНМА, СпособНачисленияАмортизацииНУ, СрокИспользованияДляВычисленияАмортизацииНУ),);
				...
				Если ДатаПервойАмортизацииНУ = NULL Тогда
					...
					Если ВыдаватьСообщения Тогда
						...
						ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Нал. расчет амортизации: для нематериального актива %1 при расчете по способу %2 не определилась дата первой амортизации!';uk='Бух. розрахунок амортизації: для нематеріального активу %1 при розрахунку способом %2 не визначилася дата першої амортизації!'"),
						                           ПредставлениеНМА, СпособНачисленияАмортизацииНУ), Отказ);
			...
			ИначеЕсли СпособНачисленияАмортизацииНУ = Перечисления.СпособыНачисленияАмортизацииНМА.Кумулятивный Тогда
				...
				Если ДатаПервойАмортизацииНУ = NULL Тогда
					...
					Если ВыдаватьСообщения Тогда
						...
						ОбщегоНазначения.СообщитьИнформацию(ТекстСообщенияНУ + " при расчете по способу " + СпособНачисленияАмортизацииНУ +  " не определилась дата первой амортизации!");
			...
			Иначе 
				...
				Если ВыдаватьСообщения  Тогда
					...
					ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщенияНУ + " установлен способ расчета амортизации, не поддерживаемый в налоговом учете (" + СпособНачисленияАмортизацииНУ +  ")!", Отказ);
...
Процедура ПроверкаДублированияЗаписейСостоянийНМАУпр(Период, ТаблицаЗаписей,Отказ,Заголовок) Экспорт
	...
	Для А = 1 по НаборСостояний.Количество() Цикл
		Если НМА_ПоСостояниям[А-1].Количество()>0 тогда
			...
			Пока Выборка.Следующий() Цикл
				ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Для НМА <%1> уже зафиксировано состояние <%2> по управленческому учету"
...
Процедура ПроверкаДублированияЗаписейСостоянийНМА(Организация, Период, ТаблицаЗаписей,Отказ,Заголовок) Экспорт
	...
	Для А = 1 по НаборСостояний.Количество() Цикл
		Если НМА_ПоСостояниям[А-1].Количество()>0 тогда
			...
			Пока Выборка.Следующий() Цикл
				ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Для НМА <%1> в организации <%2> уже зафиксировано состояние <%3> по бухгалтерскому учету"
...
Процедура ДополнитьТабличнуюЧастьСведениямиОбОСБухНалогРегл(ДатаДока, ТекОрганизация, ТаблицаПоОС,
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	СпособыОтраженияБУ.СпособыОтраженияРасходовПоАмортизации КАК НаправлениеБУ,
...
Процедура ЗаполнитьТабличнуюЧастьОсновныхСредств(ДатаРасчета, ОС, ТекОрганизация, ОтражатьВУправленческомУчете, ОтражатьВБухгалтерскомУчете, 
	...
	|		                      &ДатаИтогов,
	|		                      ОсновноеСредство В (ВЫБРАТЬ ОсновноеСредство ИЗ ОсновныеСредства)
	|			                  И Организация = &Организация) КАК ПереоценкаОСБУ
	|	ПО ОсновныеСредства.ОсновноеСредство = ПереоценкаОСБУ.ОсновноеСредство";
