...
Функция СформироватьЗапросПоСпособамОтраженияРасходов(
	...
	|	СпособыОтраженияРасходов.НалоговоеНазначение,
			...
			Пока Выборка.Следующий() Цикл
				...
				Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
					...
					Если ГруппаНалоговогоНазначенияЗатрат = Перечисления.ГруппыНалоговыхНазначений.НалоговыеНазначенияАктивовИВзаиморасчетовПоНДС Тогда
						НоваяСтрока.НалоговоеНазначениеДоходовИЗатрат 	= Строка.НалоговоеНазначение;
						Если НЕ ЗначениеЗаполнено(НоваяСтрока.НалоговоеНазначениеДоходовИЗатрат) Тогда
							ХарактерЗатратБУ = УправлениеЗатратами.ПолучитьХарактерЗатратПоСчетуЗатрат(Выборка.СчетЗатрат, Неопределено);
					...
					Если ЗначениеЗаполнено(Строка.СчетУчетаБУ) И (СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015 ИЛИ ГруппаНалоговогоНазначенияЗатрат = Перечисления.ГруппыНалоговыхНазначений.НалоговыеНазначенияАктивовИВзаиморасчетовПоНДС) Тогда
						Отказ = Ложь;
						УправлениеВнеоборотнымиАктивами.ПроверитьСоответствиеНалоговогоНазначенияАмортизации(
							Строка.Номенклатура, 
							НоваяСтрока.НалоговоеНазначениеДоходовИЗатрат, 
							НоваяСтрока.НалоговоеНазначение, 
							Отказ
						);
						Если Отказ Тогда
							НоваяСтрока = ТаблицаОшибок.Добавить();
							// Текст сообщения выведен ранее в ПроверитьСоответствиеНалоговогоНазначенияАмортизации
							Продолжить;
						КонецЕсли;	
					КонецЕсли;	
				...
				Тогда
					СформироватьДвиженияПоРегиструБухгалтерииПогашениеСтоимости(
						...
						НоваяСтрока,
						Строка.Склад,
						?(Строка.ВидМатериала = Перечисления.ВидыМатериалов.МалоценныйБыстроизнашивающийсяПредмет, КоличествоБух, 0),
						СтоимостьБух,
						СтоимостьНал,
						КодОперации,
						ТаблицаОшибок,
...
Процедура РассчитатьПогашеннуюСтоимостьПриПередачеВЭксплуатацию(
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
			...
			ПогашеннаяСтоимостьНал = НалоговыйУчет.ОпределитьСтоимостьНУОС(Выборка.НалоговоеНазначение, Выборка.СтоимостьБух, Выборка.СтоимостьНал, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльС2015);
		КонецЕсли;
...
Процедура ПогашениеСтоимостиПриПередачеВЭксплуатацию(
	...
	Если СтруктураПараметров.ЕстьСтрокиОтражатьВБухгалтерскомУчете Тогда
		СтруктураШапкиДокумента.Вставить("ЕстьНалогНаПрибыль", СтруктураПараметров.ЕстьНалогНаПрибыль);
		СтруктураШапкиДокумента.Вставить("ЕстьНалогНаПрибыльДо2015", СтруктураПараметров.ЕстьНалогНаПрибыльДо2015);
		СтруктураШапкиДокумента.Вставить("ЕстьНалогНаПрибыльС2015", СтруктураПараметров.ЕстьНалогНаПрибыльС2015);
	КонецЕсли;	
