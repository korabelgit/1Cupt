...
Функция ПолучитьНазваниеОбъекта(СчетУчета) Экспорт
	Возврат БухгалтерскийУчетПовтИсп.ПолучитьНазваниеОбъекта(СчетУчета);
КонецФункции
...
Процедура ОткрытьОписаниеСчета(Счет, ФормаВладелец = Неопределено) Экспорт
	...
	Макет.КодЯзыкаМакета = ЛокализацияПовтИсп.КодЯзыкаИнформационнойБазы();
	...
	// Заголовок описания
	ЗаголовокОписания = НСтр("ru='<h1>Счет ';uk='<h1>Рахунок '", ЛокализацияПовтИсп.КодЯзыкаИнформационнойБазы()) + Счет.Код + "<br>" + Счет.Наименование + "</h1>";
	
	// Список ссылок на описания субсчетов
	СписокСубсчетов = "";
	Выборка = ПланСчетов.Выбрать(Счет);
	Пока Выборка.Следующий() Цикл
		СписокСубсчетов = СписокСубсчетов + ?(ПустаяСтрока(СписокСубсчетов), НСтр("ru='<h2>К счету открыты следующие субсчета ';uk='<h2>До рахунку відкриті наступні субрахунки'", ЛокализацияПовтИсп.КодЯзыкаИнформационнойБазы()) + ?(Счет.Уровень() > 0, СокрЛП(Счет.Уровень() + 1) + НСтр("ru='-го уровня';uk='-го рівня'", ЛокализацияПовтИсп.КодЯзыкаИнформационнойБазы()), "") + ":</h2><ul>", "");
	...
	Если НЕ Счет.Родитель.Пустая() Тогда
		ОписаниеРодителя = "<p><a href='ОписаниеСчета=" + ЗначениеВСтрокуВнутр(Счет.Родитель) + "'>" + НСтр("ru='Описание счета ';uk='Опис рахунку '", ЛокализацияПовтИсп.КодЯзыкаИнформационнойБазы()) + Счет.Родитель.Код + " """ + Счет.Родитель.Наименование + """</a></p>"
	КонецЕсли;
	...
	Форма.Заголовок = НСтр("ru='Описание счета: ';uk='Опис рахунку: '", ЛокализацияПовтИсп.КодЯзыкаИнформационнойБазы()) + Счет.Код;
	Форма.Открыть();
...
// Проверяет используются счета-классы в качестве групп и возвращает признак использования (булево)
Функция ПолучитьИспользоватьКлассыСчетовВКачествеГрупп() Экспорт
	
	Возврат БухгалтерскийУчетПовтИсп.ПолучитьИспользоватьКлассыСчетовВКачествеГрупп();
...
// Проверяет, принадлежит ли счет счету-родителю или массиву счетов-родителей 
//
// Параметры
//  Счет  		 	- ПланСчетов.Ссылка
//  				  Проверяемый счет
//
//  СчетРодитель 	- ПланСчетов.Ссылка или Массив ПланСчетов.Ссылка
//                 	  Счет-родитель или массив счетов-родителей
//
// Возвращаемое значение:
...
Функция СчетПринадлежитРодителю(Счет, СчетРодитель) Экспорт
	...
	Если ТипЗнч(СчетРодитель) = Тип("Массив") Тогда
		Для каждого ИндСчетРодитель Из СчетРодитель Цикл
			Результат = БухгалтерскийУчетПовтИсп.СчетПринадлежитРодителю(Счет, ИндСчетРодитель);
			Если Результат Тогда
				Прервать;
			КонецЕсли; 
		КонецЦикла; 
	Иначе	
		Результат = БухгалтерскийУчетПовтИсп.СчетПринадлежитРодителю(Счет, СчетРодитель);
	КонецЕсли;
...
Функция СчетСписанияРБП(Субконто1, Субконто2, Субконто3) Экспорт
	
	ТипРБП = Тип("СправочникСсылка.РасходыБудущихПериодов");
	СчетБУ = Неопределено;
	  
	Если ЗначениеЗаполнено(Субконто1) И ТипЗнч(Субконто1) = ТипРБП Тогда
		СчетБУ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Субконто1, "СчетБУ");
	ИначеЕсли ЗначениеЗаполнено(Субконто1) И ТипЗнч(Субконто1) = ТипРБП Тогда
		СчетБУ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Субконто2, "СчетБУ");
	ИначеЕсли ЗначениеЗаполнено(Субконто1) И ТипЗнч(Субконто1) = ТипРБП Тогда
		СчетБУ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Субконто3, "СчетБУ");
	КонецЕсли;
	
	Возврат СчетБУ;
	
КонецФункции // СчетСписанияРБП()
...
Функция ПолучитьИспользуемыеКлассыСчетовРасходов(Дата, Организация) Экспорт
	Возврат БухгалтерскийУчетПовтИсп.ПолучитьИспользуемыеКлассыСчетовРасходов(Дата, Организация);
КонецФункции
...
// (есть ли соответствующая запись в регистре сведений МонетарныеСтатьиВВалютеКромеОперационных) 
//
Функция ВидСтатьиДляПереоценки(
	СтрокаТаблицы, 
	КвоСубконто, 
	ПереоценкаПоПравиламДоПриказа627 = Ложь
	)
...
КонецФункции // ВидСтатьиДляПереоценки


// Возвращает структуру счетов для проведения курсовых разниц
Функция ПараметрыУчетаКурсовыхРазниц(
	КлассСчетовРасходов
	) Экспорт
...
КонецФункции // ПараметрыУчетаКурсовыхРазниц
...
Функция ПодготовкаСтруктурыПараметровДляПереоценкиСчета(Ссылка,Заголовок = Неопределено) Экспорт
	...
	Для Каждого Реквизит из СтруктураПараметров Цикл
		СтруктураПараметров.Вставить(
			Реквизит.Ключ, 
			?(РеквизитыДокумента.Найти(Реквизит.Ключ) = Неопределено,
				Неопределено,
				ОбщегоНазначения.ПолучитьЗначениеРеквизита(Ссылка, Реквизит.Ключ)
			)
		);
	КонецЦикла;
    ...
    СсылкаДата = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Ссылка, "Дата");
	...
	Если Не(БухгалтерскийУчетРасчетовСКонтрагентами.ОпределениеНаправленияДвиженияДляДокументаДвиженияДенежныхСредств(Ссылка.Метаданные().Имя).НаправлениеДвижения = Неопределено) тогда
		...
		Если НЕ ЗначениеЗаполнено(СтруктураПараметров["ДатаОплаты"]) тогда
			СтруктураПараметров.Вставить("Период", СсылкаДата);	
		Иначе
			СтруктураПараметров.Вставить(
				"Период",
				?(ОбщегоНазначения.ПолучитьЗначениеРеквизита(Ссылка, "Оплачено"),
					УправлениеДенежнымиСредствами.ПолучитьДатуДвижений(СсылкаДата, СтруктураПараметров["ДатаОплаты"]),
					СсылкаДата
				)
			);	
		Конецесли;
	...
	СтруктураПараметров.Вставить("ЕстьНалогНаПрибыльДо2015", СтруктураПараметров.ЕстьНалогНаПрибыль И (СтруктураПараметров.Период <  глЗначениеПеременной("ДатаНКУ2015")));
	СтруктураПараметров.Вставить("ЕстьНалогНаПрибыльС2015" , СтруктураПараметров.ЕстьНалогНаПрибыль И (СтруктураПараметров.Период >= глЗначениеПеременной("ДатаНКУ2015")));
...
// Процедура рассчитывает курсовые разницы и формирует проводки в БУ
Процедура ПереоценкаСчетаРегл(
	СтруктураПараметров,
	ПроводкиБУ,
	СчетПереоценки, 
	ВалютаРегламентированногоУчета,
	ВставитьПервой      = Истина,
	РассчитыватьОстатки = Истина,
	ЧастичнаяПереоценка = Ложь,
	ПереоценкаПоПравиламДоПриказа627 = Ложь
	) Экспорт
	...
	Если Не(БухгалтерскийУчетПовтИсп.ВалютныйСчет(ТекСчет)) Тогда
		Возврат;
	КонецЕсли;
	...
	ДатаКурса = ДатаДока;
	Если НЕ СтруктураПараметров.Свойство("ДатаКурса", ДатаКурса) Тогда
		ДатаКурса = ДатаДока;
	КонецЕсли;
	...
	Если НЕ СтруктураПараметров.Свойство("ЕстьНалогНаПрибыльДо2015", ЕстьНалогНаПрибыльДо2015) Тогда
		ЕстьНалогНаПрибыльДо2015 = Ложь;
	КонецЕсли;
	...
	Если РассчитыватьОстатки Тогда
		...
		Для Каждого Стр Из РеестрОстатков Цикл
			...
			РасчетныйОстатокВалРегУчета = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(
											ОстатокИнаяВал, 
											Стр.Валюта, 
											ВалютаРегламентированногоУчета,
											ДанныеОВалютеОстатка.Курс, 
											ДанныеОВалютеРег.Курс,
											ДанныеОВалютеОстатка.Кратность,
											ДанныеОВалютеРег.Кратность
										  );
...
// При расчете курсовой разницы учитывается сумма операции документа
Процедура ПереоценкаСчетаВПределахСуммыОперацииРегл(
	СтруктураПараметров, 
	ПроводкиБУ, 
	СчетПереоценки, 
	ВалютаРегламентированногоУчета="", 
	...
	Для Каждого Стр Из РеестрОстатков Цикл
		...
		Если ОстатокИнаяВал <> 0 Тогда
			Если СуммаДляПереоценки <> ОстатокИнаяВал Тогда
...
Функция ПереоценкаСчетовДокументаРегл(
	СтруктураПараметров, 
	ПроводкиБУ, 
	ВалютаРегламентированногоУчета, 
	ДвиженияБылиЗаписаны = Ложь
	) Экспорт
	...
	Если СтруктураПараметров.Период >= глЗначениеПеременной("ДатаНачалаПереоценкиВалютыПоПравиламПриказа627") Тогда
	...
	Если ДвиженияБылиЗаписаны Тогда
		...
		|		СУММА(ВалютныеСчетаПроводок.СуммаОперацииВал) КАК СуммаОперацииВал
		|	ИЗ
		|		(ВЫБРАТЬ
		...
		|           ХозрасчетныйДвиженияССубконто.ВалютнаяСуммаДт КАК СуммаОперацииВал
		|
		|		ИЗ
		...
		|           -ХозрасчетныйДвиженияССубконто.ВалютнаяСуммаКт КАК СуммаОперацииВал
		|		ИЗ
		|			РегистрБухгалтерии.Хозрасчетный.ДвиженияССубконто(
		|				,
		|				,
		...
		Пока Выборка.Следующий() Цикл
			...
			ТекЗаписьТаблицы.СуммаОперации = Выборка.СуммаОперацииВал;
	...
	Иначе
		...
		Для Каждого ТекПроводка Из ПроводкиБУ Цикл
			...
			Если БухгалтерскийУчетПовтИсп.ВалютныйСчет(ТекПроводка.СчетКт) Тогда
				...
				Иначе
					НайденныеСтроки[0].СуммаОперации = НайденныеСтроки[0].СуммаОперации + (-ТекПроводка.ВалютнаяСуммаКт);
				КонецЕсли;
  			КонецЕсли;
			
			Если БухгалтерскийУчетПовтИсп.ВалютныйСчет(ТекПроводка.СчетДт) Тогда
				...
				Иначе
					НайденныеСтроки[0].СуммаОперации = НайденныеСтроки[0].СуммаОперации + ТекПроводка.ВалютнаяСуммаДт;
	  			КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого ТекСчет Из ТаблицаСчетов Цикл
		...
		Если ПереоценкаПоПравиламПриказа627 Тогда
			ПереоценкаСчетаВПределахСуммыОперацииРегл(
				СтруктураПараметров, 
				ПроводкиБУ, 
				СчетДляПереоценки, 
				ВалютаРегламентированногоУчета, 
		...
		Иначе
			ПереоценкаСчетаРегл(
				...
				СчетДляПереоценки, 
				ВалютаРегламентированногоУчета, 
				Истина, // ВставитьПервой
				,       // РассчитыватьОстатки = Истина  
				,       // ЧастичнаяПереоценка = Ложь 
				Истина  // ПереоценкаПоПравиламДоПриказа627
			);	
		КонецЕсли;
