...
Процедура НачалоВыбораЗначенияСделки(ДокументОбъект, ФормаДокумента, ЭлементФормы, ДоговорКонтрагента, Сделка,
	...
	Если (СтрокаТипаЗаказа = "ЗаказПоставщику") или (СтрокаТипаЗаказа = "ЗаказПокупателя") Тогда
		...
		Если ЕстьТипСчет Тогда
			...
			Если НЕ ЗначениеЗаполнено(Сделка) ИЛИ ТипЗнч(Сделка) <> Тип("ДокументСсылка." + ТипДокументаСделки) Тогда	
...
КонецФункции // ПроверитьМодифицированностьДокумента
...
КонецФункции // ПроверитьМодифицированностьОбъекта
...
КонецПроцедуры // ОткрытьСвойстваСправочника
...
КонецПроцедуры // ОбновитьСвойстваСправочника
...
КонецФункции // ПолучитьМассивНедоступныхВидовОпераций
...
//  ПризнакВалютный 	            – <Булево> - признак "Валютный", по которому будет ограничиваться список - результат функции
//
// Возвращаемое значение:
//   <СписокЗначений> – список счетов, среди которых будет производиться выбор в форме документа
//
Функция ЗаполнитьСписокВыбораСчетовДенежныхСредств(МассивСчетовОтбора, ОтборПоПризнакуВалютный = Ложь, ПризнакВалютный = Неопределено, МассивСчетовИсключений = Неопределено) Экспорт
	...
	ОтбиратьПоПризнакуВалютный = ОтборПоПризнакуВалютный И (ПризнакВалютный <> Неопределено);
	
	Запрос.УстановитьПараметр("ОтбиратьПоПризнакуВалютный", ОтбиратьПоПризнакуВалютный);
	
	Если ОтбиратьПоПризнакуВалютный Тогда
		Запрос.УстановитьПараметр("Валютный", ПризнакВалютный);
	Иначе
