...
Процедура ЗаполнитьДополнительныеПоляПоУмолчанию(ОтчетОбъект) Экспорт
	...
	Для Каждого Группировка Из ДанныеОтчета.Группировка Цикл 
		...
		ИначеЕсли ТипПоля.СодержитТип(Тип("ДокументСсылка.ПоступлениеТоваровУслуг"))
			...
			ИЛИ ТипПоля.СодержитТип(Тип("ДокументСсылка.АккредитивПолученный"))
			...
			ИЛИ ТипПоля.СодержитТип(Тип("ДокументСсылка.ЗаписьКнигиПриобретения"))
			ИЛИ ТипПоля.СодержитТип(Тип("ДокументСсылка.ПлатежноеПоручениеВходящее"))
			ИЛИ ТипПоля.СодержитТип(Тип("ДокументСсылка.ПлатежноеТребованиеПолученное"))
			ИЛИ ТипПоля.СодержитТип(Тип("ДокументСсылка.ПлатежноеТребованиеПоручениеПолученное"))
			ИЛИ ТипПоля.СодержитТип(Тип("ДокументСсылка.ПлатежныйОрдерПоступлениеДенежныхСредств"))
			ИЛИ ТипПоля.СодержитТип(Тип("ДокументСсылка.ПлатежныйОрдерСписаниеДенежныхСредств"))
			ИЛИ ТипПоля.СодержитТип(Тип("ДокументСсылка.ПолучениеУслугПоПереработке"))
					...
					ИначеЕсли Найти(Отбор.Поле, "Субконто") = 1 тогда
						...
						ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.АнализСубконто" И ФормироватьКарточкуСчета Тогда
							...
							Если НайденнаяСтрока <> Неопределено Тогда
								...
								Если (Найти(Строка(Отбор.Поле), "СвязанноеПоле") > 0) Тогда
									Сообщить(НСтр("ru='Сумма расшифровки может не совпадать с требуемой. Не удалось установить отбор по полю ';uk='Сума розшифрування може не збігатися з необхідною. Не вдалося встановити відбір по полю '")+Строка(Отбор.Поле));
								Иначе
