...
Процедура ВыполнитьПогашениеПотребностиЗаказаНаПроизводствоПоРаспределению(ДокОбъект, ВидДокумента, ИмяРеквизитаЗаказ, СтруктураШапкиДокумента, ИмяТабличнойЧастиПродукция)
	...
	Иначе	
	 ...
	 Заголовок = ОбщегоНазначения.ПредставлениеДокументаПриПроведении(ДокОбъект.Ссылка);
				...
				Иначе
					...
					Пока МассивСтрок.Количество() > 0 И КолОстатки > 0 Цикл
						...
						Для каждого Строка из СтрокиУжеРазмещено цикл
							КоличествоУжеРазмещено = КоличествоУжеРазмещено + Макс(Строка.Количество,0);
						КонецЦикла;
						
						КолОстатки = КолОстатки - КоличествоУжеРазмещено;
						Если КолОстатки <= 0 Тогда 
							Продолжить; 
						КонецЕсли;
						...
						// Переведем количество документа в количество в ед. хранения остатков
						ДокКоличествоВЕдХранения = МассивСтрок[0].Количество 
							* ОбходХарактеристика.Коэффициент 
							/ ОбходХарактеристика.КоэффициентЕдиницыХранения;
							
						// Определим зарезервированное/размещеное количество в ед. хранения остатков
						МожноРазместить = Мин(КолОстатки, ДокКоличествоВЕдХранения);
						
						// Переведем зарезервированное/размещеное количество в ед. документа
						РазмещеноВЕдДокумента = МожноРазместить 
							* ОбходХарактеристика.КоэффициентЕдиницыХранения 
							/ ОбходХарактеристика.Коэффициент;
						
						НоваяСтрока.Количество = РазмещеноВЕдДокумента;
						НоваяСтрока.Размещение = Обход.Размещение;
						
						МассивСтрок[0].Количество = МассивСтрок[0].Количество - РазмещеноВЕдДокумента;
