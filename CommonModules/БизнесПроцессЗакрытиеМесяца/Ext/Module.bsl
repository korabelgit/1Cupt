...
//	1) В точке маршрута "Расчет себестоимости УУ" в документе не должны быть установлен признак БУ
//	2) В точке маршрута "Расчет себестоимости регл" в документе не дожен быть установлен признак УУ
Функция СкорректироватьОтражениеВУчете(ЗначенияПризнаковОтраженияВУчетах, ТочкаМаршрута, ПериодРегистрации) Экспорт
	...
	ИначеЕсли ИмяТочкиМаршрута = НРег("ПереоценитьВалютныеСредства") Тогда
		// переоценку валюты по БУ выполняем только на конец квартала 
		Если КонецМесяца(ПериодРегистрации) <> КонецКвартала(ПериодРегистрации) Тогда
			// не проверяем признак БУ
			НовыеЗначенияПризнаковОтраженияВУчетах.Вставить("ОтражатьВБухгалтерскомУчете",	Ложь);
		КонецЕсли;	
	КонецЕсли;
...
КонецФункции // СкорректироватьОтражениеВУчете
...
Функция ЗаполнитьМассивНедоступныхРегламентныхОпераций(ДатаАнализа, Организация = Неопределено, МестоВызова = "ИзБизнесПроцесса") Экспорт
	...
	Для Сч = 1 по ОбластьСписокНастроек.ВысотаТаблицы Цикл
		...
		ПризнакДоступности = Число(СокрЛП(ОбластьСписокНастроек.Область(Сч,4,Сч,4).Текст));
	...
	Если МестоВызова = "ИзБизнесПроцесса" Тогда
		// регламентную переоценку запасов выполняем только на конец квартала 
		Если КонецМесяца(ДатаАнализа) <> КонецКвартала(ДатаАнализа) Тогда
			МассивОпераций.Добавить(НРег("ВыполнитьРегламентнуюПереоценкуЗапасов"));	
			МассивОпераций.Добавить(НРег("ПереоценитьВалютныеСредства"));	
		КонецЕсли;
	КонецЕсли;	
	
	Возврат МассивОпераций;
	
КонецФункции // ЗаполнитьМассивНедоступныхРегламентныхОпераций
...
КонецФункции // БизнесПроцессЗавершен
...
КонецФункции // ОпределитьПериодичностьДокумента
