...
Функция ЗаполнятьОборотныеСубконто(СтруктураШапкиДокумента)
	...
	Возврат СтруктураШапкиДокумента.ЗаполнятьСтатьиИПодразделенияВПроводкахПриРаспределенииЗатрат;
	
КонецФункции // ЗаполнятьОборотныеСубконто
...
Функция ПолучитьНаборыДвижений(
	...
	Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
		...
		ДвиженияПартииТоваровПереданные  = ?(СтруктураИмен = Неопределено ИЛИ СтруктураИмен.Свойство("ПартииТоваровПереданные"), 			Движения.ПартииТоваровПереданные, Неопределено);
	...
	ИначеЕсли СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		ДвиженияПартииТоваровПереданные  = ?(СтруктураИмен = Неопределено ИЛИ СтруктураИмен.Свойство("ПартииТоваровПереданные"), 			Движения.ПартииТоваровПереданныеБухгалтерскийУчет, Неопределено);
	...
	СтруктураДвижений.Вставить("ДвиженияПартииТоваровПереданные", 	ДвиженияПартииТоваровПереданные);
...
Функция СформироватьТекстЗапросаЗаполнениеКорректировкиВстречногоВыпускаПродукции()
	...
	|			ВстречныйВыпускПродукции.Затрата
	|		ИЗ
	|			ВстречныйВыпускПродукции КАК ВстречныйВыпускПродукции
	|	)	
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|						
	...
	|		ВстречныйВыпускПродукции КАК РегистрВстречныйВыпуск
	|	ПО
	...
	|;
	|////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВстречныйВыпускПродукции
	|;
	|////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ СписанныеТовары
	|";
...
Функция СформироватьТекстЗапросаПоИспользованиюПолуфабрикатов()
	...
	|		ГДЕ
	|			ИсключаемаяНоменклатура.Период МЕЖДУ &НачДата И &КонДата
	|			И ИсключаемаяНоменклатура.Активность
	|			//ДляРеглУчета И ИсключаемаяНоменклатура.Организация = &Организация
	|
	|		) КАК ИсключаемаяНоменклатура
	|	ПО
...
// Из базы распределения исключается продукция, полностью списанная на собственные нужды (ОПЗ, АЗ, Брак в производстве),
...
Процедура ОпределитьНоменклатуруИсключаемуюИзБазыРаспределения(СтруктураШапкиДокумента) Экспорт
	...
	Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
		...
		|	И РаспределениеЗатратПоПеределам.Активность
...
Функция СформироватьТекстЗапросаПоЗатратамНаВыпускНаработка()
	...
	|		И ЗатратыНаВыпуск.ЗаказНЗП = НезавершенноеПроизводство.Заказ
	|		//ДляРеглУчета И ЗатратыНаВыпуск.СчетУчета = НезавершенноеПроизводство.СчетУчета
	|		//ДляБухУчета И ЗатратыНаВыпуск.НалоговоеНазначение = НезавершенноеПроизводство.НалоговоеНазначение 
	|		
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		РегистрНакопления.НезавершенноеПроизводство%СуффиксУчета%.Остатки(&КонГраница,
	|			//ДляРеглУчета Организация = &Организация
	...
	|		//ДляБухУчета И ЗатратыНаВыпуск.НалоговоеНазначение = НезавершенноеПроизводствоАналитикаНЗП.НалоговоеНазначение 
	|		
	|ГДЕ
...
Процедура СформироватьДвиженияПоРегиструЗатратыНаВыпускПродукцииНаработкаРасход(
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		НоваяСтрока.СуммаНУ = НалоговыйУчет.ОпределитьСтоимостьНУ(НоваяСтрока.НалоговоеНазначение, СуммаНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
...
Функция СформироватьЗапросПоРаспределениюЗатрат(
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете И СтруктураШапкиДокумента.РазрешитьКорректировкиИспользования Тогда
		...
		ИндексНалоговоеНазначение = "";
	Иначе
		УсловиеНалоговоеНазначение = " //ДляБухУчета И ((ВыпускПродукции.НалоговоеНазначение = РегЗатраты.НалоговоеНазначение) ИЛИ 
		...
		|";
		ИндексНалоговоеНазначение = " //ДляБухУчета  ,РегВыпуск.НалоговоеНазначение ";
	КонецЕсли; 
	...
	|
	|	СУММА(ЕСТЬNULL(ВыпускПродукции.КоличествоВыпуск, 0)) КАК КоличествоВыпуск,
	|
	...
	|
	|	ПО
	|		ВыпускПродукции.Подразделение = РегЗатраты.Подразделение
	|	 	И ВыпускПродукции.НоменклатурнаяГруппа = РегЗатраты.НоменклатурнаяГруппа
	|	 	И ВыпускПродукции.Продукция = РегЗатраты.Продукция
	|	 	И ВыпускПродукции.ХарактеристикаПродукции = РегЗатраты.ХарактеристикаПродукции
	|	 	И ВыпускПродукции.СерияПродукции = РегЗатраты.СерияПродукции
	|	 	И ВыпускПродукции.Спецификация = РегЗатраты.Спецификация
	|		И ВыпускПродукции.Заказ = РегЗатраты.Заказ
	|	 	//ДляРеглУчета И ВыпускПродукции.СчетУчетаНЗП = РегЗатраты.СчетУчета
	|"      +  УсловиеНалоговоеНазначение  + "
	|
	|ГДЕ
	...
	|	И (ВыпускПродукции.КоличествоВыпуск = 0 ИЛИ ВыпускПродукции.КоличествоВыпуск Есть NULL)
...
Функция СформироватьТекстЗапросаПоЗатратамНаВыпуск(СтруктураШапкиДокумента)
	...
	|   //ДляБухУчета ВЫБОР КОГДА ВыпускПродукции.КодОперации В (&КодыОперацийВыпускИспользоватьНалоговоеНазначениеПолучатель) ТОГДА 
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		РегистрВстречныйВыпуск КАК РегистрВстречныйВыпуск
	|	ПО
	|		ЗатратыНаВыпуск.Затрата     = РегистрВстречныйВыпуск.Затрата
	|		И ЗатратыНаВыпуск.Продукция = РегистрВстречныйВыпуск.Продукция
	|
	...
	|		ВыпускПродукции КАК ВыпускПродукции
	|	ПО
	|		ВыпускПродукции.Продукция                            = ЗатратыНаВыпуск.Продукция
	|		И ВыпускПродукции.Подразделение                      = ЗатратыНаВыпуск.Подразделение
	...
	|;
	|////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ РегистрВстречныйВыпуск
	|;
	|////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ИсключаемаяНоменклатура
	|
	|";
...
Процедура УстановитьПараметрыЗапросПоЗатратамНаВыпуск(
	...
	МассивКодовОперацийВыпускИспользоватьНалоговоеНазначениеПолучатель = Новый Массив;
	МассивКодовОперацийВыпускИспользоватьНалоговоеНазначениеПолучатель.Добавить(Перечисления.КодыОперацийВыпускПродукции.КорректировкаНЗПРасч);
	МассивКодовОперацийВыпускИспользоватьНалоговоеНазначениеПолучатель.Добавить(Перечисления.КодыОперацийВыпускПродукции.ОприходованиеНЗП);
	МассивКодовОперацийВыпускИспользоватьНалоговоеНазначениеПолучатель.Добавить(Перечисления.КодыОперацийВыпускПродукции.ВозвратМатериаловИзПроизводстваРасч);
	МассивКодовОперацийВыпускИспользоватьНалоговоеНазначениеПолучатель.Добавить(Перечисления.КодыОперацийВыпускПродукции.СписаниеНЗП);
	Запрос.УстановитьПараметр("КодыОперацийВыпускИспользоватьНалоговоеНазначениеПолучатель", МассивКодовОперацийВыпускИспользоватьНалоговоеНазначениеПолучатель);
...
Функция ПолучитьМассивЗатрат(
	...
	|	И РаспределениеЗатрат.Активность 
	...
	Пока ВыборкаПоЗатратам.Следующий() Цикл
		...
		// ... оптимизация 
			...
			Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете 
				...
				И НЕ ВыборкаПоЗатратам.НалоговоеНазначение = ВыборкаПоЗатратамНаВыпуск.НалоговоеНазначениеПоФакту
				И НЕ ВыборкаПоЗатратам.СтатусМатериальныхЗатрат = Перечисления.СтатусыМатериальныхЗатратНаПроизводство.ПринятыеВПереработку
				И НЕ ВыборкаПоЗатратамНаВыпуск.КодОперации = Перечисления.КодыОперацийЗатратыНаВыпускПродукции.КорректировкаНЗПРасч
				
				Тогда
				
				// Если в учетной политике не разрешены автоматические корректировки,
				// пропускаем записи, в которых не равны налоговые назначения затрат 
				// и налоговое назначение-получатель выпуска продукции
				// за исключением:
				// распределения затрат, принятых в переработку - пустое налоговое назначение в затратах
				// корректировки НЗП по расчитываемой стоимости - всегда разрешены корректировки
			...
			Если Не СтруктураШапкиДокумента.ОтражатьВУправленческомУчете 
			   ...
			   И (ВыборкаПоЗатратамНаВыпуск.НоменклатурнаяГруппа <> ВыборкаПоЗатратамНаВыпуск.НоменклатурнаяГруппаНЗП
					...
					ИЛИ (СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете 
						И ВыборкаПоЗатратамНаВыпуск.НалоговоеНазначение <> ВыборкаПоЗатратамНаВыпуск.НалоговоеНазначениеПоФакту 
						И ВыборкаПоЗатратам.СтатусМатериальныхЗатрат <> Перечисления.СтатусыМатериальныхЗатратНаПроизводство.ПринятыеВПереработку
						И ВыборкаПоЗатратамНаВыпуск.КодОперации <> Перечисления.КодыОперацийЗатратыНаВыпускПродукции.КорректировкаНЗПРасч
						)
					Тогда
			...
			НоваяСтрокаПриход.НалоговоеНазначение   = ВыборкаПоЗатратамНаВыпуск.НалоговоеНазначение;
			НоваяСтрокаПриход.СчетУчета         	= ВыборкаПоЗатратам.СчетУчета;
...
Процедура ПолучитьНачальныйИКонечныйНомерПередела(
	...
	|			И РаспределениеЗатратПолное.Активность 
...
Функция СформироватьТекстЗапросаПоВыпускуПродукцииИЗатратамНаВыпуск()
	...
	|	И РаспределениеПродукцииПоПеределам.Активность 
...
Функция СформироватьТекстЗапросаПоВыпускуПродукции()
	...
	|	//ДляУпрУчета				И Распределение.Активность 
...
Процедура СформироватьДвиженияПоРегиструВыпускПродукции(
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		НоваяСтрока.СтоимостьНУ  = НалоговыйУчет.ОпределитьСтоимостьНУ(НоваяСтрока.НалоговоеНазначениеПолучатель, СуммаНУКорректировки, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
...
Процедура СформироватьДвиженияПоРегиструПартииТоваровНаСкладах(
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		НоваяСтрока.СтоимостьНУ = НалоговыйУчет.ОпределитьСтоимостьНУ(НоваяСтрока.НалоговоеНазначение, СуммаНУКорректировки, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
		НоваяСтрока.НДСВходящий = НДСВходящийКорректировки;
		НоваяСтрока.НДСКредит   = НДСКредитКорректировки;
...
Процедура СформироватьДвиженияПоРегиструЗатраты(
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		НоваяСтрока.СуммаНУ = НалоговыйУчет.ОпределитьСтоимостьНУ(НоваяСтрока.НалоговоеНазначение, СуммаНУКорректировки, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
		НоваяСтрока.НДСВходящий = НДСВходящийКорректировки;
		НоваяСтрока.НДСКредит = НДСКредитКорректировки;
...
Функция ПолучитьСодержаниеПроводкиПоХарактеруЗатрат(ХарактерЗатрат)
	...
	ИначеЕсли ХарактерЗатрат = Перечисления.ХарактерЗатрат.ПрочиеОперационныеРасходы Тогда	
		СодержаниеПроводки = "Распределение прочих операционных расходов";
		
	ИначеЕсли ХарактерЗатрат = Перечисления.ХарактерЗатрат.РасходыНаСбыт Тогда	
		СодержаниеПроводки = "Распределение расходов на сбыт";
...
КонецФункции // ПолучитьСодержаниеПроводкиПоХарактеруЗатрат
...
Процедура СформироватьПроводкуПоВыпускуПродукции(
	...
	Если ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВБухгалтерскомУчете Тогда	
		...
		СуммаНУДт = НалоговыйУчет.ОпределитьСтоимостьНУ(НалоговоеНазначениеДт, СуммаНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
		
		НалоговоеНазначениеКт = СтрокаВыборки.НалоговоеНазначение;
		СуммаНУКт = НалоговыйУчет.ОпределитьСтоимостьНУ(НалоговоеНазначениеКт, СуммаНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
...
Процедура СформироватьВременнуюТаблицуСпособыРаспределенияРасширеннаяАналитика(
    ...
    |
	|	ВЫБОР 
	|		КОГДА СпособыРаспределения.Подразделение = ТаблицаПодходящихКлючей.Подразделение ТОГДА
	|			// В способе распределения указано конкретное подразделение
	|			10
	|		КОГДА (СпособыРаспределения.РаспределятьНаПодчиненныеПодразделения) ТОГДА
	|			// В способ распределения распространяется на подчиненные подразделения
	|			// Чем дальше родитель, тем меньше приоритет
	|			ВЫБОР 
	|				КОГДА СпособыРаспределения.Подразделение = ТаблицаПодходящихКлючей.Подразделение.Родитель ТОГДА
	|					9
	|				КОГДА СпособыРаспределения.Подразделение = ТаблицаПодходящихКлючей.Подразделение.Родитель.Родитель ТОГДА
	|					8
	|				КОГДА СпособыРаспределения.Подразделение = ТаблицаПодходящихКлючей.Подразделение.Родитель.Родитель.Родитель ТОГДА
	|				 	7
	|				ИНАЧЕ
	|					6	
	|			КОНЕЦ
	|	ИНАЧЕ
	|		// В способе распределения не указано подразделение, т.е. выбрано <Все подразделения>
	|		0
	|	КОНЕЦ КАК Приоритет,
	|
	|		
...
Процедура СформироватьВременнуюТаблицуСпособыРаспределения(
	...
	|	КОНЕЦ КАК ЭтоСпособРаспределенияРодителя,
	|	ВЫБОР 
	|		КОГДА Подразделения.Подразделение ЕСТЬ NULL ТОГДА
	|			// Таблицы не соединились. В способе распределения указано конкретное подразделение,
	|			// способ не распространяется на подчиненные подразделения
	|			10
	|		КОГДА (СпособыРаспределения.СпособРаспределенияЗатрат.РаспределятьНаПодчиненныеПодразделения) ТОГДА
	|			// Таблицы соединились. В способ распределения распространяется на подчиненные подразделения
	|			// Чем дальше родитель, тем меньше приоритет
	|			ВЫБОР 
	|				КОГДА СпособыРаспределения.Подразделение = Подразделения.Подразделение ТОГДА
	|					9
	|				КОГДА СпособыРаспределения.Подразделение = Подразделения.Родитель ТОГДА
	|					8
	|				КОГДА СпособыРаспределения.Подразделение = Подразделения.Родитель.Родитель ТОГДА
	|					7
	|				КОГДА СпособыРаспределения.Подразделение = Подразделения.Родитель.Родитель.Родитель ТОГДА
	|				 	6
	|				ИНАЧЕ
	|					5	
	|			КОНЕЦ
	|	ИНАЧЕ
	|		// Таблицы соединились по пустому подразделению, т.е. в способе распределения выбрано <Все подразделения>
	|		0
	|	КОНЕЦ КАК Приоритет,
	|		
	|	СпособыРаспределения.ХарактерРаспределенияЗатрат,
...
Функция СформироватьТекстЗапросаПоСпособамРаспределенияЗатратРасширеннаяАналитика()
	...
	|			СпособыРаспределения.АналитикаУчетаЗатрат,
	|			СпособыРаспределения.ЭтоСпособРаспределенияРодителя,
	|			СпособыРаспределения.Приоритет,
	|			МАКСИМУМ(СпособыРаспределения.Период) КАК Период
	|		ИЗ 
	|			(ВЫБРАТЬ
	|				СпособыРаспределенияПоПериодамИПриоритетам.АналитикаВидаУчета,
	|				СпособыРаспределенияПоПериодамИПриоритетам.АналитикаУчетаЗатрат,
	|				СпособыРаспределенияПоПериодамИПриоритетам.ЭтоСпособРаспределенияРодителя,
	|				СпособыРаспределенияПоПериодамИПриоритетам.Период,
	|				МАКСИМУМ(СпособыРаспределенияПоПериодамИПриоритетам.Приоритет) КАК Приоритет
	|			ИЗ 
	|				СпособыРаспределения%СуффиксУчета% КАК СпособыРаспределенияПоПериодамИПриоритетам
	|			СГРУППИРОВАТЬ ПО
	|				СпособыРаспределенияПоПериодамИПриоритетам.АналитикаВидаУчета,
	|				СпособыРаспределенияПоПериодамИПриоритетам.АналитикаУчетаЗатрат,
	|				СпособыРаспределенияПоПериодамИПриоритетам.ЭтоСпособРаспределенияРодителя,
	...
	|		И СпособыРаспределения.Период = СпособыРаспределенияПоПериодам.Период
	|		И СпособыРаспределения.Приоритет = СпособыРаспределенияПоПериодам.Приоритет
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ (
	|		ВЫБРАТЬ РАЗЛИЧНЫЕ
	|			СпособыРаспределения.АналитикаВидаУчета,
...
Функция СформироватьТекстЗапросаПоСпособамРаспределенияЗатрат()
	...
	|		ИЗ 
	|			(ВЫБРАТЬ
	|				//ДляРеглУчета СпособыРаспределенияПоПериодамИПриоритетам.СчетЗатрат,
	|				СпособыРаспределенияПоПериодамИПриоритетам.СтатьяЗатрат,
	|				СпособыРаспределенияПоПериодамИПриоритетам.Подразделение,
	|				СпособыРаспределенияПоПериодамИПриоритетам.ЭтоСпособРаспределенияРодителя,
	|				СпособыРаспределенияПоПериодамИПриоритетам.Период,
	|				МАКСИМУМ(СпособыРаспределенияПоПериодамИПриоритетам.Приоритет) КАК Приоритет
	|			ИЗ 
...
Функция ПроверитьИУстановитьОтборПостроителяЗапросаПоБазеРаспределения(
	...
	Если ОтборСоответствуетАналитике
	   ...
	   И ПостроительЗапроса.ДоступныеПоля.Найти("Продукция") <> Неопределено Тогда
		...
		Если ПостроительЗапроса.Отбор.Продукция.Значение <> СтрокаВыборки.Продукция
		 ИЛИ ПостроительЗапроса.Отбор.ХарактеристикаПродукции.Значение <> СтрокаВыборки.ХарактеристикаПродукции
		 ИЛИ ПостроительЗапроса.Отбор.СерияПродукции.Значение <> СтрокаВыборки.СерияПродукции Тогда
			ОтборСоответствуетАналитике = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОтборСоответствуетАналитике Тогда
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете 
			...
			Если ПостроительЗапроса.Отбор.НалоговоеНазначениеПоФакту.Значение <> СтрокаВыборки.НалоговоеНазначение Тогда
	  			ОтборСоответствуетАналитике = Ложь;
			КонецЕсли;
	...
	Если Не ОтборСоответствуетАналитике Тогда
		...
		Иначе
			Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете 
				И НЕ СтруктураШапкиДокумента.РазрешитьКорректировкиИспользования 
				И ПостроительЗапроса.ДоступныеПоля.Найти("НалоговоеНазначениеПоФакту") <> Неопределено Тогда
...
Процедура СформироватьДвиженияПоРегиструНезавершенноеПроизводствоРаспределениеЗатрат(
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		Если Не ЗначениеЗаполнено(НоваяСтрока.НалоговоеНазначение) 
			И (ЗначениеЗаполнено(СтрокаВыборки.Заказ) И ТипЗнч(СтрокаВыборки.Заказ) = Тип("ДокументСсылка.ЗаказПокупателя") И СтрокаВыборки.Заказ.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.Переработка) 
		...
		НоваяСтрока.СтоимостьНУ = НалоговыйУчет.ОпределитьСтоимостьНУ(НоваяСтрока.НалоговоеНазначение, СуммаНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
		НоваяСтрока.НДСВходящий = НДСВходящий;
		НоваяСтрока.НДСКредит   = НДСКредит;
...
Процедура СформироватьДвиженияПоРегиструЗатратыНаВыпускПродукцииРаспределениеЗатрат(
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		Если Не ЗначениеЗаполнено(НоваяСтрока.НалоговоеНазначениеПоФакту) 
			И (ЗначениеЗаполнено(СтрокаВыборкиБаза.Заказ) И ТипЗнч(СтрокаВыборкиБаза.Заказ) = Тип("ДокументСсылка.ЗаказПокупателя") И СтрокаВыборкиБаза.Заказ.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.Переработка) Тогда
			СтруктураПараметров = Новый Структура;
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		НоваяСтрока.СуммаНУ 	= НалоговыйУчет.ОпределитьСтоимостьНУ(НоваяСтрока.НалоговоеНазначениеПоФакту, СуммаНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
		НоваяСтрока.НДСВходящий = НДСВходящий;
		НоваяСтрока.НДСКредит 	= НДСКредит;
	КонецЕсли; 
...
Процедура СформироватьПроводкуПоРаспределениюЗатрат(
	...
	Если СтрокаВыборкиБаза.СчетУчета = СтрокаВыборки.СчетУчета
	   И (ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВБухгалтерскомУчете И СтрокаВыборкиБаза.НалоговоеНазначениеПоФакту = СтрокаВыборки.НалоговоеНазначение) 
	...
	Если ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВБухгалтерскомУчете Тогда
		...
		Если (СтрокаВыборкиБаза.СчетУчета <> СтрокаВыборки.СчетУчета) 
			И НЕ НалоговыйУчетПовтИсп.ПолучитьГруппуНалоговогоНазначения(СтрокаВыборкиБаза.НалоговоеНазначениеПоФакту) = Перечисления.ГруппыНалоговыхНазначений.НалоговыеНазначенияАктивовИВзаиморасчетовПоНДС
			И НалоговыйУчетПовтИсп.ПолучитьГруппуНалоговогоНазначения(СтрокаВыборки.НалоговоеНазначение) = Перечисления.ГруппыНалоговыхНазначений.НалоговыеНазначенияАктивовИВзаиморасчетовПоНДС 
		...
		НоваяПроводка.СуммаНУДт = НалоговыйУчет.ОпределитьСтоимостьНУ(НоваяПроводка.НалоговоеНазначениеДт, СуммаНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015); 
	...
	Если ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВБухгалтерскомУчете Тогда
		...
		НоваяПроводка.СуммаНУКт = НалоговыйУчет.ОпределитьСтоимостьНУ(НоваяПроводка.НалоговоеНазначениеКт, СуммаНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015); 
	КонецЕсли;
...
Функция СформироватьТекстЗапросаПоПрямымПроизводственнымЗатратам()
	...
	|		//ДляБухУчета И НезавершенноеПроизводство.НалоговоеНазначение = ИнвентаризацияНЗП.НалоговоеНазначение
	|
	|УПОРЯДОЧИТЬ ПО
	|	//ДляРеглУчета НезавершенноеПроизводство.Организация,
	|	//ДляРеглУчета НезавершенноеПроизводство.СчетУчета,
...
Процедура РаспределитьЗатратыВНезавершенноеПроизводство(
	...
	Если СуммаОстаток = 0 
		И ((ХарактерЗатрат = Перечисления.ХарактерЗатрат.ОбщепроизводственныеРасходы И СтруктураШапкиДокумента.ДиректКостингОбщепроизводственныеЗатратыНУ)
			...
			Иначе
				Если ВсегоБазаРаспределения <= ВыборкаПоЗатратам.БазаРаспределенияПриНормальнойМощности Тогда
					...
					Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете  И СуммаСписанияНаСебестоимость > 0 Тогда
						...
						Если СтруктураШапкиДокумента.ДиректКостингОбщепроизводственныеЗатратыНУ Тогда
							СуммаНУСписанияНаСебестоимость 		= 0;
							НДСВходящийСписанияНаСебестоимость 	= 0;
							НДСКредитСписанияНаСебестоимость 	= 0;
						Иначе
							СуммаНУСписанияНаСебестоимость 		= Окр(СуммаНУОстаток * КоэфСписанияНУ, 2);
							НДСВходящийСписанияНаСебестоимость 	= Окр(НДСВходящийОстаток * КоэфСписанияНУ, 2);
							НДСКредитСписанияНаСебестоимость 	= Окр(НДСКредитОстаток * КоэфСписанияНУ, 2);
						КонецЕсли;
	...
	Пока ВыборкаБаза.Следующий() Цикл
		...
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете И ХарактерЗатрат = Перечисления.ХарактерЗатрат.ОбщепроизводственныеРасходы И СтруктураШапкиДокумента.ДиректКостингОбщепроизводственныеЗатратыНУ Тогда
			СуммаНУ     = 0;
			НДСВходящий = 0;
			НДСКредит   = 0;
		Иначе
...
Процедура СформироватьПроводкуПоСписаниюЗатратНаСебестоимость(
	...
	Если ЗаполнятьОборотныеСубконто(СтруктураШапкиДокумента) ИЛИ Не УправлениеПроизводством.ПроверитьСубконтоНаОборотное(НоваяПроводка.СчетДт, "СтатьиЗатрат") Тогда
		БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетДт, НоваяПроводка.СубконтоДт, "СтатьиЗатрат", СтатьяЗатрат);
	КонецЕсли;	
	
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете И СтруктураШапкиДокумента.Дата >= глЗначениеПеременной("ДатаНачалаРаспределенияОПЗвНУ") Тогда
...
Процедура РаспределениеОбщепроизводственныхИАдминистративныхРасходов(	
	...
	Если Не РезультатЗапросаПоЗатратам.Пустой()
	   ...
	   И Не РезультатЗапросаБазаРаспределения.Пустой() Тогда	
		...
		РаспределитьЗатратыПоБазеРаспределения(
			...
			СтруктураДвижений.ДвиженияЗатратыНаВыпуск,
			СтруктураДвижений.Операция,
			?(СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете, СтруктураДвижений.ДвиженияКорректировкиЗатраты_НУ, Неопределено)
			);
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете И СтруктураДвижений.ДвиженияКорректировкиЗатраты_НУ.Модифицированность() Тогда
	  	СтруктураДвижений.ДвиженияКорректировкиЗатраты_НУ.Записать(Ложь);
	КонецЕсли;
...
//  ХарактерЗатрат - ПеречислениеСсылка.ХарактерЗатрат - Текущий характер затрат
//  Сумма - Число - Сумма затрат
//  СуммаНУ - Число - Сумма затрат по НУ
//  РегистрБухгалтерииНаборЗаписей - РегистрБухгалтерииНаборЗаписей - Набор записей регистра бухгалтерии.
...
Функция СформироватьЗапросПоПартиямТоваров(СтруктураШапкиДокумента, РасчетПоНУ = Ложь)
   ...
   ТекстЗапроса = "
	...
	|
	|	ВЫБОР
	|		КОГДА (НЕ СписанныеТовары.СкладПолучатель ЕСТЬ NULL)
	|			ТОГДА СписанныеТовары.СкладПолучатель
	|		КОГДА (НЕ ПартииТоваровНаСкладахПолучатель.Склад ЕСТЬ NULL)
	|			ТОГДА ПартииТоваровНаСкладахПолучатель.Склад
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК СкладПолучатель,
	|	ВЫБОР
	|	//ДляУпрУчета 	КОГДА (НЕ СписанныеТовары.Подразделение ЕСТЬ NULL)
	|	//ДляУпрУчета 		ТОГДА СписанныеТовары.Подразделение
	|	//ДляРеглУчета 	КОГДА (НЕ СписанныеТовары.ПодразделениеОрганизации ЕСТЬ NULL)
	|	//ДляРеглУчета 		ТОГДА СписанныеТовары.ПодразделениеОрганизации
	|	//ДляУпрУчета 	КОГДА (НЕ Себестоимость.Подразделение ЕСТЬ NULL )
	|	//ДляУпрУчета 		ТОГДА Себестоимость.Подразделение
	...
	|	КОНЕЦ КАК Подразделение,
	|	ВЫБОР
	|		КОГДА (НЕ СписанныеТовары.Продукция ЕСТЬ NULL)
	|			ТОГДА СписанныеТовары.Продукция
	|		КОГДА (НЕ БракВПроизводстве.Продукция ЕСТЬ NULL)
	|			ТОГДА БракВПроизводстве.Продукция
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Продукция,
	|	ВЫБОР
	|		КОГДА (НЕ СписанныеТовары.ХарактеристикаНоменклатурыНовая ЕСТЬ NULL)
	|			ТОГДА СписанныеТовары.ХарактеристикаНоменклатурыНовая
	...
	|	ВЫБОР
	|		КОГДА (НЕ СписанныеТовары.СерияНоменклатурыНовая ЕСТЬ NULL)
	|			ТОГДА СписанныеТовары.СерияНоменклатурыНовая
	...
	|	ВЫБОР
	|		КОГДА (НЕ СписанныеТовары.ОбъектСтроительства ЕСТЬ NULL)
	|			ТОГДА СписанныеТовары.ОбъектСтроительства
	|		КОГДА (НЕ СтроительствоОС.ОбъектСтроительства ЕСТЬ NULL)
	|			ТОГДА СтроительствоОС.ОбъектСтроительства
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ОбъектСтроительства,
	|	//ДляУпрУчета ВЫБОР
	|	//ДляУпрУчета 	КОГДА (НЕ СписанныеТовары.Проект ЕСТЬ NULL)
	|	//ДляУпрУчета 		ТОГДА СписанныеТовары.Проект
	|	//ДляУпрУчета 	КОГДА (НЕ Затраты.Проект ЕСТЬ NULL)
	|	//ДляУпрУчета 		ТОГДА Затраты.Проект
	|	//ДляУпрУчета 	ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	//ДляУпрУчета КОНЕЦ КАК Проект,
	|	//ДляБухУчета ЕСТЬNULL(СписанныеТовары.НалоговоеНазначениеПоФакту, ПартииТоваровНаСкладах.НалоговоеНазначениеПоФакту) КАК НалоговоеНазначениеПоФакту,
	|	//ДляБухУчета ЕСТЬNULL(СписанныеТовары.НалоговоеНазначениеНовое, ПартииТоваровНаСкладах.НалоговоеНазначениеПоФакту) КАК НалоговоеНазначениеПолучатель,
	|	ЕСТЬNULL(СписанныеТовары.ДоговорКонтрагента, ПартииТоваровПереданные.ДоговорКонтрагента) КАК ДоговорКонтрагента,
	|	ЕСТЬNULL(СписанныеТовары.ДокументПередачи, ПартииТоваровПереданные.ДокументПередачи) КАК ДокументПередачи,
	|	//ДляУпрУчета ПартииТоваровНаСкладах.СтатусПартии КАК СтатусПартии,
	|	//ДляУпрУчета ЕСТЬNULL(СписанныеТовары.СтатусПередачи, ПартииТоваровПереданные.СтатусПередачи) КАК СтатусПередачи,
	|	ВЫБОР
	|		КОГДА (НЕ СписанныеТовары.НоменклатурнаяГруппа ЕСТЬ NULL)
	|			ТОГДА СписанныеТовары.НоменклатурнаяГруппа
	...
	|	ВЫБОР
	|		КОГДА (НЕ СписанныеТовары.ЗаказСписания ЕСТЬ NULL)
	|			ТОГДА СписанныеТовары.ЗаказСписания
	|		//ДляУпрУчета КОГДА (НЕ Себестоимость.ЗаказПокупателя ЕСТЬ NULL)
	|		//ДляУпрУчета 	ТОГДА Себестоимость.ЗаказПокупателя
	...
	|
	|	СУММА(ВЫБОР
	|		КОГДА ПартииТоваровНаСкладах.КодОперации В (&КодыПереработка) И ПартииТоваровНаСкладах.ВидДвижения = &ВидДвиженияРасход
	|			ТОГДА ПартииТоваровНаСкладах.Стоимость
	|		ИНАЧЕ 0
	|	КОНЕЦ) КАК СуммаПереработка,
	|
	|	//ДляБухУчета СУММА(ВЫБОР
	|	//ДляБухУчета 	КОГДА ПартииТоваровНаСкладах.КодОперации В (&КодыПереработка) И ПартииТоваровНаСкладах.ВидДвижения = &ВидДвиженияРасход
	|	//ДляБухУчета 		ТОГДА ПартииТоваровНаСкладах.СтоимостьНУ
	|	//ДляБухУчета 	ИНАЧЕ 0
	|	//ДляБухУчета КОНЕЦ) КАК СуммаНУПереработка,
	|
	|	//ДляБухУчета СУММА(ВЫБОР
	|	//ДляБухУчета 	КОГДА ПартииТоваровНаСкладах.КодОперации В (&КодыПереработка) И ПартииТоваровНаСкладах.ВидДвижения = &ВидДвиженияРасход
	|	//ДляБухУчета 		ТОГДА ПартииТоваровНаСкладах.НДСВходящий
	|	//ДляБухУчета 	ИНАЧЕ 0
	|	//ДляБухУчета КОНЕЦ) КАК НДСВходящийПереработка,
	|
	|	//ДляБухУчета СУММА(ВЫБОР
	|	//ДляБухУчета 	КОГДА ПартииТоваровНаСкладах.КодОперации В (&КодыПереработка) И ПартииТоваровНаСкладах.ВидДвижения = &ВидДвиженияРасход
	|	//ДляБухУчета 		ТОГДА ПартииТоваровНаСкладах.НДСКредит
	|	//ДляБухУчета 	ИНАЧЕ 0
	|	//ДляБухУчета КОНЕЦ) КАК НДСКредитПереработка,
	|
	...
	|	    ПО (ВЫБОР КОГДА ПартииТоваровНаСкладах.ДокументДвижения <> НЕОПРЕДЕЛЕНО
	|		        ТОГДА ПартииТоваровНаСкладах.ДокументДвижения
	|		        ИНАЧЕ ПартииТоваровНаСкладах.Регистратор
	|	       КОНЕЦ) = СписанныеТовары.Регистратор
	|			И ПартииТоваровНаСкладах.НомерСтрокиСписанныхТоваров = СписанныеТовары.НомерСтроки
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.Затраты%СуффиксУчета% КАК Затраты
	|		ПО ПартииТоваровНаСкладах.Регистратор = Затраты.Регистратор
	|			И ПартииТоваровНаСкладах.НомерКорСтроки = Затраты.НомерСтроки
	|			И (ПартииТоваровНаСкладах.КодОперации В (&КодыЗатраты))   
	|			//ДляБухУчета И ПартииТоваровНаСкладах.КорСчет = Затраты.СчетУчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.НезавершенноеПроизводство%СуффиксУчета% КАК НЗП
	|		ПО ПартииТоваровНаСкладах.Регистратор = НЗП.Регистратор
	|			И ПартииТоваровНаСкладах.НомерКорСтроки = НЗП.НомерСтроки
	|			И (ПартииТоваровНаСкладах.КодОперации В (&КодыНЗП))   
	|			//ДляБухУчета И ПартииТоваровНаСкладах.КорСчет = НЗП.СчетУчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.БракВПроизводстве%СуффиксУчета% КАК БракВПроизводстве
	|		ПО ПартииТоваровНаСкладах.Регистратор = БракВПроизводстве.Регистратор
	|			И ПартииТоваровНаСкладах.НомерКорСтроки = БракВПроизводстве.НомерСтроки
	|			И (ПартииТоваровНаСкладах.КодОперации В (&КодыБрак))   
	|			//ДляБухУчета И ПартииТоваровНаСкладах.КорСчет = БракВПроизводстве.СчетУчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.СтроительствоОбъектовОсновныхСредств%СуффиксУчета% КАК СтроительствоОС
	|		ПО ПартииТоваровНаСкладах.Регистратор = СтроительствоОС.Регистратор
	|   			И ПартииТоваровНаСкладах.НомерКорСтроки = СтроительствоОС.НомерСтроки
	|			И (ПартииТоваровНаСкладах.КодОперации В (&КодыЗатратыОС))   
	|			//ДляБухУчета И ПартииТоваровНаСкладах.КорСчет = СтроительствоОС.СчетУчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ПартииТоваровПереданные%СуффиксУчета% КАК ПартииТоваровПереданные
	|		ПО ПартииТоваровНаСкладах.Регистратор = ПартииТоваровПереданные.Регистратор
	|			И ПартииТоваровНаСкладах.НомерКорСтроки = ПартииТоваровПереданные.НомерСтроки
	|			И (ПартииТоваровНаСкладах.КодОперации В (&КодыПереработка))   
	|			//ДляБухУчета И ПартииТоваровНаСкладах.КорСчет = ПартииТоваровПереданные.СчетУчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ПартииТоваровНаСкладах%СуффиксУчета% КАК ПартииТоваровНаСкладахПолучатель
	|		ПО ПартииТоваровНаСкладах.Регистратор = ПартииТоваровНаСкладахПолучатель.Регистратор
	|			И ПартииТоваровНаСкладах.НомерКорСтроки = ПартииТоваровНаСкладахПолучатель.НомерСтроки
	|			И (ПартииТоваровНаСкладах.КодОперации В (&КодыПеремещение))   
	|			//ДляБухУчета И ПартииТоваровНаСкладах.КорСчет = ПартииТоваровНаСкладахПолучатель.СчетУчета
	|		//ДляУпрУчета ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ПродажиСебестоимость КАК Себестоимость
	|			//ДляУпрУчета ПО ПартииТоваровНаСкладах.Регистратор = Себестоимость.Регистратор
	|			//ДляУпрУчета 	И ПартииТоваровНаСкладах.НомерКорСтроки = Себестоимость.НомерСтроки
	|
	|
	|ГДЕ
	|	ПартииТоваровНаСкладах.Период МЕЖДУ &НачДата И &КонДата
	...
	|	//ДляУпрУчета 	ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	//ДляУпрУчета КОНЕЦ,
	|	//ДляБухУчета ЕСТЬNULL(СписанныеТовары.НалоговоеНазначениеПоФакту, ПартииТоваровНаСкладах.НалоговоеНазначениеПоФакту),
	|	//ДляБухУчета ЕСТЬNULL(СписанныеТовары.НалоговоеНазначениеНовое, ПартииТоваровНаСкладах.НалоговоеНазначениеПоФакту),
	|	ЕСТЬNULL(СписанныеТовары.ДоговорКонтрагента, ПартииТоваровПереданные.ДоговорКонтрагента),
	|	ЕСТЬNULL(СписанныеТовары.ДокументПередачи, ПартииТоваровПереданные.ДокументПередачи),
	|	//ДляУпрУчета ПартииТоваровНаСкладах.СтатусПартии,
	|	//ДляУпрУчета ЕСТЬNULL(СписанныеТовары.СтатусПередачи, ПартииТоваровПереданные.СтатусПередачи),
	|	ВЫБОР
	|		КОГДА (НЕ СписанныеТовары.НоменклатурнаяГруппа ЕСТЬ NULL)
	|			ТОГДА СписанныеТовары.НоменклатурнаяГруппа
	|		КОГДА (НЕ НЗП.НоменклатурнаяГруппа ЕСТЬ NULL)
	...
	КодыПереработка = Новый Массив;
	КодыПереработка.Добавить(Перечисления.КодыОперацийПартииТоваров.ПередачаВПереработку);
	КодыПереработка.Добавить(Перечисления.КодыОперацийПартииТоваров.ПередачаНаКомиссию);
	Запрос.УстановитьПараметр("КодыПереработка", КодыПереработка);
...
Процедура РаспределениеВсехТранспортноЗаготовительныхРасходов(
	...
	|ЗатратыНаСтроительствоОбъектовОсновныхСредств, ЗатратыОбороты, Операция, СтроительствоОбъектовОС_БУ, КорректировкиЗатраты_НУ";
	...
	Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
		РаспределениеТранспортноЗаготовительныхРасходов(СтруктураШапкиДокумента, СтруктураДвижений, ТабТоваров);
	ИначеЕсли СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		РаспределениеТранспортноЗаготовительныхРасходов(СтруктураШапкиДокумента, СтруктураДвижений, ТабТоваров, "Материалы");
		РаспределениеТранспортноЗаготовительныхРасходов(СтруктураШапкиДокумента, СтруктураДвижений, ТабТоваров, "Товары");
	КонецЕсли; 
	...
	Если СтруктураДвижений.ДвиженияПартииТоваровПереданные.Модифицированность() Тогда
		СтруктураДвижений.ДвиженияПартииТоваровПереданные.Записать(Ложь);
	КонецЕсли;
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		Если СтруктураДвижений.ДвиженияКорректировкиЗатраты_НУ.Модифицированность() Тогда
		  	СтруктураДвижений.ДвиженияКорректировкиЗатраты_НУ.Записать(Ложь);
		КонецЕсли;
...
Функция СформироватьМассивСчетовВИерархии(МассивСчетов) Экспорт
...
Процедура РаспределениеТранспортноЗаготовительныхРасходов(
	...
	МассивКоэфСуммаПереработка= Новый Массив;	
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		МассивКоэфСуммаНУПереработка= Новый Массив;	
		...
		МассивКоэфНДСКредитПереработка= Новый Массив;	
		...
		МассивКоэфНДСВходящийПереработка= Новый Массив;	
	...
	Пока ВыборкаПоЗатратам.Следующий() Цикл
		...
		МассивКоэфСуммаПереработка.Очистить();
		...
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
			...
			МассивКоэфСуммаНУПереработка.Очистить();
			...
			МассивКоэфНДСКредитПереработка.Очистить();
			...
			МассивКоэфНДСВходящийПереработка.Очистить();
		...
		Пока ВыборкаПартии.Следующий() Цикл
			...
			МассивКоэфСуммаПереработка.Добавить(ВыборкаПартии.СуммаПереработка);
			...
			Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
				...
				МассивКоэфСуммаНУПереработка.Добавить(ВыборкаПартии.СуммаНУПереработка);
				...
				МассивКоэфНДСКредитПереработка.Добавить(ВыборкаПартии.НДСКредитПереработка);
				...
				МассивКоэфНДСВходящийПереработка.Добавить(ВыборкаПартии.НДСВходящийПереработка);
			...
			ОбщаяСуммаПереработка= ОбщаяСуммаПереработка + ВыборкаПартии.СуммаПереработка;
			...
			Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
				...
				ОбщаяСуммаНУПереработка= ОбщаяСуммаНУПереработка + ВыборкаПартии.СуммаНУПереработка;
				...
				ОбщаяНДСКредитПереработка= ОбщаяНДСКредитПереработка + ВыборкаПартии.НДСКредитПереработка;
				...
				ОбщаяНДСВходящийПереработка= ОбщаяНДСВходящийПереработка + ВыборкаПартии.НДСВходящийПереработка;
		...
		Пока ВыборкаНачОст.Следующий() Цикл 
			...
			СуммаНачальныйОстаток  = СуммаНачальныйОстаток + ?(ВыборкаНачОст.СуммаНачальныйОстаток = NULL, 0, ВыборкаНачОст.СуммаНачальныйОстаток);
			
			Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
				СуммаНУНачальныйОстаток     = СуммаНУНачальныйОстаток + ?(ВыборкаНачОст.СуммаНУНачальныйОстаток = NULL, 0, ВыборкаНачОст.СуммаНУНачальныйОстаток);
				НДСКредитНачальныйОстаток   = НДСКредитНачальныйОстаток + ?(ВыборкаНачОст.НДСВходящийНачальныйОстаток = NULL, 0, ВыборкаНачОст.НДСВходящийНачальныйОстаток);
				НДСВходящийНачальныйОстаток = НДСВходящийНачальныйОстаток + ?(ВыборкаНачОст.НДСКредитНачальныйОстаток = NULL, 0, ВыборкаНачОст.НДСКредитНачальныйОстаток);
			КонецЕсли; 
		КонецЦикла;	
		...
		СписаноВсегоСумма = ОбщаяСуммаПеремещение + ОбщаяСуммаНЗП + ОбщаяСуммаЗатраты + ОбщаяСуммаБрак + ОбщаяСуммаЗатратыОС + ОбщаяСуммаПереработка + ОбщаяСуммаРеализация;
		...
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
			...
			СписаноВсегоСуммаНУ = ОбщаяСуммаНУПеремещение + ОбщаяСуммаНУНЗП + ОбщаяСуммаНУЗатраты + ОбщаяСуммаНУБрак + ОбщаяСуммаНУЗатратыОС + ОбщаяСуммаНУПереработка + ОбщаяСуммаНУРеализация;
			
			СписаноВсегоНДСКредит = ОбщаяНДСКредитПеремещение + ОбщаяНДСКредитНЗП + ОбщаяНДСКредитЗатраты + ОбщаяНДСКредитБрак + ОбщаяНДСКредитЗатратыОС + ОбщаяНДСКредитПереработка + ОбщаяНДСКредитРеализация;
			
			СписаноВсегоНДСВходящий = ОбщаяНДСВходящийПеремещение + ОбщаяНДСВходящийНЗП + ОбщаяНДСВходящийЗатраты + ОбщаяНДСВходящийБрак + ОбщаяНДСВходящийЗатратыОС + ОбщаяНДСВходящийПереработка + ОбщаяНДСВходящийРеализация;
		...
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
			СуммаНУТЗР     = Окр(ВыборкаПоЗатратам.СуммаНУОстаток     * КоэфТЗРСумма, 2);
			НДСКредитТЗР   = Окр(ВыборкаПоЗатратам.НДСКредитОстаток   * КоэфТЗРСумма, 2);
			НДСВходящийТЗР = Окр(ВыборкаПоЗатратам.НДСВходящийОстаток * КоэфТЗРСумма, 2);
		Иначе	
			СуммаВРТЗР = 0;
			НДСКредитТЗР = 0;
			НДСВходящийТЗР = 0;
		...
		СуммаПереработка = ?(СписаноВсегоСумма=0, 0, Окр(СуммаТЗР * ОбщаяСуммаПереработка / СписаноВсегоСумма, 2));
		СуммаРеализация  = СуммаТЗР - СуммаПеремещение - СуммаНЗП - СуммаЗатраты - СуммаБрак - СуммаЗатратыОС - СуммаПереработка;
		
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
			
			
			СуммаНУПеремещение = ?(СписаноВсегоСумма=0, 0, Окр(СуммаНУТЗР * ОбщаяСуммаПеремещение / СписаноВсегоСумма, 2));
			СуммаНУНЗП         = ?(СписаноВсегоСумма=0, 0, Окр(СуммаНУТЗР * ОбщаяСуммаНЗП / СписаноВсегоСумма, 2));
			СуммаНУЗатраты     = ?(СписаноВсегоСумма=0, 0, Окр(СуммаНУТЗР * ОбщаяСуммаЗатраты / СписаноВсегоСумма, 2));
			СуммаНУБрак        = ?(СписаноВсегоСумма=0, 0, Окр(СуммаНУТЗР * ОбщаяСуммаБрак / СписаноВсегоСумма, 2));
			СуммаНУЗатратыОС   = ?(СписаноВсегоСумма=0, 0, Окр(СуммаНУТЗР * ОбщаяСуммаЗатратыОС / СписаноВсегоСумма, 2));
			СуммаНУПереработка = ?(СписаноВсегоСумма=0, 0, Окр(СуммаНУТЗР * ОбщаяСуммаПереработка / СписаноВсегоСумма, 2));
			СуммаНУРеализация  = СуммаНУТЗР - СуммаНУПеремещение - СуммаНУНЗП - СуммаНУЗатраты - СуммаНУБрак - СуммаНУЗатратыОС - СуммаНУПереработка;
			
			НДСКредитПеремещение = ?(СписаноВсегоСумма=0, 0, Окр(НДСКредитТЗР * ОбщаяСуммаПеремещение / СписаноВсегоСумма, 2));
			НДСКредитНЗП         = ?(СписаноВсегоСумма=0, 0, Окр(НДСКредитТЗР * ОбщаяСуммаНЗП / СписаноВсегоСумма, 2));
			НДСКредитЗатраты     = ?(СписаноВсегоСумма=0, 0, Окр(НДСКредитТЗР * ОбщаяСуммаЗатраты / СписаноВсегоСумма, 2));
			НДСКредитБрак        = ?(СписаноВсегоСумма=0, 0, Окр(НДСКредитТЗР * ОбщаяСуммаБрак / СписаноВсегоСумма, 2));
			НДСКредитЗатратыОС   = ?(СписаноВсегоСумма=0, 0, Окр(НДСКредитТЗР * ОбщаяСуммаЗатратыОС / СписаноВсегоСумма, 2));
			НДСКредитПереработка = ?(СписаноВсегоСумма=0, 0, Окр(НДСКредитТЗР * ОбщаяСуммаПереработка / СписаноВсегоСумма, 2));
			НДСКредитРеализация  = НДСКредитТЗР - НДСКредитПеремещение - НДСКредитНЗП - НДСКредитЗатраты - НДСКредитБрак - НДСКредитЗатратыОС - НДСКредитПереработка;
			
			НДСВходящийПеремещение = ?(СписаноВсегоСумма=0, 0, Окр(НДСВходящийТЗР * ОбщаяСуммаПеремещение / СписаноВсегоСумма, 2));
			НДСВходящийНЗП         = ?(СписаноВсегоСумма=0, 0, Окр(НДСВходящийТЗР * ОбщаяСуммаНЗП / СписаноВсегоСумма, 2));
			НДСВходящийЗатраты     = ?(СписаноВсегоСумма=0, 0, Окр(НДСВходящийТЗР * ОбщаяСуммаЗатраты / СписаноВсегоСумма, 2));
			НДСВходящийБрак        = ?(СписаноВсегоСумма=0, 0, Окр(НДСВходящийТЗР * ОбщаяСуммаБрак / СписаноВсегоСумма, 2));
			НДСВходящийЗатратыОС   = ?(СписаноВсегоСумма=0, 0, Окр(НДСВходящийТЗР * ОбщаяСуммаЗатратыОС / СписаноВсегоСумма, 2));
			НДСВходящийПереработка = ?(СписаноВсегоСумма=0, 0, Окр(НДСВходящийТЗР * ОбщаяСуммаПереработка / СписаноВсегоСумма, 2));
			НДСВходящийРеализация  = НДСВходящийТЗР - НДСВходящийПеремещение - НДСВходящийНЗП - НДСВходящийЗатраты - НДСВходящийБрак - НДСВходящийЗатратыОС - НДСВходящийПереработка;
		КонецЕсли; 
		...
		МассивСуммаПереработка = ОбщегоНазначения.РаспределитьПропорционально(СуммаПереработка, МассивКоэфСуммаПереработка, 2, Ложь);
		МассивСуммаРеализация  = ОбщегоНазначения.РаспределитьПропорционально(СуммаРеализация, МассивКоэфСуммаРеализация, 2, Ложь);
		
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
			
			
			МассивСуммаНУПеремещение = ОбщегоНазначения.РаспределитьПропорционально(СуммаНУПеремещение, МассивКоэфСуммаПеремещение, 2, Ложь);
			МассивСуммаНУНЗП         = ОбщегоНазначения.РаспределитьПропорционально(СуммаНУНЗП, МассивКоэфСуммаНЗП, 2, Ложь);
			МассивСуммаНУЗатраты     = ОбщегоНазначения.РаспределитьПропорционально(СуммаНУЗатраты, МассивКоэфСуммаЗатраты, 2, Ложь);
			МассивСуммаНУБрак        = ОбщегоНазначения.РаспределитьПропорционально(СуммаНУБрак, МассивКоэфСуммаБрак, 2, Ложь);
			МассивСуммаНУЗатратыОС   = ОбщегоНазначения.РаспределитьПропорционально(СуммаНУЗатратыОС, МассивКоэфСуммаЗатратыОС, 2, Ложь);
			МассивСуммаНУПереработка = ОбщегоНазначения.РаспределитьПропорционально(СуммаНУПереработка, МассивКоэфСуммаПереработка, 2, Ложь);
			МассивСуммаНУРеализация  = ОбщегоНазначения.РаспределитьПропорционально(СуммаНУРеализация, МассивКоэфСуммаРеализация, 2, Ложь);
				
			МассивНДСКредитПеремещение = ОбщегоНазначения.РаспределитьПропорционально(НДСКредитПеремещение, МассивКоэфСуммаПеремещение, 2, Ложь);
			МассивНДСКредитНЗП         = ОбщегоНазначения.РаспределитьПропорционально(НДСКредитНЗП, МассивКоэфСуммаНЗП, 2, Ложь);
			МассивНДСКредитЗатраты     = ОбщегоНазначения.РаспределитьПропорционально(НДСКредитЗатраты, МассивКоэфСуммаЗатраты, 2, Ложь);
			МассивНДСКредитБрак        = ОбщегоНазначения.РаспределитьПропорционально(НДСКредитБрак, МассивКоэфСуммаБрак, 2, Ложь);
			МассивНДСКредитЗатратыОС   = ОбщегоНазначения.РаспределитьПропорционально(НДСКредитЗатратыОС, МассивКоэфСуммаЗатратыОС, 2, Ложь);
			МассивНДСКредитПереработка = ОбщегоНазначения.РаспределитьПропорционально(НДСКредитПереработка, МассивКоэфСуммаПереработка, 2, Ложь);
			МассивНДСКредитРеализация  = ОбщегоНазначения.РаспределитьПропорционально(НДСКредитРеализация, МассивКоэфСуммаРеализация, 2, Ложь);
			
			МассивНДСВходящийПеремещение = ОбщегоНазначения.РаспределитьПропорционально(НДСВходящийПеремещение, МассивКоэфСуммаПеремещение, 2, Ложь);
			МассивНДСВходящийНЗП         = ОбщегоНазначения.РаспределитьПропорционально(НДСВходящийНЗП, МассивКоэфСуммаНЗП, 2, Ложь);
			МассивНДСВходящийЗатраты     = ОбщегоНазначения.РаспределитьПропорционально(НДСВходящийЗатраты, МассивКоэфСуммаЗатраты, 2, Ложь);
			МассивНДСВходящийБрак        = ОбщегоНазначения.РаспределитьПропорционально(НДСВходящийБрак, МассивКоэфСуммаБрак, 2, Ложь);
			МассивНДСВходящийЗатратыОС   = ОбщегоНазначения.РаспределитьПропорционально(НДСВходящийЗатратыОС, МассивКоэфСуммаЗатратыОС, 2, Ложь);
			МассивНДСВходящийПереработка = ОбщегоНазначения.РаспределитьПропорционально(НДСВходящийПереработка, МассивКоэфСуммаПереработка, 2, Ложь);
			МассивНДСВходящийРеализация  = ОбщегоНазначения.РаспределитьПропорционально(НДСВходящийРеализация, МассивКоэфСуммаРеализация, 2, Ложь);
		...
		Пока ВыборкаПартии.Следующий() Цикл
			...
			ДосписаниеСуммаПереработка = ?(МассивСуммаПереработка = Неопределено, 0, МассивСуммаПереработка[Индекс]);
			...
			Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
				...
				ДосписаниеСуммаНУПереработка = ?(МассивСуммаНУПереработка = Неопределено, 0, МассивСуммаНУПереработка[Индекс]);
				...
				ДосписаниеНДСКредитПереработка = ?(МассивНДСКредитПереработка = Неопределено, 0, МассивНДСКредитПереработка[Индекс]);
				...
				ДосписаниеНДСВходящийПереработка = ?(МассивНДСВходящийПереработка = Неопределено, 0, МассивНДСВходящийПереработка[Индекс]);
			...
			Иначе
				...
				ДосписаниеСуммаНУПереработка = 0;
				...
				ДосписаниеНДСКредитПереработка = 0;
				...
				ДосписаниеНДСВходящийПереработка = 0;
			...
			ИначеЕсли ДосписаниеСуммаПереработка <> 0 ИЛИ ДосписаниеСуммаНУПереработка <> 0 ИЛИ ДосписаниеНДСКредитПереработка <> 0 ИЛИ ДосписаниеНДСВходящийПереработка <> 0 Тогда
				НаправлениеИспользования = "Переработка";
				Сумма       = ДосписаниеСуммаПереработка;
				СуммаНУ     = ДосписаниеСуммаНУПереработка;
				НДСКредит   = ДосписаниеНДСКредитПереработка;
				НДСВходящий = ДосписаниеНДСВходящийПереработка;
...
Функция КорректировкиТЗРНУ(
	...
	НалоговоеНазначениеЗатрат      = СтрокаВыборкиЗатраты.НалоговоеНазначение;
	НалоговоеНазначениеПолучатель  = СтрокаПолучатель.НалоговоеНазначение;
	...
	// Формирование движений по регистру КорректировкиЗатратыНалоговыйУчет
	СтруктураПолучатель = Новый Структура("НалоговоеНазначениеПоФакту", НалоговоеНазначениеПолучатель);
	СформироватьДвиженияПоРегиструКорректировкиНалоговыйУчет(
		СтруктураШапкиДокумента,
		СтрокаВыборкиЗатраты,
		СтруктураПолучатель,
	...
	Если СтруктураКорректировкаНДСКредит.ВосстановлениеНДС Тогда
		...
		НоваяПроводка.Активность = Истина;
		НоваяПроводка.Период = СтруктураШапкиДокумента.Период;
		НоваяПроводка.Содержание  = "Корректировка налогового кредита по использованию ТЗР";
		НоваяПроводка.Сумма       = СуммаКорректировкиСтоимость;
		
		НоваяПроводка.СчетДт      = СтрокаПолучатель.СчетУчета;
		
		БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетДт, НоваяПроводка.СубконтоДт, "Подразделения"       , СтрокаПолучатель.Подразделение);
		БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетДт, НоваяПроводка.СубконтоДт, "НоменклатурныеГруппы", СтрокаПолучатель.НоменклатурнаяГруппа);
		БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетДт, НоваяПроводка.СубконтоДт, "СтатьиЗатрат", СтрокаПолучатель.СтатьяЗатрат);
		
		НоваяПроводка.СуммаНУДт              = СуммаКорректировкиСтоимостьНУ;
		НоваяПроводка.НалоговоеНазначениеДт  = НалоговоеНазначениеПолучатель;
	...
	Возврат СтруктураКорректировкаНДСКредит;
	
КонецФункции // КорректировкиТЗРНУ
...
Процедура ДосписатьТЗРПоНаправлениюИспользования(
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		Иначе
			НоваяПроводка.НалоговоеНазначениеДт = СтрокаВыборкиПартии.НалоговоеНазначениеПоФакту;
		КонецЕсли;	
		НоваяПроводка.СуммаНУДт = НалоговыйУчет.ОпределитьСтоимостьНУ(НоваяПроводка.НалоговоеНазначениеДт, СуммаНУ, СтруктураШапкиДокумента.ЕстьНалогНаПрибыльДо2015);
	...
	Если НаправлениеИспользования = "НЗП" Тогда
		...
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда 
			СтруктураКорректировкаНДСКредит = КорректировкиТЗРНУ(
				СтруктураШапкиДокумента, 
				СтрокаВыборкиЗатраты,
				НоваяСтрока,
				Сумма,
				НДСВходящий,
				НДСКредит,
				СтруктураДвижений.ДвиженияКорректировкиЗатраты_НУ,
				СтруктураДвижений.Операция
				);
				
			Если СтруктураКорректировкаНДСКредит <> Неопределено Тогда
				
				НоваяСтрокаКорректировка = НаборЗаписейНезавершенноеПроизводство.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрокаКорректировка, НоваяСтрока);
				
				НоваяСтрокаКорректировка.Стоимость   = СтруктураКорректировкаНДСКредит.СуммаКорректировкиСтоимость;
				НоваяСтрокаКорректировка.СтоимостьНУ = СтруктураКорректировкаНДСКредит.СуммаКорректировкиСтоимостьНУ;
				НоваяСтрокаКорректировка.НДСВходящий = 0;
				НоваяСтрокаКорректировка.НДСКредит   = СтруктураКорректировкаНДСКредит.КорректировкаНДСКредит;
	...
	ИначеЕсли НаправлениеИспользования = "ЗатратыОС" Тогда
		...
		ИначеЕсли СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
			...
			СтруктураКорректировкаНДСКредит = НалоговыйУчет.ПолучитьСуммуКорректировкиНалоговогоКредита(
				СтрокаВыборкиЗатраты.НалоговоеНазначение,
				СтрокаВыборкиПартии.НалоговоеНазначениеПолучатель,
				НДСВходящий,
				НДСКредит,
				Сумма, // СтоимостьБУ
				СтруктураШапкиДокумента.Дата
			...
			Если СтруктураКорректировкаНДСКредит.ЕстьКорректировки Тогда
				...
				// Формирование движений по регистру КорректировкиЗатратыНалоговыйУчет
				СтруктураПолучатель = Новый Структура("НалоговоеНазначениеПоФакту", СтрокаВыборкиПартии.НалоговоеНазначениеПолучатель);
				СформироватьДвиженияПоРегиструКорректировкиНалоговыйУчет(
					СтруктураШапкиДокумента,
					СтрокаВыборкиЗатраты,
					СтруктураПолучатель,
				...
				СформироватьДвиженияПоРегиструСтроительствоОбъектовОсновныхСредств(
					СтруктураШапкиДокумента,
					СтрокаВыборки,
					СтруктураКорректировкаНДСКредит.СуммаКорректировкиСтоимость,
					СтруктураКорректировкаНДСКредит.СуммаКорректировкиСтоимостьНУ,
					0, // НДСВходящий, 
					СтруктураКорректировкаНДСКредит.КорректировкаНДСКредит, 
					СтруктураДвижений.ДвиженияСтроительствоОбъектовОС_БУ
				);
				...
				Если СтруктураКорректировкаНДСКредит.ВосстановлениеНДС Тогда
					...
					РегистрБухгалтерииНаборЗаписей = СтруктураДвижений.Операция;
					// проводка вида Дт 1522 Кт 6443 Сторно
					НоваяПроводка = РегистрБухгалтерииНаборЗаписей.Добавить();
					НоваяПроводка.Организация = СтруктураШапкиДокумента.Организация;
					НоваяПроводка.Активность = Истина;
					НоваяПроводка.Период = СтруктураШапкиДокумента.Период;
					...
					БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетДт, НоваяПроводка.СубконтоДт, "ОбъектыСтроительства" , СтрокаВыборкиПартии.ОбъектСтроительства);
					БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетДт, НоваяПроводка.СубконтоДт, "СтатьиЗатрат", СтрокаВыборкиПартии.СтатьяЗатрат);
	...
	ИначеЕсли НаправлениеИспользования = "Брак" Тогда
		...
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда 
			...
			Если СтруктураКорректировкаНДСКредит <> Неопределено Тогда
				...
				НоваяСтрокаКорректировка = НаборЗаписейБракВПроизводстве.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрокаКорректировка, НоваяСтрока);
				
				НоваяСтрокаКорректировка.Сумма       = СтруктураКорректировкаНДСКредит.СуммаКорректировкиСтоимость;
				НоваяСтрокаКорректировка.СуммаНУ     = СтруктураКорректировкаНДСКредит.СуммаКорректировкиСтоимостьНУ;
				НоваяСтрокаКорректировка.НДСВходящий = 0;
				НоваяСтрокаКорректировка.НДСКредит   = СтруктураКорректировкаНДСКредит.КорректировкаНДСКредит;
	...
	ИначеЕсли НаправлениеИспользования = "Переработка" Тогда
		
		// досписание ТЗР при передаче в переработку
		
		НаборЗаписейПартииТоваровПереданные = СтруктураДвижений.ДвиженияПартииТоваровПереданные;
		
		НоваяСтрока = НаборЗаписейПартииТоваровПереданные.Добавить();
		НоваяСтрока.Активность = Истина;
		НоваяСтрока.ВидДвижения = ВидДвиженияНакопления.Приход;
		НоваяСтрока.Период = СтруктураШапкиДокумента.Период;
		НоваяСтрока.Организация = СтрокаВыборкиПартии.Организация;
		...
		НоваяСтрока.ДоговорКонтрагента         = СтрокаВыборкиПартии.ДоговорКонтрагента;
		НоваяСтрока.ДокументПередачи           = СтрокаВыборкиПартии.ДокументПередачи;
		Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
			НоваяСтрока.СтатусПередачи  = СтрокаВыборкиПартии.СтатусПередачи;
			НоваяСтрока.СтатусПартии    = СтрокаВыборкиПартии.СтатусПартии;
		КонецЕсли; 
		...
		НоваяСтрока.КодОперации    = Перечисления.КодыОперацийПартииТоваров.ПоступлениеДопРасходов; 
		...
		НоваяСтрокаТабТоваров = ТабТоваров.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрокаТабТоваров, НоваяСтрока);
		
		// Если в наборе записей 1000 строк, запишем набор.
		Если НаборЗаписейПартииТоваровПереданные.Количество() = 1000 Тогда
			НаборЗаписейПартииТоваровПереданные.Записать(Ложь);
		КонецЕсли;
		...
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда 
			...
			Если СтруктураКорректировкаНДСКредит.ЕстьКорректировки Тогда
				...
				НоваяСтрокаКорректировка = НаборЗаписейПартииТоваровПереданные.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрокаКорректировка, НоваяСтрока);
				...
				Если СтруктураКорректировкаНДСКредит.ВосстановлениеНДС Тогда
					...
					РегистрБухгалтерииНаборЗаписей = СтруктураДвижений.Операция;
					// проводка вида Дт 206 Кт 6443 Сторно
					НоваяПроводка = РегистрБухгалтерииНаборЗаписей.Добавить();
					НоваяПроводка.Организация = СтруктураШапкиДокумента.Организация;
					НоваяПроводка.Активность = Истина;
					НоваяПроводка.Период = СтруктураШапкиДокумента.Период;
					...
					БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетДт, НоваяПроводка.СубконтоДт, "Номенклатура", СтрокаВыборкиПартии.Номенклатура);
					БухгалтерскийУчет.УстановитьСубконто(НоваяПроводка.СчетДт, НоваяПроводка.СубконтоДт, "Контрагенты",  СтрокаВыборкиПартии.ДоговорКонтрагента.Владелец);
	...
	ИначеЕсли НаправлениеИспользования = "Себестоимость" Тогда
		...
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда 
			...
			СтруктураКорректировкаНДСКредит = НалоговыйУчет.ПолучитьСуммуКорректировкиНалоговогоКредита(
				СтрокаВыборкиЗатраты.НалоговоеНазначение,
				СтрокаВыборкиПартии.НалоговоеНазначение,
				НДСВходящий,
				НДСКредит,
				Сумма, // СтоимостьБУ
				СтруктураШапкиДокумента.Дата
			...
			Если СтруктураКорректировкаНДСКредит.ЕстьКорректировки Тогда
				...
				// Формирование движений по регистру КорректировкиЗатратыНалоговыйУчет
				СтруктураПолучатель = Новый Структура("НалоговоеНазначениеПоФакту", СтрокаВыборкиПартии.НалоговоеНазначение);
				СформироватьДвиженияПоРегиструКорректировкиНалоговыйУчет(
					СтруктураШапкиДокумента,
					СтрокаВыборкиЗатраты,
					СтруктураПолучатель,
				...
				Если СтруктураКорректировкаНДСКредит.ВосстановлениеНДС Тогда
					...
					РегистрБухгалтерииНаборЗаписей = СтруктураДвижений.Операция;
					// проводка вида Дт 901 Кт 6443 Сторно
					НоваяПроводка = РегистрБухгалтерииНаборЗаписей.Добавить();
					НоваяПроводка.Организация = СтруктураШапкиДокумента.Организация;
					НоваяПроводка.Активность = Истина;
					НоваяПроводка.Период = СтруктураШапкиДокумента.Период;
...
Процедура СформироватьДвиженияПоРегиструЗатратыПриходИзТЗР(
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда 
		...
		Если СтруктураКорректировкаНДСКредит <> Неопределено Тогда
			...
			НоваяСтрокаКорректировка = НаборЗаписейЗатраты.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаКорректировка, НоваяСтрока);
...
Функция ПолучитьМассивВыполняемыхДействийТрадиционныйРежим(ПериодРегистрации, Организация, ОтражатьВУправленческомУчете, ОтражатьВБухгалтерскомУчете, СообщатьОбОшибке)
	...
	Если НачалоМесяца(ПериодРегистрации) < глЗначениеПеременной("ДатаНачалаРаспределенияОПЗвНУ") Тогда
...
КонецФункции // ПолучитьМассивВыполняемыхДействий
