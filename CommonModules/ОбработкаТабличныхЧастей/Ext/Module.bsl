...
Процедура ЗаполнитьСтавкуНДСТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ВидДокумента = "") Экспорт
	...
	ОписаниеТиповТабличнаяЧастьОбработки = Новый ОписаниеТипов("ОбработкаТабличнаяЧастьСтрока.ОбработкаТабличнойЧастиТовары.Товары, ОбработкаТабличнаяЧастьСтрока.ФормированиеНалоговыхНакладных.Товары, ОбработкаТабличнаяЧастьСтрока.ФормированиеНалоговыхНакладных.Услуги");
...
Процедура БСУРассчитатьНоминальныеРеквизитыТЧ(СтрокаТабличнойЧасти, ДокументОбъект, ИмяРеквизитаСумма = "Сумма", РассчитатьЦенуПоСумме  = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура) Тогда
		...
		Если НЕ СтрокаТабличнойЧасти.Номенклатура.ВидНоменклатуры.УчитываетсяПоНоминальнойСтоимости Тогда
			...
			СтрокаТабличнойЧасти.СуммаНоминальная = СтрокаТабличнойЧасти[ИмяРеквизитаСумма] - СуммаВключенногоНДС;
			Если РассчитатьЦенуПоСумме Тогда
				СтрокаТабличнойЧасти.ЦенаНоминальная  = ?(СтрокаТабличнойЧасти.Количество <> 0, СтрокаТабличнойЧасти.СуммаНоминальная / СтрокаТабличнойЧасти.Количество, 0);
			Иначе
				СтрокаТабличнойЧасти.ЦенаНоминальная  = СтрокаТабличнойЧасти.Цена - ?(СтрокаТабличнойЧасти.Количество <> 0, СуммаВключенногоНДС/СтрокаТабличнойЧасти.Количество, СуммаВключенногоНДС);
			КонецЕсли;	
			
		Иначе  //заполнение для бланков, которые учитываются по номинальной стоимости	
...
Процедура РассчитатьСуммуНДСиАкцизногоНалогаТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("Сумма", МетаданныеДокумента, ИмяТабличнойЧасти) И
		...
		Если СуммаВключаетАкциз И ЕстьАкцизныйНалог Тогда
			...
			СтрокаТабличнойЧасти.СуммаАкцизногоНалога = УчетАкцизногоНалога.РассчитатьСуммуАкцизногоНалога(
															СтрокаТабличнойЧасти.Сумма, 
															СуммаВключаетАкциз, 
															СтрокаТабличнойЧасти.СтатьяДекларацииПоАкцизномуНалогу, 
															СтрокаТабличнойЧасти.ПодакцизныеТоварыДляКоммерческогоИспользования
														);
		...
		СтрокаТабличнойЧасти.СуммаНДС = Ценообразование.РассчитатьСуммуНДС(
											СуммаДляРасчетаНДС,
	    	                                УчитыватьНДС, 
											СуммаВключаетНДС,
	            	                        Ценообразование.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.СтавкаНДС)
...
КонецПроцедуры // РассчитатьСуммуНДСиАкцизногоНалогаТабЧасти()
...
Процедура ЗаполнитьСтатьюДекларацииПоАкцизномуНалогуТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект) Экспорт
	...
	// Заполнить СтатьяДекларацииПоАкцизномуНалогу
