...
Процедура ОбновитьФормуТиповогоОтчетаПоКомпоновщику(ОтчетОбъект, ФормаОтчета) Экспорт
	...
	Иначе
		...
		Попытка
			ПериодФормированияОтчетаПараметр = ОтчетОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ПериодФормированияОтчета"));
			ПериодФормированияОтчетаЗначение = ПериодФормированияОтчетаПараметр.Значение;
			Если ЗначениеЗаполнено(ПериодФормированияОтчетаЗначение.ДатаНачала) ИЛИ ЗначениеЗаполнено(ПериодФормированияОтчетаЗначение.ДатаОкончания) Тогда
				ФормаОтчета.СтандартныйПериод = ПериодФормированияОтчетаЗначение;
				ФормаОтчета.ЭлементыФормы.ДинамическийОтборСтандартныйПериодПользователя = ПериодФормированияОтчетаЗначение.Вариант;
			КонецЕсли; 
		Исключение
			// 
		КонецПопытки; 
...
КонецПроцедуры // ОбновитьФормуТиповогоОтчетаПоКомпоновщику
...
Функция ПолучитьПоследнийИспользуемыйВариант(Ссылка)
	...
	Иначе
		Если ЗначениеЗаполнено(Значение[Ссылка]) И Значение[Ссылка].ПолучитьОбъект() = Неопределено Тогда
			Возврат Справочники.СохраненныеНастройки.ПустаяСсылка();
		Иначе
...
Процедура ОчиститьПоследнююИспользуемуюНастройку(ОтчетНазвание) Экспорт
	...
	Значение["ОтчетОбъект." + СокрЛП(ОтчетНазвание)] = Справочники.СохраненныеНастройки.ПустаяСсылка();
	СохранитьЗначение("ПоследниеИспользованныеНастройкиПользователя", Значение);
		
КонецПроцедуры // ОчиститьПоследнююИспользуемуюНастройку
...
Функция ПолучитьОписаниеНастроекОтчета(КомпоновщикНастроек)
	...
	ИначеЕсли ПредставлениеЭлементаОтчета = Перечисления.ПредставленияЭлементовОтчетов.КроссТаблица Тогда
		...
		КонецЕсли;
		ТипКолонок = НСтр("ru='Группировки колонок';uk='Груповання стовпчиків'", ЯзыкФормированияПечатныхФормОтчетов);
	ИначеЕсли ПредставлениеЭлементаОтчета = Перечисления.ПредставленияЭлементовОтчетов.Диаграмма Тогда
		...
		Иначе
			Строки = Новый Массив;
		КонецЕсли;
		ТипСтрок = НСтр("ru='Группировки серий';uk='Груповання серій'", ЯзыкФормированияПечатныхФормОтчетов);
		Если ЭлементОтчета.Точки.Количество() > 0 Тогда
			Колонки = ПолучитьМассивГруппировок(ЭлементОтчета.Точки[0], КомпоновщикНастроек); 
		Иначе
			Колонки = Новый Массив;
		КонецЕсли;
		ТипКолонок = НСтр("ru='Группировки точек';uk='Груповання точок'", ЯзыкФормированияПечатныхФормОтчетов);
	КонецЕсли;
...
КонецФункции // ПоказательДоступен
