...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь,  ЕстьОшибки = Ложь) Экспорт
		...
		Если Префикс = "Верх" Тогда
			...
			Иначе
				Если ДатаНач = '00010101000000' ИЛИ ДатаКон = '00010101000000' Тогда
					ОбластьЗначение.Параметры.Период = НСтр("ru='Период: ';uk='Період: '", КодЯзыкаПечать) + Формат(ДатаНач, "ДФ = ""дд.ММ.гггг""; ДП = ""...""") 
	...
	ОбластьШапкаТаблицы.Область(2,2).Текст = ЛокализацияПовтИсп.ПолучитьПереводТекста(ЗаголовокКолонки);
