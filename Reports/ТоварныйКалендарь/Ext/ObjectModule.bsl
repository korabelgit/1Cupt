...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь,  ЕстьОшибки = Ложь) Экспорт
	...
	Для Каждого Строка Из ТаблицаСписокВсехФильтров Цикл
		...
		Если НайденныеСтроки.Количество() = 1 Тогда
			...
			СтрФильтры = СтрФильтры 
						 + ", "
						 + ЛокализацияПовтИсп.ПолучитьПереводТекста(НайденныеСтроки[0].ПредставлениеФильтра)
						 + " " 
						 + ЛокализацияПовтИсп.ПолучитьПереводТекста(СтрТипФильтра, КодЯзыкаПечать, ЛокализацияПовтИсп.КодЯзыкаСистемы())
						 + " " 
	...
	Для Сч = 1 По 3 Цикл
		...
		Если Префикс = "Верх" Тогда
			...
			ОбластьЗначение.Параметры.Период = НСтр("ru='По состоянию заказов на ';uk='Станом замовлень на '", КодЯзыкаПечать) + Формат(ДатаОтчета, "ДЛФ=ДД");

			Если  ДатаОтчета = '00010101000000' Тогда
				ОбластьЗначение.Параметры.Период = НСтр("ru='По состоянию заказов на момент последнего движения ';uk='Станом замовлень на момент останнього руху '", КодЯзыкаПечать);
			Иначе
				ОбластьЗначение.Параметры.Период = НСтр("ru='По состоянию заказов на ';uk='Станом замовлень на '", КодЯзыкаПечать) + Формат(ДатаОтчета, "ДФ = ""дд.ММ.гггг""");
			КонецЕсли;
	...
	ОбластьЗначение.Область(2,1).Текст = ЛокализацияПовтИсп.ПолучитьПереводТекста(ЗаголовокКолонки);
