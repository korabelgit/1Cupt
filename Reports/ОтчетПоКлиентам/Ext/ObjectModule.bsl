...
Процедура УстановитьНачальныеНастройки(ДополнительныеПараметры = Неопределено) Экспорт
	...
	|		NULL КАК Номенклатура,
	|		NULL КАК ХарактеристикаНоменклатуры,
	|		ВзаиморасчетыСКонтрагентамиОстаткиИОбороты.Регистратор КАК ДокументДвижения,
	|		0 КАК СуммаНачальныйДолг,
	|		0 КАК СуммаКонечныйДолг,
	|		0 КАК СуммаПриход,
	|		0 КАК СуммаРасход,
	|		ВзаиморасчетыСКонтрагентамиОстаткиИОбороты.СуммаУпрПриход КАК СуммаОплаченоДеб,
	|		ВзаиморасчетыСКонтрагентамиОстаткиИОбороты.СуммаУпрРасход КАК СуммаОплаченоКред,
	|		0 КАК КоличествоПриход,
	|		0 КАК КоличествоРасход,
	|		0 КАК КоличествоЕдиницДляОтчетовПриход,
	|		0 КАК КоличествоЕдиницДляОтчетовРасход
	|	ИЗ
	|		РегистрНакопления.ВзаиморасчетыСКонтрагентами.Обороты(&ДатаНач, &ДатаКон, Регистратор, {ДоговорКонтрагента.Организация.* КАК Организация, ДоговорКонтрагента.Владелец.* КАК Контрагент, ДоговорКонтрагента.* КАК ДоговорКонтрагента, Сделка.* КАК ДокументСделка}) КАК ВзаиморасчетыСКонтрагентамиОстаткиИОбороты
	|	ГДЕ
	|		ВзаиморасчетыСКонтрагентамиОстаткиИОбороты.Регистратор ССЫЛКА Документ.КорректировкаДолга
	|	{ГДЕ
	|		ВзаиморасчетыСКонтрагентамиОстаткиИОбороты.Регистратор.* КАК ДокументДвижения}
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	...
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("КоличествоПриход", "Количество (в ед. хранения) (приход от контрагента)");
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("КоличествоРасход", "Количество (в ед. хранения) (расход на контрагента)");
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("КоличествоЕдиницДляОтчетовПриход", "Количество (в ед. отчетов) (приход от контрагента)");
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("КоличествоЕдиницДляОтчетовРасход", "Количество (в ед. отчетов) (расход на контрагента)");
	...
	УниверсальныйОтчет.ДобавитьПоказатель("КоличествоПриход", "Количество (в ед. хранения)", Истина, "ЧЦ=15; ЧДЦ=3", "Приход", "Приход от контрагента");
	УниверсальныйОтчет.ДобавитьПоказатель("КоличествоЕдиницДляОтчетовПриход", "Количество (в ед. отчетов)", Ложь, "ЧЦ=15; ЧДЦ=3", "Приход", "Приход от контрагента");
	...
	УниверсальныйОтчет.ДобавитьПоказатель("КоличествоРасход", "Количество (в ед. хранения)", Истина, "ЧЦ=15; ЧДЦ=3", "Расход", "Расход на контрагента");
	УниверсальныйОтчет.ДобавитьПоказатель("КоличествоЕдиницДляОтчетовРасход", "Количество (в ед. отчетов)", Ложь, "ЧЦ=15; ЧДЦ=3", "Расход", "Расход на контрагента");
