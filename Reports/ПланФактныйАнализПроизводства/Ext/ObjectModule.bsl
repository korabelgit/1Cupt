	...
	Для Каждого Показатель из Элементы Цикл
		...
		Если ТипЗнч(Показатель) = Тип("ГруппаВыбранныхПолейКомпоновкиДанных") Тогда
			...
			Если (Показатель.Заголовок = "Отклонение количества" ИЛИ Показатель.Заголовок = "Отклонение стоимости") И Показатель.Использование Тогда
				...
				ПоказательЗаголовок = Показатель.Заголовок;
				Если ПоказательЗаголовок = "Отклонение количества" И ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов() = "uk" Тогда
					ПоказательЗаголовок = "Відхилення кількості";
				ИначеЕсли ПоказательЗаголовок = "Отклонение стоимости" И ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов() = "uk" Тогда	
				    ПоказательЗаголовок = "Відхилення вартості";
				КонецЕсли; 
				РезультатПоиска = ТабличныйДокумент.НайтиТекст(ПоказательЗаголовок,, ТабличныйДокумент.Область(ОбластьЭталонныйПериод.Верх, ОбластьЭталонныйПериод.Лево, ТабличныйДокумент.ВысотаТаблицы, ТабличныйДокумент.ШиринаТаблицы));
...
Процедура УстановитьСхемуКомпоновки() Экспорт
	...
	Для каждого СравниваемыеДанные из ДанныеОтчета Цикл
		...
		УстановитьПараметр(ПараметрДатаКонТек, СхемаКомпоновки, НовыйНабор, "ДатаКон" + Формат(СравниваемыеДанные.НомерСтроки, "ЧГ=0"), ?(СравниваемыеДанные.ДатаКон = Дата('00010101'), СравниваемыеДанные.ДатаКон, Новый Граница(КонецДня(СравниваемыеДанные.ДатаКон), ВидГраницы.Включая)), "ДатаКонТек");
...
Функция ПолучитьПредставлениеДанных(Данные)
	...
	Иначе
		...
		Результат = Результат + НСтр("ru='Сравниваемый период.';uk='Порівнюваний період.'", КодЯзыкаПечать);
