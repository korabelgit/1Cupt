...
Функция ПолеЗапросаПоПредставлению(Показатель)
	...
	Если ВидОтчета = "ОрганизационнаяДиаграмма" Тогда
		...
		Если (Показатель.Представление =НСтр("ru='Фамилия';uk='Прізвище'",КодЯзыкаПечать))
		 или (Показатель.Представление =НСтр("ru='Имя';uk='Імя'",КодЯзыкаПечать))
	...
	Пока Выборка.Следующий() Цикл
		Для Каждого Строка Из Показатели цикл
			...
			Если Строка.Представление = НСтр("ru='Фотография';uk='Фотографія'",КодЯзыкаПечать) Тогда
				...
				Продолжить;
			КонецЕсли;	
			Если (Строка.Представление = НСтр("ru='Наименование';uk='Найменування'",КодЯзыкаПечать)) ИЛИ (Строка.Представление = НСтр("ru='Фамилия';uk='Прізвище'",КодЯзыкаПечать)) ИЛИ (Строка.Представление = НСтр("ru='Имя';uk='Імя'",КодЯзыкаПечать)) ИЛИ 
...
Процедура СформироватьСписокПоказателей() Экспорт
	...
	ИначеЕсли ВидОтчета = "ОрганизационнаяДиаграмма" Тогда	
        ...
        СтрПоказатели.Представление		= НСтр("ru='Имя';uk='Імя'",КодЯзыкаПечать);
