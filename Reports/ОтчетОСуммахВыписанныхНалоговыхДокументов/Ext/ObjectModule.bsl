...
Процедура УстановитьНачальныеНастройки(ДополнительныеПараметры = Неопределено) Экспорт
	...
	Текст = 
		  ...
		  |	ВЫБОР КОГДА НДС.ВозвратнаяТара = ИСТИНА ТОГДА ""Возвратная тара"" ИНАЧЕ ""Товары, услуги"" КОНЕЦ КАК ВозвратнаяТара,
		  |	НДС.СобытиеНДС,
		  |	НДС.СтавкаНДС,
...
Процедура СформироватьОтчет(ТабличныйДокумент) Экспорт
	...
	ИначеЕсли ВРЕГ(ВидОтчета) = ВРЕГ("УсловнаяПродажа") Тогда	
		...
		СписокЗначенийСобытиеНДС.Добавить(Перечисления.СобытияОжидаемыйИПодтвержденныйНДСПродаж.УсловнаяПродажаВозврат);
