			...
			Для Каждого Ячейка Из СтрокаМакета.Ячейки Цикл
				...
				Если ТипЗнч(ПараметрРасшифровки) = Тип("ПараметрОбластиРасшифровкаКомпоновкиДанных") Тогда 
					...
					Если ДанныеОтчета.ПоказателиОтчета.НУ.Значение Тогда
						Если ПараметрРасшифровки.ВыраженияПолей.Найти("НалоговоеНазначение") = Неопределено Тогда 
							ПараметрСчет = ПараметрРасшифровки.ВыраженияПолей.Добавить();
							ПараметрСчет.Поле      = "НалоговоеНазначение";
							ПараметрСчет.Выражение = "ОсновнойНабор.НалоговоеНазначение";
						КонецЕсли;
						Если ПараметрРасшифровки.ВыраженияПолей.Найти("НалоговоеНазначениеКор") = Неопределено Тогда
							ПараметрКорСчет = ПараметрРасшифровки.ВыраженияПолей.Добавить();
							ПараметрКорСчет.Поле      = "НалоговоеНазначениеКор";
							ПараметрКорСчет.Выражение = "ОсновнойНабор.НалоговоеНазначениеКор";
						КонецЕсли;
					КонецЕсли;	
				КонецЕсли;
			КонецЦикла;
...
Процедура ДоработатьКомпоновщикПередВыводом(ВнешниеНаборыДанных) Экспорт
	...
	Для Каждого ЭлементМассива Из МассивПоказателей Цикл
		Если ПоказателиОтчета[ЭлементМассива].Значение И ЭлементМассива <> "ВалютнаяСумма" Тогда
			Если ТиповыеОтчеты.ПоказательДоступен(КомпоновщикНастроек.Настройки.ДоступныеПоляВыбора, "ОборотыЗаПериод." + ЭлементМассива + "ОборотДт") Тогда
			...
			Иначе
				ТиповыеОтчеты.ДобавитьВыбранноеПоле(ГруппаДт, "ОборотыЗаПериод.ПустаяЯчейкаДт");
				ТиповыеОтчеты.ДобавитьВыбранноеПоле(ГруппаКт, "ОборотыЗаПериод.ПустаяЯчейкаКт");
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Если ДанныеОтчета.ПоказателиОтчета.НУ.Значение Тогда
		ТиповыеОтчеты.ДобавитьВыбранноеПоле(Структура.Выбор,  "НалоговоеНазначение");
		ТиповыеОтчеты.ДобавитьВыбранноеПоле(Структура.Выбор,  "НалоговоеНазначениеКор");
	КонецЕсли;	
...
Функция ПолучитьТекстЗаголовка(ОрганизацияВНачале = Истина) Экспорт 
	...
	Если ПустаяСтрока(ТекстКорСубконто) Тогда
		ОбщийТекстСубконто = ОбщийТекстСубконто + НСтр("ru=' и ...';uk=' і ...'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
	Иначе
		ОбщийТекстСубконто = ОбщийТекстСубконто + " и " + ТекстКорСубконто;
	КонецЕсли;
	ЗаголовокОтчета = НСтр("ru='Обороты между субконто ';uk='Обороти між субконто '", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()) + ОбщийТекстСубконто + СтандартныеОтчеты.ПолучитьПредставлениеПериода(ЭтотОбъект);
